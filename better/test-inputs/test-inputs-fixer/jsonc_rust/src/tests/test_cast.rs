...
extern "C" fn getit(mut new_obj: *mut json_object, mut field: *const i8) {
    let mut o: *mut json_object = 0 as *mut json_object;
    if (unsafe { json_object_object_get_ex(new_obj, field, &mut o) }) == 0 {
        (unsafe { printf(
            b"Field %s does not exist\n\0" as *const u8 as *const i8,
            field,
        ) });
    }
    let mut o_type: json_type = unsafe { json_object_get_type(o) };
    (unsafe { printf(
...
extern "C" fn checktype(mut new_obj: *mut json_object, mut field: *const i8) {
    let mut o: *mut json_object = new_obj;
    if !field.is_null() && (unsafe { json_object_object_get_ex(new_obj, field, &mut o) }) == 0 {
        (unsafe { printf(
            b"Field %s does not exist\n\0" as *const u8 as *const i8,
            field,
        ) });
