...
fn main_0() -> i32 {
    ...
    let mut jso: *mut json_object = unsafe { json_tokener_parse(input) };
    (unsafe { printf(
        b"jso.to_string()=%s\n\0" as *const u8 as *const i8,
        json_object_to_json_string(jso),
    ) });
    let mut rv: i32 = 0;
    rv = unsafe { json_c_visit(
        jso,
        0 as i32,
...
extern "C" fn skip_arrays(
    ...
    if (unsafe { json_object_get_type(jso) }) as u32 == json_type_array as i32 as u32 {
        return 7547 as i32;
    }
    return 0 as i32;
}
