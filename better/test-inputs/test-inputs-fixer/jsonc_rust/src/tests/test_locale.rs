...
fn main_0(mut _argc: i32, mut _argv: *mut *mut i8) -> i32 {
    ...
    new_obj = unsafe { json_tokener_parse(b"[1.2,3.4,123456.78,5.0,2.3e10]\0" as *const u8 as *const i8) };
    (unsafe { snprintf(
        buf2.as_mut_ptr(),
        ::std::mem::size_of::<[i8; 10]>() as u64,
        b"%f\0" as *const u8 as *const i8,
    ...
    let mut ii: u32 = 0;
    ii = 0 as i32 as u32;
    while (ii as u64) < (unsafe { json_object_array_length(new_obj) }) {
        let mut val: *mut json_object = unsafe { json_object_array_get_idx(new_obj, ii as size_t) };
        (unsafe { printf(
