...
extern "C" fn test_case_parse() {
    ...
    (unsafe { json_tokener_set_flags(tok, 0x1 as i32) });
    new_obj = unsafe { json_tokener_parse_ex(tok, b"True\0" as *const u8 as *const i8, 4 as i32) };
    if new_obj.is_null() {
    } else {
        (unsafe { __assert_fail(
            ...
            34 as i32 as u32,
            (*::std::mem::transmute::<&[u8; 27], &[i8; 27]>(b"void test_case_parse(void)\0"))
                .as_ptr(),
        ) });
    }
    new_obj = unsafe { json_tokener_parse_ex(tok, b"False\0" as *const u8 as *const i8, 5 as i32) };
    if new_obj.is_null() {
    } else {
        (unsafe { __assert_fail(
            b"new_obj == NULL\0" as *const u8 as *const i8,
    ...
    new_obj = unsafe { json_tokener_parse_ex(tok, b"Null\0" as *const u8 as *const i8, 4 as i32) };
    if new_obj.is_null() {
    } else {
        (unsafe { __assert_fail(
            b"new_obj == NULL\0" as *const u8 as *const i8,
