...
extern "C" {
    ...
    fn json_tokener_set_flags(tok: *mut json_tokener, flags: i32);
    fn json_tokener_parse_ex(tok: *mut json_tokener, str: *const i8, len: i32) -> *mut json_object;
    fn json_type_to_name(o_type: json_type) -> *const i8;
    fn json_c_visit(
        jso: *mut json_object,
        future_flags: i32,
...
fn main_0() -> i32 {
    static mut separator: [i8; 35] = unsafe {
        *::std::mem::transmute::<&[u8; 35], &[i8; 35]>(b"==================================\0")
    };
    test_basic_parse();
...
extern "C" fn single_incremental_parse(mut test_string: *const i8, mut clear_serializer_0: i32) {
    ...
    all_at_once_obj = unsafe { json_tokener_parse(test_string) };
    if clear_serializer_0 != 0 {
        do_clear_serializer(all_at_once_obj);
    }
    all_at_once_str = unsafe { json_object_to_json_string(all_at_once_obj) };
    tok = unsafe { json_tokener_new() };
    let mut test_string_len: size_t = (unsafe { strlen(test_string) }).wrapping_add(1 as i32 as u64);
    ...
    while ii < test_string_len {
        ...
        new_obj = unsafe { json_tokener_parse_ex(tok, &*test_string.offset(ii as isize), len_to_parse) };
        jerr = unsafe { json_tokener_get_error(tok) };
    ...
    new_str = unsafe { json_object_to_json_string(new_obj) };
    ...
    if !all_at_once_obj.is_null() {
        (unsafe { json_object_put(all_at_once_obj) });
    }
    if !new_obj.is_null() {
...
extern "C" fn single_basic_parse(mut test_string: *const i8, mut clear_serializer_0: i32) {
    let mut new_obj: *mut json_object = 0 as *mut json_object;
    new_obj = unsafe { json_tokener_parse(test_string) };
    if clear_serializer_0 != 0 {
        do_clear_serializer(new_obj);
    }
    (unsafe { printf(
...
extern "C" fn do_clear_serializer(mut jso: *mut json_object) {
    (unsafe { json_c_visit(
        jso,
        0 as i32,
...
extern "C" fn clear_serializer(
    ...
    mut _flags: i32,
    mut _parent_jso: *mut json_object,
    mut _jso_key: *const i8,
    mut _jso_index: *mut size_t,
    mut _userarg: *mut libc::c_void,
) -> i32 {
    if !jso.is_null() {
        (unsafe { json_object_set_serializer(jso, None, 0 as *mut libc::c_void, None) });
    }
    return 0 as i32;
}
extern "C" fn test_verbose_parse() {
    ...
    new_obj = unsafe { json_tokener_parse_verbose(b"{ foo }\0" as *const u8 as *const i8, &mut error) };
    if error as u32 == json_tokener_error_parse_object_key_name as i32 as u32 {
        ...
        (unsafe { __assert_fail(
            ...
            b"/home/xial/json-c/tests/test_parse.c\0" as *const u8 as *const i8,
            235 as i32 as u32,
            (*::std::mem::transmute::<&[u8; 30], &[i8; 30]>(b"void test_verbose_parse(void)\0"))
                .as_ptr(),
        ) });
    }
    if new_obj.is_null() {
    ...
    new_obj = unsafe { json_tokener_parse(b"{ foo }\0" as *const u8 as *const i8) };
    if new_obj.is_null() {
    } else {
        (unsafe { __assert_fail(
            b"new_obj == NULL\0" as *const u8 as *const i8,
...
extern "C" fn test_incremental_parse() {
    ...
    new_obj = unsafe { json_tokener_parse(string_to_parse) };
    if new_obj.is_null() {
    ...
    while !(unsafe { incremental_steps[ii as usize].string_to_parse }).is_null() {
        ...
        let mut step: *mut incremental_step =
            (unsafe { &mut *incremental_steps.as_mut_ptr().offset(ii as isize) }) as *mut incremental_step;
        let mut length: i32 = unsafe { (*step).length };
        let mut expected_char_offset: size_t = 0;
        (unsafe { json_tokener_set_flags(tok, (*step).tok_flags) });
        if length == -(1 as i32) {
            length = (unsafe { strlen((*step).string_to_parse) }) as i32;
        }
        if (unsafe { (*step).char_offset }) == -(1 as i32) {
            expected_char_offset = length as size_t;
        } else {
            expected_char_offset = (unsafe { (*step).char_offset }) as size_t;
        }
        (unsafe { printf(
        ...
        new_obj = unsafe { json_tokener_parse_ex(tok, (*step).string_to_parse, length) };
        jerr = unsafe { json_tokener_get_error(tok) };
        if (unsafe { (*step).expected_error }) as u32 != json_tokener_success as i32 as u32 {
            if !new_obj.is_null() {
                (unsafe { printf(
                    b"ERROR: invalid object returned: %s\n\0" as *const u8 as *const i8,
                    json_object_to_json_string(new_obj),
                ) });
            } else if jerr as u32 != (unsafe { (*step).expected_error }) as u32 {
                (unsafe { printf(
                    b"ERROR: got wrong error: %s\n\0" as *const u8 as *const i8,
                    json_tokener_error_desc(jerr),
                ) });
            } else if (unsafe { json_tokener_get_parse_end(tok) }) != expected_char_offset {
                (unsafe { printf(
        ...
        if (unsafe { (*step).reset_tokener }) & 1 as i32 != 0 {
            (unsafe { json_tokener_reset(tok) });
        }
        if this_step_ok != 0 {
            num_ok += 1;
        } else {
