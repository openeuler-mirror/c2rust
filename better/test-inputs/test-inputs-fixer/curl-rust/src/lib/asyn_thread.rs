...
extern "C" {
    ...
    fn Curl_thread_create(
        func: Option<unsafe extern "C" fn(*mut libc::c_void) -> u32>,
        arg: *mut libc::c_void,
    ) -> *mut pthread_t;
    fn Curl_thread_destroy(hnd: *mut pthread_t);
    fn Curl_thread_join(hnd: *mut *mut pthread_t) -> i32;
...
#[no_mangle]
pub extern "C" fn Curl_resolver_init(
    mut _easy: *mut Curl_easy,
    mut resolver: *mut *mut libc::c_void,
) -> CURLcode {
    (unsafe { *resolver = Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<resdata>() as u64,
    ) });
    if (unsafe { *resolver }).is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_resolver_cleanup(mut resolver: *mut libc::c_void) {
    (unsafe { Curl_cfree.expect("non-null function pointer")(resolver) });
}
#[no_mangle]
pub extern "C" fn Curl_resolver_duphandle(
    mut easy: *mut Curl_easy,
    mut to: *mut *mut libc::c_void,
    mut _from: *mut libc::c_void,
) -> CURLcode {
    return Curl_resolver_init(easy, to);
}
#[no_mangle]
pub extern "C" fn Curl_resolver_cancel(mut data: *mut Curl_easy) {
    destroy_async_data(unsafe { &mut (*data).state.async_0 });
}
extern "C" fn conn_thread_sync_data(mut data: *mut Curl_easy) -> *mut thread_sync_data {
    return unsafe { &mut (*(*data).state.async_0.tdata).tsd };
}
extern "C" fn destroy_thread_sync_data(mut tsd: *mut thread_sync_data) {
    if !(unsafe { (*tsd).mtx }).is_null() {
        (unsafe { pthread_mutex_destroy((*tsd).mtx) });
        (unsafe { Curl_cfree.expect("non-null function pointer")((*tsd).mtx as *mut libc::c_void) });
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")((*tsd).hostname as *mut libc::c_void) });
    if !(unsafe { (*tsd).res }).is_null() {
        (unsafe { Curl_freeaddrinfo((*tsd).res) });
    }
    if (unsafe { (*tsd).sock_pair[1 as i32 as usize] }) != -(1 as i32) {
        (unsafe { close((*tsd).sock_pair[1 as i32 as usize]) });
    }
    (unsafe { memset(
...
extern "C" fn init_thread_sync_data(
    mut td: *mut thread_data,
    mut hostname: *const i8,
    mut port: i32,
    mut hints: *const addrinfo,
) -> i32 {
    let mut tsd: *mut thread_sync_data = unsafe { &mut (*td).tsd };
    (unsafe { memset(
        tsd as *mut libc::c_void,
        0 as i32,
        ::std::mem::size_of::<thread_sync_data>() as u64,
    ) });
    let fresh0 = unsafe { &mut ((*tsd).td) };
    *fresh0 = td;
    (unsafe { (*tsd).port = port });
    (unsafe { (*tsd).done = 1 as i32 });
    (unsafe { (*tsd).hints = *hints });
    let fresh1 = unsafe { &mut ((*tsd).mtx) };
    *fresh1 = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<pthread_mutex_t>() as u64,
    ) }) as *mut pthread_mutex_t;
    if !(unsafe { (*tsd).mtx }).is_null() {
        (unsafe { pthread_mutex_init((*tsd).mtx, 0 as *const pthread_mutexattr_t) });
        ...
        {
            (unsafe { (*tsd).sock_pair[0 as i32 as usize] = -(1 as i32) });
            (unsafe { (*tsd).sock_pair[1 as i32 as usize] = -(1 as i32) });
        } else {
            (unsafe { (*tsd).sock_error = CURLE_OK as i32 });
            let fresh2 = unsafe { &mut ((*tsd).hostname) };
            *fresh2 = unsafe { Curl_cstrdup.expect("non-null function pointer")(hostname) };
            if !(unsafe { (*tsd).hostname }).is_null() {
                return 1 as i32;
            }
        }
    }
...
extern "C" fn getaddrinfo_complete(mut data: *mut Curl_easy) -> i32 {
    let mut tsd: *mut thread_sync_data = conn_thread_sync_data(data);
    let mut rc: i32 = 0;
    rc = (unsafe { Curl_addrinfo_callback(data, (*tsd).sock_error, (*tsd).res) }) as i32;
    let fresh3 = unsafe { &mut ((*tsd).res) };
    *fresh3 = 0 as *mut Curl_addrinfo;
    return rc;
}
extern "C" fn getaddrinfo_thread(mut arg: *mut libc::c_void) -> u32 {
    let mut tsd: *mut thread_sync_data = arg as *mut thread_sync_data;
    let mut td: *mut thread_data = unsafe { (*tsd).td };
    ...
    if rc != 0 {
        (unsafe { (*tsd).sock_error = if *__errno_location() != 0 {
            *__errno_location()
        } else {
            rc
        } });
        if (unsafe { (*tsd).sock_error }) == 0 as i32 {
            (unsafe { (*tsd).sock_error = -(10 as i32) });
        }
    }
    (unsafe { pthread_mutex_lock((*tsd).mtx) });
    if (unsafe { (*tsd).done }) != 0 {
        (unsafe { pthread_mutex_unlock((*tsd).mtx) });
        destroy_thread_sync_data(tsd);
        (unsafe { Curl_cfree.expect("non-null function pointer")(td as *mut libc::c_void) });
    } else {
        if (unsafe { (*tsd).sock_pair[1 as i32 as usize] }) != -(1 as i32) {
            ...
            {
                (unsafe { (*tsd).sock_error = *__errno_location() });
            }
        }
        (unsafe { (*tsd).done = 1 as i32 });
        (unsafe { pthread_mutex_unlock((*tsd).mtx) });
...
extern "C" fn destroy_async_data(mut async_0: *mut Curl_async) {
    if !(unsafe { (*async_0).tdata }).is_null() {
        let mut td: *mut thread_data = unsafe { (*async_0).tdata };
        let mut done: i32 = 0;
        let mut sock_rd: curl_socket_t = unsafe { (*td).tsd.sock_pair[0 as i32 as usize] };
        let mut data: *mut Curl_easy = unsafe { (*td).tsd.data };
        (unsafe { pthread_mutex_lock((*td).tsd.mtx) });
        done = unsafe { (*td).tsd.done };
        (unsafe { (*td).tsd.done = 1 as i32 });
        (unsafe { pthread_mutex_unlock((*td).tsd.mtx) });
        if done == 0 {
            (unsafe { Curl_thread_destroy((*td).thread_hnd) });
        } else {
            if !(unsafe { (*td).thread_hnd }).is_null() {
                (unsafe { Curl_thread_join(&mut (*td).thread_hnd) });
            }
            destroy_thread_sync_data(unsafe { &mut (*td).tsd });
            (unsafe { Curl_cfree.expect("non-null function pointer")((*async_0).tdata as *mut libc::c_void) });
        }
        (unsafe { Curl_multi_closed(data, sock_rd) });
        (unsafe { close(sock_rd) });
    }
    let fresh4 = unsafe { &mut ((*async_0).tdata) };
    *fresh4 = 0 as *mut thread_data;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*async_0).hostname as *mut libc::c_void) });
    let fresh5 = unsafe { &mut ((*async_0).hostname) };
    *fresh5 = 0 as *mut i8;
}
extern "C" fn init_resolve_thread(
    mut data: *mut Curl_easy,
    mut hostname: *const i8,
    mut port: i32,
    mut hints: *const addrinfo,
) -> bool {
    let mut td: *mut thread_data = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<thread_data>() as u64,
    ) }) as *mut thread_data;
    let mut err: i32 = 12 as i32;
    let mut asp: *mut Curl_async = unsafe { &mut (*data).state.async_0 };
    let fresh6 = unsafe { &mut ((*data).state.async_0.tdata) };
    *fresh6 = td;
    if !td.is_null() {
        (unsafe { (*asp).port = port });
        (unsafe { (*asp).set_done(0 as i32 as bit) });
        (unsafe { (*asp).status = 0 as i32 });
        let fresh7 = unsafe { &mut ((*asp).dns) };
        *fresh7 = 0 as *mut Curl_dns_entry;
        let fresh8 = unsafe { &mut ((*td).thread_hnd) };
        *fresh8 = 0 as *mut pthread_t;
        if init_thread_sync_data(td, hostname, port, hints) == 0 {
            let fresh9 = unsafe { &mut ((*asp).tdata) };
            *fresh9 = 0 as *mut thread_data;
            (unsafe { Curl_cfree.expect("non-null function pointer")(td as *mut libc::c_void) });
        } else {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*asp).hostname as *mut libc::c_void) });
            let fresh10 = unsafe { &mut ((*asp).hostname) };
            *fresh10 = unsafe { Curl_cstrdup.expect("non-null function pointer")(hostname) };
            if !(unsafe { (*asp).hostname }).is_null() {
                (unsafe { (*td).tsd.done = 0 as i32 });
                let fresh11 = unsafe { &mut ((*td).thread_hnd) };
                *fresh11 = unsafe { Curl_thread_create(
                    Some(getaddrinfo_thread as unsafe extern "C" fn(*mut libc::c_void) -> u32),
...
extern "C" fn thread_wait_resolv(
    mut data: *mut Curl_easy,
    ...
    td = unsafe { (*data).state.async_0.tdata };
    if (unsafe { Curl_thread_join(&mut (*td).thread_hnd) }) != 0 {
        if !entry.is_null() {
            result = getaddrinfo_complete(data) as CURLcode;
        }
    }
    let fresh12 = unsafe { &mut ((*data).state.async_0) };
    (*fresh12).set_done(1 as i32 as bit);
    if !entry.is_null() {
        (unsafe { *entry = (*data).state.async_0.dns });
    }
    if (unsafe { (*data).state.async_0.dns }).is_null() && report as i32 != 0 {
        result = unsafe { Curl_resolver_error(data) };
    }
    destroy_async_data(unsafe { &mut (*data).state.async_0 });
    if (unsafe { (*data).state.async_0.dns }).is_null() && report as i32 != 0 {
...
#[no_mangle]
pub extern "C" fn Curl_resolver_kill(mut data: *mut Curl_easy) {
    let mut td: *mut thread_data = unsafe { (*data).state.async_0.tdata };
    if !td.is_null() && !(unsafe { (*td).thread_hnd }).is_null() {
...
#[no_mangle]
pub extern "C" fn Curl_resolver_wait_resolv(
    mut data: *mut Curl_easy,
    mut entry: *mut *mut Curl_dns_entry,
) -> CURLcode {
...
#[no_mangle]
pub extern "C" fn Curl_resolver_is_resolved(
    mut data: *mut Curl_easy,
    mut entry: *mut *mut Curl_dns_entry,
) -> CURLcode {
    let mut td: *mut thread_data = unsafe { (*data).state.async_0.tdata };
    ...
    if done != 0 {
        ...
        if (unsafe { (*data).state.async_0.dns }).is_null() {
            let mut result: CURLcode = unsafe { Curl_resolver_error(data) };
            destroy_async_data(unsafe { &mut (*data).state.async_0 });
            return result;
        }
        destroy_async_data(unsafe { &mut (*data).state.async_0 });
        ...
        let mut elapsed: timediff_t = unsafe { Curl_timediff(Curl_now(), (*data).progress.t_startsingle) };
        if elapsed < 0 as i32 as i64 {
            elapsed = 0 as i32 as timediff_t;
        }
        if (unsafe { (*td).poll_interval }) == 0 as i32 as u32 {
            (unsafe { (*td).poll_interval = 1 as i32 as u32 });
        } else if elapsed >= (unsafe { (*td).interval_end }) {
            let fresh13 = unsafe { &mut ((*td).poll_interval) };
            *fresh13 = (*fresh13).wrapping_mul(2 as i32 as u32);
        }
        if (unsafe { (*td).poll_interval }) > 250 as i32 as u32 {
            (unsafe { (*td).poll_interval = 250 as i32 as u32 });
        }
        (unsafe { (*td).interval_end = elapsed + (*td).poll_interval as i64 });
        (unsafe { Curl_expire(data, (*td).poll_interval as timediff_t, EXPIRE_ASYNC_NAME) });
    }
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_resolver_getsock(
    mut data: *mut Curl_easy,
    ...
    let mut reslv: *mut resdata = (unsafe { (*data).state.async_0.resolver }) as *mut resdata;
    let mut td: *mut thread_data = unsafe { (*data).state.async_0.tdata };
    if !td.is_null() {
        (unsafe { *socks.offset(0 as i32 as isize) = (*td).tsd.sock_pair[0 as i32 as usize] });
        let fresh14 = unsafe { &mut ((*td).tsd.data) };
        *fresh14 = data;
        ret_val = (1 as i32) << 0 as i32;
    } else {
        ms = unsafe { Curl_timediff(Curl_now(), (*reslv).start) };
        ...
        (unsafe { Curl_expire(data, milli, EXPIRE_ASYNC_NAME) });
    }
    return ret_val;
}
#[no_mangle]
pub extern "C" fn Curl_resolver_getaddrinfo(
    mut data: *mut Curl_easy,
    mut hostname: *const i8,
    mut port: i32,
    mut waitp: *mut i32,
    ...
    hints.ai_family = pf;
    hints.ai_socktype = if (unsafe { (*(*data).conn).transport }) as u32 == TRNSPRT_TCP as i32 as u32 {
        SOCK_STREAM as i32
    } else {
        SOCK_DGRAM as i32
    };
    (unsafe { (*reslv).start = Curl_now() });
...
#[no_mangle]
pub extern "C" fn Curl_set_dns_servers(mut _data: *mut Curl_easy, mut _servers: *mut i8) -> CURLcode {
    return CURLE_NOT_BUILT_IN;
}
#[no_mangle]
pub extern "C" fn Curl_set_dns_interface(
    mut _data: *mut Curl_easy,
    mut _interf: *const i8,
) -> CURLcode {
    return CURLE_NOT_BUILT_IN;
}
#[no_mangle]
