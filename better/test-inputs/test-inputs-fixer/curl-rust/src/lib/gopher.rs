...
#[no_mangle]
pub static mut Curl_handler_gopher: Curl_handler =  {
    {
        let mut init = Curl_handler {
            scheme: b"GOPHER\0" as *const u8 as *const i8,
            setup_connection: None,
            do_it: Some(gopher_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: None,
            do_more: None,
            connect_it: None,
            connecting: None,
...
#[no_mangle]
pub static mut Curl_handler_gophers: Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_more: None,
            connect_it: Some(
                gopher_connect as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            connecting: Some(
                gopher_connecting as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            doing: None,
            proto_getsock: None,
            doing_getsock: None,
...
extern "C" fn gopher_connect(mut _data: *mut Curl_easy, mut _done: *mut bool) -> CURLcode {
    return CURLE_OK;
}
extern "C" fn gopher_connecting(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut result: CURLcode = unsafe { Curl_ssl_connect(data, conn, 0 as i32) };
    if result as u64 != 0 {
...
extern "C" fn gopher_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    ...
    let mut query: *mut i8 = unsafe { (*data).state.up.query };
    ...
    (unsafe { *done = 1 as i32 != 0 });
    if !query.is_null() {
        gopherpath = unsafe { curl_maprintf(b"%s?%s\0" as *const u8 as *const i8, path, query) };
    } else {
        gopherpath = unsafe { Curl_cstrdup.expect("non-null function pointer")(path) };
    }
    if gopherpath.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    if (unsafe { strlen(gopherpath) }) <= 2 as i32 as u64 {
        sel = b"\0" as *const u8 as *const i8 as *mut i8;
        len = unsafe { strlen(sel) };
        (unsafe { Curl_cfree.expect("non-null function pointer")(gopherpath as *mut libc::c_void) });
    } else {
        let mut newp: *mut i8 = 0 as *mut i8;
        newp = gopherpath;
        newp = unsafe { newp.offset(2 as i32 as isize) };
    ...
    k = unsafe { curlx_uztosz(len) };
    while !((unsafe { strlen(sel) }) < 1 as i32 as u64) {
        ...
        result = unsafe { Curl_client_write(data, (1 as i32) << 1 as i32, sel, amount as size_t) };
        if result as u64 != 0 {
            break;
        }
        k -= amount;
        sel = unsafe { sel.offset(amount as isize) };
        if k < 1 as i32 as i64 {
            break;
        }
        timeout_ms = unsafe { Curl_timeleft(data, 0 as *mut curltime, 0 as i32 != 0) };
        if timeout_ms < 0 as i32 as i64 {
            ...
            what = unsafe { Curl_socket_check(-(1 as i32), -(1 as i32), sockfd, timeout_ms) };
            if what < 0 as i32 {
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(sel_org as *mut libc::c_void) });
    if result as u64 == 0 {
        result = unsafe { Curl_write(
            data,
            sockfd,
