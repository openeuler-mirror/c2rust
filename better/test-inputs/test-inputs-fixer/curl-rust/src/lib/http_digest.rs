...
extern "C" {
    ...
    fn Curl_auth_decode_digest_http_message(chlg: *const i8, digest: *mut digestdata) -> CURLcode;
...
pub extern "C" fn Curl_input_digest(
    ...
    if proxy {
        digest = unsafe { &mut (*data).state.proxydigest };
    } else {
        digest = unsafe { &mut (*data).state.digest };
    }
    if (unsafe { curl_strnequal(
        ...
        || (unsafe { Curl_isspace(*header.offset(6 as i32 as isize) as u8 as i32) }) == 0
    {
    ...
    return unsafe { Curl_auth_decode_digest_http_message(header, digest) };
}
#[no_mangle]
pub extern "C" fn Curl_output_digest(
    ...
    have_chlg = if !(unsafe { (*digest).nonce }).is_null() {
        1 as i32
    } else {
        0 as i32
    } != 0;
    ...
    if (unsafe { (*authp).iestyle() }) != 0 {
        tmp = unsafe { strchr(uripath as *mut i8, '?' as i32) };
        if !tmp.is_null() {
            let mut urilen: size_t = (unsafe { tmp.offset_from(uripath as *mut i8) }) as i64 as size_t;
...
pub extern "C" fn Curl_http_auth_cleanup_digest(mut data: *mut Curl_easy) {
    (unsafe { Curl_auth_digest_cleanup(&mut (*data).state.digest) });
    (unsafe { Curl_auth_digest_cleanup(&mut (*data).state.proxydigest) });
}
