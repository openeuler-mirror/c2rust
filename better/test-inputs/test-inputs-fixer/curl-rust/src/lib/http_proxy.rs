...
extern "C" {
    ...
    fn Curl_compareheader(headerline: *const i8, header: *const i8, content: *const i8) -> bool;
    fn Curl_copy_header_value(header: *const i8) -> *mut i8;
    ...
    fn Curl_http_input_auth(data: *mut Curl_easy, proxy: bool, auth: *const i8) -> CURLcode;
...
extern "C" fn https_proxy_connect(mut data: *mut Curl_easy, mut sockindex: i32) -> CURLcode {
...
pub extern "C" fn Curl_proxy_connect(mut data: *mut Curl_easy, mut sockindex: i32) -> CURLcode {
    ...
    if (unsafe { ((*conn).bits).tunnel_proxy() }) as i32 != 0 && (unsafe { ((*conn).bits).httpproxy() }) as i32 != 0 {
        ...
        let fresh0 = unsafe { &mut ((*data).state.aptr.proxyuserpwd) };
        *fresh0 = 0 as *mut i8;
    }
    return CURLE_OK;
}
...
extern "C" fn connect_init(mut data: *mut Curl_easy, mut reinit: bool) -> CURLcode {
    ...
    if !reinit {
        ...
        let fresh1 = unsafe { &mut ((*conn).connect_state) };
        *fresh1 = s;
        (unsafe { Curl_dyn_init(&mut (*s).rcvbuf, 16384 as i32 as size_t) });
        let fresh2 = unsafe { &mut ((*s).prot_save) };
        *fresh2 = unsafe { (*data).req.p.http };
        let fresh3 = unsafe { &mut ((*data).req.p.http) };
...
extern "C" fn connect_done(mut data: *mut Curl_easy) {
    ...
    if (unsafe { (*s).tunnel_state }) as u32 != TUNNEL_EXIT as i32 as u32 {
        ...
        (unsafe { Curl_dyn_free(&mut (*s).req) });
        let fresh4 = unsafe { &mut ((*data).req.p.http) };
        *fresh4 = unsafe { (*s).prot_save };
        let fresh5 = unsafe { &mut ((*s).prot_save) };
...
extern "C" fn CONNECT_host(
    ...
    if (unsafe { Curl_checkProxyheaders(data, conn, b"Host\0" as *const u8 as *const i8) }).is_null() {
    ...
    (unsafe { *connecthostp = hostheader });
    (unsafe { *hostp = host });
    return CURLE_OK;
}
extern "C" fn CONNECT(
    ...
    loop {
        ...
        if TUNNEL_INIT as i32 as u32 == (unsafe { (*s).tunnel_state }) as u32 {
            ...
            *fresh7 = 0 as *mut i8;
            (unsafe { Curl_dyn_init(req, (1024 as i32 * 1024 as i32) as size_t) });
            result = CONNECT_host(
                data,
                conn,
                hostname,
        ...
        if (unsafe { (*http).sending }) as u32 == HTTPSEND_REQUEST as i32 as u32 {
            if (unsafe { (*s).nsend }) == 0 {
                ...
                let fresh8 = unsafe { &mut ((*k).upload_fromhere) };
        ...
        while (unsafe { (*s).keepon }) as u64 != 0 {
            ...
            if result as u64 != 0 {
                ...
                {
                    let fresh11 = unsafe { &mut ((*conn).bits) };
                ...
                perline = unsafe { Curl_dyn_len(&mut (*s).rcvbuf) };
                ...
                if (unsafe { ((*data).set).suppress_connect_headers() }) == 0 {
                    ...
                    result = unsafe { Curl_client_write(data, writetype, linep, perline) };
                    if result as u64 != 0 {
                        return result;
                    }
                }
                let fresh13 = unsafe { &mut ((*data).info.header_size) };
                    ...
                    || '\n' as i32 == (unsafe { *linep.offset(0 as i32 as isize) }) as i32
                {
                    if 407 as i32 == (unsafe { (*k).httpcode }) && (unsafe { ((*data).state).authproblem() }) == 0 {
                        ...
                        if (unsafe { (*s).cl }) != 0 {
                            ...
                            r_0 = unsafe { Curl_httpchunk_read(
                                data,
                            ...
                            && 407 as i32 == (unsafe { (*k).httpcode })
                    {
                        ...
                        let mut auth: *mut i8 = unsafe { Curl_copy_header_value(linep) };
                        if auth.is_null() {
                            return CURLE_OUT_OF_MEMORY;
                        }
                    ...
                    {
                        if (unsafe { (*k).httpcode }) / 100 as i32 == 2 as i32 {
                            (unsafe { Curl_infof(
                                data,
                    ...
                    ) }) != 0
                    {
                        if (unsafe { (*k).httpcode }) / 100 as i32 == 2 as i32 {
                            ...
                            (unsafe { Curl_httpchunk_init(data) });
                        }
                    } else if unsafe { Curl_compareheader(
                        linep,
                        b"Proxy-Connection:\0" as *const u8 as *const i8,
                    ...
                    {
                        (unsafe { (*data).info.httpproxycode = (*k).httpcode });
                    }
                    (unsafe { Curl_dyn_reset(&mut (*s).rcvbuf) });
                }
            }
        ...
        if (unsafe { (*s).close_connection() }) as i32 != 0 && !(unsafe { (*data).req.newurl }).is_null() {
            (unsafe { Curl_closesocket(data, conn, (*conn).sock[sockindex as usize]) });
    ...
    if (unsafe { (*data).info.httpproxycode }) / 100 as i32 != 2 as i32 {
        if (unsafe { (*s).close_connection() }) as i32 != 0 && !(unsafe { (*data).req.newurl }).is_null() {
            ...
            (unsafe { Curl_cfree.expect("non-null function pointer")((*data).req.newurl as *mut libc::c_void) });
            let fresh15 = unsafe { &mut ((*data).req.newurl) };
            *fresh15 = 0 as *mut i8;
    ...
    let fresh16 = unsafe { &mut ((*data).state.aptr.proxyuserpwd) };
    *fresh16 = 0 as *mut i8;
    let fresh17 = unsafe { &mut ((*data).state.aptr.proxyuserpwd) };
    *fresh17 = 0 as *mut i8;
    let fresh18 = unsafe { &mut ((*data).state.authproxy) };
    (*fresh18).set_done(1 as i32 as bit);
    let fresh19 = unsafe { &mut ((*data).state.authproxy) };
    ...
    let fresh20 = unsafe { &mut ((*data).req) };
    (*fresh20).set_ignorebody(0 as i32 as bit);
    let fresh21 = unsafe { &mut ((*conn).bits) };
...
pub extern "C" fn Curl_connect_free(mut data: *mut Curl_easy) {
    ...
    if !s.is_null() {
        ...
        let fresh22 = unsafe { &mut ((*conn).connect_state) };
