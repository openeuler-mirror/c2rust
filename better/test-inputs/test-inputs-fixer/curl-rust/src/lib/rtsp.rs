...
#[no_mangle]
pub static mut Curl_handler_rtsp: Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_it: Some(rtsp_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: Some(
                rtsp_done as unsafe extern "C" fn(*mut Curl_easy, CURLcode, bool) -> CURLcode,
            ),
            do_more: None,
            connect_it: Some(
                rtsp_connect as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            connecting: None,
            doing: None,
            proto_getsock: None,
...
extern "C" fn rtsp_setup_connection(
    mut data: *mut Curl_easy,
    mut _conn: *mut connectdata,
) -> CURLcode {
    let mut rtsp: *mut RTSP = 0 as *mut RTSP;
    rtsp = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<RTSP>() as u64,
    ) }) as *mut RTSP;
    let fresh0 = unsafe { &mut ((*data).req.p.rtsp) };
    *fresh0 = rtsp;
    if rtsp.is_null() {
...
extern "C" fn rtsp_connisdead(mut check: *mut connectdata) -> bool {
    let mut sval: i32 = 0;
    ...
    if sval == 0 as i32 {
        ...
        ret_val = !(unsafe { Curl_connalive(check) });
    }
    return ret_val;
}
extern "C" fn rtsp_conncheck(
    mut _data: *mut Curl_easy,
    mut conn: *mut connectdata,
...
extern "C" fn rtsp_connect(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut httpStatus: CURLcode = CURLE_OK;
    httpStatus = unsafe { Curl_http_connect(data, done) };
    if (unsafe { (*data).state.rtsp_next_client_CSeq }) == 0 as i32 as i64 {
        (unsafe { (*data).state.rtsp_next_client_CSeq = 1 as i32 as i64 });
    }
    if (unsafe { (*data).state.rtsp_next_server_CSeq }) == 0 as i32 as i64 {
        (unsafe { (*data).state.rtsp_next_server_CSeq = 1 as i32 as i64 });
    }
    (unsafe { (*(*data).conn).proto.rtspc.rtp_channel = -(1 as i32) });
    return httpStatus;
}
extern "C" fn rtsp_disconnect(
    mut _data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut _dead: bool,
) -> CURLcode {
    ...
    let fresh1 = unsafe { &mut ((*conn).proto.rtspc.rtp_buf) };
    *fresh1 = 0 as *mut i8;
    return CURLE_OK;
}
extern "C" fn rtsp_done(
    mut data: *mut Curl_easy,
    mut status: CURLcode,
    mut premature: bool,
) -> CURLcode {
    let mut rtsp: *mut RTSP = unsafe { (*data).req.p.rtsp };
    let mut httpStatus: CURLcode = CURLE_OK;
    if (unsafe { (*data).set.rtspreq }) as u32 == RTSPREQ_RECEIVE as i32 as u32 {
        premature = 1 as i32 != 0;
    }
    httpStatus = unsafe { Curl_http_done(data, status, premature) };
    if !rtsp.is_null() {
        let mut CSeq_sent: i64 = unsafe { (*rtsp).CSeq_sent };
        let mut CSeq_recv: i64 = unsafe { (*rtsp).CSeq_recv };
        if (unsafe { (*data).set.rtspreq }) as u32 != RTSPREQ_RECEIVE as i32 as u32 && CSeq_sent != CSeq_recv {
            (unsafe { Curl_failf(
                data,
                b"The CSeq of this request %ld did not match the response %ld\0" as *const u8
                    as *const i8,
                CSeq_sent,
                CSeq_recv,
            ) });
            ...
            && (unsafe { (*(*data).conn).proto.rtspc.rtp_channel }) == -(1 as i32)
        {
            (unsafe { Curl_infof(
                data,
                b"Got an RTP Receive with a CSeq of %ld\0" as *const u8 as *const i8,
                CSeq_recv,
            ) });
        }
    }
...
extern "C" fn rtsp_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut result: CURLcode = CURLE_OK;
    let mut rtspreq: Curl_RtspReq = unsafe { (*data).set.rtspreq };
    let mut rtsp: *mut RTSP = unsafe { (*data).req.p.rtsp };
    ...
    (unsafe { (*rtsp).CSeq_sent = (*data).state.rtsp_next_client_CSeq });
    (unsafe { (*rtsp).CSeq_recv = 0 as i32 as i64 });
    ...
    match rtspreq as u32 {
        ...
        12 => {
            (unsafe { Curl_failf(
                data,
                b"Got invalid RTSP request: RTSPREQ_LAST\0" as *const u8 as *const i8,
            ) });
    ...
    p_session_id = unsafe { (*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize] };
    if p_session_id.is_null()
        && rtspreq as u32
            & !(RTSPREQ_OPTIONS as i32 | RTSPREQ_DESCRIBE as i32 | RTSPREQ_SETUP as i32) as u32
            != 0
    {
        (unsafe { Curl_failf(
            data,
            b"Refusing to issue an RTSP request [%s] without a session ID.\0" as *const u8
                as *const i8,
            p_request,
        ) });
        return CURLE_BAD_FUNCTION_ARGUMENT;
    }
    if !(unsafe { (*data).set.str_0[STRING_RTSP_STREAM_URI as i32 as usize] }).is_null() {
        p_stream_uri = unsafe { (*data).set.str_0[STRING_RTSP_STREAM_URI as i32 as usize] };
    } else {
        p_stream_uri = b"*\0" as *const u8 as *const i8;
    }
    p_transport = unsafe { Curl_checkheaders(data, b"Transport\0" as *const u8 as *const i8) };
    if rtspreq as u32 == RTSPREQ_SETUP as i32 as u32 && p_transport.is_null() {
        if !(unsafe { (*data).set.str_0[STRING_RTSP_TRANSPORT as i32 as usize] }).is_null() {
            (unsafe { Curl_cfree.expect("non-null function pointer")(
                (*data).state.aptr.rtsp_transport as *mut libc::c_void,
            ) });
            let fresh6 = unsafe { &mut ((*data).state.aptr.rtsp_transport) };
            *fresh6 = 0 as *mut i8;
            let fresh7 = unsafe { &mut ((*data).state.aptr.rtsp_transport) };
            *fresh7 = unsafe { curl_maprintf(
                b"Transport: %s\r\n\0" as *const u8 as *const i8,
                (*data).set.str_0[STRING_RTSP_TRANSPORT as i32 as usize],
            ) };
            if (unsafe { (*data).state.aptr.rtsp_transport }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
        } else {
            (unsafe { Curl_failf(
                data,
                b"Refusing to issue an RTSP SETUP without a Transport: header.\0" as *const u8
                    as *const i8,
            ) });
            return CURLE_BAD_FUNCTION_ARGUMENT;
        }
        p_transport = unsafe { (*data).state.aptr.rtsp_transport };
    }
    if rtspreq as u32 == RTSPREQ_DESCRIBE as i32 as u32 {
            ...
            && !(unsafe { (*data).set.str_0[STRING_ENCODING as i32 as usize] }).is_null()
        {
            ...
            let fresh8 = unsafe { &mut ((*data).state.aptr.accept_encoding) };
            *fresh8 = 0 as *mut i8;
            let fresh9 = unsafe { &mut ((*data).state.aptr.accept_encoding) };
            *fresh9 = unsafe { curl_maprintf(
                b"Accept-Encoding: %s\r\n\0" as *const u8 as *const i8,
            ...
            p_accept_encoding = unsafe { (*data).state.aptr.accept_encoding };
        }
    }
    if !(unsafe { Curl_checkheaders(data, b"User-Agent\0" as *const u8 as *const i8) }).is_null()
        && !(unsafe { (*data).state.aptr.uagent }).is_null()
    {
        (unsafe { Curl_cfree.expect("non-null function pointer")(
            (*data).state.aptr.uagent as *mut libc::c_void,
        ) });
        let fresh10 = unsafe { &mut ((*data).state.aptr.uagent) };
        *fresh10 = 0 as *mut i8;
        let fresh11 = unsafe { &mut ((*data).state.aptr.uagent) };
        *fresh11 = 0 as *mut i8;
    } else if (unsafe { Curl_checkheaders(data, b"User-Agent\0" as *const u8 as *const i8) }).is_null()
        && !(unsafe { (*data).set.str_0[STRING_USERAGENT as i32 as usize] }).is_null()
    {
        p_uagent = unsafe { (*data).state.aptr.uagent };
    }
    ...
    p_proxyuserpwd = unsafe { (*data).state.aptr.proxyuserpwd };
    p_userpwd = unsafe { (*data).state.aptr.userpwd };
    (unsafe { Curl_cfree.expect("non-null function pointer")((*data).state.aptr.ref_0 as *mut libc::c_void) });
    let fresh12 = unsafe { &mut ((*data).state.aptr.ref_0) };
    *fresh12 = 0 as *mut i8;
    if !(unsafe { (*data).state.referer }).is_null()
        && (unsafe { Curl_checkheaders(data, b"Referer\0" as *const u8 as *const i8) }).is_null()
    {
        let fresh13 = unsafe { &mut ((*data).state.aptr.ref_0) };
        *fresh13 = unsafe { curl_maprintf(
            b"Referer: %s\r\n\0" as *const u8 as *const i8,
            (*data).state.referer,
        ) };
    } else {
        let fresh14 = unsafe { &mut ((*data).state.aptr.ref_0) };
        *fresh14 = 0 as *mut i8;
    }
    p_referrer = unsafe { (*data).state.aptr.ref_0 };
    if (unsafe { ((*data).state).use_range() }) as i32 != 0
        && rtspreq as u32
            & (RTSPREQ_PLAY as i32 | RTSPREQ_PAUSE as i32 | RTSPREQ_RECORD as i32) as u32
            != 0
    {
        if (unsafe { Curl_checkheaders(data, b"Range\0" as *const u8 as *const i8) }).is_null()
            && !(unsafe { (*data).state.range }).is_null()
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")(
                (*data).state.aptr.rangeline as *mut libc::c_void,
            ) });
            let fresh15 = unsafe { &mut ((*data).state.aptr.rangeline) };
            *fresh15 = 0 as *mut i8;
            let fresh16 = unsafe { &mut ((*data).state.aptr.rangeline) };
            *fresh16 = unsafe { curl_maprintf(
                b"Range: %s\r\n\0" as *const u8 as *const i8,
                (*data).state.range,
            ) };
            p_range = unsafe { (*data).state.aptr.rangeline };
        }
    }
    if !(unsafe { Curl_checkheaders(data, b"CSeq\0" as *const u8 as *const i8) }).is_null() {
        (unsafe { Curl_failf(
            data,
            b"CSeq cannot be set as a custom header.\0" as *const u8 as *const i8,
        ) });
        return CURLE_RTSP_CSEQ_ERROR;
    }
    if !(unsafe { Curl_checkheaders(data, b"Session\0" as *const u8 as *const i8) }).is_null() {
        (unsafe { Curl_failf(
            data,
            b"Session ID cannot be set as a custom header.\0" as *const u8 as *const i8,
        ) });
        return CURLE_BAD_FUNCTION_ARGUMENT;
    }
    (unsafe { Curl_dyn_init(&mut req_buffer, (64 as i32 * 1024 as i32) as size_t) });
    ...
    let fresh17 = unsafe { &mut ((*data).state.aptr.userpwd) };
    *fresh17 = 0 as *mut i8;
    let fresh18 = unsafe { &mut ((*data).state.aptr.userpwd) };
    *fresh18 = 0 as *mut i8;
    if result as u64 != 0 {
        return result;
    }
    ...
    {
        result = unsafe { Curl_add_timecondition(data, &mut req_buffer) };
        if result as u64 != 0 {
            return result;
        }
    }
    result = unsafe { Curl_add_custom_headers(data, 0 as i32 != 0, &mut req_buffer) };
    if result as u64 != 0 {
        return result;
    }
    if rtspreq as u32 == RTSPREQ_ANNOUNCE as i32 as u32
        || rtspreq as u32 == RTSPREQ_SET_PARAMETER as i32 as u32
        || rtspreq as u32 == RTSPREQ_GET_PARAMETER as i32 as u32
    {
        if (unsafe { ((*data).set).upload() }) != 0 {
            putsize = unsafe { (*data).state.infilesize };
            (unsafe { (*data).state.httpreq = HTTPREQ_PUT });
        } else {
            postsize = if (unsafe { (*data).state.infilesize }) != -(1 as i32) as i64 {
                unsafe { (*data).state.infilesize }
            } else if !(unsafe { (*data).set.postfields }).is_null() {
                (unsafe { strlen((*data).set.postfields as *const i8) }) as curl_off_t
            } else {
                0 as i32 as i64
            };
            (unsafe { (*data).state.httpreq = HTTPREQ_POST });
        }
        if putsize > 0 as i32 as i64 || postsize > 0 as i32 as i64 {
            if (unsafe { Curl_checkheaders(data, b"Content-Length\0" as *const u8 as *const i8) }).is_null() {
                result = unsafe { Curl_dyn_addf(
                    &mut req_buffer as *mut dynbuf,
                    b"Content-Length: %ld\r\n\0" as *const u8 as *const i8,
            ...
            if rtspreq as u32 == RTSPREQ_SET_PARAMETER as i32 as u32
                || rtspreq as u32 == RTSPREQ_GET_PARAMETER as i32 as u32
            {
            ...
            let fresh19 = unsafe { &mut ((*data).state) };
            (*fresh19).set_expect100header(0 as i32 as bit);
        } else if rtspreq as u32 == RTSPREQ_GET_PARAMETER as i32 as u32 {
            (unsafe { (*data).state.httpreq = HTTPREQ_HEAD });
            let fresh20 = unsafe { &mut ((*data).set) };
            (*fresh20).set_opt_no_body(1 as i32 as bit);
        }
    }
    let fresh21 = unsafe { &mut ((*data).req) };
    (*fresh21).set_forbidchunk(1 as i32 as bit);
    result = unsafe { Curl_dyn_add(&mut req_buffer, b"\r\n\0" as *const u8 as *const i8) };
    if result as u64 != 0 {
        return result;
    }
    if postsize > 0 as i32 as i64 {
        result = unsafe { Curl_dyn_addn(&mut req_buffer, (*data).set.postfields, postsize as size_t) };
        if result as u64 != 0 {
            return result;
        }
    }
    ...
    let fresh22 = unsafe { &mut ((*data).state.rtsp_next_client_CSeq) };
    *fresh22 += 1;
...
extern "C" fn rtsp_rtp_readwrite(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    ...
    let mut rtspc: *mut rtsp_conn = unsafe { &mut (*conn).proto.rtspc };
    let mut rtp: *mut i8 = 0 as *mut i8;
    let mut rtp_dataleft: ssize_t = 0;
    let mut scratch: *mut i8 = 0 as *mut i8;
    let mut result: CURLcode = CURLE_OK;
    if !(unsafe { (*rtspc).rtp_buf }).is_null() {
        let mut newptr: *mut i8 = (unsafe { Curl_saferealloc(
            (*rtspc).rtp_buf as *mut libc::c_void,
            ((*rtspc).rtp_bufsize + *nread) as size_t,
        ) }) as *mut i8;
        if newptr.is_null() {
            let fresh23 = unsafe { &mut ((*rtspc).rtp_buf) };
            *fresh23 = 0 as *mut i8;
            (unsafe { (*rtspc).rtp_bufsize = 0 as i32 as ssize_t });
            return CURLE_OUT_OF_MEMORY;
        }
        let fresh24 = unsafe { &mut ((*rtspc).rtp_buf) };
        *fresh24 = newptr;
        (unsafe { memcpy(
            ((*rtspc).rtp_buf).offset((*rtspc).rtp_bufsize as isize) as *mut libc::c_void,
            (*k).str_0 as *const libc::c_void,
            *nread as u64,
        ) });
        let fresh25 = unsafe { &mut ((*rtspc).rtp_bufsize) };
        *fresh25 += unsafe { *nread };
        rtp = unsafe { (*rtspc).rtp_buf };
        rtp_dataleft = unsafe { (*rtspc).rtp_bufsize };
    } else {
        rtp = unsafe { (*k).str_0 };
        rtp_dataleft = unsafe { *nread };
    }
    while rtp_dataleft > 0 as i32 as i64 && (unsafe { *rtp.offset(0 as i32 as isize) }) as i32 == '$' as i32 {
        if rtp_dataleft > 4 as i32 as i64 {
            let mut rtp_length: i32 = 0;
            (unsafe { (*rtspc).rtp_channel = *rtp.offset(1 as i32 as isize) as u8 as i32 });
            rtp_length = ((unsafe { *rtp.offset(2 as i32 as isize) }) as u8 as i32) << 8 as i32
                | (unsafe { *rtp.offset(3 as i32 as isize) }) as u8 as i32;
            if rtp_dataleft < (rtp_length + 4 as i32) as i64 {
                (unsafe { *readmore = 1 as i32 != 0 });
                break;
            } else {
                result = rtp_client_write(
                    data,
                    unsafe { &mut *rtp.offset(0 as i32 as isize) },
                    (rtp_length + 4 as i32) as size_t,
                );
                if result as u64 != 0 {
                    (unsafe { Curl_failf(
                        data,
                        b"Got an error writing an RTP packet\0" as *const u8 as *const i8,
                    ) });
                    (unsafe { *readmore = 0 as i32 != 0 });
                    (unsafe { Curl_cfree.expect("non-null function pointer")(
                        (*rtspc).rtp_buf as *mut libc::c_void,
                    ) });
                    let fresh26 = unsafe { &mut ((*rtspc).rtp_buf) };
                    *fresh26 = 0 as *mut i8;
                    let fresh27 = unsafe { &mut ((*rtspc).rtp_buf) };
                    *fresh27 = 0 as *mut i8;
                    (unsafe { (*rtspc).rtp_bufsize = 0 as i32 as ssize_t });
                    return result;
                }
                rtp_dataleft -= (rtp_length + 4 as i32) as i64;
                rtp = unsafe { rtp.offset((rtp_length + 4 as i32) as isize) };
                if (unsafe { (*data).set.rtspreq }) as u32 == RTSPREQ_RECEIVE as i32 as u32 {
    ...
    if rtp_dataleft != 0 && (unsafe { *rtp.offset(0 as i32 as isize) }) as i32 == '$' as i32 {
        scratch =
            (unsafe { Curl_cmalloc.expect("non-null function pointer")(rtp_dataleft as size_t) }) as *mut i8;
        if scratch.is_null() {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*rtspc).rtp_buf as *mut libc::c_void) });
            let fresh28 = unsafe { &mut ((*rtspc).rtp_buf) };
            *fresh28 = 0 as *mut i8;
            let fresh29 = unsafe { &mut ((*rtspc).rtp_buf) };
            *fresh29 = 0 as *mut i8;
            (unsafe { (*rtspc).rtp_bufsize = 0 as i32 as ssize_t });
            return CURLE_OUT_OF_MEMORY;
        }
        ...
        (unsafe { Curl_cfree.expect("non-null function pointer")((*rtspc).rtp_buf as *mut libc::c_void) });
        let fresh30 = unsafe { &mut ((*rtspc).rtp_buf) };
        *fresh30 = 0 as *mut i8;
        let fresh31 = unsafe { &mut ((*rtspc).rtp_buf) };
        *fresh31 = scratch;
        (unsafe { (*rtspc).rtp_bufsize = rtp_dataleft });
        (unsafe { *nread = 0 as i32 as ssize_t });
        return CURLE_OK;
    }
    let fresh32 = unsafe { &mut ((*k).str_0) };
    *fresh32 = unsafe { (*fresh32).offset((*nread - rtp_dataleft) as isize) };
    let _ = rtp_dataleft > 0 as i32 as i64;
    (unsafe { *nread = rtp_dataleft });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*rtspc).rtp_buf as *mut libc::c_void) });
    let fresh33 = unsafe { &mut ((*rtspc).rtp_buf) };
    *fresh33 = 0 as *mut i8;
    let fresh34 = unsafe { &mut ((*rtspc).rtp_buf) };
    *fresh34 = 0 as *mut i8;
    (unsafe { (*rtspc).rtp_bufsize = 0 as i32 as ssize_t });
    return CURLE_OK;
}
extern "C" fn rtp_client_write(
    mut data: *mut Curl_easy,
    mut ptr: *mut i8,
    mut len: size_t,
) -> CURLcode {
    ...
    if unsafe { ((*data).set.fwrite_rtp).is_some() } {
        writeit = unsafe { (*data).set.fwrite_rtp };
        user_ptr = unsafe { (*data).set.rtp_out };
    } else {
        writeit = unsafe { (*data).set.fwrite_func };
        user_ptr = unsafe { (*data).set.out };
    }
    (unsafe { Curl_set_in_callback(data, 1 as i32 != 0) });
    wrote = unsafe { writeit.expect("non-null function pointer")(ptr, 1 as i32 as size_t, len, user_ptr) };
    (unsafe { Curl_set_in_callback(data, 0 as i32 != 0) });
    if 0x10000001 as i32 as u64 == wrote {
        (unsafe { Curl_failf(data, b"Cannot pause RTP\0" as *const u8 as *const i8) });
        return CURLE_WRITE_ERROR;
    }
    if wrote != len {
        (unsafe { Curl_failf(data, b"Failed writing RTP data\0" as *const u8 as *const i8) });
        return CURLE_WRITE_ERROR;
    }
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_rtsp_parseheader(mut data: *mut Curl_easy, mut header: *mut i8) -> CURLcode {
    ...
    ) }) != 0
    {
        ...
        if nc == 1 as i32 {
            let mut rtsp: *mut RTSP = unsafe { (*data).req.p.rtsp };
            (unsafe { (*rtsp).CSeq_recv = CSeq });
            (unsafe { (*data).state.rtsp_CSeq_recv = CSeq });
        } else {
            (unsafe { Curl_failf(
                data,
                b"Unable to read the CSeq header: [%s]\0" as *const u8 as *const i8,
                header,
            ) });
            return CURLE_RTSP_CSEQ_ERROR;
        }
    ...
    ) }) != 0
    {
        ...
        start = unsafe { header.offset(8 as i32 as isize) };
        while (unsafe { *start }) as i32 != 0 && (unsafe { Curl_isspace(*start as u8 as i32) }) != 0 {
            start = unsafe { start.offset(1) };
        }
        if (unsafe { *start }) == 0 {
            (unsafe { Curl_failf(data, b"Got a blank Session ID\0" as *const u8 as *const i8) });
            return CURLE_RTSP_SESSION_ERROR;
        }
        end = start;
        while (unsafe { *end }) as i32 != 0 && (unsafe { *end }) as i32 != ';' as i32 && (unsafe { Curl_isspace(*end as u8 as i32) }) == 0
        {
            end = unsafe { end.offset(1) };
        }
        idlen = (unsafe { end.offset_from(start) }) as i64 as size_t;
        if !(unsafe { (*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize] }).is_null() {
            if (unsafe { strlen((*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize]) }) != idlen
                || (unsafe { strncmp(
                    start,
            ...
            let fresh35 = unsafe { &mut ((*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize]) };
            *fresh35 = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
            ...
            (unsafe { memcpy(
                (*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize] as *mut libc::c_void,
                start as *const libc::c_void,
                idlen,
            ) });
            (unsafe { *((*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize]).offset(idlen as isize) =
                '\u{0}' as i32 as i8 });
        }
    }
    return CURLE_OK;
