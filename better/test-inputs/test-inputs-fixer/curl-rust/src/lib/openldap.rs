...
extern "C" {
    ...
    fn ldap_url_parse(url: *const i8, ludpp: *mut *mut LDAPURLDesc) -> i32;
    ...
    fn ldap_get_option(ld: *mut LDAP, option: i32, outvalue: *mut libc::c_void) -> i32;
    fn ldap_set_option(ld: *mut LDAP, option: i32, invalue: *const libc::c_void) -> i32;
    ...
    fn ldap_init_fd(fd: ber_socket_t, proto: i32, url: *const i8, ld: *mut *mut LDAP) -> i32;
}
...
pub struct sockbuf_io {
    pub sbi_setup: Option<unsafe extern "C" fn(*mut Sockbuf_IO_Desc, *mut libc::c_void) -> i32>,
    pub sbi_remove: Option<unsafe extern "C" fn(*mut Sockbuf_IO_Desc) -> i32>,
    pub sbi_ctrl: Option<unsafe extern "C" fn(*mut Sockbuf_IO_Desc, i32, *mut libc::c_void) -> i32>,
    pub sbi_read: Option<
        unsafe extern "C" fn(*mut Sockbuf_IO_Desc, *mut libc::c_void, ber_len_t) -> ber_slen_t,
    >,
...
#[no_mangle]
pub static mut Curl_handler_ldap: Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_it: Some(oldap_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: Some(
                oldap_done as unsafe extern "C" fn(*mut Curl_easy, CURLcode, bool) -> CURLcode,
            ),
            do_more: None,
            connect_it: Some(
                oldap_connect as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            connecting: Some(
                oldap_connecting as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            doing: None,
            proto_getsock: None,
            doing_getsock: None,
...
#[no_mangle]
pub static mut Curl_handler_ldaps: Curl_handler =  {
    {
        let mut init = Curl_handler {
...
extern "C" fn oldap_setup_connection(
    ...
    rc = unsafe { ldap_url_parse((*data).state.url, &mut lud) };
    if rc != 0 as i32 {
        let mut msg: *const i8 = b"url parsing problem\0" as *const u8 as *const i8;
        status = CURLE_URL_MALFORMAT;
        if rc > 0 as i32 && rc <= 0xa as i32 {
            if rc == 0x1 as i32 {
                status = CURLE_OUT_OF_MEMORY;
            }
            msg = unsafe { url_errs[rc as usize] };
        }
        (unsafe { Curl_failf(data, b"LDAP local: %s\0" as *const u8 as *const i8, msg) });
        return status;
    }
    proto = unsafe { ldap_pvt_url_scheme2proto((*lud).lud_scheme) };
    (unsafe { ldap_free_urldesc(lud) });
    li = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
    ...
    (unsafe { (*li).proto = proto });
    let fresh0 = unsafe { &mut ((*conn).proto.ldapc) };
    *fresh0 = li;
    (unsafe { Curl_conncontrol(conn, 0 as i32) });
    return CURLE_OK;
}
extern "C" fn oldap_connect(mut data: *mut Curl_easy, mut _done: *mut bool) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut li: *mut ldapconninfo = unsafe { (*conn).proto.ldapc };
    let mut rc: i32 = 0;
    let mut proto: i32 = 3 as i32;
    let mut hosturl: [i8; 1024] = [0; 1024];
    let mut ptr: *mut i8 = 0 as *mut i8;
    (unsafe { strcpy(hosturl.as_mut_ptr(), b"ldap\0" as *const u8 as *const i8) });
    ptr = unsafe { hosturl.as_mut_ptr().offset(4 as i32 as isize) };
    if (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 0 as i32) as u32 != 0 {
        let fresh1 = ptr;
        ptr = unsafe { ptr.offset(1) };
        (unsafe { *fresh1 = 's' as i32 as i8 });
    }
    (unsafe { curl_msnprintf(
        ptr,
        (::std::mem::size_of::<[i8; 1024]>() as u64)
            .wrapping_sub(ptr.offset_from(hosturl.as_mut_ptr()) as i64 as u64),
        b"://%s:%d\0" as *const u8 as *const i8,
        (*conn).host.name,
        (*conn).remote_port,
    ) });
    rc = unsafe { ldap_init_fd(
    ...
    if rc != 0 {
        (unsafe { Curl_failf(
            data,
            b"LDAP local: Cannot connect to %s, %s\0" as *const u8 as *const i8,
            hosturl.as_mut_ptr(),
            ldap_err2string(rc),
        ) });
        return CURLE_COULDNT_CONNECT;
    }
    (unsafe { ldap_set_option(
        (*li).ld,
        0x11 as i32,
    ...
    if (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 0 as i32) as u32 != 0 {
        ...
        result =
            unsafe { Curl_ssl_connect_nonblocking(data, conn, 0 as i32 != 0, 0 as i32, &mut (*li).ssldone) };
        if result as u64 != 0 {
            return result;
        }
    }
...
extern "C" fn oldap_connecting(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut li: *mut ldapconninfo = unsafe { (*conn).proto.ldapc };
    ...
    if (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 0 as i32) as u32 != 0 {
        if !(unsafe { (*li).ssldone }) {
            let mut result: CURLcode = unsafe { Curl_ssl_connect_nonblocking(
                data,
                conn,
                0 as i32 != 0,
                0 as i32,
            ...
            if result as u32 != 0 || !(unsafe { (*li).ssldone }) {
                return result;
            }
        }
        if !(unsafe { (*li).sslinst }) {
            let mut sb: *mut Sockbuf = 0 as *mut Sockbuf;
            (unsafe { ldap_get_option(
                (*li).ld,
                0x5008 as i32,
                &mut sb as *mut *mut Sockbuf as *mut libc::c_void,
            ) });
            (unsafe { ber_sockbuf_add_io(sb, &mut ldapsb_tls, 20 as i32, data as *mut libc::c_void) });
            (unsafe { (*li).sslinst = 1 as i32 != 0 });
            let fresh2 = unsafe { &mut ((*li).recv) };
            *fresh2 = unsafe { (*conn).recv[0 as i32 as usize] };
            let fresh3 = unsafe { &mut ((*li).send) };
            *fresh3 = unsafe { (*conn).send[0 as i32 as usize] };
        }
    }
    tvp = &mut tv;
    loop {
        if !(unsafe { (*li).didbind }) {
            ...
            if (unsafe { ((*conn).bits).user_passwd() }) != 0 {
                binddn = unsafe { (*conn).user };
                passwd.bv_val = unsafe { (*conn).passwd };
                passwd.bv_len = unsafe { strlen(passwd.bv_val) };
            } else {
                binddn = 0 as *mut i8;
                passwd.bv_val = 0 as *mut i8;
                passwd.bv_len = 0 as i32 as ber_len_t;
            }
            rc = unsafe { ldap_sasl_bind(
                (*li).ld,
                binddn,
                0 as *mut i8,
                &mut passwd,
            ...
            (unsafe { (*li).didbind = 1 as i32 != 0 });
            if !tvp.is_null() {
                return CURLE_OK;
            }
        }
        rc = unsafe { ldap_result((*li).ld, (*li).msgid, 0 as i32, tvp, &mut msg) };
        if rc < 0 as i32 {
            (unsafe { Curl_failf(
                data,
        ...
        if rc != 0 {
            (unsafe { Curl_failf(
                data,
                b"LDAP local: bind ldap_parse_result %s\0" as *const u8 as *const i8,
                ldap_err2string(rc),
            ) });
            return CURLE_LDAP_CANNOT_BIND;
        }
        ...
        if !info.is_null() {
            (unsafe { ldap_memfree(info as *mut libc::c_void) });
            info = 0 as *mut i8;
        }
        proto = 2 as i32;
        (unsafe { ldap_set_option(
    ...
    let fresh4 = unsafe { &mut ((*conn).recv[0 as i32 as usize]) };
    *fresh4 = Some(oldap_recv as Curl_recv);
    (unsafe { *done = 1 as i32 != 0 });
    return CURLE_OK;
}
extern "C" fn oldap_disconnect(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    ...
    if !li.is_null() {
        if !(unsafe { (*li).ld }).is_null() {
            ...
            (unsafe { ldap_unbind_ext(
                (*li).ld,
                0 as *mut *mut LDAPControl,
                0 as *mut *mut LDAPControl,
            ) });
            let fresh5 = unsafe { &mut ((*li).ld) };
            *fresh5 = 0 as *mut LDAP;
        }
        let fresh6 = unsafe { &mut ((*conn).proto.ldapc) };
        *fresh6 = 0 as *mut ldapconninfo;
        (unsafe { Curl_cfree.expect("non-null function pointer")(li as *mut libc::c_void) });
    }
    return CURLE_OK;
}
extern "C" fn oldap_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut li: *mut ldapconninfo = unsafe { (*conn).proto.ldapc };
    ...
    rc = unsafe { ldap_url_parse((*data).state.url, &mut ludp) };
    if rc != 0 as i32 {
        let mut msg: *const i8 = b"url parsing problem\0" as *const u8 as *const i8;
        status = CURLE_URL_MALFORMAT;
        if rc > 0 as i32 && rc <= 0xa as i32 {
    ...
    lr = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
    ...
    (unsafe { (*lr).msgid = msgid });
    let fresh7 = unsafe { &mut ((*data).req.p.ldap) };
    *fresh7 = lr;
...
extern "C" fn oldap_done(
    mut data: *mut Curl_easy,
    mut _res: CURLcode,
    mut _premature: bool,
) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut lr: *mut ldapreqinfo = unsafe { (*data).req.p.ldap };
    if !lr.is_null() {
        if (unsafe { (*lr).msgid }) != 0 {
            let mut li: *mut ldapconninfo = unsafe { (*conn).proto.ldapc };
            (unsafe { ldap_abandon_ext(
                (*li).ld,
                (*lr).msgid,
                0 as *mut *mut LDAPControl,
                0 as *mut *mut LDAPControl,
            ) });
            (unsafe { (*lr).msgid = 0 as i32 });
        }
        let fresh8 = unsafe { &mut ((*data).req.p.ldap) };
        *fresh8 = 0 as *mut ldapreqinfo;
        (unsafe { Curl_cfree.expect("non-null function pointer")(lr as *mut libc::c_void) });
    }
    return CURLE_OK;
}
extern "C" fn oldap_recv(
    mut data: *mut Curl_easy,
    mut _sockindex: i32,
    mut _buf: *mut i8,
    mut _len: size_t,
    ...
    rc = unsafe { ldap_result((*li).ld, (*lr).msgid, 0x2 as i32, &mut tv, &mut msg) };
    if rc < 0 as i32 {
        (unsafe { Curl_failf(
            data,
    ...
    ent = unsafe { ldap_first_message((*li).ld, msg) };
    while !ent.is_null() {
        ...
        msgtype = unsafe { ldap_msgtype(ent) };
        if msgtype as u64 == 0x65 as u32 as ber_tag_t {
            ...
            if rc != 0 {
                (unsafe { Curl_failf(
                    data,
                    b"LDAP local: search ldap_parse_result %s\0" as *const u8 as *const i8,
                    ldap_err2string(rc),
                ) });
                (unsafe { *err = CURLE_LDAP_SEARCH_FAILED });
            } else if code != 0 && code != 0x4 as i32 {
                (unsafe { Curl_failf(
                    data,
                    b"LDAP remote: search failed %s %s\0" as *const u8 as *const i8,
                    ldap_err2string(rc),
                    if !info.is_null() {
                        info as *const i8
                    } else {
                ...
                if code == 0x4 as i32 {
                    (unsafe { Curl_infof(
                        data,
                        b"There are more than %d entries\0" as *const u8 as *const i8,
                        (*lr).nument,
                    ) });
                }
                (unsafe { (*data).req.size = (*data).req.bytecount });
                (unsafe { *err = CURLE_OK });
                ret = 0 as i32;
            }
            (unsafe { (*lr).msgid = 0 as i32 });
            ...
            if !(msgtype as u64 != 0x64 as u32 as ber_tag_t) {
                let fresh9 = unsafe { &mut ((*lr).nument) };
                *fresh9 += 1;
                rc = unsafe { ldap_get_dn_ber((*li).ld, ent, &mut ber, &mut bv) };
                if rc < 0 as i32 {
                    (unsafe { *err = CURLE_RECV_ERROR });
                    return -(1 as i32) as ssize_t;
                }
                writeerr = unsafe { Curl_client_write(
                    data,
                    (1 as i32) << 0 as i32,
                ...
                writeerr = unsafe { Curl_client_write(data, (1 as i32) << 0 as i32, bv.bv_val, bv.bv_len) };
                if writeerr as u64 != 0 {
                    (unsafe { *err = writeerr });
                    return -(1 as i32) as ssize_t;
                }
                ...
                *fresh10 = (*fresh10 as u64).wrapping_add((bv.bv_len).wrapping_add(5 as i32 as u64))
                    as curl_off_t as curl_off_t;
                rc = unsafe { ldap_get_attribute_ber((*li).ld, ent, ber, &mut bv, &mut bvals) };
                while rc == 0 as i32 {
                    let mut i: i32 = 0;
                    ...
                    if bvals.is_null() {
                        ...
                        writeerr =
                            unsafe { Curl_client_write(data, (1 as i32) << 0 as i32, bv.bv_val, bv.bv_len) };
                        if writeerr as u64 != 0 {
                            (unsafe { *err = writeerr });
                            return -(1 as i32) as ssize_t;
                        }
                        ...
                        *fresh11 = (*fresh11 as u64)
                            .wrapping_add((bv.bv_len).wrapping_add(3 as i32 as u64))
                            as curl_off_t as curl_off_t;
                    } else {
                        i = 0 as i32;
                        while !(unsafe { (*bvals.offset(i as isize)).bv_val }).is_null() {
                            ...
                            *fresh12 = (*fresh12 as u64)
                                .wrapping_add((bv.bv_len).wrapping_add(2 as i32 as u64))
                                as curl_off_t as curl_off_t;
                            if binary == 0 {
                                if (unsafe { Curl_isspace(
                                    *((*bvals.offset(i as isize)).bv_val).offset(0 as i32 as isize)
                                        as u8 as i32,
                                ) }) != 0
                                    || (unsafe { Curl_isspace(
                                        *((*bvals.offset(i as isize)).bv_val).offset(
                                    ...
                                    ) }) != 0
                                {
                                    ...
                                    let mut j: u32 = 0;
                                    j = 0 as i32 as u32;
                                    while (j as u64) < (unsafe { (*bvals.offset(i as isize)).bv_len }) {
                                        if (unsafe { Curl_isprint(
                            ...
                            if binary != 0 || binval != 0 {
                                ...
                                let mut error: CURLcode = unsafe { Curl_base64_encode(
                                    data,
                                ...
                                if error as u64 != 0 {
                                    (unsafe { ber_memfree(bvals as *mut libc::c_void) });
                                    (unsafe { ber_free(ber, 0 as i32) });
                                    (unsafe { ldap_msgfree(msg) });
                                    (unsafe { *err = error });
                                    return -(1 as i32) as ssize_t;
                                }
                                writeerr = unsafe { Curl_client_write(
                                ...
                                let fresh13 = unsafe { &mut ((*data).req.bytecount) };
                                *fresh13 += 2 as i32 as i64;
                                if val_b64_sz > 0 as i32 as u64 {
                                    ...
                                    let fresh14 = unsafe { &mut ((*data).req.bytecount) };
                                    *fresh14 = (*fresh14 as u64).wrapping_add(val_b64_sz)
                                        as curl_off_t
                                        as curl_off_t;
                                }
                            } else {
                                ...
                                let fresh15 = unsafe { &mut ((*data).req.bytecount) };
                                *fresh15 = (*fresh15 as u64).wrapping_add(
                                    (unsafe { (*bvals.offset(i as isize)).bv_len })
                            ...
                            let fresh16 = unsafe { &mut ((*data).req.bytecount) };
                            *fresh16 += 1;
                            i += 1;
                        }
                        (unsafe { ber_memfree(bvals as *mut libc::c_void) });
                        ...
                        let fresh17 = unsafe { &mut ((*data).req.bytecount) };
                        *fresh17 += 1;
                    }
                    rc = unsafe { ldap_get_attribute_ber((*li).ld, ent, ber, &mut bv, &mut bvals) };
                }
                ...
                let fresh18 = unsafe { &mut ((*data).req.bytecount) };
                *fresh18 += 1;
                (unsafe { ber_free(ber, 0 as i32) });
            }
            ent = unsafe { ldap_next_message((*li).ld, ent) };
        }
    }
    (unsafe { ldap_msgfree(msg) });
    return ret as ssize_t;
}
extern "C" fn ldapsb_tls_setup(mut sbiod: *mut Sockbuf_IO_Desc, mut arg: *mut libc::c_void) -> i32 {
    let fresh19 = unsafe { &mut ((*sbiod).sbiod_pvt) };
    *fresh19 = arg;
    return 0 as i32;
}
extern "C" fn ldapsb_tls_remove(mut sbiod: *mut Sockbuf_IO_Desc) -> i32 {
    let fresh20 = unsafe { &mut ((*sbiod).sbiod_pvt) };
    *fresh20 = 0 as *mut libc::c_void;
    return 0 as i32;
}
extern "C" fn ldapsb_tls_close(mut _sbiod: *mut Sockbuf_IO_Desc) -> i32 {
    return 0 as i32;
}
extern "C" fn ldapsb_tls_ctrl(
    mut sbiod: *mut Sockbuf_IO_Desc,
    mut opt: i32,
    mut _arg: *mut libc::c_void,
) -> i32 {
    if opt == 8 as i32 {
        let mut data: *mut Curl_easy = (unsafe { (*sbiod).sbiod_pvt }) as *mut Curl_easy;
        return (unsafe { Curl_ssl_data_pending((*data).conn, 0 as i32) }) as i32;
    }
    return 0 as i32;
}
extern "C" fn ldapsb_tls_read(
    mut sbiod: *mut Sockbuf_IO_Desc,
    ...
    if !data.is_null() {
        ...
        if !conn.is_null() {
            ...
            ret = unsafe { ((*li).recv).expect("non-null function pointer")(
                data,
                0 as i32,
                buf as *mut i8,
                len,
            ...
            if ret < 0 as i32 as i64 && err as u32 == CURLE_AGAIN as i32 as u32 {
...
extern "C" fn ldapsb_tls_write(
    ...
    if !data.is_null() {
        ...
        if !conn.is_null() {
            ...
            ret = unsafe { ((*li).send).expect("non-null function pointer")(
...
static mut ldapsb_tls: Sockbuf_IO =  {
    {
        let mut init = sockbuf_io {
            sbi_setup: Some(
                ldapsb_tls_setup
                    as unsafe extern "C" fn(*mut Sockbuf_IO_Desc, *mut libc::c_void) -> i32,
            ),
            sbi_remove: Some(
                ldapsb_tls_remove as unsafe extern "C" fn(*mut Sockbuf_IO_Desc) -> i32,
            ),
            sbi_ctrl: Some(
                ldapsb_tls_ctrl
                    as unsafe extern "C" fn(*mut Sockbuf_IO_Desc, i32, *mut libc::c_void) -> i32,
            ),
            sbi_read: Some(
                ldapsb_tls_read
            ...
            sbi_close: Some(ldapsb_tls_close as unsafe extern "C" fn(*mut Sockbuf_IO_Desc) -> i32),
        };
        init
    }
};
