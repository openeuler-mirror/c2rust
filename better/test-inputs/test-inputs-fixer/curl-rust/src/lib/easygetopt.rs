...
extern "C" fn lookup(mut name: *const i8, mut id: CURLoption) -> *mut curl_easyoption {
    if !name.is_null() || id as u32 != 0 {
        let mut o: *mut curl_easyoption =
            (unsafe { &mut *Curl_easyopts.as_mut_ptr().offset(0 as i32 as isize) }) as *mut curl_easyoption;
        loop {
            if !name.is_null() {
                if (unsafe { Curl_strcasecompare((*o).name, name) }) != 0 {
                    return o;
                }
            } else if (unsafe { (*o).id }) as u32 == id as u32
                && (unsafe { (*o).flags }) & ((1 as i32) << 0 as i32) as u32 == 0
            {
                return o;
            }
            o = unsafe { o.offset(1) };
            if (unsafe { (*o).name }).is_null() {
                break;
            }
        }
    }
...
#[no_mangle]
pub extern "C" fn curl_easy_option_by_name(mut name: *const i8) -> *const curl_easyoption {
    return lookup(name, CURLOPT_LASTENTRY);
}
#[no_mangle]
pub extern "C" fn curl_easy_option_by_id(mut id: CURLoption) -> *const curl_easyoption {
...
pub extern "C" fn curl_easy_option_next(
    ...
    if !prev.is_null() && !(unsafe { (*prev).name }).is_null() {
        prev = unsafe { prev.offset(1) };
        if !(unsafe { (*prev).name }).is_null() {
            return prev;
        }
    } else if prev.is_null() {
        return (unsafe { &mut *Curl_easyopts.as_mut_ptr().offset(0 as i32 as isize) }) as *mut curl_easyoption;
    }
