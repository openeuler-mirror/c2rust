...
extern "C" {
    ...
    fn inflateInit_(strm: z_streamp, version: *const i8, stream_size: i32) -> i32;
    fn inflateInit2_(strm: z_streamp, windowBits: i32, version: *const i8, stream_size: i32)
        -> i32;
...
extern "C" fn zalloc_cb(mut _opaque: voidpf, mut items: u32, mut size: u32) -> voidpf {
    return unsafe { Curl_ccalloc.expect("non-null function pointer")(items as size_t, size as size_t) };
}
extern "C" fn zfree_cb(mut _opaque: voidpf, mut ptr: voidpf) {
    (unsafe { Curl_cfree.expect("non-null function pointer")(ptr) });
}
extern "C" fn process_zlib_error(mut data: *mut Curl_easy, mut z: *mut z_stream) -> CURLcode {
...
extern "C" fn exit_zlib(
    mut data: *mut Curl_easy,
    mut z: *mut z_stream,
    mut zlib_init: *mut zlibInitState,
    mut result: CURLcode,
) -> CURLcode {
    if (unsafe { *zlib_init }) as u32 == ZLIB_GZIP_HEADER as i32 as u32 {
        (unsafe { Curl_cfree.expect("non-null function pointer")((*z).next_in as *mut libc::c_void) });
        let fresh0 = unsafe { &mut ((*z).next_in) };
        *fresh0 = 0 as *mut Bytef;
    }
    if (unsafe { *zlib_init }) as u32 != ZLIB_UNINIT as i32 as u32 {
        if (unsafe { inflateEnd(z) }) != 0 as i32 && result as u32 == CURLE_OK as i32 as u32 {
            result = process_zlib_error(data, z);
        }
        (unsafe { *zlib_init = ZLIB_UNINIT });
    }
    return result;
}
extern "C" fn process_trailer(mut data: *mut Curl_easy, mut zp: *mut zlib_params) -> CURLcode {
    let mut z: *mut z_stream = unsafe { &mut (*zp).z };
    let mut result: CURLcode = CURLE_OK;
    let mut len: uInt = if (unsafe { (*z).avail_in }) < (unsafe { (*zp).trailerlen }) {
        unsafe { (*z).avail_in }
    } else {
        unsafe { (*zp).trailerlen }
    };
    let fresh1 = unsafe { &mut ((*zp).trailerlen) };
    *fresh1 = (*fresh1 as u32).wrapping_sub(len) as uInt as uInt;
    let fresh2 = unsafe { &mut ((*z).avail_in) };
    *fresh2 = (*fresh2 as u32).wrapping_sub(len) as uInt as uInt;
    let fresh3 = unsafe { &mut ((*z).next_in) };
    *fresh3 = unsafe { (*fresh3).offset(len as isize) };
    if (unsafe { (*z).avail_in }) != 0 {
        result = CURLE_WRITE_ERROR;
    }
    if result as u32 != 0 || (unsafe { (*zp).trailerlen }) == 0 {
        result = exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, result);
    } else {
...
extern "C" fn inflate_stream(
    ...
    let mut zp: *mut zlib_params =
        (unsafe { &mut (*writer).params }) as *mut *mut libc::c_void as *mut zlib_params;
    let mut z: *mut z_stream = unsafe { &mut (*zp).z };
    let mut nread: uInt = unsafe { (*z).avail_in };
    let mut orig_in: *mut Bytef = unsafe { (*z).next_in };
    ...
    {
        return exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, CURLE_WRITE_ERROR);
    }
    decomp = (unsafe { Curl_cmalloc.expect("non-null function pointer")(16384 as i32 as size_t) }) as *mut i8;
    if decomp.is_null() {
        return exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, CURLE_OUT_OF_MEMORY);
    }
    while !done {
        let mut status: i32 = 0;
        done = 1 as i32 != 0;
        let fresh4 = unsafe { &mut ((*z).next_out) };
        *fresh4 = decomp as *mut Bytef;
        ...
        if (unsafe { (*z).avail_out }) != 16384 as i32 as u32 {
            if status == 0 as i32 || status == 1 as i32 {
                ...
                if result as u64 != 0 {
                    exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, result);
                    break;
                }
            }
        }
        ...
        match status {
            ...
            -3 => {
                if (unsafe { (*zp).zlib_init }) as u32 == ZLIB_INIT as i32 as u32 {
                    ...
                    {
                        let fresh5 = unsafe { &mut ((*z).next_in) };
                        *fresh5 = orig_in;
        ...
        match current_block_21 {
            13757731702203840573 => {
                result = exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, process_zlib_error(data, z));
            }
            _ => {}
        }
    }
...
extern "C" fn deflate_init_writer(
    ...
    let fresh6 = unsafe { &mut ((*z).zalloc) };
    *fresh6 = unsafe { ::std::mem::transmute::<
        Option<unsafe extern "C" fn(voidpf, u32, u32) -> voidpf>,
        alloc_func,
    >(Some(
        zalloc_cb as unsafe extern "C" fn(voidpf, u32, u32) -> voidpf,
    )) };
    let fresh7 = unsafe { &mut ((*z).zfree) };
    *fresh7 = unsafe { ::std::mem::transmute::<Option<unsafe extern "C" fn(voidpf, voidpf) -> ()>, free_func>(
...
extern "C" fn deflate_unencode_write(
    ...
    let mut zp: *mut zlib_params =
        (unsafe { &mut (*writer).params }) as *mut *mut libc::c_void as *mut zlib_params;
    let mut z: *mut z_stream = unsafe { &mut (*zp).z };
    let fresh8 = unsafe { &mut ((*z).next_in) };
...
extern "C" fn deflate_close_writer(mut data: *mut Curl_easy, mut writer: *mut contenc_writer) {
    let mut zp: *mut zlib_params =
        (unsafe { &mut (*writer).params }) as *mut *mut libc::c_void as *mut zlib_params;
    let mut z: *mut z_stream = unsafe { &mut (*zp).z };
    exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, CURLE_OK);
}
static mut deflate_encoding: content_encoding =  {
    {
        let mut init = content_encoding {
            ...
            init_writer: Some(
                deflate_init_writer
                    as unsafe extern "C" fn(*mut Curl_easy, *mut contenc_writer) -> CURLcode,
            ),
            unencode_write: Some(
...
extern "C" fn gzip_init_writer(
    ...
    let fresh9 = unsafe { &mut ((*z).zalloc) };
    *fresh9 = unsafe { ::std::mem::transmute::<
        Option<unsafe extern "C" fn(voidpf, u32, u32) -> voidpf>,
        alloc_func,
    >(Some(
        zalloc_cb as unsafe extern "C" fn(voidpf, u32, u32) -> voidpf,
    )) };
    let fresh10 = unsafe { &mut ((*z).zfree) };
    *fresh10 = unsafe { ::std::mem::transmute::<Option<unsafe extern "C" fn(voidpf, voidpf) -> ()>, free_func>(
        Some(zfree_cb as unsafe extern "C" fn(voidpf, voidpf) -> ()),
    ) };
...
extern "C" fn gzip_unencode_write(
    ...
    if (unsafe { (*zp).zlib_init }) as u32 == ZLIB_INIT_GZIP as i32 as u32 {
        let fresh11 = unsafe { &mut ((*z).next_in) };
    ...
    match (unsafe { (*zp).zlib_init }) as u32 {
        1 => {
            ...
            match check_gzip_header(buf as *mut u8, nbytes as ssize_t, &mut hlen) as u32 {
                0 => {
                    let fresh12 = unsafe { &mut ((*z).next_in) };
                ...
                2 => {
                    (unsafe { (*z).avail_in = nbytes as uInt });
                    let fresh13 = unsafe { &mut ((*z).next_in) };
                    *fresh13 =
                ...
                1 | _ => {
                    return exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, process_zlib_error(data, z));
                }
            }
        }
        4 => {
            let mut hlen_0: ssize_t = 0;
            let fresh14 = unsafe { &mut ((*z).avail_in) };
            *fresh14 = (*fresh14 as u32).wrapping_add(nbytes as uInt) as uInt as uInt;
            let fresh15 = unsafe { &mut ((*z).next_in) };
            *fresh15 = (unsafe { Curl_saferealloc((*z).next_in as *mut libc::c_void, (*z).avail_in as size_t) })
                as *mut Bytef;
            if (unsafe { (*z).next_in }).is_null() {
                return exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, CURLE_OUT_OF_MEMORY);
            }
            ...
            match check_gzip_header(unsafe { (*z).next_in }, (unsafe { (*z).avail_in }) as ssize_t, &mut hlen_0) as u32 {
                0 => {
                    (unsafe { Curl_cfree.expect("non-null function pointer")(
                        (*z).next_in as *mut libc::c_void,
                    ) });
                    let fresh16 = unsafe { &mut ((*z).next_in) };
        ...
        3 => {
            let fresh17 = unsafe { &mut ((*z).next_in) };
            *fresh17 = buf as *mut Bytef;
            (unsafe { (*z).avail_in = nbytes as uInt });
            return process_trailer(data, zp);
        }
        5 | _ => {
            let fresh18 = unsafe { &mut ((*z).next_in) };
...
extern "C" fn gzip_close_writer(mut data: *mut Curl_easy, mut writer: *mut contenc_writer) {
    let mut zp: *mut zlib_params =
        (unsafe { &mut (*writer).params }) as *mut *mut libc::c_void as *mut zlib_params;
    let mut z: *mut z_stream = unsafe { &mut (*zp).z };
    exit_zlib(data, z, unsafe { &mut (*zp).zlib_init }, CURLE_OK);
}
static mut gzip_encoding: content_encoding =  {
    {
        let mut init = content_encoding {
...
extern "C" fn identity_close_writer(mut _data: *mut Curl_easy, mut _writer: *mut contenc_writer) {}
static mut identity_encoding: content_encoding =  {
    {
        let mut init = content_encoding {
...
pub extern "C" fn Curl_all_content_encodings() -> *mut i8 {
    ...
    while !(unsafe { *cep }).is_null() {
        ce = unsafe { *cep };
        if (unsafe { Curl_strcasecompare((*ce).name, b"identity\0" as *const u8 as *const i8) }) == 0 {
...
extern "C" fn client_unencode_write(
    mut data: *mut Curl_easy,
    mut _writer: *mut contenc_writer,
    mut buf: *const i8,
    mut nbytes: size_t,
) -> CURLcode {
    let mut k: *mut SingleRequest = unsafe { &mut (*data).req };
    if nbytes == 0 || (unsafe { (*k).ignorebody() }) as i32 != 0 {
        return CURLE_OK;
    }
    return unsafe { Curl_client_write(data, (1 as i32) << 0 as i32, buf as *mut i8, nbytes) };
}
extern "C" fn client_close_writer(mut _data: *mut Curl_easy, mut _writer: *mut contenc_writer) {}
static mut client_encoding: content_encoding =  {
    {
        let mut init = content_encoding {
            name: 0 as *const i8,
            alias: 0 as *const i8,
...
extern "C" fn error_close_writer(mut _data: *mut Curl_easy, mut _writer: *mut contenc_writer) {}
static mut error_encoding: content_encoding =  {
    {
        let mut init = content_encoding {
            name: 0 as *const i8,
            alias: 0 as *const i8,
...
extern "C" fn new_unencoding_writer(
    ...
    let mut sz: size_t = (16 as u64).wrapping_add(unsafe { (*handler).paramsize });
    let mut writer: *mut contenc_writer =
        (unsafe { Curl_ccalloc.expect("non-null function pointer")(1 as i32 as size_t, sz) })
            as *mut contenc_writer;
    if !writer.is_null() {
        let fresh21 = unsafe { &mut ((*writer).handler) };
        *fresh21 = handler;
        let fresh22 = unsafe { &mut ((*writer).downstream) };
        *fresh22 = downstream;
...
#[no_mangle]
pub extern "C" fn Curl_unencode_cleanup(mut data: *mut Curl_easy) {
    let mut k: *mut SingleRequest = unsafe { &mut (*data).req };
    let mut writer: *mut contenc_writer = unsafe { (*k).writer_stack };
    while !writer.is_null() {
        let fresh23 = unsafe { &mut ((*k).writer_stack) };
        *fresh23 = unsafe { (*writer).downstream };
        (unsafe { ((*(*writer).handler).close_writer).expect("non-null function pointer")(data, writer) });
        (unsafe { Curl_cfree.expect("non-null function pointer")(writer as *mut libc::c_void) });
        writer = unsafe { (*k).writer_stack };
    }
}
extern "C" fn find_encoding(mut name: *const i8, mut len: size_t) -> *const content_encoding {
    let mut cep: *const *const content_encoding = 0 as *const *const content_encoding;
    cep = unsafe { encodings.as_ptr() };
    while !(unsafe { *cep }).is_null() {
        let mut ce: *const content_encoding = unsafe { *cep };
        if (unsafe { Curl_strncasecompare(name, (*ce).name, len) }) != 0
            && (unsafe { *((*ce).name).offset(len as isize) }) == 0
            || !(unsafe { (*ce).alias }).is_null()
                && (unsafe { Curl_strncasecompare(name, (*ce).alias, len) }) != 0
...
pub extern "C" fn Curl_build_unencoding_stack(
    ...
    loop {
        ...
        while (unsafe { Curl_isspace(*enclist as u8 as i32) }) != 0 || (unsafe { *enclist }) as i32 == ',' as i32 {
            enclist = unsafe { enclist.offset(1) };
        }
        name = enclist;
        namelen = 0 as i32 as size_t;
        while (unsafe { *enclist }) as i32 != 0 && (unsafe { *enclist }) as i32 != ',' as i32 {
            if (unsafe { Curl_isspace(*enclist as u8 as i32) }) == 0 {
        ...
        {
            (unsafe { (*k).set_chunk(1 as i32 as bit) });
            ...
            if (unsafe { (*k).writer_stack }).is_null() {
                let fresh24 = unsafe { &mut ((*k).writer_stack) };
            ...
            writer = new_unencoding_writer(data, encoding, unsafe { (*k).writer_stack });
            if writer.is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
            let fresh25 = unsafe { &mut ((*k).writer_stack) };
            *fresh25 = writer;
        }
        if !((unsafe { *enclist }) != 0) {
            break;
