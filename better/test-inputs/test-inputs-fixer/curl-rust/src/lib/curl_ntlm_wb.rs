...
extern "C" fn ntlm_wb_cleanup(mut ntlm: *mut ntlmdata) {
    if (unsafe { (*ntlm).ntlm_auth_hlpr_socket }) != -(1 as i32) {
        (unsafe { close((*ntlm).ntlm_auth_hlpr_socket) });
        (unsafe { (*ntlm).ntlm_auth_hlpr_socket = -(1 as i32) });
    }
    if (unsafe { (*ntlm).ntlm_auth_hlpr_pid }) != 0 {
        let mut i: i32 = 0;
        i = 0 as i32;
        while i < 4 as i32 {
            let mut ret: pid_t = unsafe { waitpid((*ntlm).ntlm_auth_hlpr_pid, 0 as *mut i32, 1 as i32) };
            if ret == (unsafe { (*ntlm).ntlm_auth_hlpr_pid }) || (unsafe { *__errno_location() }) == 10 as i32 {
                break;
            }
            match i {
                0 => {
                    (unsafe { kill((*ntlm).ntlm_auth_hlpr_pid, 15 as i32) });
                }
                1 => {
                    (unsafe { Curl_wait_ms(1 as i32 as timediff_t) });
                }
                2 => {
                    (unsafe { kill((*ntlm).ntlm_auth_hlpr_pid, 9 as i32) });
                }
                3 | _ => {}
            }
            i += 1;
        }
        (unsafe { (*ntlm).ntlm_auth_hlpr_pid = 0 as i32 });
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")((*ntlm).challenge as *mut libc::c_void) });
    let fresh0 = unsafe { &mut ((*ntlm).challenge) };
    *fresh0 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*ntlm).response as *mut libc::c_void) });
    let fresh1 = unsafe { &mut ((*ntlm).response) };
    *fresh1 = 0 as *mut i8;
}
extern "C" fn ntlm_wb_init(
    ...
    if (unsafe { (*ntlm).ntlm_auth_hlpr_socket }) != -(1 as i32) || (unsafe { (*ntlm).ntlm_auth_hlpr_pid }) != 0 {
        return CURLE_OK;
    }
    username = userp;
    if username.is_null() || (unsafe { *username.offset(0 as i32 as isize) }) == 0 {
        username = unsafe { getenv(b"NTLMUSER\0" as *const u8 as *const i8) };
        if username.is_null() || (unsafe { *username.offset(0 as i32 as isize) }) == 0 {
            username = unsafe { getenv(b"LOGNAME\0" as *const u8 as *const i8) };
        }
        if username.is_null() || (unsafe { *username.offset(0 as i32 as isize) }) == 0 {
            username = unsafe { getenv(b"USER\0" as *const u8 as *const i8) };
        }
        if (username.is_null() || (unsafe { *username.offset(0 as i32 as isize) }) == 0)
            && (unsafe { getpwuid_r(
                geteuid(),
    ...
    slash = unsafe { strpbrk(username, b"\\/\0" as *const u8 as *const i8) };
    if !slash.is_null() {
        domain = unsafe { Curl_cstrdup.expect("non-null function pointer")(username) };
        if domain.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        slash = unsafe { domain.offset(slash.offset_from(username) as i64 as isize) };
        (unsafe { *slash = '\u{0}' as i32 as i8 });
    ...
    if (unsafe { access(ntlm_auth, 1 as i32) }) != 0 as i32 {
        (unsafe { Curl_failf(
            data,
            b"Could not access ntlm_auth: %s errno %d: %s\0" as *const u8 as *const i8,
            ntlm_auth,
            *__errno_location(),
            Curl_strerror(
                *__errno_location(),
    ...
    } else if (unsafe { socketpair(1 as i32, SOCK_STREAM as i32, 0 as i32, sockfds.as_mut_ptr()) }) != 0 {
        (unsafe { Curl_failf(
            data,
            b"Could not open socket pair. errno %d: %s\0" as *const u8 as *const i8,
            *__errno_location(),
            Curl_strerror(
                *__errno_location(),
                buffer.as_mut_ptr(),
        ...
        child_pid = unsafe { fork() };
        if child_pid == -(1 as i32) {
            (unsafe { close(sockfds[0 as i32 as usize]) });
            (unsafe { close(sockfds[1 as i32 as usize]) });
            (unsafe { Curl_failf(
                data,
            ...
            if child_pid == 0 {
                (unsafe { close(sockfds[0 as i32 as usize]) });
                if (unsafe { dup2(sockfds[1 as i32 as usize], 0 as i32) }) == -(1 as i32) {
                    (unsafe { Curl_failf(
                        data,
                        b"Could not redirect child stdin. errno %d: %s\0" as *const u8 as *const i8,
                        *__errno_location(),
                        Curl_strerror(
                            *__errno_location(),
                            buffer.as_mut_ptr(),
                ...
                if (unsafe { dup2(sockfds[1 as i32 as usize], 1 as i32) }) == -(1 as i32) {
                    (unsafe { Curl_failf(
                        data,
                ...
                (unsafe { Curl_failf(
                    data,
                    b"Could not execl(). errno %d: %s\0" as *const u8 as *const i8,
                    *__errno_location(),
                    Curl_strerror(
                        *__errno_location(),
                        buffer.as_mut_ptr(),
            ...
            (unsafe { close(sockfds[1 as i32 as usize]) });
            (unsafe { (*ntlm).ntlm_auth_hlpr_socket = sockfds[0 as i32 as usize] });
            (unsafe { (*ntlm).ntlm_auth_hlpr_pid = child_pid });
            (unsafe { Curl_cfree.expect("non-null function pointer")(domain as *mut libc::c_void) });
            (unsafe { Curl_cfree.expect("non-null function pointer")(ntlm_auth_alloc as *mut libc::c_void) });
            return CURLE_OK;
        }
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")(domain as *mut libc::c_void) });
...
extern "C" fn ntlm_wb_response(
    ...
    let mut len_in: size_t = unsafe { strlen(input) };
    ...
    let mut buf: *mut u8 = (unsafe { (*data).state.buffer }) as *mut u8;
    (unsafe { Curl_dyn_init(&mut b, 100000 as i32 as size_t) });
    loop {
        if !(len_in > 0 as i32 as u64) {
            current_block = 11812396948646013369;
            break;
        }
        let mut written: ssize_t = unsafe { send(
        ...
        if written == -(1 as i32) as i64 {
            ...
            input = unsafe { input.offset(written as isize) };
            len_in = (len_in as u64).wrapping_sub(written as u64) as size_t as size_t;
        }
    }
    match current_block {
        11812396948646013369 => {
            loop {
                let mut size: ssize_t = unsafe { recv(
                    (*ntlm).ntlm_auth_hlpr_socket,
                ...
                if size == -(1 as i32) as i64 {
                    ...
                    if (unsafe { Curl_dyn_addn(&mut b, buf as *const libc::c_void, size as size_t) }) as u64 != 0
                    {
                        current_block = 119169402052537339;
                        break;
                    }
                    len_out = unsafe { Curl_dyn_len(&mut b) };
                    ptr = unsafe { Curl_dyn_ptr(&mut b) };
                    if !(len_out != 0
                        && (unsafe { *ptr.offset(len_out.wrapping_sub(1 as i32 as u64) as isize) }) as i32
                            == '\n' as i32)
                    {
                        continue;
                    }
                    (unsafe { *ptr.offset(len_out.wrapping_sub(1 as i32 as u64) as isize) =
                        '\u{0}' as i32 as i8 });
            ...
            match current_block {
                ...
                _ => {
                    if !(state as u32 == NTLMSTATE_TYPE1 as i32 as u32
                        ...
                        && (unsafe { *ptr.offset(0 as i32 as isize) }) as i32 == 'P' as i32
                        && (unsafe { *ptr.offset(1 as i32 as isize) }) as i32 == 'W' as i32)
                    {
                        if !(len_out < 4 as i32 as u64) {
                            if !(state as u32 == NTLMSTATE_TYPE1 as i32 as u32
                                && ((unsafe { *ptr.offset(0 as i32 as isize) }) as i32 != 'Y' as i32
                                    || (unsafe { *ptr.offset(1 as i32 as isize) }) as i32 != 'R' as i32
                                    || (unsafe { *ptr.offset(2 as i32 as isize) }) as i32 != ' ' as i32))
                            {
                                if !(state as u32 == NTLMSTATE_TYPE2 as i32 as u32
                                    && ((unsafe { *ptr.offset(0 as i32 as isize) }) as i32 != 'K' as i32
                                        || (unsafe { *ptr.offset(1 as i32 as isize) }) as i32 != 'K' as i32
                                        || (unsafe { *ptr.offset(2 as i32 as isize) }) as i32 != ' ' as i32)
                                    && ((unsafe { *ptr.offset(0 as i32 as isize) }) as i32 != 'A' as i32
                                        || (unsafe { *ptr.offset(1 as i32 as isize) }) as i32 != 'F' as i32
                                        || (unsafe { *ptr.offset(2 as i32 as isize) }) as i32 != ' ' as i32))
                                {
                                    let fresh2 = unsafe { &mut ((*ntlm).response) };
                                    *fresh2 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                                        ptr.offset(3 as i32 as isize),
                                    ) };
                                    (unsafe { Curl_dyn_free(&mut b) });
                                    if (unsafe { (*ntlm).response }).is_null() {
                                        return CURLE_OUT_OF_MEMORY;
                                    }
                                    return CURLE_OK;
                                }
...
#[no_mangle]
pub extern "C" fn Curl_input_ntlm_wb(
    mut data: *mut Curl_easy,
    ...
    header = unsafe { header.offset(strlen(b"NTLM\0" as *const u8 as *const i8) as isize) };
    while (unsafe { *header }) as i32 != 0 && (unsafe { Curl_isspace(*header as u8 as i32) }) != 0 {
        header = unsafe { header.offset(1) };
    }
    if (unsafe { *header }) != 0 {
        let fresh3 = unsafe { &mut ((*ntlm).challenge) };
        *fresh3 = unsafe { Curl_cstrdup.expect("non-null function pointer")(header) };
        if (unsafe { (*ntlm).challenge }).is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        (unsafe { *state = NTLMSTATE_TYPE2 });
    } else {
        if (unsafe { *state }) as u32 == NTLMSTATE_LAST as i32 as u32 {
            (unsafe { Curl_infof(data, b"NTLM auth restarted\0" as *const u8 as *const i8) });
            Curl_http_auth_cleanup_ntlm_wb(conn);
        } else if (unsafe { *state }) as u32 == NTLMSTATE_TYPE3 as i32 as u32 {
            (unsafe { Curl_infof(data, b"NTLM handshake rejected\0" as *const u8 as *const i8) });
            Curl_http_auth_cleanup_ntlm_wb(conn);
            (unsafe { *state = NTLMSTATE_NONE });
            return CURLE_REMOTE_ACCESS_DENIED;
        } else {
            if (unsafe { *state }) as u32 >= NTLMSTATE_TYPE1 as i32 as u32 {
                (unsafe { Curl_infof(
                    data,
                    b"NTLM handshake failure (internal error)\0" as *const u8 as *const i8,
                ) });
                return CURLE_REMOTE_ACCESS_DENIED;
            }
        }
...
#[no_mangle]
pub extern "C" fn Curl_output_ntlm_wb(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut proxy: bool,
) -> CURLcode {
    ...
    if proxy {
        allocuserpwd = unsafe { &mut (*data).state.aptr.proxyuserpwd };
        userp = unsafe { (*conn).http_proxy.user };
        ntlm = unsafe { &mut (*conn).proxyntlm };
        state = unsafe { &mut (*conn).proxy_ntlm_state };
        authp = unsafe { &mut (*data).state.authproxy };
    } else {
        allocuserpwd = unsafe { &mut (*data).state.aptr.userpwd };
        userp = unsafe { (*conn).user };
        ntlm = unsafe { &mut (*conn).ntlm };
        state = unsafe { &mut (*conn).http_ntlm_state };
        authp = unsafe { &mut (*data).state.authhost };
    }
    (unsafe { (*authp).set_done(0 as i32 as bit) });
    if userp.is_null() {
    ...
    match (unsafe { *state }) as u32 {
        2 => {
            let mut input: *mut i8 =
                unsafe { curl_maprintf(b"TT %s\n\0" as *const u8 as *const i8, (*ntlm).challenge) };
            if input.is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
            res = ntlm_wb_response(data, ntlm, input, unsafe { *state });
            (unsafe { Curl_cfree.expect("non-null function pointer")(input as *mut libc::c_void) });
            if res as u64 != 0 {
                return res;
            }
            (unsafe { Curl_cfree.expect("non-null function pointer")(*allocuserpwd as *mut libc::c_void) });
            (unsafe { *allocuserpwd = curl_maprintf(
            ...
            (unsafe { *state = NTLMSTATE_TYPE3 });
            (unsafe { (*authp).set_done(1 as i32 as bit) });
            Curl_http_auth_cleanup_ntlm_wb(conn);
            if (unsafe { *allocuserpwd }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
        ...
        1 | _ => {
            ...
            res = ntlm_wb_response(data, ntlm, b"YR\n\0" as *const u8 as *const i8, unsafe { *state });
            if res as u64 != 0 {
                return res;
            }
            (unsafe { Curl_cfree.expect("non-null function pointer")(*allocuserpwd as *mut libc::c_void) });
            ...
            (unsafe { Curl_cfree.expect("non-null function pointer")((*ntlm).response as *mut libc::c_void) });
            let fresh4 = unsafe { &mut ((*ntlm).response) };
            *fresh4 = 0 as *mut i8;
            if (unsafe { *allocuserpwd }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
    ...
    match current_block_56 {
        13041636594629627168 => {
            (unsafe { Curl_cfree.expect("non-null function pointer")(*allocuserpwd as *mut libc::c_void) });
            (unsafe { *allocuserpwd = 0 as *mut i8 });
            (unsafe { (*authp).set_done(1 as i32 as bit) });
        }
        _ => {}
    }
...
#[no_mangle]
pub extern "C" fn Curl_http_auth_cleanup_ntlm_wb(mut conn: *mut connectdata) {
    ntlm_wb_cleanup(unsafe { &mut (*conn).ntlm });
    ntlm_wb_cleanup(unsafe { &mut (*conn).proxyntlm });
}
