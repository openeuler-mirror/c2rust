...
extern "C" {
    ...
    fn Curl_get_line(buf: *mut i8, len: i32, input: *mut FILE) -> *mut i8;
    fn Curl_memrchr(s: *const libc::c_void, c: i32, n: size_t) -> *mut libc::c_void;
    fn Curl_getdate_capped(p: *const i8) -> time_t;
    fn Curl_rand_hex(data: *mut Curl_easy, rnd: *mut u8, num: size_t) -> CURLcode;
    fn Curl_rename(oldpath: *const i8, newpath: *const i8) -> i32;
...
extern "C" fn freecookie(mut co: *mut Cookie) {
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).expirestr as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).domain as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).path as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).spath as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).name as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).value as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).maxage as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*co).version as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")(co as *mut libc::c_void) });
}
extern "C" fn tailmatch(mut cooke_domain: *const i8, mut hostname: *const i8) -> bool {
    let mut cookie_domain_len: size_t = unsafe { strlen(cooke_domain) };
...
extern "C" fn pathmatch(mut cookie_path: *const i8, mut request_uri: *const i8) -> bool {
    ...
    cookie_path_len = unsafe { strlen(cookie_path) };
    if 1 as i32 as u64 == cookie_path_len {
        return 1 as i32 != 0;
    }
    uri_path = unsafe { Curl_cstrdup.expect("non-null function pointer")(request_uri) };
    if uri_path.is_null() {
        return 0 as i32 != 0;
    }
    pos = unsafe { strchr(uri_path, '?' as i32) };
    if !pos.is_null() {
        (unsafe { *pos = 0 as i32 as i8 });
    }
    if 0 as i32 as u64 == (unsafe { strlen(uri_path) })
        || (unsafe { *uri_path.offset(0 as i32 as isize) }) as i32 != '/' as i32
    {
    ...
    uri_path_len = unsafe { strlen(uri_path) };
    if uri_path_len < cookie_path_len {
        ret = 0 as i32 != 0;
    } else if (unsafe { strncmp(cookie_path, uri_path, cookie_path_len) }) != 0 {
        ret = 0 as i32 != 0;
    } else if cookie_path_len == uri_path_len {
        ret = 1 as i32 != 0;
    } else if (unsafe { *uri_path.offset(cookie_path_len as isize) }) as i32 == '/' as i32 {
        ret = 1 as i32 != 0;
    } else {
        ret = 0 as i32 != 0;
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")(uri_path as *mut libc::c_void) });
    return ret;
}
extern "C" fn get_top_domain(domain: *const i8, mut outlen: *mut size_t) -> *const i8 {
    let mut len: size_t = 0 as i32 as size_t;
    let mut first: *const i8 = 0 as *const i8;
    let mut last: *const i8 = 0 as *const i8;
    if !domain.is_null() {
        len = unsafe { strlen(domain) };
        last = (unsafe { Curl_memrchr(domain as *const libc::c_void, '.' as i32, len) }) as *const i8;
        if !last.is_null() {
            first = (unsafe { Curl_memrchr(
            ...
            if !first.is_null() {
                ...
                len = (len as u64).wrapping_sub((unsafe { first.offset_from(domain) }) as i64 as u64) as size_t
                    as size_t;
            }
        }
    }
...
extern "C" fn cookie_hash_domain(mut domain: *const i8, len: size_t) -> size_t {
    let mut end: *const i8 = unsafe { domain.offset(len as isize) };
    let mut h: size_t = 5381 as i32 as size_t;
    while domain < end {
        h = (h as u64).wrapping_add(h << 5 as i32) as size_t as size_t;
        let fresh0 = domain;
        domain = unsafe { domain.offset(1) };
        h ^= (unsafe { Curl_raw_toupper(*fresh0) }) as u64;
    }
    return h.wrapping_rem(256 as i32 as u64);
}
extern "C" fn cookiehash(domain: *const i8) -> size_t {
    let mut top: *const i8 = 0 as *const i8;
    let mut len: size_t = 0;
    if domain.is_null() || (unsafe { Curl_host_is_ipnum(domain) }) as i32 != 0 {
        return 0 as i32 as size_t;
    }
    top = get_top_domain(domain, &mut len);
    return cookie_hash_domain(top, len);
}
extern "C" fn sanitize_cookie_path(mut cookie_path: *const i8) -> *mut i8 {
    let mut len: size_t = 0;
    let mut new_path: *mut i8 = unsafe { Curl_cstrdup.expect("non-null function pointer")(cookie_path) };
    if new_path.is_null() {
        return 0 as *mut i8;
    }
    len = unsafe { strlen(new_path) };
    if (unsafe { *new_path.offset(0 as i32 as isize) }) as i32 == '"' as i32 {
        (unsafe { memmove(
    ...
    if len != 0 && (unsafe { *new_path.offset(len.wrapping_sub(1 as i32 as u64) as isize) }) as i32 == '"' as i32
    {
        (unsafe { *new_path.offset(len.wrapping_sub(1 as i32 as u64) as isize) = 0 as i32 as i8 });
        len = len.wrapping_sub(1);
    }
    if (unsafe { *new_path.offset(0 as i32 as isize) }) as i32 != '/' as i32 {
...
#[no_mangle]
pub extern "C" fn Curl_cookie_loadfiles(mut data: *mut Curl_easy) {
    let mut list: *mut curl_slist = unsafe { (*data).state.cookielist };
    if !list.is_null() {
        ...
        while !list.is_null() {
            let mut newcookies: *mut CookieInfo = Curl_cookie_init(
                ...
                unsafe { (*list).data },
                unsafe { (*data).cookies },
                (unsafe { ((*data).set).cookiesession() }) != 0,
            );
            if newcookies.is_null() {
                (unsafe { Curl_infof(
                    data,
                    b"ignoring failed cookie_init for %s\0" as *const u8 as *const i8,
                    (*list).data,
                ) });
            } else {
                let fresh1 = unsafe { &mut ((*data).cookies) };
                *fresh1 = newcookies;
            }
            list = unsafe { (*list).next };
        }
        (unsafe { curl_slist_free_all((*data).state.cookielist) });
        let fresh2 = unsafe { &mut ((*data).state.cookielist) };
        *fresh2 = 0 as *mut curl_slist;
        (unsafe { Curl_share_unlock(data, CURL_LOCK_DATA_COOKIE) });
    }
}
extern "C" fn strstore(mut str: *mut *mut i8, mut newstr: *const i8) {
    (unsafe { Curl_cfree.expect("non-null function pointer")(*str as *mut libc::c_void) });
    (unsafe { *str = Curl_cstrdup.expect("non-null function pointer")(newstr) });
}
extern "C" fn remove_expired(mut cookies: *mut CookieInfo) {
    let mut co: *mut Cookie = 0 as *mut Cookie;
    let mut nx: *mut Cookie = 0 as *mut Cookie;
    let mut now: curl_off_t = unsafe { time(0 as *mut time_t) };
    let mut i: u32 = 0;
    if now < (unsafe { (*cookies).next_expiration }) && (unsafe { (*cookies).next_expiration }) != 0x7fffffffffffffff as i64 {
        return;
    } else {
        (unsafe { (*cookies).next_expiration = 0x7fffffffffffffff as i64 });
    }
    i = 0 as i32 as u32;
    while i < 256 as i32 as u32 {
        let mut pv: *mut Cookie = 0 as *mut Cookie;
        co = unsafe { (*cookies).cookies[i as usize] };
        while !co.is_null() {
            nx = unsafe { (*co).next };
            if (unsafe { (*co).expires }) != 0 && (unsafe { (*co).expires }) < now {
                if pv.is_null() {
                    let fresh3 = unsafe { &mut ((*cookies).cookies[i as usize]) };
                    *fresh3 = unsafe { (*co).next };
                } else {
                    let fresh4 = unsafe { &mut ((*pv).next) };
                    *fresh4 = unsafe { (*co).next };
                }
                let fresh5 = unsafe { &mut ((*cookies).numcookies) };
                *fresh5 -= 1;
                freecookie(co);
            } else {
                if (unsafe { (*co).expires }) != 0 && (unsafe { (*co).expires }) < (unsafe { (*cookies).next_expiration }) {
                    (unsafe { (*cookies).next_expiration = (*co).expires });
                }
                pv = co;
            }
            co = nx;
...
extern "C" fn bad_domain(mut domain: *const i8) -> bool {
    return (unsafe { strchr(domain, '.' as i32) }).is_null()
        && (unsafe { Curl_strcasecompare(domain, b"localhost\0" as *const u8 as *const i8) }) == 0;
}
#[no_mangle]
pub extern "C" fn Curl_cookie_add(
    ...
    let mut now: time_t = unsafe { time(0 as *mut time_t) };
    let mut replace_old: bool = 0 as i32 != 0;
    let mut badcookie: bool = 0 as i32 != 0;
    let mut myhash: size_t = 0;
    co = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<Cookie>() as u64,
    ) }) as *mut Cookie;
    if co.is_null() {
    ...
    if httpheader {
        ...
        let mut linelength: size_t = unsafe { strlen(lineptr) };
        if linelength > 5000 as i32 as u64 {
            (unsafe { Curl_cfree.expect("non-null function pointer")(co as *mut libc::c_void) });
            return 0 as *mut Cookie;
        }
        semiptr = unsafe { strchr(lineptr, ';' as i32) };
        while (unsafe { *lineptr }) as i32 != 0
            && ((unsafe { *lineptr }) as u8 as i32 == ' ' as i32 || (unsafe { *lineptr }) as u8 as i32 == '\t' as i32)
        {
            lineptr = unsafe { lineptr.offset(1) };
        }
        ptr = lineptr;
        loop {
                ...
                ) })
            {
                ...
                let mut len: size_t = unsafe { strlen(what.as_mut_ptr()) };
                let mut nlen: size_t = unsafe { strlen(name.as_mut_ptr()) };
                let mut endofn: *const i8 = (unsafe { &*ptr.offset(nlen as isize) }) as *const i8;
                    ...
                    || nlen.wrapping_add(len) > 4096 as i32 as u64
                {
                    ...
                    (unsafe { Curl_infof(
                        data,
                        b"oversized cookie dropped, name/val %zu + %zu bytes\0" as *const u8
                            as *const i8,
                        nlen,
                        len,
                    ) });
                ...
                sep = if (unsafe { *endofn }) as i32 == '=' as i32 {
                    1 as i32
                } else {
                    0 as i32
                } != 0;
                if nlen != 0 {
                    endofn = unsafe { endofn.offset(-1) };
                    if (unsafe { *endofn }) as u8 as i32 == ' ' as i32 || (unsafe { *endofn }) as u8 as i32 == '\t' as i32 {
                        while (unsafe { *endofn }) as i32 != 0
                            && ((unsafe { *endofn }) as u8 as i32 == ' ' as i32
                                || (unsafe { *endofn }) as u8 as i32 == '\t' as i32)
                            && nlen != 0
                        {
                            endofn = unsafe { endofn.offset(-1) };
                ...
                while len != 0
                    && (what[len.wrapping_sub(1 as i32 as u64) as usize] as u8 as i32 == ' ' as i32
                        || what[len.wrapping_sub(1 as i32 as u64) as usize] as u8 as i32
                            == '\t' as i32)
                {
                    what[len.wrapping_sub(1 as i32 as u64) as usize] = 0 as i32 as i8;
                    len = len.wrapping_sub(1);
                }
                whatptr = what.as_mut_ptr();
                while (unsafe { *whatptr }) as i32 != 0
                    && ((unsafe { *whatptr }) as u8 as i32 == ' ' as i32 || (unsafe { *whatptr }) as u8 as i32 == '\t' as i32)
                {
                    whatptr = unsafe { whatptr.offset(1) };
                }
                    ...
                    && name[1 as i32 as usize] as i32 == '_' as i32
                {
                    ...
                    {
                        let fresh6 = unsafe { &mut ((*co).prefix) };
                        *fresh6 = (*fresh6 as i32 | (1 as i32) << 0 as i32) as u8;
                    } else if (unsafe { strncmp(
                        b"__Host-\0" as *const u8 as *const i8,
                        name.as_mut_ptr(),
                        7 as i32 as u64,
                    ) }) == 0
                    {
                        let fresh7 = unsafe { &mut ((*co).prefix) };
                        *fresh7 = (*fresh7 as i32 | (1 as i32) << 1 as i32) as u8;
                    }
                }
                if (unsafe { (*co).name }).is_null() {
                    if !sep {
                        badcookie = 1 as i32 != 0;
                        break;
                    } else {
                        let fresh8 = unsafe { &mut ((*co).name) };
                        *fresh8 =
                            unsafe { Curl_cstrdup.expect("non-null function pointer")(name.as_mut_ptr()) };
                        let fresh9 = unsafe { &mut ((*co).value) };
                        *fresh9 = unsafe { Curl_cstrdup.expect("non-null function pointer")(whatptr) };
                        done = 1 as i32 != 0;
                        if (unsafe { (*co).name }).is_null() || (unsafe { (*co).value }).is_null() {
                            badcookie = 1 as i32 != 0;
                            break;
                        }
                    }
                    ...
                    if (unsafe { Curl_strcasecompare(b"secure\0" as *const u8 as *const i8, name.as_mut_ptr()) })
                        != 0
                    {
                        if secure as i32 != 0 || !(unsafe { (*c).running }) {
                            (unsafe { (*co).secure = 1 as i32 != 0 });
                        } else {
                            badcookie = 1 as i32 != 0;
                            break;
                        }
                    ...
                    {
                        (unsafe { (*co).httponly = 1 as i32 != 0 });
                    } else if sep {
                        done = 0 as i32 != 0;
                    }
                }
                if !done {
                    if (unsafe { Curl_strcasecompare(b"path\0" as *const u8 as *const i8, name.as_mut_ptr()) })
                        != 0
                    {
                        strstore(unsafe { &mut (*co).path }, whatptr);
                        if (unsafe { (*co).path }).is_null() {
                            badcookie = 1 as i32 != 0;
                            break;
                        } else {
                            ...
                            let fresh10 = unsafe { &mut ((*co).spath) };
                            *fresh10 = sanitize_cookie_path(unsafe { (*co).path });
                            if (unsafe { (*co).spath }).is_null() {
                                badcookie = 1 as i32 != 0;
                                break;
                            }
                        }
                    ...
                    ) }) != 0
                    {
                        ...
                        if '.' as i32 == (unsafe { *whatptr.offset(0 as i32 as isize) }) as i32 {
                            whatptr = unsafe { whatptr.offset(1) };
                        }
                        is_ip =
                            unsafe { Curl_host_is_ipnum(if !domain.is_null() { domain } else { whatptr }) };
                        if domain.is_null()
                            || is_ip as i32 != 0 && (unsafe { strcmp(whatptr, domain) }) == 0
                            || !is_ip && tailmatch(whatptr, domain) as i32 != 0
                        {
                            strstore(unsafe { &mut (*co).domain }, whatptr);
                            if (unsafe { (*co).domain }).is_null() {
                                badcookie = 1 as i32 != 0;
                                break;
                            } else if !is_ip {
                                (unsafe { (*co).tailmatch = 1 as i32 != 0 });
                            }
                        } else {
                            badcookie = 1 as i32 != 0;
                            (unsafe { Curl_infof(
                                data,
                                b"skipped cookie with bad tailmatch domain: %s\0" as *const u8
                                    as *const i8,
                                whatptr,
                            ) });
                        }
                    ...
                    {
                        strstore(unsafe { &mut (*co).version }, whatptr);
                        if (unsafe { (*co).version }).is_null() {
                            badcookie = 1 as i32 != 0;
                            break;
                        }
                    } else if (unsafe { Curl_strcasecompare(
                    ...
                    {
                        strstore(unsafe { &mut (*co).maxage }, whatptr);
                        if (unsafe { (*co).maxage }).is_null() {
                            badcookie = 1 as i32 != 0;
                            break;
                        }
                    } else if (unsafe { Curl_strcasecompare(
                    ...
                    {
                        strstore(unsafe { &mut (*co).expirestr }, whatptr);
                        if (unsafe { (*co).expirestr }).is_null() {
                            badcookie = 1 as i32 != 0;
                            break;
                        }
                    }
            ...
            if semiptr.is_null() || (unsafe { *semiptr }) == 0 {
                semiptr = 0 as *const i8;
            } else {
                ptr = unsafe { semiptr.offset(1 as i32 as isize) };
                while (unsafe { *ptr }) as i32 != 0
                    && ((unsafe { *ptr }) as u8 as i32 == ' ' as i32 || (unsafe { *ptr }) as u8 as i32 == '\t' as i32)
                {
                    ptr = unsafe { ptr.offset(1) };
                }
                semiptr = unsafe { strchr(ptr, ';' as i32) };
                if semiptr.is_null() && (unsafe { *ptr }) as i32 != 0 {
                    semiptr = unsafe { strchr(ptr, '\u{0}' as i32) };
                }
            }
            if semiptr.is_null() {
                break;
        ...
        if !(unsafe { (*co).maxage }).is_null() {
            ...
            if offt as u32 == CURL_OFFT_FLOW as i32 as u32 {
                (unsafe { (*co).expires = 0x7fffffffffffffff as i64 });
            } else if offt as u64 == 0 {
                if (unsafe { (*co).expires }) == 0 {
                    (unsafe { (*co).expires = 1 as i32 as curl_off_t });
                } else if 0x7fffffffffffffff as i64 - now < (unsafe { (*co).expires }) {
                    (unsafe { (*co).expires = 0x7fffffffffffffff as i64 });
                } else {
                    let fresh11 = unsafe { &mut ((*co).expires) };
                    *fresh11 += now;
                }
            }
        } else if !(unsafe { (*co).expirestr }).is_null() {
            (unsafe { (*co).expires = Curl_getdate_capped((*co).expirestr) });
            if (unsafe { (*co).expires }) == 0 as i32 as i64 {
                (unsafe { (*co).expires = 1 as i32 as curl_off_t });
            } else if (unsafe { (*co).expires }) < 0 as i32 as i64 {
                (unsafe { (*co).expires = 0 as i32 as curl_off_t });
            }
        }
        if !badcookie && (unsafe { (*co).domain }).is_null() {
            if !domain.is_null() {
                let fresh12 = unsafe { &mut ((*co).domain) };
                *fresh12 = unsafe { Curl_cstrdup.expect("non-null function pointer")(domain) };
                if (unsafe { (*co).domain }).is_null() {
                    badcookie = 1 as i32 != 0;
                }
            }
        }
        if !badcookie && (unsafe { (*co).path }).is_null() && !path.is_null() {
            let mut queryp: *mut i8 = unsafe { strchr(path, '?' as i32) };
            let mut endslash: *mut i8 = 0 as *mut i8;
            if queryp.is_null() {
                endslash = unsafe { strrchr(path, '/' as i32) };
            } else {
                endslash = (unsafe { Curl_memrchr(
            ...
            if !endslash.is_null() {
                let mut pathlen: size_t =
                    ((unsafe { endslash.offset_from(path) }) as i64 + 1 as i32 as i64) as size_t;
                let fresh13 = unsafe { &mut ((*co).path) };
                *fresh13 = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
                ...
                if !(unsafe { (*co).path }).is_null() {
                    ...
                    (unsafe { *((*co).path).offset(pathlen as isize) = 0 as i32 as i8 });
                    let fresh14 = unsafe { &mut ((*co).spath) };
                    *fresh14 = sanitize_cookie_path(unsafe { (*co).path });
                    if (unsafe { (*co).spath }).is_null() {
                        badcookie = 1 as i32 != 0;
                    }
                } else {
        ...
        if badcookie as i32 != 0 || (unsafe { (*co).name }).is_null() {
            freecookie(co);
            return 0 as *mut Cookie;
        }
    } else {
        ...
        {
            lineptr = unsafe { lineptr.offset(10 as i32 as isize) };
            (unsafe { (*co).httponly = 1 as i32 != 0 });
        }
        if (unsafe { *lineptr.offset(0 as i32 as isize) }) as i32 == '#' as i32 {
            (unsafe { Curl_cfree.expect("non-null function pointer")(co as *mut libc::c_void) });
            return 0 as *mut Cookie;
        }
        ptr_0 = unsafe { strchr(lineptr, '\r' as i32) };
        if !ptr_0.is_null() {
            (unsafe { *ptr_0 = 0 as i32 as i8 });
        }
        ptr_0 = unsafe { strchr(lineptr, '\n' as i32) };
        if !ptr_0.is_null() {
            (unsafe { *ptr_0 = 0 as i32 as i8 });
        }
        firstptr = unsafe { strtok_r(lineptr, b"\t\0" as *const u8 as *const i8, &mut tok_buf) };
        ptr_0 = firstptr;
        fields = 0 as i32;
        while !ptr_0.is_null() && !badcookie {
            ...
            match fields {
                0 => {
                    if (unsafe { *ptr_0.offset(0 as i32 as isize) }) as i32 == '.' as i32 {
                        ptr_0 = unsafe { ptr_0.offset(1) };
                    }
                    let fresh15 = unsafe { &mut ((*co).domain) };
                    *fresh15 = unsafe { Curl_cstrdup.expect("non-null function pointer")(ptr_0) };
                    if (unsafe { (*co).domain }).is_null() {
                        badcookie = 1 as i32 != 0;
                    }
                ...
                1 => {
                    (unsafe { (*co).tailmatch =
                        if Curl_strcasecompare(ptr_0, b"TRUE\0" as *const u8 as *const i8) != 0 {
                            1 as i32
                        } else {
                            0 as i32
                        } != 0 });
                ...
                2 => {
                    if (unsafe { strcmp(b"TRUE\0" as *const u8 as *const i8, ptr_0) }) != 0
                        && (unsafe { strcmp(b"FALSE\0" as *const u8 as *const i8, ptr_0) }) != 0
                    {
                        let fresh16 = unsafe { &mut ((*co).path) };
                        *fresh16 = unsafe { Curl_cstrdup.expect("non-null function pointer")(ptr_0) };
                        if (unsafe { (*co).path }).is_null() {
                            badcookie = 1 as i32 != 0;
                        } else {
                            let fresh17 = unsafe { &mut ((*co).spath) };
                            *fresh17 = sanitize_cookie_path(unsafe { (*co).path });
                            if (unsafe { (*co).spath }).is_null() {
                                badcookie = 1 as i32 != 0;
                            }
                        }
                        ...
                        let fresh18 = unsafe { &mut ((*co).path) };
                        *fresh18 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                        ...
                        let fresh19 = unsafe { &mut ((*co).spath) };
                        *fresh19 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                            b"/\0" as *const u8 as *const i8,
                        ) };
                        if (unsafe { (*co).spath }).is_null() {
                            badcookie = 1 as i32 != 0;
                ...
                4 => {
                    if (unsafe { curlx_strtoofft(ptr_0, 0 as *mut *mut i8, 10 as i32, &mut (*co).expires) })
                        as u64
                        != 0
                    {
                        badcookie = 1 as i32 != 0;
                ...
                5 => {
                    let fresh20 = unsafe { &mut ((*co).name) };
                    *fresh20 = unsafe { Curl_cstrdup.expect("non-null function pointer")(ptr_0) };
                    if (unsafe { (*co).name }).is_null() {
                        badcookie = 1 as i32 != 0;
                    } else if (unsafe { Curl_strncasecompare(
                        b"__Secure-\0" as *const u8 as *const i8,
                        (*co).name,
                        9 as i32 as size_t,
                    ) }) != 0
                    {
                        let fresh21 = unsafe { &mut ((*co).prefix) };
                        *fresh21 = (*fresh21 as i32 | (1 as i32) << 0 as i32) as u8;
                    } else if (unsafe { Curl_strncasecompare(
                        b"__Host-\0" as *const u8 as *const i8,
                        (*co).name,
                        7 as i32 as size_t,
                    ...
                    {
                        let fresh22 = unsafe { &mut ((*co).prefix) };
                        *fresh22 = (*fresh22 as i32 | (1 as i32) << 1 as i32) as u8;
                    }
                    current_block_187 = 4402115142504265260;
                }
                6 => {
                    let fresh23 = unsafe { &mut ((*co).value) };
                    *fresh23 = unsafe { Curl_cstrdup.expect("non-null function pointer")(ptr_0) };
            ...
            match current_block_187 {
                15249084414975911002 => {
                    (unsafe { (*co).secure = 0 as i32 != 0 });
                    if (unsafe { Curl_strcasecompare(ptr_0, b"TRUE\0" as *const u8 as *const i8) }) != 0 {
                        if secure as i32 != 0 || (unsafe { (*c).running }) as i32 != 0 {
                            (unsafe { (*co).secure = 1 as i32 != 0 });
                        } else {
                            badcookie = 1 as i32 != 0;
                        }
        ...
        if 6 as i32 == fields {
            let fresh24 = unsafe { &mut ((*co).value) };
            *fresh24 =
    ...
    if (unsafe { (*co).prefix }) as i32 & (1 as i32) << 0 as i32 != 0 {
        if !(unsafe { (*co).secure }) {
            freecookie(co);
            return 0 as *mut Cookie;
        }
    }
    if (unsafe { (*co).prefix }) as i32 & (1 as i32) << 1 as i32 != 0 {
        if (unsafe { (*co).secure }) as i32 != 0
            && !(unsafe { (*co).path }).is_null()
            && (unsafe { strcmp((*co).path, b"/\0" as *const u8 as *const i8) }) == 0 as i32
            && !(unsafe { (*co).tailmatch })
        {
        } else {
            freecookie(co);
    ...
    if !(unsafe { (*c).running }) && (unsafe { (*c).newsession }) as i32 != 0 && (unsafe { (*co).expires }) == 0 {
        freecookie(co);
        return 0 as *mut Cookie;
    }
    (unsafe { (*co).livecookie = (*c).running });
    let fresh25 = unsafe { &mut ((*c).lastct) };
    *fresh25 += 1;
    (unsafe { (*co).creationtime = *fresh25 });
    if !noexpire {
        remove_expired(c);
    }
    if !data.is_null()
        && (!domain.is_null() && !(unsafe { (*co).domain }).is_null() && !(unsafe { Curl_host_is_ipnum((*co).domain) }))
    {
        let mut psl: *const psl_ctx_t = unsafe { Curl_psl_use(data) };
        let mut acceptable: i32 = 0;
        if !psl.is_null() {
            acceptable = unsafe { psl_is_cookie_domain_acceptable(psl, domain, (*co).domain) };
            (unsafe { Curl_psl_release(data) });
        } else {
        ...
        if acceptable == 0 {
            (unsafe { Curl_infof(
                data,
                b"cookie '%s' dropped, domain '%s' must not set cookies for '%s'\0" as *const u8
                    as *const i8,
                (*co).name,
                domain,
                (*co).domain,
    ...
    myhash = cookiehash(unsafe { (*co).domain });
    clist = unsafe { (*c).cookies[myhash as usize] };
    replace_old = 0 as i32 != 0;
    while !clist.is_null() {
        if (unsafe { Curl_strcasecompare((*clist).name, (*co).name) }) != 0 {
            if !(unsafe { (*clist).domain }).is_null() && !(unsafe { (*co).domain }).is_null() {
                if (unsafe { Curl_strcasecompare((*clist).domain, (*co).domain) }) != 0
                    && (unsafe { (*clist).tailmatch }) as i32 == (unsafe { (*co).tailmatch }) as i32
                {
                    replace_old = 1 as i32 != 0;
                }
            } else if (unsafe { (*clist).domain }).is_null() && (unsafe { (*co).domain }).is_null() {
                replace_old = 1 as i32 != 0;
            }
            if replace_old {
                if !(unsafe { (*clist).spath }).is_null() && !(unsafe { (*co).spath }).is_null() {
                    if (unsafe { (*clist).secure }) as i32 != 0 && !(unsafe { (*co).secure }) && !secure {
                        let mut cllen: size_t = 0;
                        let mut sep_0: *const i8 = 0 as *const i8;
                        sep_0 = unsafe { strchr(((*clist).spath).offset(1 as i32 as isize), '/' as i32) };
                        if !sep_0.is_null() {
                            cllen = (unsafe { sep_0.offset_from((*clist).spath) }) as i64 as size_t;
                        } else {
                            cllen = unsafe { strlen((*clist).spath) };
                        }
                        if (unsafe { Curl_strncasecompare((*clist).spath, (*co).spath, cllen) }) != 0 {
                            freecookie(co);
                            return 0 as *mut Cookie;
                        }
                    } else if (unsafe { Curl_strcasecompare((*clist).spath, (*co).spath) }) != 0 {
                        replace_old = 1 as i32 != 0;
                    } else {
                        replace_old = 0 as i32 != 0;
                    }
                } else if (unsafe { (*clist).spath }).is_null() && (unsafe { (*co).spath }).is_null() {
                    replace_old = 1 as i32 != 0;
                } else {
                    replace_old = 0 as i32 != 0;
                }
            }
            if replace_old as i32 != 0 && !(unsafe { (*co).livecookie }) && (unsafe { (*clist).livecookie }) as i32 != 0 {
                freecookie(co);
                return 0 as *mut Cookie;
            }
            if replace_old {
                let fresh26 = unsafe { &mut ((*co).next) };
                *fresh26 = unsafe { (*clist).next };
                (unsafe { (*co).creationtime = (*clist).creationtime });
                (unsafe { Curl_cfree.expect("non-null function pointer")((*clist).name as *mut libc::c_void) });
                (unsafe { Curl_cfree.expect("non-null function pointer")((*clist).value as *mut libc::c_void) });
                (unsafe { Curl_cfree.expect("non-null function pointer")(
                    (*clist).domain as *mut libc::c_void,
                ) });
                (unsafe { Curl_cfree.expect("non-null function pointer")((*clist).path as *mut libc::c_void) });
                (unsafe { Curl_cfree.expect("non-null function pointer")((*clist).spath as *mut libc::c_void) });
                (unsafe { Curl_cfree.expect("non-null function pointer")(
    ...
    if unsafe { (*c).running } {
        (unsafe { Curl_infof(
            data,
            b"%s cookie %s=\"%s\" for domain %s, path %s, expire %ld\0" as *const u8 as *const i8,
    ...
    if !replace_old {
        if !lastc.is_null() {
            let fresh27 = unsafe { &mut ((*lastc).next) };
            *fresh27 = co;
        } else {
            let fresh28 = unsafe { &mut ((*c).cookies[myhash as usize]) };
            *fresh28 = co;
        }
        let fresh29 = unsafe { &mut ((*c).numcookies) };
        *fresh29 += 1;
    }
    if (unsafe { (*co).expires }) != 0 && (unsafe { (*co).expires }) < (unsafe { (*c).next_expiration }) {
        (unsafe { (*c).next_expiration = (*co).expires });
    }
    return co;
}
#[no_mangle]
pub extern "C" fn Curl_cookie_init(
    mut data: *mut Curl_easy,
    mut file: *const i8,
    ...
    if inc.is_null() {
        c = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
            1 as i32 as size_t,
        ...
        let fresh30 = unsafe { &mut ((*c).filename) };
        *fresh30 = unsafe { Curl_cstrdup.expect("non-null function pointer")(if !file.is_null() {
            file
        } else {
            b"none\0" as *const u8 as *const i8
        }) };
        if (unsafe { (*c).filename }).is_null() {
            current_block = 3471203105459249385;
        } else {
            (unsafe { (*c).next_expiration = 0x7fffffffffffffff as i64 });
    ...
    match current_block {
        11650488183268122163 => {
            (unsafe { (*c).running = 0 as i32 != 0 });
            if !file.is_null() && (unsafe { strcmp(file, b"-\0" as *const u8 as *const i8) }) == 0 {
                fp = unsafe { stdin };
                fromfile = 0 as i32 != 0;
            } else if !file.is_null() && (unsafe { *file }) == 0 {
                fp = 0 as *mut FILE;
            } else {
                fp = if !file.is_null() {
                    unsafe { fopen(file, b"r\0" as *const u8 as *const i8) }
                } else {
                    0 as *mut FILE
                };
            }
            (unsafe { (*c).newsession = newsession });
            if !fp.is_null() {
                let mut lineptr: *mut i8 = 0 as *mut i8;
                let mut headerline: bool = false;
                line = (unsafe { Curl_cmalloc.expect("non-null function pointer")(5000 as i32 as size_t) })
                    as *mut i8;
                if line.is_null() {
                    current_block = 3471203105459249385;
                } else {
                    while !(unsafe { Curl_get_line(line, 5000 as i32, fp) }).is_null() {
                        ...
                        {
                            lineptr = (unsafe { &mut *line.offset(11 as i32 as isize) }) as *mut i8;
                        ...
                        while (unsafe { *lineptr }) as i32 != 0
                            && ((unsafe { *lineptr }) as u8 as i32 == ' ' as i32
                                || (unsafe { *lineptr }) as u8 as i32 == '\t' as i32)
                        {
                            lineptr = unsafe { lineptr.offset(1) };
                        }
...
extern "C" fn cookie_sort(mut p1: *const libc::c_void, mut p2: *const libc::c_void) -> i32 {
    let mut c1: *mut Cookie = unsafe { *(p1 as *mut *mut Cookie) };
    let mut c2: *mut Cookie = unsafe { *(p2 as *mut *mut Cookie) };
    let mut l1: size_t = 0;
    let mut l2: size_t = 0;
    l1 = if !(unsafe { (*c1).path }).is_null() {
        unsafe { strlen((*c1).path) }
    } else {
        0 as i32 as u64
    };
    l2 = if !(unsafe { (*c2).path }).is_null() {
        unsafe { strlen((*c2).path) }
    } else {
        0 as i32 as u64
    };
    if l1 != l2 {
    ...
    l1 = if !(unsafe { (*c1).domain }).is_null() {
        unsafe { strlen((*c1).domain) }
    } else {
        0 as i32 as u64
    };
    l2 = if !(unsafe { (*c2).domain }).is_null() {
        unsafe { strlen((*c2).domain) }
    } else {
        0 as i32 as u64
    };
    if l1 != l2 {
    ...
    l1 = if !(unsafe { (*c1).name }).is_null() {
        unsafe { strlen((*c1).name) }
    } else {
        0 as i32 as u64
    };
    l2 = if !(unsafe { (*c2).name }).is_null() {
        unsafe { strlen((*c2).name) }
    } else {
        0 as i32 as u64
    };
    if l1 != l2 {
    ...
    return if (unsafe { (*c2).creationtime }) > (unsafe { (*c1).creationtime }) {
        1 as i32
    } else {
        -(1 as i32)
    };
}
extern "C" fn cookie_sort_ct(mut p1: *const libc::c_void, mut p2: *const libc::c_void) -> i32 {
    let mut c1: *mut Cookie = unsafe { *(p1 as *mut *mut Cookie) };
    let mut c2: *mut Cookie = unsafe { *(p2 as *mut *mut Cookie) };
    return if (unsafe { (*c2).creationtime }) > (unsafe { (*c1).creationtime }) {
        1 as i32
...
extern "C" fn dup_cookie(mut src: *mut Cookie) -> *mut Cookie {
    let mut current_block: u64;
    let mut d: *mut Cookie = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<Cookie>() as u64,
        1 as i32 as size_t,
    ) }) as *mut Cookie;
    if !d.is_null() {
        if !(unsafe { (*src).expirestr }).is_null() {
            let fresh32 = unsafe { &mut ((*d).expirestr) };
            *fresh32 = unsafe { Curl_cstrdup.expect("non-null function pointer")((*src).expirestr) };
            if (unsafe { (*d).expirestr }).is_null() {
        ...
        match current_block {
            7815301370352969686 => {
                if !(unsafe { (*src).domain }).is_null() {
                    let fresh33 = unsafe { &mut ((*d).domain) };
                    *fresh33 = unsafe { Curl_cstrdup.expect("non-null function pointer")((*src).domain) };
                    if (unsafe { (*d).domain }).is_null() {
                        current_block = 3152575373803700262;
                    } else {
                ...
                match current_block {
                    3152575373803700262 => {}
                    _ => {
                        if !(unsafe { (*src).path }).is_null() {
                            let fresh34 = unsafe { &mut ((*d).path) };
                            *fresh34 =
                                unsafe { Curl_cstrdup.expect("non-null function pointer")((*src).path) };
                            if (unsafe { (*d).path }).is_null() {
                                current_block = 3152575373803700262;
                            } else {
                        ...
                        match current_block {
                            3152575373803700262 => {}
                            _ => {
                                if !(unsafe { (*src).spath }).is_null() {
                                    let fresh35 = unsafe { &mut ((*d).spath) };
                                    *fresh35 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                                        (*src).spath,
                                    ) };
                                    if (unsafe { (*d).spath }).is_null() {
                                        current_block = 3152575373803700262;
                                    } else {
                                ...
                                match current_block {
                                    3152575373803700262 => {}
                                    _ => {
                                        if !(unsafe { (*src).name }).is_null() {
                                            let fresh36 = unsafe { &mut ((*d).name) };
                                            *fresh36 = unsafe { Curl_cstrdup
                                                .expect("non-null function pointer")(
                                                (*src).name
                                            ) };
                                            if (unsafe { (*d).name }).is_null() {
                                                current_block = 3152575373803700262;
                                            } else {
                                        ...
                                        match current_block {
                                            3152575373803700262 => {}
                                            _ => {
                                                if !(unsafe { (*src).value }).is_null() {
                                                    let fresh37 = unsafe { &mut ((*d).value) };
                                                    *fresh37 = unsafe { Curl_cstrdup
                                                        .expect("non-null function pointer")(
                                                        (*src).value,
                                                    ) };
                                                    if (unsafe { (*d).value }).is_null() {
                                                        current_block = 3152575373803700262;
                                                    } else {
                                                ...
                                                match current_block {
                                                    3152575373803700262 => {}
                                                    _ => {
                                                        if !(unsafe { (*src).maxage }).is_null() {
                                                            let fresh38 = unsafe { &mut ((*d).maxage) };
                                                            *fresh38 = unsafe { Curl_cstrdup.expect(
                                                                "non-null function pointer",
                                                            )(
                                                                (*src).maxage
                                                            ) };
                                                            if (unsafe { (*d).maxage }).is_null() {
                                                                current_block = 3152575373803700262;
                                                            } else {
                                                                current_block =
                                                        ...
                                                        match current_block {
                                                            3152575373803700262 => {}
                                                            _ => {
                                                                if !(unsafe { (*src).version }).is_null() {
                                                                    let fresh39 =
                                                                        unsafe { &mut ((*d).version) };
                                                                    *fresh39 = unsafe { Curl_cstrdup.expect(
                                                                        "non-null function pointer",
                                                                    )(
                                                                        (*src).version
                                                                    ) };
                                                                    if (unsafe { (*d).version }).is_null() {
                                                                        current_block =
                                                                            3152575373803700262;
                                                                    } else {
                                                                        current_block =
                                                                ...
                                                                match current_block {
                                                                    ...
                                                                    _ => {
                                                                        ...
                                                                        (unsafe { (*d).tailmatch =
                                                                            (*src).tailmatch });
                                                                        (unsafe { (*d).secure = (*src).secure });
                                                                        (unsafe { (*d).livecookie =
                                                                            (*src).livecookie });
                                                                        (unsafe { (*d).httponly =
                                                                            (*src).httponly });
                                                                        (unsafe { (*d).creationtime =
                                                                            (*src).creationtime });
                                                                        current_block =
...
#[no_mangle]
pub extern "C" fn Curl_cookie_getlist(
    mut c: *mut CookieInfo,
    mut host: *const i8,
    mut path: *const i8,
    mut secure: bool,
    ...
    if c.is_null() || (unsafe { (*c).cookies[myhash as usize] }).is_null() {
        return 0 as *mut Cookie;
    }
    remove_expired(c);
    is_ip = unsafe { Curl_host_is_ipnum(host) };
    co = unsafe { (*c).cookies[myhash as usize] };
    loop {
        if co.is_null() {
            current_block = 4808432441040389987;
            break;
        }
        if if (unsafe { (*co).secure }) as i32 != 0 {
            secure as i32
        } else {
            1 as i32
        } != 0
        {
            if (unsafe { (*co).domain }).is_null()
                || (unsafe { (*co).tailmatch }) as i32 != 0
                    && !is_ip
                    && tailmatch(unsafe { (*co).domain }, host) as i32 != 0
                || (!(unsafe { (*co).tailmatch }) || is_ip as i32 != 0)
                    && (unsafe { Curl_strcasecompare(host, (*co).domain) }) != 0
            {
                if (unsafe { (*co).spath }).is_null() || pathmatch(unsafe { (*co).spath }, path) as i32 != 0 {
                    newco = dup_cookie(co);
                    if newco.is_null() {
                        current_block = 4964016911898087089;
                        break;
                    }
                    let fresh40 = unsafe { &mut ((*newco).next) };
                    *fresh40 = mainco;
                    mainco = newco;
    ...
    match current_block {
        4808432441040389987 => {
            if matches != 0 {
                ...
                let mut i: size_t = 0;
                array = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
                    (::std::mem::size_of::<*mut Cookie>() as u64).wrapping_mul(matches),
                ) }) as *mut *mut Cookie;
                if array.is_null() {
                    current_block = 4964016911898087089;
                } else {
                    ...
                    while !co.is_null() {
                        let fresh41 = i;
                        i = i.wrapping_add(1);
                        let fresh42 = unsafe { &mut (*array.offset(fresh41 as isize)) };
                        *fresh42 = co;
                        co = unsafe { (*co).next };
                    }
                    (unsafe { qsort(
                    ...
                    mainco = unsafe { *array.offset(0 as i32 as isize) };
                    i = 0 as i32 as size_t;
                    while i < matches.wrapping_sub(1 as i32 as u64) {
                        let fresh43 = unsafe { &mut ((**array.offset(i as isize)).next) };
                        *fresh43 = unsafe { *array.offset(i.wrapping_add(1 as i32 as u64) as isize) };
                        i = i.wrapping_add(1);
                    }
                    let fresh44 = unsafe { &mut ((**array
                        .offset(matches.wrapping_sub(1 as i32 as u64) as isize))
                    .next) };
                    *fresh44 = 0 as *mut Cookie;
                    (unsafe { Curl_cfree.expect("non-null function pointer")(array as *mut libc::c_void) });
...
#[no_mangle]
pub extern "C" fn Curl_cookie_clearall(mut cookies: *mut CookieInfo) {
    if !cookies.is_null() {
        let mut i: u32 = 0;
        i = 0 as i32 as u32;
        while i < 256 as i32 as u32 {
            Curl_cookie_freelist(unsafe { (*cookies).cookies[i as usize] });
            let fresh45 = unsafe { &mut ((*cookies).cookies[i as usize]) };
            *fresh45 = 0 as *mut Cookie;
            i = i.wrapping_add(1);
        }
        (unsafe { (*cookies).numcookies = 0 as i32 as i64 });
    }
}
#[no_mangle]
pub extern "C" fn Curl_cookie_freelist(mut co: *mut Cookie) {
...
#[no_mangle]
pub extern "C" fn Curl_cookie_clearsess(mut cookies: *mut CookieInfo) {
    ...
    i = 0 as i32 as u32;
    while i < 256 as i32 as u32 {
        if !(unsafe { (*cookies).cookies[i as usize] }).is_null() {
            prev = unsafe { (*cookies).cookies[i as usize] };
            curr = prev;
            first = curr;
            while !curr.is_null() {
                next = unsafe { (*curr).next };
                if (unsafe { (*curr).expires }) == 0 {
                    if first == curr {
                        first = next;
                    }
                    if prev == curr {
                        ...
                        let fresh46 = unsafe { &mut ((*prev).next) };
                        *fresh46 = next;
                    }
                    freecookie(curr);
                    let fresh47 = unsafe { &mut ((*cookies).numcookies) };
                    *fresh47 -= 1;
                } else {
                    prev = curr;
                }
            ...
            let fresh48 = unsafe { &mut ((*cookies).cookies[i as usize]) };
            *fresh48 = first;
        }
        i = i.wrapping_add(1);
    }
...
#[no_mangle]
pub extern "C" fn Curl_cookie_cleanup(mut c: *mut CookieInfo) {
    if !c.is_null() {
        let mut i: u32 = 0;
        (unsafe { Curl_cfree.expect("non-null function pointer")((*c).filename as *mut libc::c_void) });
        i = 0 as i32 as u32;
        while i < 256 as i32 as u32 {
            Curl_cookie_freelist(unsafe { (*c).cookies[i as usize] });
            i = i.wrapping_add(1);
        }
        (unsafe { Curl_cfree.expect("non-null function pointer")(c as *mut libc::c_void) });
    }
}
extern "C" fn get_netscape_format(mut co: *const Cookie) -> *mut i8 {
    return unsafe { curl_maprintf(
        ...
        if (*co).tailmatch as i32 != 0
            && !((*co).domain).is_null()
            && *((*co).domain).offset(0 as i32 as isize) as i32 != '.' as i32
        {
            b".\0" as *const u8 as *const i8
        } else {
            b"\0" as *const u8 as *const i8
...
extern "C" fn cookie_output(
    ...
    remove_expired(c);
    if (unsafe { strcmp(b"-\0" as *const u8 as *const i8, filename) }) == 0 {
        out = unsafe { stdout };
        ...
        tempstore = unsafe { curl_maprintf(
        ...
        out = unsafe { fopen(tempstore, b"w\0" as *const u8 as *const i8) };
        if out.is_null() {
    ...
    match current_block {
        15652330335145281839 => {
            (unsafe { fputs (b"# Netscape HTTP Cookie File\n# https://curl.se/docs/http-cookies.html\n# This file was generated by libcurl! Edit at your own risk.\n\n\0" as * const u8 as * const i8 , out ,) }) ;
            if (unsafe { (*c).numcookies }) != 0 {
                let mut i: u32 = 0;
                let mut nvalid: size_t = 0 as i32 as size_t;
                let mut array: *mut *mut Cookie = 0 as *mut *mut Cookie;
                array = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
                    1 as i32 as size_t,
                    (::std::mem::size_of::<*mut Cookie>() as u64)
                ...
                if array.is_null() {
                    ...
                    i = 0 as i32 as u32;
                    while i < 256 as i32 as u32 {
                        co = unsafe { (*c).cookies[i as usize] };
                        while !co.is_null() {
                            if !(unsafe { (*co).domain }).is_null() {
                                let fresh49 = nvalid;
                                nvalid = nvalid.wrapping_add(1);
                                let fresh50 = unsafe { &mut (*array.offset(fresh49 as isize)) };
                                *fresh50 = co;
                            }
                            co = unsafe { (*co).next };
                        }
                    ...
                    loop {
                        ...
                        let mut format_ptr: *mut i8 =
                            get_netscape_format(unsafe { *array.offset(i as isize) });
                        if format_ptr.is_null() {
                            ...
                            (unsafe { curl_mfprintf(out, b"%s\n\0" as *const u8 as *const i8, format_ptr) });
                            (unsafe { Curl_cfree.expect("non-null function pointer")(
            ...
            match current_block {
                ...
                _ => {
                    if !use_stdout {
                        (unsafe { fclose(out) });
                        out = 0 as *mut FILE;
                        if (unsafe { Curl_rename(tempstore, filename) }) != 0 {
                            (unsafe { unlink(tempstore) });
                            error = CURLE_WRITE_ERROR;
                            current_block = 7867765481719854029;
                        } else {
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(tempstore as *mut libc::c_void) });
    return error;
}
extern "C" fn cookie_list(mut data: *mut Curl_easy) -> *mut curl_slist {
    ...
    let mut line: *mut i8 = 0 as *mut i8;
    let mut i: u32 = 0;
    if (unsafe { (*data).cookies }).is_null() || (unsafe { (*(*data).cookies).numcookies }) == 0 as i32 as i64 {
        return 0 as *mut curl_slist;
    }
    i = 0 as i32 as u32;
    while i < 256 as i32 as u32 {
        c = unsafe { (*(*data).cookies).cookies[i as usize] };
        while !c.is_null() {
            if !(unsafe { (*c).domain }).is_null() {
                line = get_netscape_format(c);
                if line.is_null() {
                    (unsafe { curl_slist_free_all(list) });
                    return 0 as *mut curl_slist;
                }
                beg = unsafe { Curl_slist_append_nodup(list, line) };
                if beg.is_null() {
                    (unsafe { Curl_cfree.expect("non-null function pointer")(line as *mut libc::c_void) });
                    (unsafe { curl_slist_free_all(list) });
                    return 0 as *mut curl_slist;
...
#[no_mangle]
pub extern "C" fn Curl_cookie_list(mut data: *mut Curl_easy) -> *mut curl_slist {
    let mut list: *mut curl_slist = 0 as *mut curl_slist;
    (unsafe { Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE) });
...
#[no_mangle]
pub extern "C" fn Curl_flush_cookies(mut data: *mut Curl_easy, mut cleanup: bool) {
    let mut res: CURLcode = CURLE_OK;
    if !(unsafe { (*data).set.str_0[STRING_COOKIEJAR as i32 as usize] }).is_null() {
        ...
        res = cookie_output(
            data,
            unsafe { (*data).cookies },
            unsafe { (*data).set.str_0[STRING_COOKIEJAR as i32 as usize] },
        );
        if res as u64 != 0 {
            (unsafe { Curl_infof(
                data,
                b"WARNING: failed to save cookies in %s: %s\0" as *const u8 as *const i8,
                (*data).set.str_0[STRING_COOKIEJAR as i32 as usize],
                curl_easy_strerror(res),
            ) });
        }
    } else {
        if cleanup as i32 != 0 && !(unsafe { (*data).state.cookielist }).is_null() {
            (unsafe { curl_slist_free_all((*data).state.cookielist) });
            let fresh51 = unsafe { &mut ((*data).state.cookielist) };
            *fresh51 = 0 as *mut curl_slist;
        }
        (unsafe { Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE) });
    }
    if cleanup as i32 != 0
        && ((unsafe { (*data).share }).is_null() || (unsafe { (*data).cookies }) != (unsafe { (*(*data).share).cookies }))
    {
        Curl_cookie_cleanup(unsafe { (*data).cookies });
        let fresh52 = unsafe { &mut ((*data).cookies) };
