...
extern "C" {
    ...
    fn Curl_hash_clean_with_criterium(
        ...
        comp: Option<unsafe extern "C" fn(*mut libc::c_void, *mut libc::c_void) -> i32>,
    );
    fn Curl_hash_str(key: *mut libc::c_void, key_length: size_t, slots_num: size_t) -> size_t;
    ...
    fn Curl_str2addr(dotted: *mut i8, port: i32) -> *mut Curl_addrinfo;
    fn Curl_resolver_getsock(data: *mut Curl_easy, sock: *mut curl_socket_t) -> i32;
    fn Curl_resolver_is_resolved(data: *mut Curl_easy, dns: *mut *mut Curl_dns_entry) -> CURLcode;
    fn Curl_getaddrinfo(
        data: *mut Curl_easy,
        hostname: *const i8,
        port: i32,
    ...
    fn Curl_rand(data: *mut Curl_easy, rnd: *mut u8, num: size_t) -> CURLcode;
    ...
    fn Curl_doh_is_resolved(data: *mut Curl_easy, dns: *mut *mut Curl_dns_entry) -> CURLcode;
...
#[no_mangle]
pub extern "C" fn Curl_num_addresses(mut addr: *const Curl_addrinfo) -> i32 {
...
#[no_mangle]
pub extern "C" fn Curl_printable_address(
    mut ai: *const Curl_addrinfo,
    mut buf: *mut i8,
    mut bufsize: size_t,
) {
    (unsafe { *buf.offset(0 as i32 as isize) = 0 as i32 as i8 });
    match unsafe { (*ai).ai_family } {
        2 => {
            let mut sa4: *const sockaddr_in =
                (unsafe { (*ai).ai_addr }) as *const libc::c_void as *const sockaddr_in;
            let mut ipaddr4: *const in_addr = unsafe { &(*sa4).sin_addr };
            (unsafe { inet_ntop(
                (*ai).ai_family,
        ...
        10 => {
            let mut sa6: *const sockaddr_in6 =
                (unsafe { (*ai).ai_addr }) as *const libc::c_void as *const sockaddr_in6;
            let mut ipaddr6: *const in6_addr = unsafe { &(*sa6).sin6_addr };
            (unsafe { inet_ntop(
                (*ai).ai_family,
...
extern "C" fn create_hostcache_id(
    mut name: *const i8,
    mut port: i32,
    mut ptr: *mut i8,
    mut buflen: size_t,
) {
    let mut len: size_t = unsafe { strlen(name) };
    ...
    loop {
        ...
        (unsafe { *fresh4 = ({
            ...
            if ::std::mem::size_of::<i32>() as u64 > 1 as i32 as u64 {
                ...
                let fresh3 = name;
                name = name.offset(1);
                __res = *(*__ctype_tolower_loc()).offset(*fresh3 as u8 as i32 as isize);
            }
            __res
        }) as i8 });
    }
...
extern "C" fn hostcache_timestamp_remove(
    ...
    return (0 as i32 as i64 != (unsafe { (*c).timestamp })
        && (unsafe { (*data).now }) - (unsafe { (*c).timestamp }) >= (unsafe { (*data).cache_timeout })) as i32;
}
extern "C" fn hostcache_prune(
    ...
    user.now = now;
    (unsafe { Curl_hash_clean_with_criterium(
        hostcache,
...
#[no_mangle]
pub extern "C" fn Curl_hostcache_prune(mut data: *mut Curl_easy) {
    let mut now: time_t = 0;
    if (unsafe { (*data).set.dns_cache_timeout }) == -(1 as i32) as i64 || (unsafe { (*data).dns.hostcache }).is_null() {
        return;
    }
    ...
    (unsafe { time(&mut now) });
    hostcache_prune(unsafe { (*data).dns.hostcache }, unsafe { (*data).set.dns_cache_timeout }, now);
    if !(unsafe { (*data).share }).is_null() {
...
extern "C" fn fetch_addr(
    ...
    entry_len = unsafe { strlen(entry_id.as_mut_ptr()) };
    dns = (unsafe { Curl_hash_pick(
        (*data).dns.hostcache,
        entry_id.as_mut_ptr() as *mut libc::c_void,
        entry_len.wrapping_add(1 as i32 as u64),
    ) }) as *mut Curl_dns_entry;
    if dns.is_null() && (unsafe { ((*data).state).wildcard_resolve() }) as i32 != 0 {
        create_hostcache_id(
    ...
    if !dns.is_null() && (unsafe { (*data).set.dns_cache_timeout }) != -(1 as i32) as i64 {
        let mut user: hostcache_prune_data = hostcache_prune_data {
            cache_timeout: 0,
            now: 0,
        };
        (unsafe { time(&mut user.now) });
        user.cache_timeout = unsafe { (*data).set.dns_cache_timeout };
        ...
        ) != 0
        {
            (unsafe { Curl_infof(
                data,
                b"Hostname in DNS cache was stale, zapped\0" as *const u8 as *const i8,
            ) });
...
#[no_mangle]
pub extern "C" fn Curl_fetch_addr(
    mut data: *mut Curl_easy,
    mut hostname: *const i8,
    mut port: i32,
) -> *mut Curl_dns_entry {
    ...
    if !dns.is_null() {
        let fresh5 = unsafe { &mut ((*dns).inuse) };
        *fresh5 += 1;
    }
    if !(unsafe { (*data).share }).is_null() {
        (unsafe { Curl_share_unlock(data, CURL_LOCK_DATA_DNS) });
...
extern "C" fn Curl_shuffle_addr(
    mut data: *mut Curl_easy,
    mut addr: *mut *mut Curl_addrinfo,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let num_addrs: i32 = Curl_num_addresses(unsafe { *addr });
    if num_addrs > 1 as i32 {
        ...
        nodes = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
            (num_addrs as u64).wrapping_mul(::std::mem::size_of::<*mut Curl_addrinfo>() as u64),
        ) }) as *mut *mut Curl_addrinfo;
        if !nodes.is_null() {
            let mut i: i32 = 0;
            let mut rnd: *mut u32 = 0 as *mut u32;
            let rnd_size: size_t =
                (num_addrs as u64).wrapping_mul(::std::mem::size_of::<u32>() as u64);
            let fresh6 = unsafe { &mut (*nodes.offset(0 as i32 as isize)) };
            *fresh6 = unsafe { *addr };
            i = 1 as i32;
            while i < num_addrs {
                let fresh7 = unsafe { &mut (*nodes.offset(i as isize)) };
                *fresh7 = unsafe { (**nodes.offset((i - 1 as i32) as isize)).ai_next };
                i += 1;
            }
            rnd = (unsafe { Curl_cmalloc.expect("non-null function pointer")(rnd_size) }) as *mut u32;
            if !rnd.is_null() {
                if (unsafe { Curl_rand(data, rnd as *mut u8, rnd_size) }) as u32 == CURLE_OK as i32 as u32 {
                    let mut swap_tmp: *mut Curl_addrinfo = 0 as *mut Curl_addrinfo;
                    i = num_addrs - 1 as i32;
                    while i > 0 as i32 {
                        swap_tmp = unsafe { *nodes.offset(
                            (*rnd.offset(i as isize)).wrapping_rem((i + 1 as i32) as u32) as isize,
                        ) };
                        let fresh8 = unsafe { &mut (*nodes.offset(
                            (*rnd.offset(i as isize)).wrapping_rem((i + 1 as i32) as u32) as isize,
                        )) };
                        *fresh8 = unsafe { *nodes.offset(i as isize) };
                        let fresh9 = unsafe { &mut (*nodes.offset(i as isize)) };
                        *fresh9 = swap_tmp;
                        i -= 1;
                    }
                    i = 1 as i32;
                    while i < num_addrs {
                        let fresh10 = unsafe { &mut ((**nodes.offset((i - 1 as i32) as isize)).ai_next) };
                        *fresh10 = unsafe { *nodes.offset(i as isize) };
                        i += 1;
                    }
                    let fresh11 = unsafe { &mut ((**nodes.offset((num_addrs - 1 as i32) as isize)).ai_next) };
                    *fresh11 = 0 as *mut Curl_addrinfo;
                    (unsafe { *addr = *nodes.offset(0 as i32 as isize) });
                }
                (unsafe { Curl_cfree.expect("non-null function pointer")(rnd as *mut libc::c_void) });
            } else {
                result = CURLE_OUT_OF_MEMORY;
            }
            (unsafe { Curl_cfree.expect("non-null function pointer")(nodes as *mut libc::c_void) });
        } else {
            result = CURLE_OUT_OF_MEMORY;
        }
    }
...
#[no_mangle]
pub extern "C" fn Curl_cache_addr(
    mut data: *mut Curl_easy,
    ...
    if (unsafe { ((*data).set).dns_shuffle_addresses() }) != 0 {
        let mut result: CURLcode = Curl_shuffle_addr(data, &mut addr);
        if result as u64 != 0 {
            return 0 as *mut Curl_dns_entry;
        }
    }
    dns = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
    ...
    entry_len = unsafe { strlen(entry_id.as_mut_ptr()) };
    (unsafe { (*dns).inuse = 1 as i32 as i64 });
    let fresh12 = unsafe { &mut ((*dns).addr) };
    *fresh12 = addr;
    (unsafe { time(&mut (*dns).timestamp) });
    if (unsafe { (*dns).timestamp }) == 0 as i32 as i64 {
        (unsafe { (*dns).timestamp = 1 as i32 as time_t });
    }
    dns2 = (unsafe { Curl_hash_add(
        (*data).dns.hostcache,
        entry_id.as_mut_ptr() as *mut libc::c_void,
        entry_len.wrapping_add(1 as i32 as u64),
        dns as *mut libc::c_void,
    ...
    if dns2.is_null() {
        (unsafe { Curl_cfree.expect("non-null function pointer")(dns as *mut libc::c_void) });
        return 0 as *mut Curl_dns_entry;
    }
    dns = dns2;
    let fresh13 = unsafe { &mut ((*dns).inuse) };
    *fresh13 += 1;
    return dns;
}
extern "C" fn get_localhost6(mut port: i32) -> *mut Curl_addrinfo {
    let mut ca: *mut Curl_addrinfo = 0 as *mut Curl_addrinfo;
    let ss_size: size_t = ::std::mem::size_of::<sockaddr_in6>() as u64;
    let hostlen: size_t = unsafe { strlen(b"localhost\0" as *const u8 as *const i8) };
    ...
    ca = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
    ...
    (unsafe { (*ca).ai_flags = 0 as i32 });
    (unsafe { (*ca).ai_family = 10 as i32 });
    (unsafe { (*ca).ai_socktype = SOCK_STREAM as i32 });
    (unsafe { (*ca).ai_protocol = IPPROTO_TCP as i32 });
    (unsafe { (*ca).ai_addrlen = ss_size as curl_socklen_t });
    let fresh14 = unsafe { &mut ((*ca).ai_next) };
    *fresh14 = 0 as *mut Curl_addrinfo;
    let fresh15 = unsafe { &mut ((*ca).ai_addr) };
    *fresh15 = (unsafe { (ca as *mut i8).offset(::std::mem::size_of::<Curl_addrinfo>() as u64 as isize) })
    ...
    let fresh16 = unsafe { &mut ((*ca).ai_canonname) };
    *fresh16 = unsafe { ((*ca).ai_addr as *mut i8).offset(ss_size as isize) };
    (unsafe { strcpy((*ca).ai_canonname, b"localhost\0" as *const u8 as *const i8) });
    return ca;
}
extern "C" fn get_localhost(mut port: i32) -> *mut Curl_addrinfo {
    let mut ca: *mut Curl_addrinfo = 0 as *mut Curl_addrinfo;
    ...
    (unsafe { (*ca).ai_flags = 0 as i32 });
    (unsafe { (*ca).ai_family = 2 as i32 });
    (unsafe { (*ca).ai_socktype = SOCK_STREAM as i32 });
    (unsafe { (*ca).ai_protocol = IPPROTO_TCP as i32 });
    (unsafe { (*ca).ai_addrlen = ss_size as curl_socklen_t });
    let fresh17 = unsafe { &mut ((*ca).ai_addr) };
    *fresh17 = (unsafe { (ca as *mut i8).offset(::std::mem::size_of::<Curl_addrinfo>() as u64 as isize) })
        as *mut libc::c_void as *mut sockaddr;
    (unsafe { memcpy(
        (*ca).ai_addr as *mut libc::c_void,
    ...
    let fresh18 = unsafe { &mut ((*ca).ai_canonname) };
    *fresh18 = unsafe { ((*ca).ai_addr as *mut i8).offset(ss_size as isize) };
    (unsafe { strcpy((*ca).ai_canonname, b"localhost\0" as *const u8 as *const i8) });
    let fresh19 = unsafe { &mut ((*ca).ai_next) };
    *fresh19 = get_localhost6(port);
    return ca;
}
#[no_mangle]
pub extern "C" fn Curl_ipv6works(mut data: *mut Curl_easy) -> bool {
    if !data.is_null() {
        return unsafe { (*(*data).multi).ipv6_works };
    } else {
        let mut ipv6_works: i32 = -(1 as i32);
        let mut s: curl_socket_t = unsafe { socket(10 as i32, SOCK_DGRAM as i32, 0 as i32) };
        if s == -(1 as i32) {
            ipv6_works = 0 as i32;
        } else {
            ipv6_works = 1 as i32;
...
#[no_mangle]
pub extern "C" fn Curl_host_is_ipnum(mut hostname: *const i8) -> bool {
...
pub extern "C" fn Curl_resolv(
    ...
    (unsafe { *entry = 0 as *mut Curl_dns_entry });
    let fresh20 = unsafe { &mut ((*conn).bits) };
    ...
    if !dns.is_null() {
        ...
        let fresh21 = unsafe { &mut ((*dns).inuse) };
        *fresh21 += 1;
    ...
    if dns.is_null() {
        ...
        if unsafe { ((*data).set.resolver_start).is_some() } {
            let mut st: i32 = 0;
            (unsafe { Curl_set_in_callback(data, 1 as i32 != 0) });
            st = unsafe { ((*data).set.resolver_start).expect("non-null function pointer")(
        ...
        if addr.is_null() {
            if (unsafe { (*conn).ip_version }) as i32 == 2 as i32 && !Curl_ipv6works(data) {
                return CURLRESOLV_ERROR;
            }
            if (unsafe { Curl_strcasecompare(hostname, b"localhost\0" as *const u8 as *const i8) }) != 0 {
                addr = get_localhost(port);
            } else if allowDOH as i32 != 0 && (unsafe { ((*data).set).doh() }) as i32 != 0 && !ipnum {
                addr = unsafe { Curl_doh(data, hostname, port, &mut respwait) };
            } else {
                if !(unsafe { Curl_ipvalid(data, conn) }) {
                    return CURLRESOLV_ERROR;
                }
                addr = unsafe { Curl_getaddrinfo(data, hostname, port, &mut respwait) };
            }
        }
        if addr.is_null() {
            if respwait != 0 {
...
#[no_mangle]
pub extern "C" fn Curl_resolv_timeout(
    mut data: *mut Curl_easy,
    mut hostname: *const i8,
    mut port: i32,
    mut entry: *mut *mut Curl_dns_entry,
...
#[no_mangle]
pub extern "C" fn Curl_resolv_unlock(mut data: *mut Curl_easy, mut dns: *mut Curl_dns_entry) {
    if !data.is_null() && !(unsafe { (*data).share }).is_null() {
        (unsafe { Curl_share_lock(data, CURL_LOCK_DATA_DNS, CURL_LOCK_ACCESS_SINGLE) });
    }
...
extern "C" fn freednsentry(mut freethis: *mut libc::c_void) {
    let mut dns: *mut Curl_dns_entry = freethis as *mut Curl_dns_entry;
    let fresh22 = unsafe { &mut ((*dns).inuse) };
    *fresh22 -= 1;
    if (unsafe { (*dns).inuse }) == 0 as i32 as i64 {
        (unsafe { Curl_freeaddrinfo((*dns).addr) });
        (unsafe { Curl_cfree.expect("non-null function pointer")(dns as *mut libc::c_void) });
    }
}
#[no_mangle]
pub extern "C" fn Curl_mk_dnscache(mut hash: *mut Curl_hash) -> i32 {
    return unsafe { Curl_hash_init(
        hash,
        7 as i32,
        Some(Curl_hash_str as unsafe extern "C" fn(*mut libc::c_void, size_t, size_t) -> size_t),
        Some(
...
#[no_mangle]
pub extern "C" fn Curl_hostcache_clean(mut data: *mut Curl_easy, mut hash: *mut Curl_hash) {
    if !data.is_null() && !(unsafe { (*data).share }).is_null() {
        (unsafe { Curl_share_lock(data, CURL_LOCK_DATA_DNS, CURL_LOCK_ACCESS_SINGLE) });
    }
    (unsafe { Curl_hash_clean(hash) });
    if !data.is_null() && !(unsafe { (*data).share }).is_null() {
        (unsafe { Curl_share_unlock(data, CURL_LOCK_DATA_DNS) });
    }
}
#[no_mangle]
pub extern "C" fn Curl_loadhostpairs(mut data: *mut Curl_easy) -> CURLcode {
    let mut current_block: u64;
    let mut hostp: *mut curl_slist = 0 as *mut curl_slist;
    let mut hostname: [i8; 256] = [0; 256];
    let mut port: i32 = 0 as i32;
    let fresh23 = unsafe { &mut ((*data).state) };
    (*fresh23).set_wildcard_resolve(0 as i32 as bit);
    hostp = unsafe { (*data).state.resolve };
    while !hostp.is_null() {
        let mut entry_id: [i8; 262] = [0; 262];
        if !(unsafe { (*hostp).data }).is_null() {
            if (unsafe { *((*hostp).data).offset(0 as i32 as isize) }) as i32 == '-' as i32 {
                let mut entry_len: size_t = 0;
                if 2 as i32
                    != (unsafe { sscanf(
                    ...
                    ) })
                {
                    (unsafe { Curl_infof(
                        data,
                        b"Couldn't parse CURLOPT_RESOLVE removal entry '%s'\0" as *const u8
                            as *const i8,
                        (*hostp).data,
                    ) });
                } else {
                ...
                host_begin = unsafe { (*hostp).data };
                if (unsafe { *host_begin.offset(0 as i32 as isize) }) as i32 == '+' as i32 {
                    host_begin = unsafe { host_begin.offset(1) };
                    permanent = 0 as i32 != 0;
                }
                host_end = unsafe { strchr(host_begin, ':' as i32) };
                if !(host_end.is_null()
                    || (unsafe { host_end.offset_from(host_begin) }) as i64
                        >= ::std::mem::size_of::<[i8; 256]>() as u64 as ptrdiff_t)
                {
                    (unsafe { memcpy(
                    ...
                    hostname[(unsafe { host_end.offset_from(host_begin) }) as i64 as usize] =
                        '\u{0}' as i32 as i8;
                    port_ptr = unsafe { host_end.offset(1 as i32 as isize) };
                    tmp_port = unsafe { strtoul(port_ptr, &mut end_ptr, 10 as i32) };
                    if !(tmp_port > (32767 as i32 * 2 as i32 + 1 as i32) as u64
                        || end_ptr == port_ptr
                        || (unsafe { *end_ptr }) as i32 != ':' as i32)
                    {
                        port = tmp_port as i32;
                        addresses = unsafe { end_ptr.offset(1 as i32 as isize) };
                        loop {
                            if !((unsafe { *end_ptr }) != 0) {
                            ...
                            addr_begin = unsafe { end_ptr.offset(1 as i32 as isize) };
                            addr_end = unsafe { strchr(addr_begin, ',' as i32) };
                            if addr_end.is_null() {
                                addr_end = unsafe { addr_begin.offset(strlen(addr_begin) as isize) };
                            }
                            end_ptr = addr_end;
                            if (unsafe { *addr_begin }) as i32 == '[' as i32 {
                                if addr_end == addr_begin
                                    || (unsafe { *addr_end.offset(-(1 as i32 as isize)) }) as i32 != ']' as i32
                                {
                                    current_block = 2415380317517078313;
                                    break;
                                }
                                addr_begin = unsafe { addr_begin.offset(1) };
                                addr_end = unsafe { addr_end.offset(-1) };
                            }
                            alen = (unsafe { addr_end.offset_from(addr_begin) }) as i64 as size_t;
                            if alen == 0 {
                                continue;
                            }
                            if alen >= ::std::mem::size_of::<[i8; 64]>() as u64 {
                                current_block = 2415380317517078313;
                                break;
                            }
                            (unsafe { memcpy(
                            ...
                            ai = unsafe { Curl_str2addr(address.as_mut_ptr(), port) };
                            if ai.is_null() {
                                (unsafe { Curl_infof(
                                    data,
                                ...
                                let fresh24 = unsafe { &mut ((*tail).ai_next) };
                                *fresh24 = ai;
                ...
                if error {
                    (unsafe { Curl_failf(
                        data,
                        b"Couldn't parse CURLOPT_RESOLVE entry '%s'!\0" as *const u8 as *const i8,
                        (*hostp).data,
                    ) });
                    (unsafe { Curl_freeaddrinfo(head) });
                ...
                entry_len_0 = unsafe { strlen(entry_id.as_mut_ptr()) };
                if !(unsafe { (*data).share }).is_null() {
                    (unsafe { Curl_share_lock(data, CURL_LOCK_DATA_DNS, CURL_LOCK_ACCESS_SINGLE) });
                }
                dns = (unsafe { Curl_hash_pick(
                ...
                if !dns.is_null() {
                    (unsafe { Curl_infof(
                        data,
                        b"RESOLVE %s:%d is - old addresses discarded!\0" as *const u8 as *const i8,
                        hostname.as_mut_ptr(),
                        port,
                    ) });
                    (unsafe { Curl_hash_delete(
                ...
                if !dns.is_null() {
                    if permanent {
                        (unsafe { (*dns).timestamp = 0 as i32 as time_t });
                    }
                    let fresh25 = unsafe { &mut ((*dns).inuse) };
                    *fresh25 -= 1;
                }
                if !(unsafe { (*data).share }).is_null() {
                    (unsafe { Curl_share_unlock(data, CURL_LOCK_DATA_DNS) });
                ...
                (unsafe { Curl_infof(
                    data,
                    b"Added %s:%d:%s to DNS cache%s\0" as *const u8 as *const i8,
                    hostname.as_mut_ptr(),
                    port,
                    addresses,
                ...
                if hostname[0 as i32 as usize] as i32 == '*' as i32
                    && hostname[1 as i32 as usize] as i32 == '\u{0}' as i32
                {
                    (unsafe { Curl_infof(
                        data,
                        b"RESOLVE %s:%d is wildcard, enabling wildcard checks\0" as *const u8
                            as *const i8,
                        hostname.as_mut_ptr(),
                        port,
                    ) });
                    let fresh26 = unsafe { &mut ((*data).state) };
                    (*fresh26).set_wildcard_resolve(1 as i32 as bit);
                }
            }
        }
        hostp = unsafe { (*hostp).next };
    }
    let fresh27 = unsafe { &mut ((*data).state.resolve) };
    *fresh27 = 0 as *mut curl_slist;
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_resolv_check(
    mut data: *mut Curl_easy,
    mut dns: *mut *mut Curl_dns_entry,
) -> CURLcode {
    if (unsafe { ((*(*data).conn).bits).doh() }) != 0 {
        return unsafe { Curl_doh_is_resolved(data, dns) };
    }
    return unsafe { Curl_resolver_is_resolved(data, dns) };
}
#[no_mangle]
pub extern "C" fn Curl_resolv_getsock(
    mut data: *mut Curl_easy,
    ...
    return unsafe { Curl_resolver_getsock(data, socks) };
}
#[no_mangle]
pub extern "C" fn Curl_once_resolved(
    mut data: *mut Curl_easy,
    mut protocol_done: *mut bool,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    if !(unsafe { (*data).state.async_0.dns }).is_null() {
        let fresh28 = unsafe { &mut ((*conn).dns_entry) };
        *fresh28 = unsafe { (*data).state.async_0.dns };
        let fresh29 = unsafe { &mut ((*data).state.async_0.dns) };
        *fresh29 = 0 as *mut Curl_dns_entry;
    }
    result = unsafe { Curl_setup_conn(data, protocol_done) };
    if result as u64 != 0 {
        (unsafe { Curl_detach_connnection(data) });
        (unsafe { Curl_conncache_remove_conn(data, conn, 1 as i32 != 0) });
        (unsafe { Curl_disconnect(data, conn, 1 as i32 != 0) });
    }
    return result;
}
#[no_mangle]
pub extern "C" fn Curl_resolver_error(mut data: *mut Curl_easy) -> CURLcode {
