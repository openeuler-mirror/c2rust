...
#[no_mangle]
pub extern "C" fn Curl_speedinit(mut data: *mut Curl_easy) {
    (unsafe { memset(
...
#[no_mangle]
pub extern "C" fn Curl_speedcheck(mut data: *mut Curl_easy, mut now: curltime) -> CURLcode {
    if (unsafe { (*data).req.keepon }) & (1 as i32) << 4 as i32 != 0 {
        return CURLE_OK;
    }
    if (unsafe { (*data).progress.current_speed }) >= 0 as i32 as i64 && (unsafe { (*data).set.low_speed_time }) != 0 {
        if (unsafe { (*data).progress.current_speed }) < (unsafe { (*data).set.low_speed_limit }) {
            if (unsafe { (*data).state.keeps_speed.tv_sec }) == 0 {
                (unsafe { (*data).state.keeps_speed = now });
            } else {
                let mut howlong: timediff_t = unsafe { Curl_timediff(now, (*data).state.keeps_speed) };
                if howlong >= (unsafe { (*data).set.low_speed_time }) * 1000 as i32 as i64 {
                    (unsafe { Curl_failf (data , b"Operation too slow. Less than %ld bytes/sec transferred the last %ld seconds\0" as * const u8 as * const i8 , (* data) . set . low_speed_limit , (* data) . set . low_speed_time ,) }) ;
    ...
    if (unsafe { (*data).set.low_speed_limit }) != 0 {
        (unsafe { Curl_expire(data, 1000 as i32 as timediff_t, EXPIRE_SPEEDCHECK) });
    }
    return CURLE_OK;
}
