...
pub extern "C" fn Curl_httpchunk_init(mut data: *mut Curl_easy) {
    ...
    (unsafe { (*chunk).hexindex = 0 as i32 as u8 });
    (unsafe { (*chunk).state = CHUNK_HEX });
    (unsafe { Curl_dyn_init(&mut (*conn).trailer, 4096 as i32 as size_t) });
}
#[no_mangle]
pub extern "C" fn Curl_httpchunk_read(
    ...
    while length != 0 {
        ...
        match (unsafe { (*ch).state }) as u32 {
            ...
            2 => {
                piece = unsafe { curlx_sotouz(if (*ch).datasize >= length {
                    length
                } else {
                    (*ch).datasize
                }) };
                if (unsafe { ((*data).set).http_te_skip() }) == 0 && (unsafe { (*k).ignorebody() }) == 0 {
                    if (unsafe { ((*data).set).http_ce_skip() }) == 0 && !(unsafe { (*k).writer_stack }).is_null() {
                        result = unsafe { Curl_unencode_write(data, (*k).writer_stack, datap, piece) };
                    } else {
                        result = unsafe { Curl_client_write(data, (1 as i32) << 0 as i32, datap, piece) };
                    }
                    if result as u64 != 0 {
                        (unsafe { *extrap = result });
                        return CHUNKE_PASSTHRU_ERROR;
                ...
                let fresh1 = unsafe { &mut ((*ch).datasize) };
            ...
            5 => {
                if (unsafe { *datap }) as i32 == 0xd as i32 || (unsafe { *datap }) as i32 == 0xa as i32 {
                    let mut tr: *mut i8 = unsafe { Curl_dyn_ptr(&mut (*conn).trailer) };
                    if !tr.is_null() {
                        ...
                        tr = unsafe { Curl_dyn_ptr(&mut (*conn).trailer) };
                        trlen = unsafe { Curl_dyn_len(&mut (*conn).trailer) };
                        result = CURLE_OK as i32 as CURLcode;
                        if result as u64 != 0 {
                            return CHUNKE_BAD_CHUNK;
                        }
                        if (unsafe { ((*data).set).http_te_skip() }) == 0 {
                            result = unsafe { Curl_client_write(data, (1 as i32) << 1 as i32, tr, trlen) };
                            if result as u64 != 0 {
                                (unsafe { *extrap = result });
                                return CHUNKE_PASSTHRU_ERROR;
                            }
                        }
                        (unsafe { Curl_dyn_reset(&mut (*conn).trailer) });
...
#[no_mangle]
pub extern "C" fn Curl_chunked_strerror(mut code: CHUNKcode) -> *const i8 {
    match code as i32 {
