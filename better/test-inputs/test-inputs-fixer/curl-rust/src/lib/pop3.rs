...
extern "C" {
    ...
    fn Curl_MD5_update(context: *mut MD5_context, data: *const u8, len: u32) -> CURLcode;
    fn Curl_MD5_final(context: *mut MD5_context, result: *mut u8) -> CURLcode;
...
pub type Curl_MD5_final_func = Option<unsafe extern "C" fn(*mut u8, *mut libc::c_void) -> ()>;
pub type Curl_MD5_update_func =
    Option<unsafe extern "C" fn(*mut libc::c_void, *const u8, u32) -> ()>;
...
#[no_mangle]
pub static mut Curl_handler_pop3: Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_it: Some(pop3_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: Some(
                pop3_done as unsafe extern "C" fn(*mut Curl_easy, CURLcode, bool) -> CURLcode,
            ),
            do_more: None,
            connect_it: Some(
                pop3_connect as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            connecting: Some(
                pop3_multi_statemach as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            doing: Some(pop3_doing as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            proto_getsock: Some(
                pop3_getsock
...
#[no_mangle]
pub static mut Curl_handler_pop3s: Curl_handler =  {
    {
        let mut init = Curl_handler {
...
static mut saslpop3: SASLproto =  {
    {
        let mut init = SASLproto {
            ...
            getmessage: Some(pop3_get_message as unsafe extern "C" fn(*mut i8, *mut *mut i8) -> ()),
        };
        init
    }
};
extern "C" fn pop3_to_pop3s(mut conn: *mut connectdata) {
    let fresh0 = unsafe { &mut ((*conn).handler) };
    *fresh0 = unsafe { &Curl_handler_pop3s };
    let fresh1 = unsafe { &mut ((*conn).bits) };
    (*fresh1).set_tls_upgraded(1 as i32 as bit);
}
extern "C" fn pop3_endofresp(
    ...
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    ...
    if (unsafe { (*pop3c).state }) as u32 == POP3_CAPA as i32 as u32 {
        if len >= 1 as i32 as u64 && (unsafe { *line.offset(0 as i32 as isize) }) as i32 == '.' as i32 {
            (unsafe { *resp = '+' as i32 });
        } else {
            (unsafe { *resp = '*' as i32 });
        }
...
extern "C" fn pop3_get_message(mut buffer: *mut i8, mut outptr: *mut *mut i8) {
...
extern "C" fn state(mut data: *mut Curl_easy, mut newstate: pop3state) {
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*(*data).conn).proto.pop3c };
    (unsafe { (*pop3c).state = newstate });
}
extern "C" fn pop3_perform_capa(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    (unsafe { (*pop3c).sasl.authmechs = 0 as i32 as u16 });
    (unsafe { (*pop3c).sasl.authused = 0 as i32 as u16 });
    (unsafe { (*pop3c).tls_supported = 0 as i32 != 0 });
...
extern "C" fn pop3_perform_starttls(
...
extern "C" fn pop3_perform_upgrade_tls(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
) -> CURLcode {
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut result: CURLcode =
        unsafe { Curl_ssl_connect_nonblocking(data, conn, 0 as i32 != 0, 0 as i32, &mut (*pop3c).ssldone) };
    if result as u64 == 0 {
        if (unsafe { (*pop3c).state }) as u32 != POP3_UPGRADETLS as i32 as u32 {
            state(data, POP3_UPGRADETLS);
        }
        if unsafe { (*pop3c).ssldone } {
            pop3_to_pop3s(conn);
...
extern "C" fn pop3_perform_user(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
...
extern "C" fn pop3_perform_apop(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut i: size_t = 0;
    ...
    ctxt = unsafe { Curl_MD5_init(Curl_DIGEST_MD5.as_ptr()) };
    if ctxt.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { Curl_MD5_update(
        ctxt,
    ...
    (unsafe { Curl_MD5_final(ctxt, digest.as_mut_ptr()) });
    i = 0 as i32 as size_t;
    while i < 16 as i32 as u64 {
        (unsafe { curl_msnprintf(
            &mut *secret
                .as_mut_ptr()
                .offset((2 as i32 as u64).wrapping_mul(i) as isize) as *mut i8,
            3 as i32 as size_t,
...
extern "C" fn pop3_continue_auth(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut resp: *const i8,
) -> CURLcode {
...
extern "C" fn pop3_perform_authentication(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    ...
    if !(unsafe { Curl_sasl_can_authenticate(&mut (*pop3c).sasl, conn) }) {
        state(data, POP3_STOP);
        return result;
    }
    if (unsafe { (*pop3c).authtypes }) & (unsafe { (*pop3c).preftype }) & ((1 as i32) << 2 as i32) as u32 != 0 {
        result = unsafe { Curl_sasl_start(&mut (*pop3c).sasl, data, conn, 0 as i32 != 0, &mut progress) };
        if result as u64 == 0 {
    ...
    if result as u64 == 0 && progress as u32 == SASL_IDLE as i32 as u32 {
        if (unsafe { (*pop3c).authtypes }) & (unsafe { (*pop3c).preftype }) & ((1 as i32) << 1 as i32) as u32 != 0 {
            result = pop3_perform_apop(data, conn);
        } else if (unsafe { (*pop3c).authtypes }) & (unsafe { (*pop3c).preftype }) & ((1 as i32) << 0 as i32) as u32 != 0 {
...
extern "C" fn pop3_perform_command(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut pop3: *mut POP3 = unsafe { (*data).req.p.pop3 };
    let mut command: *const i8 = 0 as *const i8;
    if (unsafe { *((*pop3).id).offset(0 as i32 as isize) }) as i32 == '\u{0}' as i32
        || (unsafe { ((*data).set).list_only() }) as i32 != 0
    {
        command = b"LIST\0" as *const u8 as *const i8;
        if (unsafe { *((*pop3).id).offset(0 as i32 as isize) }) as i32 != '\u{0}' as i32 {
            (unsafe { (*pop3).transfer = PPTRANSFER_INFO });
        }
    } else {
    ...
    if (unsafe { *((*pop3).id).offset(0 as i32 as isize) }) as i32 != '\u{0}' as i32 {
        result = unsafe { Curl_pp_sendf(
            ...
            if !((*pop3).custom).is_null()
                && *((*pop3).custom).offset(0 as i32 as isize) as i32 != '\u{0}' as i32
            {
...
extern "C" fn pop3_perform_quit(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = unsafe { Curl_pp_sendf(
        data,
        &mut (*conn).proto.pop3c.pp as *mut pingpong,
        b"%s\0" as *const u8 as *const i8,
...
extern "C" fn pop3_state_servergreet_resp(
    mut data: *mut Curl_easy,
    mut pop3code: i32,
    mut _instate: pop3state,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    ...
    if pop3code != '+' as i32 {
        ...
        if len >= 4 as i32 as u64
            && (unsafe { *line.offset(len.wrapping_sub(2 as i32 as u64) as isize) }) as i32 == '>' as i32
        {
            let mut i: size_t = 0;
            i = 3 as i32 as size_t;
            while i < len.wrapping_sub(2 as i32 as u64) {
                if (unsafe { *line.offset(i as isize) }) as i32 == '<' as i32 {
                    let mut timestamplen: size_t =
                        len.wrapping_sub(1 as i32 as u64).wrapping_sub(i);
                    let mut at: *mut i8 = 0 as *mut i8;
                    if timestamplen == 0 {
                        break;
                    }
                    let fresh3 = unsafe { &mut ((*pop3c).apoptimestamp) };
                    *fresh3 = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
                        1 as i32 as size_t,
                        timestamplen.wrapping_add(1 as i32 as u64),
                    ) }) as *mut i8;
                    if (unsafe { (*pop3c).apoptimestamp }).is_null() {
                        break;
                    }
                    (unsafe { memcpy(
                    ...
                    (unsafe { *((*pop3c).apoptimestamp).offset(timestamplen as isize) = '\u{0}' as i32 as i8 });
                    at = unsafe { strchr((*pop3c).apoptimestamp, '@' as i32) };
                    if at.is_null() {
                        (unsafe { Curl_cfree.expect("non-null function pointer")(
                            (*pop3c).apoptimestamp as *mut libc::c_void,
                        ) });
                        let fresh4 = unsafe { &mut ((*pop3c).apoptimestamp) };
                        *fresh4 = 0 as *mut i8;
                    } else {
                        (unsafe { (*pop3c).authtypes |= ((1 as i32) << 1 as i32) as u32 });
                    }
                    break;
                } else {
                    i = i.wrapping_add(1);
...
extern "C" fn pop3_state_capa_resp(
    mut data: *mut Curl_easy,
    mut pop3code: i32,
    mut _instate: pop3state,
) -> CURLcode {
    ...
    if pop3code == '*' as i32 {
        ...
        {
            (unsafe { (*pop3c).tls_supported = 1 as i32 != 0 });
        ...
        {
            (unsafe { (*pop3c).authtypes |= ((1 as i32) << 0 as i32) as u32 });
        } else if len >= 5 as i32 as u64
            && (unsafe { memcmp(
                line as *const libc::c_void,
                b"SASL \0" as *const u8 as *const i8 as *const libc::c_void,
                5 as i32 as u64,
            ) }) == 0
        {
            (unsafe { (*pop3c).authtypes |= ((1 as i32) << 2 as i32) as u32 });
            ...
            loop {
                ...
                if mechbit as i32 != 0 && llen == wordlen {
                    let fresh5 = unsafe { &mut ((*pop3c).sasl.authmechs) };
        ...
        if (unsafe { (*data).set.use_ssl }) as u64 == 0 || (unsafe { ((*conn).ssl[0 as i32 as usize]).use_0() }) as i32 != 0 {
            result = pop3_perform_authentication(data, conn);
        } else if pop3code == '+' as i32 && (unsafe { (*pop3c).tls_supported }) as i32 != 0 {
            result = pop3_perform_starttls(data, conn);
        } else if (unsafe { (*data).set.use_ssl }) as u32 <= CURLUSESSL_TRY as i32 as u32 {
            result = pop3_perform_authentication(data, conn);
        } else {
            (unsafe { Curl_failf(data, b"STLS not supported.\0" as *const u8 as *const i8) });
            result = CURLE_USE_SSL_FAILED;
        }
    }
    return result;
}
extern "C" fn pop3_state_starttls_resp(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut pop3code: i32,
    mut _instate: pop3state,
    ...
    let mut result: CURLcode = CURLE_OK;
    if (unsafe { (*(*data).conn).proto.pop3c.pp.cache_size }) != 0 {
...
extern "C" fn pop3_state_auth_resp(
    mut data: *mut Curl_easy,
    mut pop3code: i32,
    mut _instate: pop3state,
) -> CURLcode {
    ...
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut progress: saslprogress = SASL_IDLE;
    result = unsafe { Curl_sasl_continue(&mut (*pop3c).sasl, data, conn, pop3code, &mut progress) };
    if result as u64 == 0 {
        match progress as u32 {
            ...
            0 => {
                if (unsafe { (*pop3c).authtypes }) & (unsafe { (*pop3c).preftype }) & ((1 as i32) << 1 as i32) as u32 != 0 {
                    result = pop3_perform_apop(data, conn);
                } else if (unsafe { (*pop3c).authtypes }) & (unsafe { (*pop3c).preftype }) & ((1 as i32) << 0 as i32) as u32
                    != 0
                {
                    result = pop3_perform_user(data, conn);
                } else {
...
extern "C" fn pop3_state_apop_resp(
    mut data: *mut Curl_easy,
    mut pop3code: i32,
    mut _instate: pop3state,
) -> CURLcode {
...
extern "C" fn pop3_state_user_resp(
    mut data: *mut Curl_easy,
    mut pop3code: i32,
    mut _instate: pop3state,
) -> CURLcode {
...
extern "C" fn pop3_state_pass_resp(
    mut data: *mut Curl_easy,
    mut pop3code: i32,
    mut _instate: pop3state,
) -> CURLcode {
...
extern "C" fn pop3_state_command_resp(
    mut data: *mut Curl_easy,
    mut pop3code: i32,
    mut _instate: pop3state,
) -> CURLcode {
    ...
    let mut pop3: *mut POP3 = unsafe { (*data).req.p.pop3 };
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut pp: *mut pingpong = unsafe { &mut (*pop3c).pp };
    if pop3code != '+' as i32 {
        state(data, POP3_STOP);
        return CURLE_RECV_ERROR;
    }
    (unsafe { (*pop3c).eob = 2 as i32 as size_t });
    (unsafe { (*pop3c).strip = 2 as i32 as size_t });
    if (unsafe { (*pop3).transfer }) as u32 == PPTRANSFER_BODY as i32 as u32 {
        ...
        if !(unsafe { (*pp).cache }).is_null() {
            if (unsafe { ((*data).set).opt_no_body() }) == 0 {
                result = Curl_pop3_write(data, unsafe { (*pp).cache }, unsafe { (*pp).cache_size });
                if result as u64 != 0 {
                    return result;
                }
            }
            (unsafe { Curl_cfree.expect("non-null function pointer")((*pp).cache as *mut libc::c_void) });
            let fresh6 = unsafe { &mut ((*pp).cache) };
            *fresh6 = 0 as *mut i8;
...
extern "C" fn pop3_statemachine(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    ...
    loop {
        result = unsafe { Curl_pp_readresp(data, sock, pp, &mut pop3code, &mut nread) };
        if result as u64 != 0 {
            return result;
        }
        if pop3code == 0 {
        ...
        match (unsafe { (*pop3c).state }) as u32 {
            1 => {
                result = pop3_state_servergreet_resp(data, pop3code, unsafe { (*pop3c).state });
            }
            2 => {
                result = pop3_state_capa_resp(data, pop3code, unsafe { (*pop3c).state });
            }
            3 => {
                result = pop3_state_starttls_resp(data, conn, pop3code, unsafe { (*pop3c).state });
            }
            5 => {
                result = pop3_state_auth_resp(data, pop3code, unsafe { (*pop3c).state });
            }
            6 => {
                result = pop3_state_apop_resp(data, pop3code, unsafe { (*pop3c).state });
            }
            7 => {
                result = pop3_state_user_resp(data, pop3code, unsafe { (*pop3c).state });
            }
            8 => {
                result = pop3_state_pass_resp(data, pop3code, unsafe { (*pop3c).state });
            }
            9 => {
                result = pop3_state_command_resp(data, pop3code, unsafe { (*pop3c).state });
            }
            10 | _ => {
                state(data, POP3_STOP);
            }
        ...
        if !(result as u64 == 0
            && (unsafe { (*pop3c).state }) as u32 != POP3_STOP as i32 as u32
...
extern "C" fn pop3_multi_statemach(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    if (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 0 as i32) as u32 != 0 && !(unsafe { (*pop3c).ssldone }) {
        ...
        if result as u32 != 0 || !(unsafe { (*pop3c).ssldone }) {
            return result;
        }
    }
    result = unsafe { Curl_pp_statemach(data, &mut (*pop3c).pp, 0 as i32 != 0, 0 as i32 != 0) };
    (unsafe { *done = if (*pop3c).state as u32 == POP3_STOP as i32 as u32 {
        1 as i32
    } else {
        0 as i32
    } != 0 });
...
extern "C" fn pop3_block_statemach(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    ...
    let mut result: CURLcode = CURLE_OK;
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    while (unsafe { (*pop3c).state }) as u32 != POP3_STOP as i32 as u32 && result as u64 == 0 {
        result = unsafe { Curl_pp_statemach(data, &mut (*pop3c).pp, 1 as i32 != 0, disconnecting) };
    }
    return result;
}
extern "C" fn pop3_init(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut pop3: *mut POP3 = 0 as *mut POP3;
    let fresh7 = unsafe { &mut ((*data).req.p.pop3) };
    *fresh7 = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<POP3>() as u64,
        1 as i32 as size_t,
    ) }) as *mut POP3;
    pop3 = *fresh7;
    if pop3.is_null() {
...
extern "C" fn pop3_getsock(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut socks: *mut curl_socket_t,
) -> i32 {
    return unsafe { Curl_pp_getsock(data, &mut (*conn).proto.pop3c.pp, socks) };
}
extern "C" fn pop3_connect(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut pp: *mut pingpong = unsafe { &mut (*pop3c).pp };
    ...
    let fresh8 = unsafe { &mut ((*pp).statemachine) };
    *fresh8 = Some(
        pop3_statemachine as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata) -> CURLcode,
    );
    let fresh9 = unsafe { &mut ((*pp).endofresp) };
    *fresh9 = Some(
        pop3_endofresp
            as unsafe extern "C" fn(
                *mut Curl_easy,
    ...
    (unsafe { (*pop3c).preftype = !(0 as u32) });
    (unsafe { Curl_sasl_init(&mut (*pop3c).sasl, &saslpop3) });
...
extern "C" fn pop3_done(
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")((*pop3).id as *mut libc::c_void) });
    let fresh10 = unsafe { &mut ((*pop3).id) };
    *fresh10 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*pop3).custom as *mut libc::c_void) });
    let fresh11 = unsafe { &mut ((*pop3).custom) };
    *fresh11 = 0 as *mut i8;
    (unsafe { (*pop3).transfer = PPTRANSFER_BODY });
    return result;
}
extern "C" fn pop3_perform(
    mut data: *mut Curl_easy,
...
extern "C" fn pop3_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    (unsafe { *done = 0 as i32 != 0 });
...
extern "C" fn pop3_disconnect(
    ...
    (unsafe { Curl_pp_disconnect(&mut (*pop3c).pp) });
    (unsafe { Curl_sasl_cleanup(conn, (*pop3c).sasl.authused as u32) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*pop3c).apoptimestamp as *mut libc::c_void) });
    let fresh12 = unsafe { &mut ((*pop3c).apoptimestamp) };
    *fresh12 = 0 as *mut i8;
    return CURLE_OK;
}
extern "C" fn pop3_dophase_done(mut _data: *mut Curl_easy, mut _connected: bool) -> CURLcode {
    return CURLE_OK;
}
extern "C" fn pop3_doing(mut data: *mut Curl_easy, mut dophase_done: *mut bool) -> CURLcode {
...
extern "C" fn pop3_regular_transfer(
    mut data: *mut Curl_easy,
    mut dophase_done: *mut bool,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
...
extern "C" fn pop3_setup_connection(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
) -> CURLcode {
...
extern "C" fn pop3_parse_url_options(mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut pop3c: *mut pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut ptr: *const i8 = unsafe { (*conn).options };
    (unsafe { (*pop3c).sasl.resetprefs = 1 as i32 != 0 });
    while result as u64 == 0 && !ptr.is_null() && (unsafe { *ptr }) as i32 != 0 {
        ...
        ) }) != 0
        {
            ...
            {
                (unsafe { (*pop3c).preftype = ((1 as i32) << 1 as i32) as u32 });
                (unsafe { (*pop3c).sasl.prefmech = 0 as i32 as u16 });
                result = CURLE_OK;
            }
        } else {
    ...
    if (unsafe { (*pop3c).preftype }) != ((1 as i32) << 1 as i32) as u32 {
        match (unsafe { (*pop3c).sasl.prefmech }) as i32 {
            0 => {
                (unsafe { (*pop3c).preftype = 0 as i32 as u32 });
            }
            65503 => {
                (unsafe { (*pop3c).preftype = !(0 as u32) });
            }
            _ => {
                (unsafe { (*pop3c).preftype = ((1 as i32) << 2 as i32) as u32 });
            }
        }
    }
    return result;
}
extern "C" fn pop3_parse_url_path(mut data: *mut Curl_easy) -> CURLcode {
    let mut pop3: *mut POP3 = unsafe { (*data).req.p.pop3 };
...
extern "C" fn pop3_parse_custom_request(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut pop3: *mut POP3 = unsafe { (*data).req.p.pop3 };
...
#[no_mangle]
pub extern "C" fn Curl_pop3_write(
    mut data: *mut Curl_easy,
    mut str: *mut i8,
    mut nread: size_t,
) -> CURLcode {
    ...
    i = 0 as i32 as size_t;
    while i < nread {
        let mut prev: size_t = unsafe { (*pop3c).eob };
        match (unsafe { *str.offset(i as isize) }) as i32 {
            13 => {
                if (unsafe { (*pop3c).eob }) == 0 as i32 as u64 {
                    let fresh14 = unsafe { &mut ((*pop3c).eob) };
                ...
                } else if (unsafe { (*pop3c).eob }) == 3 as i32 as u64 {
                    let fresh15 = unsafe { &mut ((*pop3c).eob) };
                    *fresh15 = (*fresh15).wrapping_add(1);
                } else {
                    (unsafe { (*pop3c).eob = 1 as i32 as size_t });
                }
            }
            10 => {
                if (unsafe { (*pop3c).eob }) == 1 as i32 as u64 || (unsafe { (*pop3c).eob }) == 4 as i32 as u64 {
                    let fresh16 = unsafe { &mut ((*pop3c).eob) };
                    *fresh16 = (*fresh16).wrapping_add(1);
                } else {
                    (unsafe { (*pop3c).eob = 0 as i32 as size_t });
                }
            }
            46 => {
                if (unsafe { (*pop3c).eob }) == 2 as i32 as u64 {
                    let fresh17 = unsafe { &mut ((*pop3c).eob) };
        ...
        if prev != 0 && prev >= (unsafe { (*pop3c).eob }) {
            while prev != 0 && (unsafe { (*pop3c).strip }) != 0 {
                prev = prev.wrapping_sub(1);
                let fresh18 = unsafe { &mut ((*pop3c).strip) };
                *fresh18 = (*fresh18).wrapping_sub(1);
            }
            if prev != 0 {
                if strip_dot as i32 != 0 && prev.wrapping_sub(1 as i32 as u64) > 0 as i32 as u64 {
                    result = unsafe { Curl_client_write(
                        data,
                        (1 as i32) << 0 as i32,
    ...
    if (unsafe { (*pop3c).eob }) == 5 as i32 as u64 {
        result = unsafe { Curl_client_write(
            data,
            (1 as i32) << 0 as i32,
            b"\r\n.\r\n\0" as *const u8 as *const i8 as *mut i8,
