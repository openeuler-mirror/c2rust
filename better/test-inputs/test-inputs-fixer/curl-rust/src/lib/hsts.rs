...
#[no_mangle]
pub extern "C" fn Curl_hsts_init() -> *mut hsts {
    let mut h: *mut hsts = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<hsts>() as u64,
        1 as i32 as size_t,
    ) }) as *mut hsts;
    if !h.is_null() {
        (unsafe { Curl_llist_init(&mut (*h).list, None) });
    }
    return h;
}
extern "C" fn hsts_free(mut e: *mut stsentry) {
    (unsafe { Curl_cfree.expect("non-null function pointer")((*e).host as *mut i8 as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")(e as *mut libc::c_void) });
}
#[no_mangle]
pub extern "C" fn Curl_hsts_cleanup(mut hp: *mut *mut hsts) {
    let mut h: *mut hsts = unsafe { *hp };
    if !h.is_null() {
        let mut e: *mut Curl_llist_element = 0 as *mut Curl_llist_element;
        let mut n: *mut Curl_llist_element = 0 as *mut Curl_llist_element;
        e = unsafe { (*h).list.head };
        while !e.is_null() {
            let mut sts: *mut stsentry = (unsafe { (*e).ptr }) as *mut stsentry;
            n = unsafe { (*e).next };
            hsts_free(sts);
            e = n;
        }
        (unsafe { Curl_cfree.expect("non-null function pointer")((*h).filename as *mut libc::c_void) });
        (unsafe { Curl_cfree.expect("non-null function pointer")(h as *mut libc::c_void) });
        (unsafe { *hp = 0 as *mut hsts });
    }
}
extern "C" fn hsts_entry() -> *mut stsentry {
    return (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<stsentry>() as u64,
        1 as i32 as size_t,
    ) }) as *mut stsentry;
}
extern "C" fn hsts_create(
    mut h: *mut hsts,
    ...
    (unsafe { (*sts).expires = expires });
    (unsafe { (*sts).includeSubDomains = subdomains });
    let fresh0 = unsafe { &mut ((*sts).host) };
    *fresh0 = unsafe { Curl_cstrdup.expect("non-null function pointer")(hostname) };
    if (unsafe { (*sts).host }).is_null() {
        (unsafe { Curl_cfree.expect("non-null function pointer")(sts as *mut libc::c_void) });
        return CURLE_OUT_OF_MEMORY;
    }
...
#[no_mangle]
pub extern "C" fn Curl_hsts_parse(
    mut h: *mut hsts,
    mut hostname: *const i8,
    mut header: *const i8,
) -> CURLcode {
    ...
    loop {
        while (unsafe { *p }) as i32 != 0 && (unsafe { Curl_isspace(*p as u8 as i32) }) != 0 {
            p = unsafe { p.offset(1) };
        }
        ...
        ) }) != 0
        {
            ...
            offt = unsafe { curlx_strtoofft(p, &mut endp, 10 as i32, &mut expires) };
        ...
        ) }) != 0
        {
            ...
            p = unsafe { p.offset(17 as i32 as isize) };
            gotinc = 1 as i32 != 0;
        } else {
    ...
    if expires == 0 {
        ...
        if !sts.is_null() {
            (unsafe { Curl_llist_remove(&mut (*h).list, &mut (*sts).node, 0 as *mut libc::c_void) });
            hsts_free(sts);
        }
        return CURLE_OK;
    }
...
pub extern "C" fn Curl_hsts(
    ...
    if !h.is_null() {
        ...
        while !e.is_null() {
            let mut sts: *mut stsentry = (unsafe { (*e).ptr }) as *mut stsentry;
            n = unsafe { (*e).next };
            if (unsafe { (*sts).expires }) <= now {
                (unsafe { Curl_llist_remove(&mut (*h).list, &mut (*sts).node, 0 as *mut libc::c_void) });
                hsts_free(sts);
            } else {
                if subdomain as i32 != 0 && (unsafe { (*sts).includeSubDomains }) as i32 != 0 {
                    let mut ntail: size_t = unsafe { strlen((*sts).host) };
                    if ntail < hlen {
                        let mut offs: size_t = hlen.wrapping_sub(ntail);
                        if (unsafe { *hostname.offset(offs.wrapping_sub(1 as i32 as u64) as isize) }) as i32
                            == '.' as i32
                ...
                if (unsafe { Curl_strcasecompare(hostname, (*sts).host) }) != 0 {
                    return sts;
                }
            }
            e = n;
...
extern "C" fn hsts_push(
    ...
    e.name = (unsafe { (*sts).host }) as *mut i8;
    e.namelen = unsafe { strlen((*sts).host) };
    e.set_includeSubDomains((unsafe { (*sts).includeSubDomains }) as u32);
    if (unsafe { (*sts).expires }) != 0x7fffffffffffffff as i64 {
        result = unsafe { Curl_gmtime((*sts).expires, &mut stamp) };
        if result as u64 != 0 {
            return result;
        }
    ...
    sc = unsafe { ((*data).set.hsts_write).expect("non-null function pointer")(
        data,
        &mut e,
        i,
        (*data).set.hsts_write_userp,
    ) };
    (unsafe { *stop = sc as u32 != CURLSTS_OK as i32 as u32 });
...
extern "C" fn hsts_out(mut sts: *mut stsentry, mut fp: *mut FILE) -> CURLcode {
    let mut stamp: tm = tm {
        tm_sec: 0,
        tm_min: 0,
        tm_hour: 0,
    ...
    if (unsafe { (*sts).expires }) != 0x7fffffffffffffff as i64 {
        let mut result: CURLcode = unsafe { Curl_gmtime((*sts).expires, &mut stamp) };
        if result as u64 != 0 {
            return result;
        }
...
#[no_mangle]
pub extern "C" fn Curl_hsts_save(
    mut data: *mut Curl_easy,
    mut h: *mut hsts,
    mut file: *const i8,
) -> CURLcode {
    ...
    if file.is_null() && !(unsafe { (*h).filename }).is_null() {
        file = unsafe { (*h).filename };
    }
    if !((unsafe { (*h).flags }) as i64 & ((1 as i32) << 1 as i32) as i64 != 0
        || file.is_null()
        || (unsafe { *file.offset(0 as i32 as isize) }) == 0)
    {
        ...
        if out.is_null() {
            ...
            (unsafe { fputs (b"# Your HSTS cache. https://curl.se/docs/hsts.html\n# This file was generated by libcurl! Edit at your own risk.\n\0" as * const u8 as * const i8 , out ,) }) ;
            e = unsafe { (*h).list.head };
            while !e.is_null() {
                let mut sts: *mut stsentry = (unsafe { (*e).ptr }) as *mut stsentry;
                n = unsafe { (*e).next };
    ...
    if unsafe { ((*data).set.hsts_write).is_some() } {
        let mut i: curl_index = curl_index { index: 0, total: 0 };
        i.total = unsafe { (*h).list.size };
        i.index = 0 as i32 as size_t;
        e = unsafe { (*h).list.head };
        while !e.is_null() {
            let mut sts_0: *mut stsentry = (unsafe { (*e).ptr }) as *mut stsentry;
...
extern "C" fn hsts_add(mut h: *mut hsts, mut line: *mut i8) -> CURLcode {
    ...
    if 2 as i32 == rc {
        let mut expires: time_t =
            if (unsafe { strcmp(date.as_mut_ptr(), b"unlimited\0" as *const u8 as *const i8) }) != 0 {
                unsafe { Curl_getdate_capped(date.as_mut_ptr()) }
            } else {
...
extern "C" fn hsts_pull(mut data: *mut Curl_easy, mut h: *mut hsts) -> CURLcode {
    if unsafe { ((*data).set.hsts_read).is_some() } {
        let mut sc: CURLSTScode = CURLSTS_OK;
        loop {
            ...
            e.namelen = (::std::mem::size_of::<[i8; 257]>() as u64).wrapping_sub(1 as i32 as u64);
            e.set_includeSubDomains(0 as i32 as u32);
            e.expire[0 as i32 as usize] = 0 as i32 as i8;
            (unsafe { *(e.name).offset(0 as i32 as isize) = 0 as i32 as i8 });
            sc = unsafe { ((*data).set.hsts_read).expect("non-null function pointer")(
                data,
                &mut e,
                (*data).set.hsts_read_userp,
            ) };
            if sc as u32 == CURLSTS_OK as i32 as u32 {
                let mut expires: time_t = 0;
                let mut result: CURLcode = CURLE_OK;
                if (unsafe { *(e.name).offset(0 as i32 as isize) }) == 0 {
                    return CURLE_BAD_FUNCTION_ARGUMENT;
                }
                if e.expire[0 as i32 as usize] != 0 {
                    expires = unsafe { Curl_getdate_capped((e.expire).as_mut_ptr()) };
                } else {
                    expires = 0x7fffffffffffffff as i64;
                }
                result = hsts_create(
...
extern "C" fn hsts_load(mut h: *mut hsts, mut file: *const i8) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut line: *mut i8 = 0 as *mut i8;
    let mut fp: *mut FILE = 0 as *mut FILE;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*h).filename as *mut libc::c_void) });
    let fresh1 = unsafe { &mut ((*h).filename) };
    *fresh1 = unsafe { Curl_cstrdup.expect("non-null function pointer")(file) };
    if (unsafe { (*h).filename }).is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    ...
    if !fp.is_null() {
        ...
        if line.is_null() {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*h).filename as *mut libc::c_void) });
            let fresh2 = unsafe { &mut ((*h).filename) };
            *fresh2 = 0 as *mut i8;
            (unsafe { fclose(fp) });
            return CURLE_OUT_OF_MEMORY;
        } else {
...
#[no_mangle]
pub extern "C" fn Curl_hsts_loadfile(
...
#[no_mangle]
pub extern "C" fn Curl_hsts_loadcb(mut data: *mut Curl_easy, mut h: *mut hsts) -> CURLcode {
    if !h.is_null() {
