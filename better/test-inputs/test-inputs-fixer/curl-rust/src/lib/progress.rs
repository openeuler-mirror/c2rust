...
#[no_mangle]
pub extern "C" fn Curl_pgrsDone(mut data: *mut Curl_easy) -> i32 {
    let mut rc: i32 = 0;
...
#[no_mangle]
pub extern "C" fn Curl_pgrsResetTransferSizes(mut data: *mut Curl_easy) {
    Curl_pgrsSetDownloadSize(data, -(1 as i32) as curl_off_t);
    Curl_pgrsSetUploadSize(data, -(1 as i32) as curl_off_t);
}
#[no_mangle]
pub extern "C" fn Curl_pgrsTime(mut data: *mut Curl_easy, mut timer: timerid) -> curltime {
    ...
    match timer as u32 {
        ...
        2 => {
            ...
            let fresh0 = unsafe { &mut ((*data).progress) };
        ...
        3 => {
            delta = unsafe { &mut (*data).progress.t_nslookup };
        }
        4 => {
            delta = unsafe { &mut (*data).progress.t_connect };
        }
        5 => {
            delta = unsafe { &mut (*data).progress.t_appconnect };
        }
        6 => {
            delta = unsafe { &mut (*data).progress.t_pretransfer };
        }
        7 => {
            delta = unsafe { &mut (*data).progress.t_starttransfer };
            if (unsafe { ((*data).progress).is_t_startransfer_set() }) != 0 {
                return now;
            } else {
                let fresh1 = unsafe { &mut ((*data).progress) };
    ...
    if !delta.is_null() {
        let mut us: timediff_t = unsafe { Curl_timediff_us(now, (*data).progress.t_startsingle) };
...
#[no_mangle]
pub extern "C" fn Curl_pgrsStartNow(mut data: *mut Curl_easy) {
    (unsafe { (*data).progress.speeder_c = 0 as i32 });
    (unsafe { (*data).progress.start = Curl_now() });
    let fresh2 = unsafe { &mut ((*data).progress) };
...
pub extern "C" fn Curl_pgrsLimitWaitTime(
    ...
    actual = unsafe { Curl_timediff(now, start) };
    if actual < minimum {
...
#[no_mangle]
pub extern "C" fn Curl_pgrsSetDownloadCounter(mut data: *mut Curl_easy, mut size: curl_off_t) {
    (unsafe { (*data).progress.downloaded = size });
}
#[no_mangle]
pub extern "C" fn Curl_ratelimit(mut data: *mut Curl_easy, mut now: curltime) {
    if (unsafe { (*data).set.max_recv_speed }) != 0 {
        if (unsafe { Curl_timediff(now, (*data).progress.dl_limit_start) }) >= 3000 as i32 as i64 {
    ...
    if (unsafe { (*data).set.max_send_speed }) != 0 {
        if (unsafe { Curl_timediff(now, (*data).progress.ul_limit_start) }) >= 3000 as i32 as i64 {
...
#[no_mangle]
pub extern "C" fn Curl_pgrsSetUploadCounter(mut data: *mut Curl_easy, mut size: curl_off_t) {
    (unsafe { (*data).progress.uploaded = size });
}
#[no_mangle]
pub extern "C" fn Curl_pgrsSetDownloadSize(mut data: *mut Curl_easy, mut size: curl_off_t) {
...
#[no_mangle]
pub extern "C" fn Curl_pgrsSetUploadSize(mut data: *mut Curl_easy, mut size: curl_off_t) {
    if size >= 0 as i32 as i64 {
...
extern "C" fn progress_calc(mut data: *mut Curl_easy, mut now: curltime) -> bool {
    let mut timetoshow: bool = 0 as i32 != 0;
    let p: *mut Progress = unsafe { &mut (*data).progress };
    ...
    if (unsafe { (*p).lastshow }) != now.tv_sec {
        ...
        let fresh3 = unsafe { &mut ((*p).speeder_c) };
        *fresh3 += 1;
        ...
        if countindex != 0 {
            ...
            span_ms = unsafe { Curl_timediff(now, (*p).speeder_time[checkindex as usize]) };
...
extern "C" fn progress_meter(mut data: *mut Curl_easy) {
    ...
    let mut timespent: curl_off_t = (unsafe { (*data).progress.timespent }) / 1000000 as i32 as i64;
...
#[no_mangle]
pub extern "C" fn Curl_pgrsUpdate(mut data: *mut Curl_easy) -> i32 {
    let mut now: curltime = unsafe { Curl_now() };
