...
#[no_mangle]
pub static mut Curl_handler_mqtt: Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_it: Some(mqtt_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: None,
            do_more: None,
            connect_it: None,
            connecting: None,
            doing: Some(mqtt_doing as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            proto_getsock: None,
            doing_getsock: Some(
                mqtt_getsock
...
extern "C" fn mqtt_setup_conn(mut data: *mut Curl_easy, mut _conn: *mut connectdata) -> CURLcode {
    let mut mq: *mut MQTT = 0 as *mut MQTT;
    mq = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<MQTT>() as u64,
    ) }) as *mut MQTT;
    if mq.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    let fresh0 = unsafe { &mut ((*data).req.p.mqtt) };
    *fresh0 = mq;
    return CURLE_OK;
}
extern "C" fn mqtt_send(mut data: *mut Curl_easy, mut buf: *mut i8, mut len: size_t) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut sockfd: curl_socket_t = unsafe { (*conn).sock[0 as i32 as usize] };
    let mut mq: *mut MQTT = unsafe { (*data).req.p.mqtt };
    let mut n: ssize_t = 0;
    result = unsafe { Curl_write(data, sockfd, buf as *const libc::c_void, len, &mut n) };
    if result as u64 == 0 {
        (unsafe { Curl_debug(data, CURLINFO_HEADER_OUT, buf, n as size_t) });
    }
    if len != n as size_t {
        let mut nsend: size_t = len.wrapping_sub(n as u64);
        let mut sendleftovers: *mut i8 = (unsafe { Curl_memdup(
        ...
        let fresh1 = unsafe { &mut ((*mq).sendleftovers) };
        *fresh1 = sendleftovers;
        (unsafe { (*mq).nsend = nsend });
    } else {
        let fresh2 = unsafe { &mut ((*mq).sendleftovers) };
        *fresh2 = 0 as *mut i8;
        (unsafe { (*mq).nsend = 0 as i32 as size_t });
    }
    return result;
}
extern "C" fn mqtt_getsock(
...
extern "C" fn mqtt_encode_len(mut buf: *mut i8, mut len: size_t) -> i32 {
    let mut encoded: u8 = 0;
    let mut i: i32 = 0;
    i = 0 as i32;
    while len > 0 as i32 as u64 && i < 4 as i32 {
        encoded = len.wrapping_rem(0x80 as i32 as u64) as u8;
        len = (len as u64).wrapping_div(0x80 as i32 as u64) as size_t as size_t;
        if len != 0 {
            encoded = (encoded as i32 | 0x80 as i32) as u8;
        }
        (unsafe { *buf.offset(i as isize) = encoded as i8 });
        i += 1;
    }
    return i;
}
extern "C" fn add_passwd(
    mut passwd: *const i8,
    plen: size_t,
    mut pkt: *mut i8,
    start: size_t,
    ...
    let fresh3 = unsafe { &mut (*pkt.offset(conn_flags_pos as isize)) };
    *fresh3 = (*fresh3 as i32 | 0x40 as i32) as i8;
    (unsafe { *pkt.offset(start as isize) = (plen >> 8 as i32 & 0xff as i32 as u64) as i8 });
    (unsafe { *pkt.offset(start.wrapping_add(1 as i32 as u64) as isize) = (plen & 0xff as i32 as u64) as i8 });
    (unsafe { memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as i32 as u64) as isize) as *mut i8
            as *mut libc::c_void,
...
extern "C" fn add_user(
    ...
    let fresh4 = unsafe { &mut (*pkt.offset(conn_flags_pos as isize)) };
    *fresh4 = (*fresh4 as i32 | 0x80 as i32) as u8;
    (unsafe { *pkt.offset(start as isize) = (ulen >> 8 as i32 & 0xff as i32 as u64) as u8 });
    (unsafe { *pkt.offset(start.wrapping_add(1 as i32 as u64) as isize) = (ulen & 0xff as i32 as u64) as u8 });
    (unsafe { memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as i32 as u64) as isize) as *mut u8
            as *mut libc::c_void,
...
extern "C" fn add_client_id(
    ...
    (unsafe { *pkt.offset(start as isize) = 0 as i32 as i8 });
    (unsafe { *pkt.offset(start.wrapping_add(1 as i32 as u64) as isize) = 12 as i32 as i8 });
    (unsafe { memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as i32 as u64) as isize) as *mut i8
            as *mut libc::c_void,
...
extern "C" fn init_connpack(mut packet: *mut i8, mut remain: *mut i8, mut remain_pos: i32) -> i32 {
    (unsafe { *packet.offset(0 as i32 as isize) = 0x10 as i32 as i8 });
    (unsafe { memcpy(
        &mut *packet.offset(1 as i32 as isize) as *mut i8 as *mut libc::c_void,
        remain as *const libc::c_void,
        remain_pos as u64,
    ) });
    (unsafe { *packet.offset((remain_pos + 1 as i32) as isize) = 0 as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 2 as i32) as isize) = 0x4 as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 3 as i32) as isize) = 'M' as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 4 as i32) as isize) = 'Q' as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 5 as i32) as isize) = 'T' as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 6 as i32) as isize) = 'T' as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 7 as i32) as isize) = 0x4 as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 8 as i32) as isize) = 0x2 as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 9 as i32) as isize) = 0 as i32 as i8 });
    (unsafe { *packet.offset((remain_pos + 10 as i32) as isize) = 0x3c as i32 as i8 });
    return remain_pos + 10 as i32;
}
extern "C" fn mqtt_connect(mut data: *mut Curl_easy) -> CURLcode {
    ...
    let mut client_id: [i8; 13] =
        *(unsafe { ::std::mem::transmute::<&[u8; 13], &mut [i8; 13]>(b"curl\0\0\0\0\0\0\0\0\0") });
    let clen: size_t = unsafe { strlen(b"curl\0" as *const u8 as *const i8) };
    let mut packet: *mut i8 = 0 as *mut i8;
    let mut username: *const i8 = if !(unsafe { (*data).state.aptr.user }).is_null() {
        (unsafe { (*data).state.aptr.user }) as *const i8
    } else {
        b"\0" as *const u8 as *const i8
    };
    let ulen: size_t = unsafe { strlen(username) };
    let mut passwd: *const i8 = if !(unsafe { (*data).state.aptr.passwd }).is_null() {
        (unsafe { (*data).state.aptr.passwd }) as *const i8
    } else {
        b"\0" as *const u8 as *const i8
    };
    let plen: size_t = unsafe { strlen(passwd) };
    payloadlen = ulen
        .wrapping_add(plen)
        .wrapping_add(12 as i32 as u64)
        .wrapping_add(2 as i32 as u64);
    if ulen != 0 {
        payloadlen = (payloadlen as u64).wrapping_add(2 as i32 as u64) as size_t as size_t;
    }
    if plen != 0 {
        payloadlen = (payloadlen as u64).wrapping_add(2 as i32 as u64) as size_t as size_t;
    }
    ...
    packet = (unsafe { Curl_cmalloc.expect("non-null function pointer")(packetlen) }) as *mut i8;
    if packet.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { memset(packet as *mut libc::c_void, 0 as i32, packetlen) });
    pos = init_connpack(packet, remain.as_mut_ptr(), remain_pos);
    result = unsafe { Curl_rand_hex(
        data,
        &mut *client_id.as_mut_ptr().offset(clen as isize) as *mut i8 as *mut u8,
        (12 as i32 as u64)
            .wrapping_sub(clen)
            .wrapping_add(1 as i32 as u64),
    ) };
    rc = add_client_id(
        ...
        unsafe { strlen(client_id.as_mut_ptr()) },
        packet,
    ...
    if rc != 0 {
        ...
        if ulen != 0 {
            start_pwd = (start_pwd as u64).wrapping_add(2 as i32 as u64) as size_t as size_t;
            rc = add_user(username, ulen, packet as *mut u8, start_user, remain_pos);
            if rc != 0 {
                (unsafe { Curl_failf(
                    data,
                    b"Username is too large: [%lu]\0" as *const u8 as *const i8,
                    ulen,
                ) });
                result = CURLE_WEIRD_SERVER_REPLY;
            ...
            _ => {
                if plen != 0 {
                    ...
                    if rc != 0 {
                        (unsafe { Curl_failf(
                            data,
                            b"Password is too large: [%lu]\0" as *const u8 as *const i8,
                            plen,
                        ) });
                        result = CURLE_WEIRD_SERVER_REPLY;
                        current_block = 10351409997398868566;
    ...
    if !packet.is_null() {
        (unsafe { Curl_cfree.expect("non-null function pointer")(packet as *mut libc::c_void) });
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")((*data).state.aptr.user as *mut libc::c_void) });
    let fresh5 = unsafe { &mut ((*data).state.aptr.user) };
    *fresh5 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*data).state.aptr.passwd as *mut libc::c_void) });
    let fresh6 = unsafe { &mut ((*data).state.aptr.passwd) };
    *fresh6 = 0 as *mut i8;
    return result;
}
extern "C" fn mqtt_disconnect(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    result = mqtt_send(
        data,
...
extern "C" fn mqtt_verify_connack(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut sockfd: curl_socket_t = unsafe { (*conn).sock[0 as i32 as usize] };
    ...
    if !(result as u64 != 0) {
            ...
            || readbuf[1 as i32 as usize] as i32 != 0 as i32
        {
            (unsafe { Curl_failf(
                data,
                b"Expected %02x%02x but got %02x%02x\0" as *const u8 as *const i8,
                0 as i32,
                0 as i32,
...
extern "C" fn mqtt_get_topic(
    mut data: *mut Curl_easy,
    mut topic: *mut *mut i8,
    mut topiclen: *mut size_t,
) -> CURLcode {
    let mut path: *mut i8 = unsafe { (*data).state.up.path };
    if (unsafe { strlen(path) }) > 1 as i32 as u64 {
    ...
    (unsafe { Curl_failf(
        data,
        b"No MQTT topic found. Forgot to URL encode it?\0" as *const u8 as *const i8,
    ) });
    return CURLE_URL_MALFORMAT;
}
extern "C" fn mqtt_subscribe(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    ...
    if !(result as u64 != 0) {
        let fresh7 = unsafe { &mut ((*conn).proto.mqtt.packetid) };
        *fresh7 = (*fresh7).wrapping_add(1);
        packetlen = topiclen.wrapping_add(5 as i32 as u64);
        n = mqtt_encode_len(encodedsize.as_mut_ptr(), packetlen) as size_t;
        packetlen =
            (packetlen as u64).wrapping_add(n.wrapping_add(1 as i32 as u64)) as size_t as size_t;
        packet = (unsafe { Curl_cmalloc.expect("non-null function pointer")(packetlen) }) as *mut u8;
        if packet.is_null() {
            result = CURLE_OUT_OF_MEMORY;
        } else {
            (unsafe { *packet.offset(0 as i32 as isize) = 0x82 as i32 as u8 });
            (unsafe { memcpy(
                &mut *packet.offset(1 as i32 as isize) as *mut u8 as *mut libc::c_void,
                encodedsize.as_mut_ptr() as *const libc::c_void,
                n,
            ) });
            (unsafe { *packet.offset((1 as i32 as u64).wrapping_add(n) as isize) =
                ((*conn).proto.mqtt.packetid >> 8 as i32 & 0xff as i32 as u32) as u8 });
            (unsafe { *packet.offset((2 as i32 as u64).wrapping_add(n) as isize) =
                ((*conn).proto.mqtt.packetid & 0xff as i32 as u32) as u8 });
            (unsafe { *packet.offset((3 as i32 as u64).wrapping_add(n) as isize) =
                (topiclen >> 8 as i32 & 0xff as i32 as u64) as u8 });
            (unsafe { *packet.offset((4 as i32 as u64).wrapping_add(n) as isize) =
                (topiclen & 0xff as i32 as u64) as u8 });
            (unsafe { memcpy(
                &mut *packet.offset((5 as i32 as u64).wrapping_add(n) as isize) as *mut u8
                    as *mut libc::c_void,
                topic as *const libc::c_void,
                topiclen,
            ) });
            (unsafe { *packet.offset((5 as i32 as u64).wrapping_add(n).wrapping_add(topiclen) as isize) =
                0 as i32 as u8 });
            result = mqtt_send(data, packet as *mut i8, packetlen);
        }
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")(topic as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")(packet as *mut libc::c_void) });
    return result;
}
extern "C" fn mqtt_verify_suback(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut sockfd: curl_socket_t = unsafe { (*conn).sock[0 as i32 as usize] };
    let mut readbuf: [u8; 3] = [0; 3];
    let mut nread: ssize_t = 0;
    let mut mqtt: *mut mqtt_conn = unsafe { &mut (*conn).proto.mqtt };
    result = unsafe { Curl_read(
        data,
        sockfd,
        readbuf.as_mut_ptr() as *mut i8,
    ...
    if !(result as u64 != 0) {
        ...
        if nread < 3 as i32 as i64 {
            ...
            != (unsafe { (*mqtt).packetid }) >> 8 as i32 & 0xff as i32 as u32
            || readbuf[1 as i32 as usize] as u32 != (unsafe { (*mqtt).packetid }) & 0xff as i32 as u32
...
extern "C" fn mqtt_publish(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut payload: *mut i8 = (unsafe { (*data).set.postfields }) as *mut i8;
    ...
    let mut postfieldsize: curl_off_t = unsafe { (*data).set.postfieldsize };
    if payload.is_null() {
        return CURLE_BAD_FUNCTION_ARGUMENT;
    }
    if postfieldsize < 0 as i32 as i64 {
        payloadlen = unsafe { strlen(payload) };
    } else {
    ...
    if !(result as u64 != 0) {
        ...
        encodelen = mqtt_encode_len(encodedbytes.as_mut_ptr(), remaininglength) as size_t;
        pkt = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
            remaininglength
                .wrapping_add(1 as i32 as u64)
        ...
        if pkt.is_null() {
            ...
            let fresh8 = i;
            i = i.wrapping_add(1);
            (unsafe { *pkt.offset(fresh8 as isize) = 0x30 as i32 as u8 });
            (unsafe { memcpy(
            ...
            let fresh9 = i;
            i = i.wrapping_add(1);
            (unsafe { *pkt.offset(fresh9 as isize) = (topiclen >> 8 as i32 & 0xff as i32 as u64) as u8 });
            let fresh10 = i;
            i = i.wrapping_add(1);
            (unsafe { *pkt.offset(fresh10 as isize) = (topiclen & 0xff as i32 as u64) as u8 });
            (unsafe { memcpy(
                &mut *pkt.offset(i as isize) as *mut u8 as *mut libc::c_void,
                topic as *const libc::c_void,
                topiclen,
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(pkt as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")(topic as *mut libc::c_void) });
    return result;
}
extern "C" fn mqtt_decode_len(
    mut buf: *mut u8,
    mut buflen: size_t,
    ...
    while i < buflen && encoded as i32 & 128 as i32 != 0 {
        encoded = unsafe { *buf.offset(i as isize) };
        len = (len as u64).wrapping_add(((encoded as i32 & 127 as i32) as u64).wrapping_mul(mult))
            as size_t as size_t;
        mult = (mult as u64).wrapping_mul(128 as i32 as u64) as size_t as size_t;
        i = i.wrapping_add(1);
    }
    if !lenbytes.is_null() {
        (unsafe { *lenbytes = i });
...
extern "C" fn mqstate(mut data: *mut Curl_easy, mut state: mqttstate, mut nextstate: mqttstate) {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut mqtt: *mut mqtt_conn = unsafe { &mut (*conn).proto.mqtt };
    (unsafe { (*mqtt).state = state });
    if state as u32 == MQTT_FIRST as i32 as u32 {
        (unsafe { (*mqtt).nextstate = nextstate });
    }
}
extern "C" fn mqtt_read_publish(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut current_block: u64;
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut sockfd: curl_socket_t = unsafe { (*conn).sock[0 as i32 as usize] };
    let mut nread: ssize_t = 0;
    let mut pkt: *mut u8 = (unsafe { (*data).state.buffer }) as *mut u8;
    let mut remlen: size_t = 0;
    let mut mqtt: *mut mqtt_conn = unsafe { &mut (*conn).proto.mqtt };
    let mut mq: *mut MQTT = unsafe { (*data).req.p.mqtt };
    let mut packet: u8 = 0;
    match (unsafe { (*mqtt).state }) as u32 {
        4 => {
            current_block = 16063587588522528629;
        }
        3 | 5 => {
            packet = ((unsafe { (*mq).firstbyte }) as i32 & 0xf0 as i32) as u8;
            if packet as i32 == 0x30 as i32 {
                mqstate(data, MQTT_PUB_REMAIN, MQTT_NOSTATE);
                remlen = unsafe { (*mq).remaining_length };
                (unsafe { Curl_infof(
                    data,
                    b"Remaining length: %zd bytes\0" as *const u8 as *const i8,
                    remlen,
                ) });
                if (unsafe { (*data).set.max_filesize }) != 0 && remlen as curl_off_t > (unsafe { (*data).set.max_filesize })
                {
                    (unsafe { Curl_failf(
                        data,
                    ...
                    (unsafe { Curl_pgrsSetDownloadSize(data, remlen as curl_off_t) });
                    (unsafe { (*data).req.bytecount = 0 as i32 as curl_off_t });
                    (unsafe { (*data).req.size = remlen as curl_off_t });
                    (unsafe { (*mq).npacket = remlen });
                ...
                if packet as i32 == 0xe0 as i32 {
                    (unsafe { Curl_infof(data, b"Got DISCONNECT\0" as *const u8 as *const i8) });
    ...
    match current_block {
        4488286894823169796 => {
            ...
            let mut rest: size_t = unsafe { (*mq).npacket };
            if rest > (unsafe { (*data).set.buffer_size }) as size_t {
                rest = (unsafe { (*data).set.buffer_size }) as size_t;
            }
            result = unsafe { Curl_read(data, sockfd, pkt as *mut i8, rest, &mut nread) };
            if result as u64 != 0 {
                if CURLE_AGAIN as i32 as u32 == result as u32 {
                    (unsafe { Curl_infof(data, b"EEEE AAAAGAIN\0" as *const u8 as *const i8) });
                }
            } else if nread == 0 {
                (unsafe { Curl_infof(data, b"server disconnected\0" as *const u8 as *const i8) });
                result = CURLE_PARTIAL_FILE;
            } else {
                (unsafe { Curl_debug(data, CURLINFO_DATA_IN, pkt as *mut i8, nread as size_t) });
                let fresh11 = unsafe { &mut ((*mq).npacket) };
                *fresh11 = (*fresh11 as u64).wrapping_sub(nread as u64) as size_t as size_t;
                let fresh12 = unsafe { &mut ((*k).bytecount) };
                *fresh12 += nread;
...
extern "C" fn mqtt_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
...
extern "C" fn mqtt_doing(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut mqtt: *mut mqtt_conn = unsafe { &mut (*conn).proto.mqtt };
    let mut mq: *mut MQTT = unsafe { (*data).req.p.mqtt };
    ...
    let mut byte: u8 = 0;
    (unsafe { *done = 0 as i32 != 0 });
    if (unsafe { (*mq).nsend }) != 0 {
        let mut ptr: *mut i8 = unsafe { (*mq).sendleftovers };
        result = mqtt_send(data, unsafe { (*mq).sendleftovers }, unsafe { (*mq).nsend });
        (unsafe { Curl_cfree.expect("non-null function pointer")(ptr as *mut libc::c_void) });
        if result as u64 != 0 {
            return result;
        }
    }
    ...
    match (unsafe { (*mqtt).state }) as u32 {
        0 => {
            ...
            if nread == 0 {
                ...
                (unsafe { (*mq).npacket = 0 as i32 as size_t });
        ...
        2 => {
            ...
            if result as u64 != 0 {
                ...
                if (unsafe { (*data).state.httpreq }) as u32 == HTTPREQ_POST as i32 as u32 {
                    ...
                    (unsafe { (*mqtt).nextstate = MQTT_FIRST });
                } else {
        ...
        _ => {
            (unsafe { Curl_failf(data, b"State not handled yet\0" as *const u8 as *const i8) });
            (unsafe { *done = 1 as i32 != 0 });
            current_block_42 = 2500484646272006982;
        }
    }
    match current_block_42 {
        9618971279722602572 => {
            loop {
                ...
                let fresh13 = unsafe { &mut ((*mq).npacket) };
                let fresh14 = *fresh13;
                *fresh13 = (*fresh13).wrapping_add(1);
                (unsafe { *pkt.offset(fresh14 as isize) = byte });
                if !(byte as i32 & 0x80 as i32 != 0 && (unsafe { (*mq).npacket }) < 4 as i32 as u64) {
                    break;
                }
            }
            ...
            if !(result as u64 != 0) {
                (unsafe { (*mq).remaining_length = mqtt_decode_len(
                    &mut *pkt.offset(0 as i32 as isize),
                    (*mq).npacket,
                    0 as *mut size_t,
                ) });
                (unsafe { (*mq).npacket = 0 as i32 as size_t });
                if (unsafe { (*mq).remaining_length }) != 0 {
                    mqstate(data, unsafe { (*mqtt).nextstate }, MQTT_NOSTATE);
                } else {
                    mqstate(data, MQTT_FIRST, MQTT_FIRST);
                    if (unsafe { (*mq).firstbyte }) as i32 == 0xe0 as i32 {
                        (unsafe { Curl_infof(data, b"Got DISCONNECT\0" as *const u8 as *const i8) });
                        (unsafe { *done = 1 as i32 != 0 });
                    }
                }
