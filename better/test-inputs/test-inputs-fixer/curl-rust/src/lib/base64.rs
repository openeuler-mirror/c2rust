...
extern "C" fn decodeQuantum(mut dest: *mut u8, mut src: *const i8) -> size_t {
    ...
    while i < 4 as i32 as u64 {
        if (unsafe { *s }) as i32 == '=' as i32 {
            ...
            p = unsafe { base64.as_ptr() };
            while (unsafe { *p }) as i32 != 0 && (unsafe { *p }) as i32 != (unsafe { *s }) as i32 {
                v = v.wrapping_add(1);
                p = unsafe { p.offset(1) };
            }
            if (unsafe { *p }) as i32 == (unsafe { *s }) as i32 {
    ...
    if padding < 1 as i32 as u64 {
        (unsafe { *dest.offset(2 as i32 as isize) = curlx_ultouc(x & 0xff as u64) });
    }
    x >>= 8 as i32;
    if padding < 2 as i32 as u64 {
        (unsafe { *dest.offset(1 as i32 as isize) = curlx_ultouc(x & 0xff as u64) });
    }
    x >>= 8 as i32;
    (unsafe { *dest.offset(0 as i32 as isize) = curlx_ultouc(x & 0xff as u64) });
    return (3 as i32 as u64).wrapping_sub(padding);
}
#[no_mangle]
pub extern "C" fn Curl_base64_decode(
    mut src: *const i8,
    ...
    (unsafe { *outptr = 0 as *mut u8 });
    (unsafe { *outlen = 0 as i32 as size_t });
    srclen = unsafe { strlen(src) };
    if srclen == 0 || srclen.wrapping_rem(4 as i32 as u64) != 0 {
        return CURLE_BAD_CONTENT_ENCODING;
    }
    while (unsafe { *src.offset(length as isize) }) as i32 != '=' as i32
        && (unsafe { *src.offset(length as isize) }) as i32 != 0
    {
        length = length.wrapping_add(1);
    }
    if (unsafe { *src.offset(length as isize) }) as i32 == '=' as i32 {
        padding = padding.wrapping_add(1);
        if (unsafe { *src.offset(length.wrapping_add(1 as i32 as u64) as isize) }) as i32 == '=' as i32 {
            padding = padding.wrapping_add(1);
        }
    }
    ...
    newstr = (unsafe { Curl_cmalloc.expect("non-null function pointer")(rawlen.wrapping_add(1 as i32 as u64)) })
        as *mut u8;
    if newstr.is_null() {
    ...
    while i < numQuantums {
        ...
        if result == 0 {
            (unsafe { Curl_cfree.expect("non-null function pointer")(newstr as *mut libc::c_void) });
            return CURLE_BAD_CONTENT_ENCODING;
        }
        pos = unsafe { pos.offset(result as isize) };
        src = unsafe { src.offset(4 as i32 as isize) };
        i = i.wrapping_add(1);
    }
    (unsafe { *pos = '\u{0}' as i32 as u8 });
    (unsafe { *outptr = newstr });
    (unsafe { *outlen = rawlen });
    return CURLE_OK;
}
extern "C" fn base64_encode(
    ...
    (unsafe { *outptr = 0 as *mut i8 });
    (unsafe { *outlen = 0 as i32 as size_t });
    if insize == 0 {
        insize = unsafe { strlen(indata) };
    }
    output = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
        insize
    ...
    while insize > 0 as i32 as u64 {
        ...
        while i < 3 as i32 {
            if insize > 0 as i32 as u64 {
                inputparts += 1;
                ibuf[i as usize] = (unsafe { *indata }) as u8;
                indata = unsafe { indata.offset(1) };
        ...
        obuf[0 as i32 as usize] =
            ((ibuf[0 as i32 as usize] as i32 & 0xfc as i32) >> 2 as i32) as u8;
        obuf[1 as i32 as usize] = ((ibuf[0 as i32 as usize] as i32 & 0x3 as i32) << 4 as i32
            | (ibuf[1 as i32 as usize] as i32 & 0xf0 as i32) >> 4 as i32)
            as u8;
        obuf[2 as i32 as usize] = ((ibuf[1 as i32 as usize] as i32 & 0xf as i32) << 2 as i32
            | (ibuf[2 as i32 as usize] as i32 & 0xc0 as i32) >> 6 as i32)
            as u8;
        obuf[3 as i32 as usize] = (ibuf[2 as i32 as usize] as i32 & 0x3f as i32) as u8;
        match inputparts {
            1 => {
                (unsafe { curl_msnprintf(
                    ...
                    *table64.offset(obuf[0 as i32 as usize] as isize) as i32,
                    *table64.offset(obuf[1 as i32 as usize] as isize) as i32,
                ) });
            }
            2 => {
                (unsafe { curl_msnprintf(
                    ...
                    *table64.offset(obuf[0 as i32 as usize] as isize) as i32,
                    *table64.offset(obuf[1 as i32 as usize] as isize) as i32,
                    *table64.offset(obuf[2 as i32 as usize] as isize) as i32,
                ) });
            }
            _ => {
                (unsafe { curl_msnprintf(
                    ...
                    *table64.offset(obuf[1 as i32 as usize] as isize) as i32,
                    *table64.offset(obuf[2 as i32 as usize] as isize) as i32,
                    *table64.offset(obuf[3 as i32 as usize] as isize) as i32,
                ) });
            }
        }
        output = unsafe { output.offset(4 as i32 as isize) };
    }
    (unsafe { *output = '\u{0}' as i32 as i8 });
    (unsafe { *outptr = base64data });
    (unsafe { Curl_cfree.expect("non-null function pointer")(convbuf as *mut libc::c_void) });
    (unsafe { *outlen = strlen(base64data) });
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_base64_encode(
    ...
    return base64_encode(unsafe { base64.as_ptr() }, data, inputbuff, insize, outptr, outlen);
}
#[no_mangle]
pub extern "C" fn Curl_base64url_encode(
    mut data: *mut Curl_easy,
    mut inputbuff: *const i8,
    mut insize: size_t,
    mut outptr: *mut *mut i8,
    ...
    return base64_encode(unsafe { base64url.as_ptr() }, data, inputbuff, insize, outptr, outlen);
}
