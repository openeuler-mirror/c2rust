...
extern "C" fn convert_lineends(
    ...
    if (unsafe { ((*data).state).prev_block_had_trailing_cr() }) != 0 {
        if (unsafe { *startPtr }) as i32 == '\n' as i32 {
            (unsafe { memmove(
            ...
            let fresh0 = unsafe { &mut ((*data).state.crlf_conversions) };
            *fresh0 += 1;
        }
        let fresh1 = unsafe { &mut ((*data).state) };
        (*fresh1).set_prev_block_had_trailing_cr(0 as i32 as bit);
    }
    outPtr = (unsafe { memchr(startPtr as *const libc::c_void, '\r' as i32, size) }) as *mut i8;
    inPtr = outPtr;
    if !inPtr.is_null() {
        while inPtr < (unsafe { startPtr.offset(size as isize).offset(-(1 as i32 as isize)) }) {
            if (unsafe { memcmp(
            ...
            {
                inPtr = unsafe { inPtr.offset(1) };
                (unsafe { *outPtr = *inPtr });
                let fresh2 = unsafe { &mut ((*data).state.crlf_conversions) };
                *fresh2 += 1;
            } else if (unsafe { *inPtr }) as i32 == '\r' as i32 {
                (unsafe { *outPtr = '\n' as i32 as i8 });
            } else {
                (unsafe { *outPtr = *inPtr });
            }
        ...
        if inPtr < (unsafe { startPtr.offset(size as isize) }) {
            if (unsafe { *inPtr }) as i32 == '\r' as i32 {
                (unsafe { *outPtr = '\n' as i32 as i8 });
        ...
        if outPtr < (unsafe { startPtr.offset(size as isize) }) {
            (unsafe { *outPtr = '\u{0}' as i32 as i8 });
        }
        return (unsafe { outPtr.offset_from(startPtr) }) as i64 as size_t;
    }
    return size;
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn Curl_infof(mut data: *mut Curl_easy, mut fmt: *const i8, mut args: ...) {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_failf(mut data: *mut Curl_easy, mut fmt: *const i8, mut args: ...) {
    if ((*data).set).verbose() as i32 != 0 || !((*data).set.errorbuffer).is_null() {
        let mut ap: ::std::ffi::VaListImpl;
        let mut len: size_t = 0;
...
pub extern "C" fn Curl_write(
    ...
    num = (sockfd == (unsafe { (*conn).sock[1 as i32 as usize] })) as i32;
    bytes_written = unsafe { ((*conn).send[num as usize]).expect("non-null function pointer")(
        data,
        num,
        mem,
        len,
    ...
    (unsafe { *written = bytes_written });
    if bytes_written >= 0 as i32 as i64 {
        return CURLE_OK;
    }
    match result as u32 {
        81 => {
            (unsafe { *written = 0 as i32 as ssize_t });
            return CURLE_OK;
        }
...
#[no_mangle]
pub extern "C" fn Curl_send_plain(
    mut data: *mut Curl_easy,
    mut num: i32,
    mut mem: *const libc::c_void,
    mut len: size_t,
    ...
    let mut bytes_written: ssize_t = 0;
    conn = unsafe { (*data).conn };
    sockfd = unsafe { (*conn).sock[num as usize] };
    if CURLE_OK as i32 != 0 {
        (unsafe { *code = CURLE_OUT_OF_MEMORY });
        return -(1 as i32) as ssize_t;
    }
    bytes_written = unsafe { send(sockfd, mem, len, MSG_NOSIGNAL as i32) };
    (unsafe { *code = CURLE_OK });
    if -(1 as i32) as i64 == bytes_written {
        let mut err: i32 = unsafe { *__errno_location() };
        if 11 as i32 == err || 11 as i32 == err || 4 as i32 == err || 115 as i32 == err {
            bytes_written = 0 as i32 as ssize_t;
            (unsafe { *code = CURLE_AGAIN });
        } else {
            ...
            (unsafe { (*data).state.os_errno = err });
            (unsafe { *code = CURLE_SEND_ERROR });
        }
    }
    return bytes_written;
}
#[no_mangle]
pub extern "C" fn Curl_write_plain(
    mut data: *mut Curl_easy,
    mut sockfd: curl_socket_t,
    mut mem: *const libc::c_void,
    mut len: size_t,
    ...
    let mut num: i32 = 0;
    num = (sockfd == (unsafe { (*conn).sock[1 as i32 as usize] })) as i32;
    (unsafe { *written = Curl_send_plain(data, num, mem, len, &mut result) });
    return result;
}
#[no_mangle]
pub extern "C" fn Curl_recv_plain(
    mut data: *mut Curl_easy,
    mut num: i32,
    mut buf: *mut i8,
    mut len: size_t,
    ...
    nread = unsafe { recv(sockfd, buf as *mut libc::c_void, len, 0 as i32) };
    (unsafe { *code = CURLE_OK });
    if -(1 as i32) as i64 == nread {
        let mut err: i32 = unsafe { *__errno_location() };
        if 11 as i32 == err || 11 as i32 == err || 4 as i32 == err {
            (unsafe { *code = CURLE_AGAIN });
        } else {
            let mut buffer: [i8; 256] = [0; 256];
            (unsafe { Curl_failf(
            ...
            (unsafe { (*data).state.os_errno = err });
            (unsafe { *code = CURLE_RECV_ERROR });
        }
    }
    return nread;
}
extern "C" fn pausewrite(
    mut data: *mut Curl_easy,
    mut type_0: i32,
    mut ptr: *const i8,
    mut len: size_t,
    ...
    (unsafe { Curl_http2_stream_pause(data, 1 as i32 != 0) });
    if (unsafe { (*s).tempcount }) != 0 {
        i = 0 as i32 as u32;
        while i < (unsafe { (*s).tempcount }) {
            if (unsafe { (*s).tempwrite[i as usize].type_0 }) == type_0 {
                newtype = 0 as i32 != 0;
                break;
            } else {
                i = i.wrapping_add(1);
    ...
    if newtype {
        ...
        (unsafe { (*s).tempwrite[i as usize].type_0 = type_0 });
        let fresh7 = unsafe { &mut ((*s).tempcount) };
    ...
    (unsafe { (*k).keepon |= (1 as i32) << 4 as i32 });
    return CURLE_OK;
}
extern "C" fn chop_write(
    mut data: *mut Curl_easy,
    mut type_0: i32,
    mut optr: *mut i8,
    mut olen: size_t,
    ...
    if (unsafe { (*data).req.keepon }) & (1 as i32) << 4 as i32 != 0 {
        return pausewrite(data, type_0, ptr, len);
    }
    if type_0 & (1 as i32) << 0 as i32 != 0 {
        writebody = unsafe { (*data).set.fwrite_func };
    }
    if type_0 & (1 as i32) << 1 as i32 != 0
        && ((unsafe { ((*data).set.fwrite_header).is_some() }) || !(unsafe { (*data).set.writeheader }).is_null())
    {
        writeheader = if unsafe { ((*data).set.fwrite_header).is_some() } {
            unsafe { (*data).set.fwrite_header }
        } else {
            unsafe { (*data).set.fwrite_func }
        };
    }
    while len != 0 {
        ...
        if writebody.is_some() {
            ...
            wrote = unsafe { writebody.expect("non-null function pointer")(
                ptr,
                1 as i32 as size_t,
                chunklen,
                (*data).set.out,
            ...
            if 0x10000001 as i32 as u64 == wrote {
                if (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 4 as i32) as u32 != 0 {
                    (unsafe { Curl_failf(
                        data,
                        b"Write callback asked for PAUSE when not supported!\0" as *const u8
                            as *const i8,
                    ) });
            ...
            if wrote != chunklen {
                (unsafe { Curl_failf(
                    data,
                    b"Failure writing output to destination\0" as *const u8 as *const i8,
                ) });
                return CURLE_WRITE_ERROR;
            }
        }
        ptr = unsafe { ptr.offset(chunklen as isize) };
    ...
    if writeheader.is_some() {
        ...
        len = olen;
        (unsafe { Curl_set_in_callback(data, 1 as i32 != 0) });
        wrote_0 = unsafe { writeheader.expect("non-null function pointer")(
            ptr,
            1 as i32 as size_t,
            len,
        ...
        if wrote_0 != len {
            (unsafe { Curl_failf(data, b"Failed writing header\0" as *const u8 as *const i8) });
            return CURLE_WRITE_ERROR;
        }
    }
    return CURLE_OK;
...
#[no_mangle]
pub extern "C" fn Curl_client_write(
    mut data: *mut Curl_easy,
    mut type_0: i32,
    mut ptr: *mut i8,
    mut len: size_t,
    ...
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    if type_0 & (1 as i32) << 0 as i32 != 0
        && (unsafe { (*(*conn).handler).protocol }) & ((1 as i32) << 2 as i32 | (1 as i32) << 3 as i32) as u32
            != 0
        && (unsafe { (*conn).proto.ftpc.transfertype }) as i32 == 'A' as i32
    {
...
#[no_mangle]
pub extern "C" fn Curl_read_plain(
    mut sockfd: curl_socket_t,
    mut buf: *mut i8,
    mut bytesfromsocket: size_t,
    mut n: *mut ssize_t,
) -> CURLcode {
    let mut nread: ssize_t = unsafe { recv(sockfd, buf as *mut libc::c_void, bytesfromsocket, 0 as i32) };
    if -(1 as i32) as i64 == nread {
        let err: i32 = unsafe { *__errno_location() };
        let return_error: bool = 11 as i32 == err || 11 as i32 == err || 4 as i32 == err;
        (unsafe { *n = 0 as i32 as ssize_t });
        if return_error {
            return CURLE_AGAIN;
        }
...
pub extern "C" fn Curl_read(
    ...
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut num: i32 = (sockfd == (unsafe { (*conn).sock[1 as i32 as usize] })) as i32;
    (unsafe { *n = 0 as i32 as ssize_t });
    bytesfromsocket = if sizerequested < (unsafe { (*data).set.buffer_size }) as size_t {
        sizerequested
    } else {
        (unsafe { (*data).set.buffer_size }) as size_t
    };
    buffertofill = buf;
    nread = unsafe { ((*conn).recv[num as usize]).expect("non-null function pointer")(
        data,
        num,
        buffertofill,
        bytesfromsocket,
...
pub extern "C" fn Curl_debug(
    ...
    if (unsafe { ((*data).set).verbose() }) != 0 {
        ...
        if unsafe { ((*data).set.fdebug).is_some() } {
            (unsafe { Curl_set_in_callback(data, 1 as i32 != 0) });
            rc = unsafe { (Some(((*data).set.fdebug).expect("non-null function pointer")))
