...
pub extern "C" fn curl_share_init() -> *mut CURLSH {
    let mut share: *mut Curl_share = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
    ...
    if !share.is_null() {
        ...
        if (unsafe { Curl_mk_dnscache(&mut (*share).hostcache) }) != 0 {
...
pub unsafe extern "C" fn curl_share_setopt(
    ...
    match option as u32 {
        ...
        3 => {
            lockfunc = ::std::mem::transmute(param.arg::<*mut unsafe extern "C" fn(
                *mut CURL,
                curl_lock_data,
                curl_lock_access,
                *mut libc::c_void,
        ...
        4 => {
            unlockfunc = ::std::mem::transmute(param.arg::<*mut unsafe extern "C" fn(
                *mut CURL,
                curl_lock_data,
                *mut libc::c_void,
            ) -> ()>());
...
#[no_mangle]
pub extern "C" fn curl_share_cleanup(mut share: *mut Curl_share) -> CURLSHcode {
    ...
    (unsafe { Curl_conncache_close_all_connections(&mut (*share).conn_cache) });
    (unsafe { Curl_conncache_destroy(&mut (*share).conn_cache) });
    (unsafe { Curl_hash_destroy(&mut (*share).hostcache) });
    (unsafe { Curl_cookie_cleanup((*share).cookies) });
    if !(unsafe { (*share).sslsession }).is_null() {
        let mut i: size_t = 0;
        i = 0 as i32 as size_t;
        while i < (unsafe { (*share).max_ssl_sessions }) {
            (unsafe { Curl_ssl_kill_session(&mut *((*share).sslsession).offset(i as isize)) });
            i = i.wrapping_add(1);
        }
        (unsafe { Curl_cfree.expect("non-null function pointer")((*share).sslsession as *mut libc::c_void) });
    }
    (unsafe { Curl_psl_destroy(&mut (*share).psl) });
    if unsafe { ((*share).unlockfunc).is_some() } {
        (unsafe { ((*share).unlockfunc).expect("non-null function pointer")(
            0 as *mut CURL,
            CURL_LOCK_DATA_SHARE,
...
pub extern "C" fn Curl_share_lock(
    ...
    let mut share: *mut Curl_share = unsafe { (*data).share };
    if share.is_null() {
        return CURLSHE_INVALID;
    }
