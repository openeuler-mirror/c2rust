...
extern "C" {
    ...
    fn Curl_mime_contenttype(filename: *const i8) -> *const i8;
...
pub type curl_formget_callback =
    Option<unsafe extern "C" fn(*mut libc::c_void, *const i8, size_t) -> size_t>;
...
extern "C" fn AddHttpPost(
    ...
    post = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<curl_httppost>() as u64,
    ) }) as *mut curl_httppost;
    if !post.is_null() {
        let fresh0 = unsafe { &mut ((*post).name) };
        *fresh0 = name;
        (unsafe { (*post).namelength = (if !name.is_null() {
            if namelength != 0 {
                namelength
            } else {
                strlen(name)
        ...
        let fresh1 = unsafe { &mut ((*post).contents) };
        *fresh1 = value;
        (unsafe { (*post).contentlen = contentslength });
        let fresh2 = unsafe { &mut ((*post).buffer) };
        *fresh2 = buffer;
        (unsafe { (*post).bufferlength = bufferlength as i64 });
        let fresh3 = unsafe { &mut ((*post).contenttype) };
        *fresh3 = contenttype;
        let fresh4 = unsafe { &mut ((*post).contentheader) };
        *fresh4 = contentHeader;
        let fresh5 = unsafe { &mut ((*post).showfilename) };
        *fresh5 = showfilename;
        let fresh6 = unsafe { &mut ((*post).userp) };
        *fresh6 = userp as *mut libc::c_void;
        (unsafe { (*post).flags = flags | ((1 as i32) << 7 as i32) as i64 });
    } else {
        return 0 as *mut curl_httppost;
    }
    if !parent_post.is_null() {
        let fresh7 = unsafe { &mut ((*post).more) };
        *fresh7 = unsafe { (*parent_post).more };
        let fresh8 = unsafe { &mut ((*parent_post).more) };
        *fresh8 = post;
    } else {
        if !(unsafe { *last_post }).is_null() {
            let fresh9 = unsafe { &mut ((**last_post).next) };
            *fresh9 = post;
        } else {
            (unsafe { *httppost = post });
        }
        (unsafe { *last_post = post });
    }
    return post;
}
extern "C" fn AddFormInfo(
    mut value: *mut i8,
    mut contenttype: *mut i8,
    mut parent_form_info: *mut FormInfo,
) -> *mut FormInfo {
    let mut form_info: *mut FormInfo = 0 as *mut FormInfo;
    form_info = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<FormInfo>() as u64,
    ) }) as *mut FormInfo;
    if !form_info.is_null() {
        if !value.is_null() {
            let fresh10 = unsafe { &mut ((*form_info).value) };
            *fresh10 = value;
        }
        if !contenttype.is_null() {
            let fresh11 = unsafe { &mut ((*form_info).contenttype) };
            *fresh11 = contenttype;
        }
        (unsafe { (*form_info).flags = ((1 as i32) << 0 as i32) as i64 });
    } else {
        return 0 as *mut FormInfo;
    }
    if !parent_form_info.is_null() {
        let fresh12 = unsafe { &mut ((*form_info).more) };
        *fresh12 = unsafe { (*parent_form_info).more };
        let fresh13 = unsafe { &mut ((*parent_form_info).more) };
        *fresh13 = form_info;
    }
    return form_info;
}
extern "C" fn FormAdd(
    ...
    first_form = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<FormInfo>() as u64,
    ) }) as *mut FormInfo;
    ...
    while return_value as u32 == CURL_FORMADD_OK as i32 as u32 {
        if array_state as i32 != 0 && !forms.is_null() {
            option = unsafe { (*forms).option };
            array_value = (unsafe { (*forms).value }) as *mut i8;
            forms = unsafe { forms.offset(1) };
            if CURLFORM_END as i32 as u32 == option as u32 {
                array_state = 0 as i32 != 0;
                continue;
            }
        } else {
            option = unsafe { params.arg::<CURLformoption>() };
            if CURLFORM_END as i32 as u32 == option as u32 {
                break;
            }
        }
        ...
        match option as u32 {
            8 => {
                if array_state {
                    ...
                    forms = unsafe { params.arg::<*mut curl_forms>() };
                    if !forms.is_null() {
            ...
            2 => {
                (unsafe { (*current_form).flags |= ((1 as i32) << 2 as i32) as i64 });
            ...
            3 => {
                if (unsafe { (*current_form).namelength }) != 0 {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    (unsafe { (*current_form).namelength = if array_state as i32 != 0 {
            ...
            5 => {
                (unsafe { (*current_form).flags |= ((1 as i32) << 3 as i32) as i64 });
            ...
            6 => {
                (unsafe { (*current_form).contentslength = (if array_state as i32 != 0 {
                    array_value as size_t
                } else {
                    params.arg::<i64>() as size_t
                }) as curl_off_t });
            ...
            20 => {
                (unsafe { (*current_form).flags |= ((1 as i32) << 7 as i32) as i64 });
                (unsafe { (*current_form).contentslength = if array_state as i32 != 0 {
            ...
            7 => {
                if (unsafe { (*current_form).flags }) & ((1 as i32) << 3 as i32 | (1 as i32) << 1 as i32) as i64
                    != 0
                {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    let mut filename: *const i8 = if array_state as i32 != 0 {
                        array_value
                    } else {
                        unsafe { params.arg::<*mut i8>() }
                    };
                    if !filename.is_null() {
                        let fresh16 = unsafe { &mut ((*current_form).value) };
                        *fresh16 = unsafe { Curl_cstrdup.expect("non-null function pointer")(filename) };
                        if (unsafe { (*current_form).value }).is_null() {
                            return_value = CURL_FORMADD_MEMORY;
                        } else {
                            (unsafe { (*current_form).flags |= ((1 as i32) << 1 as i32) as i64 });
                            (unsafe { (*current_form).value_alloc = 1 as i32 != 0 });
                        }
                    } else {
                        return_value = CURL_FORMADD_NULL;
                    }
            ...
            10 => {
                let mut filename_0: *const i8 = if array_state as i32 != 0 {
                    array_value
                } else {
                    unsafe { params.arg::<*mut i8>() }
                };
                if !(unsafe { (*current_form).value }).is_null() {
                    if (unsafe { (*current_form).flags }) & ((1 as i32) << 0 as i32) as i64 != 0 {
                        if !filename_0.is_null() {
                            let mut fname: *mut i8 =
                                unsafe { Curl_cstrdup.expect("non-null function pointer")(filename_0) };
                            if fname.is_null() {
                                return_value = CURL_FORMADD_MEMORY;
                            } else {
                                form = AddFormInfo(fname, 0 as *mut i8, current_form);
                                if form.is_null() {
                                    ...
                                    (unsafe { (*form).value_alloc = 1 as i32 != 0 });
                                    current_form = form;
                    ...
                    let fresh17 = unsafe { &mut ((*current_form).value) };
                    *fresh17 = unsafe { Curl_cstrdup.expect("non-null function pointer")(filename_0) };
                    if (unsafe { (*current_form).value }).is_null() {
                        return_value = CURL_FORMADD_MEMORY;
                    } else {
                        (unsafe { (*current_form).flags |= ((1 as i32) << 0 as i32) as i64 });
                        (unsafe { (*current_form).value_alloc = 1 as i32 != 0 });
                    }
                } else {
                    return_value = CURL_FORMADD_NULL;
            ...
            12 => {
                (unsafe { (*current_form).flags |= ((1 as i32) << 5 as i32 | (1 as i32) << 4 as i32) as i64 });
                if !(unsafe { (*current_form).buffer }).is_null() {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    let mut buffer: *mut i8 = if array_state as i32 != 0 {
                        array_value
                    } else {
                        unsafe { params.arg::<*mut i8>() }
                    };
                    if !buffer.is_null() {
                        let fresh18 = unsafe { &mut ((*current_form).buffer) };
                        *fresh18 = buffer;
                        let fresh19 = unsafe { &mut ((*current_form).value) };
                        *fresh19 = buffer;
                    } else {
                        return_value = CURL_FORMADD_NULL;
                    }
            ...
            13 => {
                if (unsafe { (*current_form).bufferlength }) != 0 {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    (unsafe { (*current_form).bufferlength = if array_state as i32 != 0 {
                        array_value as size_t
                    } else {
                        params.arg::<i64>() as size_t
                    } });
            ...
            19 => {
                (unsafe { (*current_form).flags |= ((1 as i32) << 6 as i32) as i64 });
                if !(unsafe { (*current_form).userp }).is_null() {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    ...
                    if !userp.is_null() {
                        let fresh20 = unsafe { &mut ((*current_form).userp) };
                        *fresh20 = userp;
                        let fresh21 = unsafe { &mut ((*current_form).value) };
                        *fresh21 = userp;
                    } else {
                        return_value = CURL_FORMADD_NULL;
                    }
            ...
            14 => {
                let mut contenttype: *const i8 = if array_state as i32 != 0 {
                    array_value
                } else {
                    unsafe { params.arg::<*mut i8>() }
                };
                if !(unsafe { (*current_form).contenttype }).is_null() {
                    if (unsafe { (*current_form).flags }) & ((1 as i32) << 0 as i32) as i64 != 0 {
                        if !contenttype.is_null() {
                            let mut type_0: *mut i8 =
                                unsafe { Curl_cstrdup.expect("non-null function pointer")(contenttype) };
                            if type_0.is_null() {
                                return_value = CURL_FORMADD_MEMORY;
                            } else {
                                form = AddFormInfo(0 as *mut i8, type_0, current_form);
                                if form.is_null() {
                                    (unsafe { Curl_cfree.expect("non-null function pointer")(
                                    ...
                                    (unsafe { (*form).contenttype_alloc = 1 as i32 != 0 });
                                    current_form = form;
                                    form = 0 as *mut FormInfo;
                                }
                            }
                    ...
                    let fresh22 = unsafe { &mut ((*current_form).contenttype) };
                    *fresh22 = unsafe { Curl_cstrdup.expect("non-null function pointer")(contenttype) };
                    if (unsafe { (*current_form).contenttype }).is_null() {
                        return_value = CURL_FORMADD_MEMORY;
                    } else {
                        (unsafe { (*current_form).contenttype_alloc = 1 as i32 != 0 });
                    }
                } else {
                    return_value = CURL_FORMADD_NULL;
                }
            ...
            15 => {
                let mut list: *mut curl_slist = if array_state as i32 != 0 {
                    ...
                    unsafe { params.arg::<*mut curl_slist>() }
                };
                if !(unsafe { (*current_form).contentheader }).is_null() {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    let fresh23 = unsafe { &mut ((*current_form).contentheader) };
                    *fresh23 = list;
                }
                current_block_137 = 2704538829018177290;
            }
            16 | 11 => {
                let mut filename_1: *const i8 = if array_state as i32 != 0 {
                    array_value
                } else {
                    unsafe { params.arg::<*mut i8>() }
                };
                if !(unsafe { (*current_form).showfilename }).is_null() {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    let fresh24 = unsafe { &mut ((*current_form).showfilename) };
                    *fresh24 = unsafe { Curl_cstrdup.expect("non-null function pointer")(filename_1) };
                    if (unsafe { (*current_form).showfilename }).is_null() {
                        return_value = CURL_FORMADD_MEMORY;
                    } else {
                        (unsafe { (*current_form).showfilename_alloc = 1 as i32 != 0 });
                    }
                }
                current_block_137 = 2704538829018177290;
            }
        ...
        match current_block_137 {
            14371220985979925773 => {
                if !(unsafe { (*current_form).name }).is_null() {
                    return_value = CURL_FORMADD_OPTION_TWICE;
                } else {
                    ...
                    if !name.is_null() {
                        let fresh14 = unsafe { &mut ((*current_form).name) };
                        *fresh14 = name;
                    } else {
                        return_value = CURL_FORMADD_NULL;
                    }
            ...
            11213271726480235426 => {
                if !(unsafe { (*current_form).value }).is_null() {
                    ...
                    if !value.is_null() {
                        let fresh15 = unsafe { &mut ((*current_form).value) };
                        *fresh15 = value;
                    } else {
                        return_value = CURL_FORMADD_NULL;
                    }
    ...
    if CURL_FORMADD_OK as i32 as u32 != return_value as u32 {
        ...
        ptr = first_form;
        while !ptr.is_null() {
            if unsafe { (*ptr).name_alloc } {
                (unsafe { Curl_cfree.expect("non-null function pointer")((*ptr).name as *mut libc::c_void) });
                let fresh25 = unsafe { &mut ((*ptr).name) };
                *fresh25 = 0 as *mut i8;
                (unsafe { (*ptr).name_alloc = 0 as i32 != 0 });
            }
            if unsafe { (*ptr).value_alloc } {
                (unsafe { Curl_cfree.expect("non-null function pointer")((*ptr).value as *mut libc::c_void) });
                let fresh26 = unsafe { &mut ((*ptr).value) };
                *fresh26 = 0 as *mut i8;
                (unsafe { (*ptr).value_alloc = 0 as i32 != 0 });
            }
            if unsafe { (*ptr).contenttype_alloc } {
                (unsafe { Curl_cfree.expect("non-null function pointer")(
                    (*ptr).contenttype as *mut libc::c_void,
                ) });
                let fresh27 = unsafe { &mut ((*ptr).contenttype) };
                *fresh27 = 0 as *mut i8;
                (unsafe { (*ptr).contenttype_alloc = 0 as i32 != 0 });
            }
            if unsafe { (*ptr).showfilename_alloc } {
                (unsafe { Curl_cfree.expect("non-null function pointer")(
                    (*ptr).showfilename as *mut libc::c_void,
                ) });
                let fresh28 = unsafe { &mut ((*ptr).showfilename) };
                *fresh28 = 0 as *mut i8;
                (unsafe { (*ptr).showfilename_alloc = 0 as i32 != 0 });
            }
            ptr = unsafe { (*ptr).more };
        }
    }
    if CURL_FORMADD_OK as i32 as u32 == return_value as u32 {
        ...
        form = first_form;
        while !form.is_null() {
            if ((unsafe { (*form).name }).is_null() || (unsafe { (*form).value }).is_null()) && post.is_null()
                || (unsafe { (*form).contentslength }) != 0
                    && (unsafe { (*form).flags }) & ((1 as i32) << 0 as i32) as i64 != 0
                || (unsafe { (*form).flags }) & ((1 as i32) << 0 as i32) as i64 != 0
                    && (unsafe { (*form).flags }) & ((1 as i32) << 3 as i32) as i64 != 0
                || (unsafe { (*form).buffer }).is_null()
                    && (unsafe { (*form).flags }) & ((1 as i32) << 4 as i32) as i64 != 0
                    && (unsafe { (*form).flags }) & ((1 as i32) << 5 as i32) as i64 != 0
                || (unsafe { (*form).flags }) & ((1 as i32) << 1 as i32) as i64 != 0
                    && (unsafe { (*form).flags }) & ((1 as i32) << 3 as i32) as i64 != 0
            {
                return_value = CURL_FORMADD_INCOMPLETE;
                break;
            } else {
                if ((unsafe { (*form).flags }) & ((1 as i32) << 0 as i32) as i64 != 0
                    || (unsafe { (*form).flags }) & ((1 as i32) << 4 as i32) as i64 != 0)
                    && (unsafe { (*form).contenttype }).is_null()
                {
                    let mut f: *mut i8 = if (unsafe { (*form).flags }) & ((1 as i32) << 4 as i32) as i64 != 0 {
                        unsafe { (*form).showfilename }
                    } else {
                        unsafe { (*form).value }
                    };
                    let mut type_1: *const i8 = 0 as *const i8;
                    type_1 = unsafe { Curl_mime_contenttype(f) };
                    if type_1.is_null() {
                        type_1 = prevtype;
                    }
                    if type_1.is_null() {
                        type_1 = b"application/octet-stream\0" as *const u8 as *const i8;
                    }
                    let fresh29 = unsafe { &mut ((*form).contenttype) };
                    *fresh29 = unsafe { Curl_cstrdup.expect("non-null function pointer")(type_1) };
                    if (unsafe { (*form).contenttype }).is_null() {
                        return_value = CURL_FORMADD_MEMORY;
                        break;
                    } else {
                        (unsafe { (*form).contenttype_alloc = 1 as i32 != 0 });
                ...
                if !(unsafe { (*form).name }).is_null() && (unsafe { (*form).namelength }) != 0 {
                    let mut i: size_t = 0;
                    i = 0 as i32 as size_t;
                    while i < (unsafe { (*form).namelength }) {
                        if (unsafe { *((*form).name).offset(i as isize) }) == 0 {
                            return_value = CURL_FORMADD_NULL;
                            break;
                        } else {
                            i = i.wrapping_add(1);
                    ...
                    if return_value as u32 != CURL_FORMADD_OK as i32 as u32 {
                        break;
                    }
                }
                if (unsafe { (*form).flags }) & ((1 as i32) << 2 as i32) as i64 == 0 && form == first_form {
                    if !(unsafe { (*form).name }).is_null() {
                        let fresh30 = unsafe { &mut ((*form).name) };
                        *fresh30 = (unsafe { Curl_memdup(
                            (*form).name as *const libc::c_void,
                            if (*form).namelength != 0 {
                                (*form).namelength
                            } else {
                                (strlen((*form).name)).wrapping_add(1 as i32 as u64)
                            },
                        ) }) as *mut i8;
                    }
                    if (unsafe { (*form).name }).is_null() {
                        ...
                        (unsafe { (*form).name_alloc = 1 as i32 != 0 });
                    }
                }
                if (unsafe { (*form).flags })
                    & ((1 as i32) << 0 as i32
                        | (1 as i32) << 1 as i32
                        | (1 as i32) << 3 as i32
                        | (1 as i32) << 5 as i32
                    ...
                    == 0
                    && !(unsafe { (*form).value }).is_null()
                {
                    let mut clen: size_t = (unsafe { (*form).contentslength }) as size_t;
                    if clen == 0 {
                        clen = (unsafe { strlen((*form).value) }).wrapping_add(1 as i32 as u64);
                    }
                    let fresh31 = unsafe { &mut ((*form).value) };
                    *fresh31 = (unsafe { Curl_memdup((*form).value as *const libc::c_void, clen) }) as *mut i8;
                    if (unsafe { (*form).value }).is_null() {
                        return_value = CURL_FORMADD_MEMORY;
                        break;
                    } else {
                        (unsafe { (*form).value_alloc = 1 as i32 != 0 });
                ...
                post = AddHttpPost(
                    unsafe { (*form).name },
                    unsafe { (*form).namelength },
                    unsafe { (*form).value },
                    unsafe { (*form).contentslength },
                    unsafe { (*form).buffer },
                    unsafe { (*form).bufferlength },
                    unsafe { (*form).contenttype },
                    unsafe { (*form).flags },
                    unsafe { (*form).contentheader },
                    unsafe { (*form).showfilename },
                    unsafe { (*form).userp },
                    post,
                    httppost,
                ...
                if post.is_null() {
                    ...
                    if !(unsafe { (*form).contenttype }).is_null() {
                        prevtype = unsafe { (*form).contenttype };
                    }
                    form = unsafe { (*form).more };
                }
            }
        }
        if CURL_FORMADD_OK as i32 as u32 != return_value as u32 {
            ...
            ptr_0 = form;
            while !ptr_0.is_null() {
                if unsafe { (*ptr_0).name_alloc } {
                    (unsafe { Curl_cfree.expect("non-null function pointer")(
                        (*ptr_0).name as *mut libc::c_void,
                    ) });
                    let fresh32 = unsafe { &mut ((*ptr_0).name) };
                    *fresh32 = 0 as *mut i8;
                    (unsafe { (*ptr_0).name_alloc = 0 as i32 != 0 });
                }
                if unsafe { (*ptr_0).value_alloc } {
                    (unsafe { Curl_cfree.expect("non-null function pointer")(
                        (*ptr_0).value as *mut libc::c_void,
                    ) });
                    let fresh33 = unsafe { &mut ((*ptr_0).value) };
                    *fresh33 = 0 as *mut i8;
                    (unsafe { (*ptr_0).value_alloc = 0 as i32 != 0 });
                }
                if unsafe { (*ptr_0).contenttype_alloc } {
                    (unsafe { Curl_cfree.expect("non-null function pointer")(
                        (*ptr_0).contenttype as *mut libc::c_void,
                    ) });
                    let fresh34 = unsafe { &mut ((*ptr_0).contenttype) };
                    *fresh34 = 0 as *mut i8;
                    (unsafe { (*ptr_0).contenttype_alloc = 0 as i32 != 0 });
                }
                if unsafe { (*ptr_0).showfilename_alloc } {
                    (unsafe { Curl_cfree.expect("non-null function pointer")(
                        (*ptr_0).showfilename as *mut libc::c_void,
                    ) });
                    let fresh35 = unsafe { &mut ((*ptr_0).showfilename) };
                    *fresh35 = 0 as *mut i8;
                    (unsafe { (*ptr_0).showfilename_alloc = 0 as i32 != 0 });
                }
                ptr_0 = unsafe { (*ptr_0).more };
            }
        }
    }
    while !first_form.is_null() {
        let mut ptr_1: *mut FormInfo = unsafe { (*first_form).more };
        (unsafe { Curl_cfree.expect("non-null function pointer")(first_form as *mut libc::c_void) });
        first_form = ptr_1;
    }
    return return_value;
}
...
pub extern "C" fn curl_formget(
    ...
    (unsafe { Curl_mime_initpart(&mut toppart, 0 as *mut Curl_easy) });
    ...
    while result as u64 == 0 {
        ...
        let mut nread: size_t = unsafe { Curl_mime_read(
            buffer.as_mut_ptr(),
            1 as i32 as size_t,
        ...
        if nread > ::std::mem::size_of::<[i8; 8192]>() as u64
            || (unsafe { append.expect("non-null function pointer")(arg, buffer.as_mut_ptr(), nread) }) != nread
        {
    ...
    (unsafe { Curl_mime_cleanpart(&mut toppart) });
    return result as i32;
}
#[no_mangle]
pub extern "C" fn curl_formfree(mut form: *mut curl_httppost) {
    let mut next: *mut curl_httppost = 0 as *mut curl_httppost;
    if form.is_null() {
        return;
    }
    loop {
        next = unsafe { (*form).next };
        curl_formfree(unsafe { (*form).more });
        if (unsafe { (*form).flags }) & ((1 as i32) << 2 as i32) as i64 == 0 {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*form).name as *mut libc::c_void) });
        }
        if (unsafe { (*form).flags })
            & ((1 as i32) << 3 as i32 | (1 as i32) << 4 as i32 | (1 as i32) << 6 as i32) as i64
            == 0
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*form).contents as *mut libc::c_void) });
        }
        (unsafe { Curl_cfree.expect("non-null function pointer")((*form).contenttype as *mut libc::c_void) });
        (unsafe { Curl_cfree.expect("non-null function pointer")((*form).showfilename as *mut libc::c_void) });
        (unsafe { Curl_cfree.expect("non-null function pointer")(form as *mut libc::c_void) });
        form = next;
        if form.is_null() {
            break;
        }
...
extern "C" fn setname(
    ...
    if name.is_null() || len == 0 {
        return unsafe { curl_mime_name(part, name) };
    }
    zname = (unsafe { Curl_cmalloc.expect("non-null function pointer")(len.wrapping_add(1 as i32 as u64)) })
        as *mut i8;
    if zname.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { memcpy(zname as *mut libc::c_void, name as *const libc::c_void, len) });
    (unsafe { *zname.offset(len as isize) = '\u{0}' as i32 as i8 });
    res = unsafe { curl_mime_name(part, zname) };
    (unsafe { Curl_cfree.expect("non-null function pointer")(zname as *mut libc::c_void) });
    return res;
}
#[no_mangle]
pub extern "C" fn Curl_getformdata(
    ...
    (unsafe { Curl_mime_cleanpart(finalform) });
    if post.is_null() {
        return result;
    }
    form = unsafe { curl_mime_init(data) };
    if form.is_null() {
        result = CURLE_OUT_OF_MEMORY;
    }
    if result as u64 == 0 {
        result = unsafe { curl_mime_subparts(finalform, form) };
    }
    while result as u64 == 0 && !post.is_null() {
        let mut multipart: *mut curl_mime = form;
        if !(unsafe { (*post).more }).is_null() {
            part = unsafe { curl_mime_addpart(form) };
            if part.is_null() {
                result = CURLE_OUT_OF_MEMORY;
            }
            if result as u64 == 0 {
                result = setname(part, unsafe { (*post).name }, (unsafe { (*post).namelength }) as size_t);
            }
            if result as u64 == 0 {
                multipart = unsafe { curl_mime_init(data) };
                if multipart.is_null() {
                    result = CURLE_OUT_OF_MEMORY;
                }
            }
            if result as u64 == 0 {
                result = unsafe { curl_mime_subparts(part, multipart) };
            }
        }
        file = post;
        while result as u64 == 0 && !file.is_null() {
            part = unsafe { curl_mime_addpart(multipart) };
            if part.is_null() {
                result = CURLE_OUT_OF_MEMORY;
            }
            if result as u64 == 0 {
                result = unsafe { curl_mime_headers(part, (*file).contentheader, 0 as i32) };
            }
            if result as u64 == 0 && !(unsafe { (*file).contenttype }).is_null() {
                result = unsafe { curl_mime_type(part, (*file).contenttype) };
            }
            if result as u64 == 0 && (unsafe { (*post).more }).is_null() {
                result = setname(part, unsafe { (*post).name }, (unsafe { (*post).namelength }) as size_t);
            }
            if result as u64 == 0 {
                let mut clen: curl_off_t = unsafe { (*post).contentslength };
                if (unsafe { (*post).flags }) & ((1 as i32) << 7 as i32) as i64 != 0 {
                    clen = unsafe { (*post).contentlen };
                }
                if (unsafe { (*post).flags }) & ((1 as i32) << 0 as i32 | (1 as i32) << 1 as i32) as i64 != 0 {
                    if (unsafe { strcmp((*file).contents, b"-\0" as *const u8 as *const i8) }) == 0 {
                        result = unsafe { curl_mime_data_cb(
                            part,
                            ...
                            ::std::mem::transmute::<
                                ...
                                curl_seek_callback,
                            >(::std::mem::transmute::<
                                Option<unsafe extern "C" fn(*mut FILE, i64, i32) -> i32>,
                                Option<unsafe extern "C" fn() -> ()>,
                            >(Some(
                                fseek as unsafe extern "C" fn(*mut FILE, i64, i32) -> i32,
                            ))),
                            None,
                            stdin as *mut libc::c_void,
                        ) };
                    } else {
                        result = unsafe { curl_mime_filedata(part, (*file).contents) };
                    }
                    if result as u64 == 0 && (unsafe { (*post).flags }) & ((1 as i32) << 1 as i32) as i64 != 0 {
                        result = unsafe { curl_mime_filename(part, 0 as *const i8) };
                    }
                } else if (unsafe { (*post).flags }) & ((1 as i32) << 4 as i32) as i64 != 0 {
                    result = unsafe { curl_mime_data(
                        part,
                        (*post).buffer,
                ...
                } else if (unsafe { (*post).flags }) & ((1 as i32) << 6 as i32) as i64 != 0 {
                    if clen == 0 {
                        clen = -(1 as i32) as curl_off_t;
                    }
                    result = unsafe { curl_mime_data_cb(part, clen, fread_func, None, None, (*post).userp) };
                } else {
                    ...
                    result = unsafe { curl_mime_data(part, (*post).contents, uclen) };
                }
            }
            if result as u64 == 0 && !(unsafe { (*post).showfilename }).is_null() {
                if !(unsafe { (*post).more }).is_null()
                    || (unsafe { (*post).flags })
                        & ((1 as i32) << 0 as i32 | (1 as i32) << 4 as i32 | (1 as i32) << 6 as i32)
                            as i64
                        != 0
                {
                    result = unsafe { curl_mime_filename(part, (*post).showfilename) };
                }
            }
            file = unsafe { (*file).more };
        }
        post = unsafe { (*post).next };
    }
    if result as u64 != 0 {
        (unsafe { Curl_mime_cleanpart(finalform) });
    }
