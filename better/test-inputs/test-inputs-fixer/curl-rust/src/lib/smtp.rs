...
extern "C" {
    ...
    fn Curl_is_ASCII_name(hostname: *const i8) -> bool;
    ...
    fn Curl_gethostname(name: *mut i8, namelen: size_t) -> i32;
...
#[no_mangle]
pub static mut Curl_handler_smtp: Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_it: Some(smtp_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: Some(
                smtp_done as unsafe extern "C" fn(*mut Curl_easy, CURLcode, bool) -> CURLcode,
            ),
            do_more: None,
            connect_it: Some(
                smtp_connect as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            connecting: Some(
                smtp_multi_statemach as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            doing: Some(smtp_doing as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            proto_getsock: Some(
                smtp_getsock
            ...
            flags: ((1 as i32) << 2 as i32 | (1 as i32) << 6 as i32 | (1 as i32) << 10 as i32)
                as u32,
        };
        init
    }
...
#[no_mangle]
pub static mut Curl_handler_smtps: Curl_handler =  {
    {
        let mut init = Curl_handler {
...
static mut saslsmtp: SASLproto =  {
    {
        let mut init = SASLproto {
            ...
            getmessage: Some(smtp_get_message as unsafe extern "C" fn(*mut i8, *mut *mut i8) -> ()),
        };
        init
    }
};
extern "C" fn smtp_to_smtps(mut conn: *mut connectdata) {
    let fresh0 = unsafe { &mut ((*conn).handler) };
    *fresh0 = unsafe { &Curl_handler_smtps };
    let fresh1 = unsafe { &mut ((*conn).bits) };
    (*fresh1).set_tls_upgraded(1 as i32 as bit);
}
extern "C" fn smtp_endofresp(
    ...
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut result: bool = 0 as i32 != 0;
    if len < 4 as i32 as u64
        || (unsafe { Curl_isdigit(*line.offset(0 as i32 as isize) as u8 as i32) }) == 0
        || (unsafe { Curl_isdigit(*line.offset(1 as i32 as isize) as u8 as i32) }) == 0
        || (unsafe { Curl_isdigit(*line.offset(2 as i32 as isize) as u8 as i32) }) == 0
    {
        return 0 as i32 != 0;
    }
    if (unsafe { *line.offset(3 as i32 as isize) }) as i32 == ' ' as i32 || len == 5 as i32 as u64 {
        ...
        (unsafe { *resp = curlx_sltosi(strtol(tmpline.as_mut_ptr(), 0 as *mut *mut i8, 10 as i32)) });
        if (unsafe { *resp }) == 1 as i32 {
            (unsafe { *resp = 0 as i32 });
        }
    } else if (unsafe { *line.offset(3 as i32 as isize) }) as i32 == '-' as i32
        && ((unsafe { (*smtpc).state }) as u32 == SMTP_EHLO as i32 as u32
            || (unsafe { (*smtpc).state }) as u32 == SMTP_COMMAND as i32 as u32)
    {
        result = 1 as i32 != 0;
...
extern "C" fn smtp_get_message(mut buffer: *mut i8, mut outptr: *mut *mut i8) {
    ...
    if len > 4 as i32 as u64 {
        ...
        message = unsafe { buffer.offset(4 as i32 as isize) };
...
extern "C" fn state(mut data: *mut Curl_easy, mut newstate: smtpstate) {
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*(*data).conn).proto.smtpc };
    (unsafe { (*smtpc).state = newstate });
}
extern "C" fn smtp_perform_ehlo(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    (unsafe { (*smtpc).sasl.authmechs = 0 as i32 as u16 });
    (unsafe { (*smtpc).sasl.authused = 0 as i32 as u16 });
    (unsafe { (*smtpc).tls_supported = 0 as i32 != 0 });
    (unsafe { (*smtpc).auth_supported = 0 as i32 != 0 });
...
extern "C" fn smtp_perform_helo(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    (unsafe { (*smtpc).sasl.authused = 0 as i32 as u16 });
    result = unsafe { Curl_pp_sendf(
...
extern "C" fn smtp_perform_starttls(
...
extern "C" fn smtp_perform_upgrade_tls(mut data: *mut Curl_easy) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut result: CURLcode =
        unsafe { Curl_ssl_connect_nonblocking(data, conn, 0 as i32 != 0, 0 as i32, &mut (*smtpc).ssldone) };
    if result as u64 == 0 {
        if (unsafe { (*smtpc).state }) as u32 != SMTP_UPGRADETLS as i32 as u32 {
            state(data, SMTP_UPGRADETLS);
        }
        if unsafe { (*smtpc).ssldone } {
            smtp_to_smtps(conn);
...
extern "C" fn smtp_continue_auth(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut resp: *const i8,
) -> CURLcode {
...
extern "C" fn smtp_perform_authentication(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut progress: saslprogress = SASL_IDLE;
    if !(unsafe { (*smtpc).auth_supported }) || !(unsafe { Curl_sasl_can_authenticate(&mut (*smtpc).sasl, conn) }) {
        state(data, SMTP_STOP);
        return result;
    }
    result = unsafe { Curl_sasl_start(&mut (*smtpc).sasl, data, conn, 0 as i32 != 0, &mut progress) };
    if result as u64 == 0 {
...
extern "C" fn smtp_perform_command(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
    if !(unsafe { (*smtp).rcpt }).is_null() {
        let mut utf8: bool = 0 as i32 != 0;
        if (unsafe { (*smtp).custom }).is_null() || (unsafe { *((*smtp).custom).offset(0 as i32 as isize) }) == 0 {
            ...
            result = smtp_parse_address(data, unsafe { (*(*smtp).rcpt).data }, &mut address, &mut host);
            if result as u64 != 0 {
                return result;
            }
            utf8 = (unsafe { (*conn).proto.smtpc.utf8_supported }) as i32 != 0
                && (!(host.encalloc).is_null()
                    || !(unsafe { Curl_is_ASCII_name(address) })
                    || !(unsafe { Curl_is_ASCII_name(host.name) }));
            result = unsafe { Curl_pp_sendf(
                data,
                &mut (*conn).proto.smtpc.pp as *mut pingpong,
            ...
            (unsafe { Curl_free_idnconverted_hostname(&mut host) });
            (unsafe { Curl_cfree.expect("non-null function pointer")(address as *mut libc::c_void) });
        } else {
            utf8 = (unsafe { (*conn).proto.smtpc.utf8_supported }) as i32 != 0
                && (unsafe { strcmp((*smtp).custom, b"EXPN\0" as *const u8 as *const i8) }) == 0;
            result = unsafe { Curl_pp_sendf(
                data,
                &mut (*conn).proto.smtpc.pp as *mut pingpong,
        ...
        result = unsafe { Curl_pp_sendf(
            ...
            if !((*smtp).custom).is_null()
                && *((*smtp).custom).offset(0 as i32 as isize) as i32 != '\u{0}' as i32
            {
...
extern "C" fn smtp_perform_mail(mut data: *mut Curl_easy) -> CURLcode {
    ...
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut utf8: bool = 0 as i32 != 0;
    if !(unsafe { (*data).set.str_0[STRING_MAIL_FROM as i32 as usize] }).is_null() {
        let mut address: *mut i8 = 0 as *mut i8;
        let mut host: hostname = {
            let mut init = hostname {
                rawalloc: 0 as *mut i8,
        ...
        result = smtp_parse_address(
            data,
            unsafe { (*data).set.str_0[STRING_MAIL_FROM as i32 as usize] },
            &mut address,
            &mut host,
        );
        if result as u64 != 0 {
        ...
        utf8 = (unsafe { (*conn).proto.smtpc.utf8_supported }) as i32 != 0
            && (!(host.encalloc).is_null()
                ...
                || !(unsafe { Curl_is_ASCII_name(host.name) }));
        if !(host.name).is_null() {
            from = unsafe { curl_maprintf(b"<%s@%s>\0" as *const u8 as *const i8, address, host.name) };
            (unsafe { Curl_free_idnconverted_hostname(&mut host) });
        } else {
            from = unsafe { curl_maprintf(b"<%s>\0" as *const u8 as *const i8, address) };
        }
        (unsafe { Curl_cfree.expect("non-null function pointer")(address as *mut libc::c_void) });
    } else {
        from = unsafe { Curl_cstrdup.expect("non-null function pointer")(b"<>\0" as *const u8 as *const i8) };
    }
    if from.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    if !(unsafe { (*data).set.str_0[STRING_MAIL_AUTH as i32 as usize] }).is_null()
        && (unsafe { (*conn).proto.smtpc.sasl.authused }) as i32 != 0
    {
        if (unsafe { *((*data).set.str_0[STRING_MAIL_AUTH as i32 as usize]).offset(0 as i32 as isize) }) as i32
            != '\u{0}' as i32
        {
            ...
            result = smtp_parse_address(
                data,
                unsafe { (*data).set.str_0[STRING_MAIL_AUTH as i32 as usize] },
                &mut address_0,
                &mut host_0,
            );
            if result as u64 != 0 {
            ...
            if !utf8
                && (unsafe { (*conn).proto.smtpc.utf8_supported }) as i32 != 0
                && (!(host_0.encalloc).is_null()
                    || !(unsafe { Curl_is_ASCII_name(address_0) })
                    || !(unsafe { Curl_is_ASCII_name(host_0.name) }))
            {
                utf8 = 1 as i32 != 0;
            }
            if !(host_0.name).is_null() {
                auth = unsafe { curl_maprintf(
                    b"<%s@%s>\0" as *const u8 as *const i8,
                    address_0,
                    host_0.name,
                ) };
                (unsafe { Curl_free_idnconverted_hostname(&mut host_0) });
            } else {
                auth = unsafe { curl_maprintf(b"<%s>\0" as *const u8 as *const i8, address_0) };
            }
            (unsafe { Curl_cfree.expect("non-null function pointer")(address_0 as *mut libc::c_void) });
        } else {
            auth =
    ...
    if (unsafe { (*data).set.mimepost.kind }) as u32 != MIMEKIND_NONE as i32 as u32 {
        ...
        let fresh3 = unsafe { &mut ((*data).state.fread_func) };
        *fresh3 = unsafe { ::std::mem::transmute::<
            Option<unsafe extern "C" fn(*mut i8, size_t, size_t, *mut libc::c_void) -> size_t>,
            curl_read_callback,
        >(Some(
            Curl_mime_read
        ...
        let fresh4 = unsafe { &mut ((*data).state.in_0) };
        *fresh4 = (unsafe { &mut (*data).set.mimepost }) as *mut curl_mimepart as *mut libc::c_void;
    }
    if (unsafe { (*conn).proto.smtpc.size_supported }) as i32 != 0 && (unsafe { (*data).state.infilesize }) > 0 as i32 as i64
    {
        size = unsafe { curl_maprintf(b"%ld\0" as *const u8 as *const i8, (*data).state.infilesize) };
        if size.is_null() {
            (unsafe { Curl_cfree.expect("non-null function pointer")(from as *mut libc::c_void) });
            (unsafe { Curl_cfree.expect("non-null function pointer")(auth as *mut libc::c_void) });
            return CURLE_OUT_OF_MEMORY;
    ...
    if (unsafe { (*conn).proto.smtpc.utf8_supported }) as i32 != 0 && !utf8 {
        let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
        let mut rcpt: *mut curl_slist = unsafe { (*smtp).rcpt };
        while !rcpt.is_null() && !utf8 {
            if !(unsafe { Curl_is_ASCII_name((*rcpt).data) }) {
                utf8 = 1 as i32 != 0;
            }
            rcpt = unsafe { (*rcpt).next };
        }
    }
    result = unsafe { Curl_pp_sendf(
        data,
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(from as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")(auth as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")(size as *mut libc::c_void) });
    if result as u64 == 0 {
        state(data, SMTP_MAIL);
    }
    return result;
}
extern "C" fn smtp_perform_rcpt_to(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
    let mut address: *mut i8 = 0 as *mut i8;
...
extern "C" fn smtp_perform_quit(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = unsafe { Curl_pp_sendf(
        data,
        &mut (*conn).proto.smtpc.pp as *mut pingpong,
        b"%s\0" as *const u8 as *const i8,
...
extern "C" fn smtp_state_servergreet_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    if smtpcode / 100 as i32 != 2 as i32 {
        (unsafe { Curl_failf(
            data,
            b"Got unexpected smtp-server response: %d\0" as *const u8 as *const i8,
            smtpcode,
        ) });
...
extern "C" fn smtp_state_starttls_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    if (unsafe { (*(*data).conn).proto.smtpc.pp.cache_size }) != 0 {
...
extern "C" fn smtp_state_ehlo_resp(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut smtpcode: i32,
    mut _instate: smtpstate,
    ...
    if smtpcode / 100 as i32 != 2 as i32 && smtpcode != 1 as i32 {
        if (unsafe { (*data).set.use_ssl }) as u32 <= CURLUSESSL_TRY as i32 as u32
            || (unsafe { ((*conn).ssl[0 as i32 as usize]).use_0() }) as i32 != 0
        {
        ...
        {
            (unsafe { (*smtpc).tls_supported = 1 as i32 != 0 });
        } else if len >= 4 as i32 as u64
            && (unsafe { memcmp(
                line as *const libc::c_void,
        ...
        {
            (unsafe { (*smtpc).size_supported = 1 as i32 != 0 });
        } else if len >= 8 as i32 as u64
            && (unsafe { memcmp(
                line as *const libc::c_void,
                b"SMTPUTF8\0" as *const u8 as *const i8 as *const libc::c_void,
                8 as i32 as u64,
            ) }) == 0
        {
            (unsafe { (*smtpc).utf8_supported = 1 as i32 != 0 });
        ...
        {
            (unsafe { (*smtpc).auth_supported = 1 as i32 != 0 });
            line = unsafe { line.offset(5 as i32 as isize) };
            len = (len as u64).wrapping_sub(5 as i32 as u64) as size_t as size_t;
            loop {
                ...
                if mechbit as i32 != 0 && llen == wordlen {
                    let fresh5 = unsafe { &mut ((*smtpc).sasl.authmechs) };
                    *fresh5 = (*fresh5 as i32 | mechbit as i32) as u16;
                }
        ...
        if smtpcode != 1 as i32 {
            if (unsafe { (*data).set.use_ssl }) as u32 != 0 && (unsafe { ((*conn).ssl[0 as i32 as usize]).use_0() }) == 0 {
                if unsafe { (*smtpc).tls_supported } {
                    result = smtp_perform_starttls(data, conn);
                } else if (unsafe { (*data).set.use_ssl }) as u32 == CURLUSESSL_TRY as i32 as u32 {
                    result = smtp_perform_authentication(data);
                } else {
                    (unsafe { Curl_failf(data, b"STARTTLS not supported.\0" as *const u8 as *const i8) });
                    result = CURLE_USE_SSL_FAILED;
                }
            } else {
                result = smtp_perform_authentication(data);
...
extern "C" fn smtp_state_helo_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
...
extern "C" fn smtp_state_auth_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
    ...
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut progress: saslprogress = SASL_IDLE;
    result = unsafe { Curl_sasl_continue(&mut (*smtpc).sasl, data, conn, smtpcode, &mut progress) };
    if result as u64 == 0 {
...
extern "C" fn smtp_state_command_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
    ...
    let mut len: size_t = unsafe { strlen(line) };
    if !(unsafe { (*smtp).rcpt }).is_null()
        && smtpcode / 100 as i32 != 2 as i32
        && smtpcode != 553 as i32
        && smtpcode != 1 as i32
        || (unsafe { (*smtp).rcpt }).is_null() && smtpcode / 100 as i32 != 2 as i32 && smtpcode != 1 as i32
    {
        (unsafe { Curl_failf(
            data,
        ...
        if smtpcode != 1 as i32 {
            if !(unsafe { (*smtp).rcpt }).is_null() {
                let fresh6 = unsafe { &mut ((*smtp).rcpt) };
                *fresh6 = unsafe { (*(*smtp).rcpt).next };
                if !(unsafe { (*smtp).rcpt }).is_null() {
...
extern "C" fn smtp_state_mail_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
...
extern "C" fn smtp_state_rcpt_resp(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut smtpcode: i32,
    mut _instate: smtpstate,
    ...
    is_smtp_blocking_err = if is_smtp_err as i32 != 0 && (unsafe { ((*data).set).mail_rcpt_allowfails() }) == 0 {
        1 as i32
    } else {
        0 as i32
    } != 0;
    if is_smtp_err {
        (unsafe { (*smtp).rcpt_last_error = smtpcode });
        ...
        (unsafe { (*smtp).rcpt_had_ok = 1 as i32 != 0 });
    }
    if !is_smtp_blocking_err {
        let fresh7 = unsafe { &mut ((*smtp).rcpt) };
        *fresh7 = unsafe { (*(*smtp).rcpt).next };
        if !(unsafe { (*smtp).rcpt }).is_null() {
            result = smtp_perform_rcpt_to(data);
        } else if !(unsafe { (*smtp).rcpt_had_ok }) {
            (unsafe { Curl_failf(
                data,
...
extern "C" fn smtp_state_data_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
...
extern "C" fn smtp_state_postdata_resp(
    mut data: *mut Curl_easy,
    mut smtpcode: i32,
    mut _instate: smtpstate,
) -> CURLcode {
...
extern "C" fn smtp_statemachine(mut data: *mut Curl_easy, mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut sock: curl_socket_t = unsafe { (*conn).sock[0 as i32 as usize] };
    let mut smtpcode: i32 = 0;
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut pp: *mut pingpong = unsafe { &mut (*smtpc).pp };
    ...
    loop {
        result = unsafe { Curl_pp_readresp(data, sock, pp, &mut smtpcode, &mut nread) };
        if result as u64 != 0 {
            return result;
        }
        if (unsafe { (*smtpc).state }) as u32 != SMTP_QUIT as i32 as u32 && smtpcode != 1 as i32 {
            (unsafe { (*data).info.httpcode = smtpcode });
        }
        if smtpcode == 0 {
            break;
        }
        match (unsafe { (*smtpc).state }) as u32 {
            1 => {
                result = smtp_state_servergreet_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            2 => {
                result = smtp_state_ehlo_resp(data, conn, smtpcode, unsafe { (*smtpc).state });
            }
            3 => {
                result = smtp_state_helo_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            4 => {
                result = smtp_state_starttls_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            6 => {
                result = smtp_state_auth_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            7 => {
                result = smtp_state_command_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            8 => {
                result = smtp_state_mail_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            9 => {
                result = smtp_state_rcpt_resp(data, conn, smtpcode, unsafe { (*smtpc).state });
            }
            10 => {
                result = smtp_state_data_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            11 => {
                result = smtp_state_postdata_resp(data, smtpcode, unsafe { (*smtpc).state });
            }
            12 | _ => {
                state(data, SMTP_STOP);
            }
        ...
        if !(result as u64 == 0
            && (unsafe { (*smtpc).state }) as u32 != SMTP_STOP as i32 as u32
...
extern "C" fn smtp_multi_statemach(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    if (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 0 as i32) as u32 != 0 && !(unsafe { (*smtpc).ssldone }) {
        ...
        if result as u32 != 0 || !(unsafe { (*smtpc).ssldone }) {
            return result;
        }
    }
    result = unsafe { Curl_pp_statemach(data, &mut (*smtpc).pp, 0 as i32 != 0, 0 as i32 != 0) };
    (unsafe { *done = if (*smtpc).state as u32 == SMTP_STOP as i32 as u32 {
        1 as i32
    } else {
        0 as i32
    } != 0 });
...
extern "C" fn smtp_block_statemach(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    ...
    let mut result: CURLcode = CURLE_OK;
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    while (unsafe { (*smtpc).state }) as u32 != SMTP_STOP as i32 as u32 && result as u64 == 0 {
        result = unsafe { Curl_pp_statemach(data, &mut (*smtpc).pp, 1 as i32 != 0, disconnecting) };
    }
    return result;
}
extern "C" fn smtp_init(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut smtp: *mut SMTP = 0 as *mut SMTP;
    let fresh8 = unsafe { &mut ((*data).req.p.smtp) };
    *fresh8 = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<SMTP>() as u64,
        1 as i32 as size_t,
    ) }) as *mut SMTP;
    smtp = *fresh8;
    if smtp.is_null() {
...
extern "C" fn smtp_getsock(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut socks: *mut curl_socket_t,
) -> i32 {
    return unsafe { Curl_pp_getsock(data, &mut (*conn).proto.smtpc.pp, socks) };
}
extern "C" fn smtp_connect(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut pp: *mut pingpong = unsafe { &mut (*smtpc).pp };
    ...
    let fresh9 = unsafe { &mut ((*pp).statemachine) };
    *fresh9 = Some(
        smtp_statemachine as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata) -> CURLcode,
    );
    let fresh10 = unsafe { &mut ((*pp).endofresp) };
    *fresh10 = Some(
        smtp_endofresp
            as unsafe extern "C" fn(
                *mut Curl_easy,
    ...
    (unsafe { Curl_sasl_init(&mut (*smtpc).sasl, &saslsmtp) });
...
extern "C" fn smtp_done(
    ...
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
    let mut pp: *mut pingpong = unsafe { &mut (*conn).proto.smtpc.pp };
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")((*smtp).custom as *mut libc::c_void) });
    let fresh11 = unsafe { &mut ((*smtp).custom) };
    *fresh11 = 0 as *mut i8;
    if status as u64 != 0 {
        (unsafe { Curl_conncontrol(conn, 1 as i32) });
        result = status;
    } else if (unsafe { ((*data).set).connect_only() }) == 0
        && !(unsafe { (*data).set.mail_rcpt }).is_null()
        && ((unsafe { ((*data).set).upload() }) as i32 != 0 || (unsafe { (*data).set.mimepost.kind }) as u32 != 0)
    {
        if (unsafe { (*smtp).trailing_crlf }) as i32 != 0 || (unsafe { (*data).state.infilesize }) == 0 {
            eob = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                &*(b"\r\n.\r\n\0" as *const u8 as *const i8).offset(2 as i32 as isize),
            ) };
        ...
        if result as u64 != 0 {
            (unsafe { Curl_cfree.expect("non-null function pointer")(eob as *mut libc::c_void) });
            return result;
        }
        if bytes_written != len {
            let fresh12 = unsafe { &mut ((*pp).sendthis) };
            *fresh12 = eob;
            (unsafe { (*pp).sendsize = len as size_t });
            (unsafe { (*pp).sendleft = (len - bytes_written) as size_t });
        } else {
            (unsafe { (*pp).response = Curl_now() });
            (unsafe { Curl_cfree.expect("non-null function pointer")(eob as *mut libc::c_void) });
        }
        state(data, SMTP_POSTDATA);
        result = smtp_block_statemach(data, conn, 0 as i32 != 0);
    }
    (unsafe { (*smtp).transfer = PPTRANSFER_BODY });
    return result;
}
extern "C" fn smtp_perform(
    mut data: *mut Curl_easy,
    ...
    let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
    if (unsafe { ((*data).set).opt_no_body() }) != 0 {
        (unsafe { (*smtp).transfer = PPTRANSFER_INFO });
    }
    (unsafe { *dophase_done = 0 as i32 != 0 });
    let fresh13 = unsafe { &mut ((*smtp).rcpt) };
    *fresh13 = unsafe { (*data).set.mail_rcpt };
    (unsafe { (*smtp).rcpt_had_ok = 0 as i32 != 0 });
    (unsafe { (*smtp).rcpt_last_error = 0 as i32 });
    (unsafe { (*smtp).trailing_crlf = 1 as i32 != 0 });
    (unsafe { (*smtp).eob = 2 as i32 as size_t });
    if ((unsafe { ((*data).set).upload() }) as i32 != 0 || (unsafe { (*data).set.mimepost.kind }) as u32 != 0)
        && !(unsafe { (*data).set.mail_rcpt }).is_null()
    {
...
extern "C" fn smtp_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    (unsafe { *done = 0 as i32 != 0 });
...
extern "C" fn smtp_disconnect(
    ...
    (unsafe { Curl_pp_disconnect(&mut (*smtpc).pp) });
    (unsafe { Curl_sasl_cleanup(conn, (*smtpc).sasl.authused as u32) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*smtpc).domain as *mut libc::c_void) });
    let fresh14 = unsafe { &mut ((*smtpc).domain) };
    *fresh14 = 0 as *mut i8;
    return CURLE_OK;
}
extern "C" fn smtp_dophase_done(mut data: *mut Curl_easy, mut _connected: bool) -> CURLcode {
    let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
    if (unsafe { (*smtp).transfer }) as u32 != PPTRANSFER_BODY as i32 as u32 {
        (unsafe { Curl_setup_transfer(
            data,
            -(1 as i32),
            -(1 as i32) as curl_off_t,
...
extern "C" fn smtp_doing(mut data: *mut Curl_easy, mut dophase_done: *mut bool) -> CURLcode {
...
extern "C" fn smtp_regular_transfer(
    mut data: *mut Curl_easy,
    mut dophase_done: *mut bool,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
...
extern "C" fn smtp_setup_connection(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let fresh15 = unsafe { &mut ((*conn).bits) };
...
extern "C" fn smtp_parse_url_options(mut conn: *mut connectdata) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut ptr: *const i8 = unsafe { (*conn).options };
    (unsafe { (*smtpc).sasl.resetprefs = 1 as i32 != 0 });
...
extern "C" fn smtp_parse_url_path(mut data: *mut Curl_easy) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut smtpc: *mut smtp_conn = unsafe { &mut (*conn).proto.smtpc };
    let mut path: *const i8 = (unsafe { &mut *((*data).state.up.path).offset(1 as i32 as isize) }) as *mut i8;
    let mut localhost: [i8; 1025] = [0; 1025];
    if (unsafe { *path }) == 0 {
        if (unsafe { Curl_gethostname(
...
extern "C" fn smtp_parse_custom_request(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
...
extern "C" fn smtp_parse_address(
    mut data: *mut Curl_easy,
    mut fqma: *const i8,
    ...
    let mut dup: *mut i8 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
    ...
    length = unsafe { strlen(dup) };
    if length != 0 {
        if (unsafe { *dup.offset(length.wrapping_sub(1 as i32 as u64) as isize) }) as i32 == '>' as i32 {
            (unsafe { *dup.offset(length.wrapping_sub(1 as i32 as u64) as isize) = '\u{0}' as i32 as i8 });
        }
    }
    let fresh16 = unsafe { &mut ((*host).name) };
    *fresh16 = unsafe { strpbrk(dup, b"@\0" as *const u8 as *const i8) };
    if !(unsafe { (*host).name }).is_null() {
        (unsafe { *(*host).name = '\u{0}' as i32 as i8 });
        let fresh17 = unsafe { &mut ((*host).name) };
        *fresh17 = unsafe { ((*host).name).offset(1 as i32 as isize) };
        (unsafe { Curl_idnconvert_hostname(data, host) });
    }
    (unsafe { *address = dup });
    return result;
}
#[no_mangle]
pub extern "C" fn Curl_smtp_escape_eob(mut data: *mut Curl_easy, nread: ssize_t) -> CURLcode {
    let mut i: ssize_t = 0;
    let mut si: ssize_t = 0;
    let mut smtp: *mut SMTP = unsafe { (*data).req.p.smtp };
    let mut scratch: *mut i8 = unsafe { (*data).state.scratch };
    let mut newscratch: *mut i8 = 0 as *mut i8;
    let mut oldscratch: *mut i8 = 0 as *mut i8;
    let mut eob_sent: size_t = 0;
    if scratch.is_null() || (unsafe { ((*data).set).crlf() }) as i32 != 0 {
        oldscratch = scratch;
        newscratch = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
            (2 as i32 as u32).wrapping_mul((*data).set.upload_buffer_size) as size_t,
        ) }) as *mut i8;
        scratch = newscratch;
    ...
    eob_sent = unsafe { (*smtp).eob };
    i = 0 as i32 as ssize_t;
    si = 0 as i32 as ssize_t;
    while i < nread {
        if (*(unsafe { ::std::mem::transmute::<&[u8; 6], &[i8; 6]>(b"\r\n.\r\n\0") }))[(unsafe { (*smtp).eob }) as usize]
            as i32
            == (unsafe { *((*data).req.upload_fromhere).offset(i as isize) }) as i32
        {
            let fresh18 = unsafe { &mut ((*smtp).eob) };
            *fresh18 = (*fresh18).wrapping_add(1);
            if 2 as i32 as u64 == (unsafe { (*smtp).eob }) || 5 as i32 as u64 == (unsafe { (*smtp).eob }) {
                (unsafe { (*smtp).trailing_crlf = 1 as i32 != 0 });
            } else {
                (unsafe { (*smtp).trailing_crlf = 0 as i32 != 0 });
            }
        } else if (unsafe { (*smtp).eob }) != 0 {
            (unsafe { memcpy(
                &mut *scratch.offset(si as isize) as *mut i8 as *mut libc::c_void,
                &*(b"\r\n.\r\n\0" as *const u8 as *const i8).offset(eob_sent as isize) as *const i8
                    as *const libc::c_void,
                ((*smtp).eob).wrapping_sub(eob_sent),
            ) });
            si = (si as u64).wrapping_add((unsafe { (*smtp).eob }).wrapping_sub(eob_sent)) as ssize_t
                as ssize_t;
            if (*(unsafe { ::std::mem::transmute::<&[u8; 6], &[i8; 6]>(b"\r\n.\r\n\0") }))[0 as i32 as usize]
                as i32
                == (unsafe { *((*data).req.upload_fromhere).offset(i as isize) }) as i32
            {
                (unsafe { (*smtp).eob = 1 as i32 as size_t });
            } else {
                (unsafe { (*smtp).eob = 0 as i32 as size_t });
            }
            eob_sent = 0 as i32 as size_t;
            (unsafe { (*smtp).trailing_crlf = 0 as i32 != 0 });
        }
        if 3 as i32 as u64 == (unsafe { (*smtp).eob }) {
            (unsafe { memcpy(
                &mut *scratch.offset(si as isize) as *mut i8 as *mut libc::c_void,
                &*(b"\r\n..\0" as *const u8 as *const i8).offset(eob_sent as isize) as *const i8
                    as *const libc::c_void,
            ...
            si = (si as u64).wrapping_add((4 as i32 as u64).wrapping_sub(eob_sent)) as ssize_t
                as ssize_t;
            (unsafe { (*smtp).eob = 0 as i32 as size_t });
            eob_sent = 0 as i32 as size_t;
        } else if (unsafe { (*smtp).eob }) == 0 {
            let fresh19 = si;
            si = si + 1;
            (unsafe { *scratch.offset(fresh19 as isize) = *((*data).req.upload_fromhere).offset(i as isize) });
        }
        i += 1;
    }
    if (unsafe { (*smtp).eob }).wrapping_sub(eob_sent) != 0 {
        (unsafe { memcpy(
            &mut *scratch.offset(si as isize) as *mut i8 as *mut libc::c_void,
            &*(b"\r\n.\r\n\0" as *const u8 as *const i8).offset(eob_sent as isize) as *const i8
                as *const libc::c_void,
        ...
        si = (si as u64).wrapping_add((unsafe { (*smtp).eob }).wrapping_sub(eob_sent)) as ssize_t as ssize_t;
    }
    if si != nread {
        let fresh20 = unsafe { &mut ((*data).req.upload_fromhere) };
        *fresh20 = scratch;
        let fresh21 = unsafe { &mut ((*data).state.scratch) };
        *fresh21 = scratch;
        (unsafe { Curl_cfree.expect("non-null function pointer")(oldscratch as *mut libc::c_void) });
        (unsafe { (*data).req.upload_present = si });
    } else {
        (unsafe { Curl_cfree.expect("non-null function pointer")(newscratch as *mut libc::c_void) });
    }
    return CURLE_OK;
}
