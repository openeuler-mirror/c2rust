...
extern "C" {
    ...
    fn Curl_dyn_vaddf(s: *mut dynbuf, fmt: *const i8, ap: ::std::ffi::VaList) -> CURLcode;
...
#[no_mangle]
pub extern "C" fn Curl_pp_state_timeout(
    ...
    let mut response_time: timediff_t = if (unsafe { (*data).set.server_response_timeout }) != 0 {
        unsafe { (*data).set.server_response_timeout }
    } else {
        unsafe { (*pp).response_time }
    };
    timeout_ms = response_time - (unsafe { Curl_timediff(Curl_now(), (*pp).response) });
    if (unsafe { (*data).set.timeout }) != 0 && !disconnecting {
        let mut timeout2_ms: timediff_t =
            (unsafe { (*data).set.timeout }) - (unsafe { Curl_timediff(Curl_now(), (*conn).now) });
        timeout_ms = if timeout_ms < timeout2_ms {
            timeout_ms
        } else {
            timeout2_ms
        };
...
#[no_mangle]
pub extern "C" fn Curl_pp_statemach(
    mut data: *mut Curl_easy,
    mut pp: *mut pingpong,
    mut block: bool,
    mut disconnecting: bool,
    ...
    if timeout_ms <= 0 as i32 as i64 {
        (unsafe { Curl_failf(data, b"server response timeout\0" as *const u8 as *const i8) });
    ...
    if unsafe { Curl_ssl_data_pending(conn, 0 as i32) } {
        rc = 1 as i32;
    } else if Curl_pp_moredata(pp) {
        rc = 1 as i32;
    } else if (unsafe { (*pp).sendleft }) == 0 && (unsafe { Curl_ssl_data_pending(conn, 0 as i32) }) as i32 != 0 {
        rc = 1 as i32;
    } else {
    ...
    if rc == -(1 as i32) {
        (unsafe { Curl_failf(data, b"select/poll error\0" as *const u8 as *const i8) });
        result = CURLE_OUT_OF_MEMORY;
    } else if rc != 0 {
        result = unsafe { ((*pp).statemachine).expect("non-null function pointer")(data, (*data).conn) };
    }
    return result;
}
#[no_mangle]
pub extern "C" fn Curl_pp_init(mut data: *mut Curl_easy, mut pp: *mut pingpong) {
    (unsafe { (*pp).nread_resp = 0 as i32 as size_t });
    let fresh0 = unsafe { &mut ((*pp).linestart_resp) };
    *fresh0 = unsafe { (*data).state.buffer };
    (unsafe { (*pp).pending_resp = 1 as i32 != 0 });
    (unsafe { (*pp).response = Curl_now() });
}
#[no_mangle]
pub extern "C" fn Curl_pp_setup(mut pp: *mut pingpong) {
    (unsafe { Curl_dyn_init(&mut (*pp).sendbuf, (64 as i32 * 1024 as i32) as size_t) });
}
#[no_mangle]
pub extern "C" fn Curl_pp_vsendf(
    mut data: *mut Curl_easy,
    mut pp: *mut pingpong,
    mut fmt: *const i8,
    ...
    (unsafe { Curl_dyn_reset(&mut (*pp).sendbuf) });
    result = unsafe { Curl_dyn_vaddf(&mut (*pp).sendbuf, fmt, args.as_va_list()) };
    if result as u64 != 0 {
        return result;
    }
    ...
    write_len = unsafe { Curl_dyn_len(&mut (*pp).sendbuf) };
    s = unsafe { Curl_dyn_ptr(&mut (*pp).sendbuf) };
    ...
    (unsafe { Curl_debug(data, CURLINFO_HEADER_OUT, s, bytes_written as size_t) });
    if bytes_written != write_len as ssize_t {
        let fresh1 = unsafe { &mut ((*pp).sendthis) };
        *fresh1 = s;
        (unsafe { (*pp).sendsize = write_len });
        (unsafe { (*pp).sendleft = write_len.wrapping_sub(bytes_written as u64) });
    } else {
        let fresh2 = unsafe { &mut ((*pp).sendthis) };
        *fresh2 = 0 as *mut i8;
        let fresh3 = unsafe { &mut ((*pp).sendsize) };
        *fresh3 = 0 as i32 as size_t;
        (unsafe { (*pp).sendleft = *fresh3 });
        (unsafe { (*pp).response = Curl_now() });
    }
    return CURLE_OK;
}
...
#[no_mangle]
pub extern "C" fn Curl_pp_readresp(
    mut data: *mut Curl_easy,
    ...
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let buf: *mut i8 = unsafe { (*data).state.buffer };
    let mut result: CURLcode = CURLE_OK;
    (unsafe { *code = 0 as i32 });
    (unsafe { *size = 0 as i32 as size_t });
    ptr = unsafe { buf.offset((*pp).nread_resp as isize) };
    perline = (unsafe { ptr.offset_from((*pp).linestart_resp) }) as i64;
    while (unsafe { (*pp).nread_resp }) < (unsafe { (*data).set.buffer_size }) as size_t
        && (keepon as i32 != 0 && result as u64 == 0)
    {
        if !(unsafe { (*pp).cache }).is_null() {
            if (unsafe { ptr.offset((*pp).cache_size as isize) })
                > (unsafe { buf
                    ...
                    .offset(1 as i32 as isize) })
            {
                (unsafe { Curl_failf(
                    data,
                    b"cached response data too big to handle\0" as *const u8 as *const i8,
                ) });
            ...
            *fresh4 = 0 as *mut i8;
            ...
            result = unsafe { Curl_read(
                ...
                sockfd,
                ptr,
                ((*data).set.buffer_size as u64).wrapping_sub((*pp).nread_resp),
                &mut gotbytes,
            ) };
        ...
        if keepon {
            if gotbytes <= 0 as i32 as i64 {
                ...
                (unsafe { Curl_failf(data, b"response reading failed\0" as *const u8 as *const i8) });
            } else {
                let mut i: ssize_t = 0;
                let mut clipamount: ssize_t = 0 as i32 as ssize_t;
                let mut restart: bool = 0 as i32 != 0;
                let fresh5 = unsafe { &mut ((*data).req.headerbytecount) };
                *fresh5 += gotbytes;
                let fresh6 = unsafe { &mut ((*pp).nread_resp) };
                *fresh6 = (*fresh6 as u64).wrapping_add(gotbytes as u64) as size_t as size_t;
                ...
                while i < gotbytes {
                    ...
                    if (unsafe { *ptr }) as i32 == '\n' as i32 {
                        ...
                        if unsafe { ((*pp).endofresp).expect("non-null function pointer")(
                            data,
                            conn,
                            (*pp).linestart_resp,
                            perline as size_t,
                            ...
                            let mut n: size_t =
                                (unsafe { ptr.offset_from((*pp).linestart_resp) }) as i64 as size_t;
                            (unsafe { memmove(
                                buf as *mut libc::c_void,
                                (*pp).linestart_resp as *const libc::c_void,
                                n,
                            ) });
                            (unsafe { *buf.offset(n as isize) = 0 as i32 as i8 });
                            keepon = 0 as i32 != 0;
                            let fresh7 = unsafe { &mut ((*pp).linestart_resp) };
                            *fresh7 = unsafe { ptr.offset(1 as i32 as isize) };
                            i += 1;
                            (unsafe { *size = (*pp).nread_resp });
                            (unsafe { (*pp).nread_resp = 0 as i32 as size_t });
                            break;
                        } else {
                            perline = 0 as i32 as ssize_t;
                            let fresh8 = unsafe { &mut ((*pp).linestart_resp) };
                            *fresh8 = unsafe { ptr.offset(1 as i32 as isize) };
                        }
                    }
                ...
                if !keepon && i != gotbytes {
                    ...
                    if perline == gotbytes && gotbytes > (unsafe { (*data).set.buffer_size }) / 2 as i32 as i64 {
                        (unsafe { Curl_infof (data , b"Excessive server response line length received, %zd bytes. Stripping\0" as * const u8 as * const i8 , gotbytes ,) }) ;
                        restart = 1 as i32 != 0;
                        clipamount = 40 as i32 as ssize_t;
                    } else if (unsafe { (*pp).nread_resp })
                        > ((unsafe { (*data).set.buffer_size }) as size_t).wrapping_div(2 as i32 as u64)
                    {
                        clipamount = perline;
                        restart = 1 as i32 != 0;
                    }
                ...
                if clipamount != 0 {
                    (unsafe { (*pp).cache_size = clipamount as size_t });
                    let fresh9 = unsafe { &mut ((*pp).cache) };
                    *fresh9 = (unsafe { Curl_cmalloc.expect("non-null function pointer")((*pp).cache_size) })
                        as *mut i8;
                    if !(unsafe { (*pp).cache }).is_null() {
                        (unsafe { memcpy(
                ...
                if restart {
                    (unsafe { (*pp).nread_resp = 0 as i32 as size_t });
                    let fresh10 = unsafe { &mut ((*pp).linestart_resp) };
                    *fresh10 = buf;
                    ptr = *fresh10;
                    perline = 0 as i32 as ssize_t;
                }
...
#[no_mangle]
pub extern "C" fn Curl_pp_getsock(
    mut data: *mut Curl_easy,
    mut pp: *mut pingpong,
...
#[no_mangle]
pub extern "C" fn Curl_pp_flushsend(mut data: *mut Curl_easy, mut pp: *mut pingpong) -> CURLcode {
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut written: ssize_t = 0;
    let mut sock: curl_socket_t = unsafe { (*conn).sock[0 as i32 as usize] };
    let mut result: CURLcode = unsafe { Curl_write(
        data,
        sock,
        ((*pp).sendthis)
    ...
    if written != (unsafe { (*pp).sendleft }) as ssize_t {
        let fresh11 = unsafe { &mut ((*pp).sendleft) };
        *fresh11 = (*fresh11 as u64).wrapping_sub(written as u64) as size_t as size_t;
    } else {
        let fresh12 = unsafe { &mut ((*pp).sendthis) };
        *fresh12 = 0 as *mut i8;
        let fresh13 = unsafe { &mut ((*pp).sendsize) };
        *fresh13 = 0 as i32 as size_t;
        (unsafe { (*pp).sendleft = *fresh13 });
        (unsafe { (*pp).response = Curl_now() });
    }
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_pp_disconnect(mut pp: *mut pingpong) -> CURLcode {
    (unsafe { Curl_dyn_free(&mut (*pp).sendbuf) });
    (unsafe { Curl_cfree.expect("non-null function pointer")((*pp).cache as *mut libc::c_void) });
    let fresh14 = unsafe { &mut ((*pp).cache) };
    *fresh14 = 0 as *mut i8;
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_pp_moredata(mut pp: *mut pingpong) -> bool {
    return if (unsafe { (*pp).sendleft }) == 0 && !(unsafe { (*pp).cache }).is_null() && (unsafe { (*pp).nread_resp }) < (unsafe { (*pp).cache_size })
    {
        1 as i32
    } else {
        0 as i32
