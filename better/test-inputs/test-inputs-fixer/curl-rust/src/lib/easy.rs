...
extern "C" {
    ...
    fn curl_multi_setopt(multi_handle: *mut CURLM, option: CURLMoption, _: ...) -> CURLMcode;
    ...
    fn Curl_resolver_global_init() -> i32;
    ...
    fn Curl_multi_handle(hashsize: i32, chashsize: i32) -> *mut Curl_multi;
...
extern "C" fn sigpipe_ignore(mut data: *mut Curl_easy, mut ig: *mut sigpipe_ignore) {
    ...
    if (unsafe { ((*data).set).no_signal() }) == 0 {
        let mut action: sigaction = sigaction {
            __sigaction_handler: C2RustUnnamed_6 { sa_handler: None },
...
pub static mut Curl_cfree: curl_free_callback = unsafe {
    ::std::mem::transmute::<Option<unsafe extern "C" fn(*mut libc::c_void) -> ()>, curl_free_callback>(
...
pub static mut Curl_cstrdup: curl_strdup_callback = unsafe {
    ::std::mem::transmute::<Option<unsafe extern "C" fn(*const i8) -> *mut i8>, curl_strdup_callback>(
...
pub static mut Curl_ccalloc: curl_calloc_callback = unsafe {
    ::std::mem::transmute::<
        Option<unsafe extern "C" fn(u64, u64) -> *mut libc::c_void>,
        curl_calloc_callback,
    >(Some(
        calloc as unsafe extern "C" fn(u64, u64) -> *mut libc::c_void,
    ))
};
extern "C" fn global_init(mut flags: i64, mut memoryfuncs: bool) -> CURLcode {
    let fresh0 = unsafe { initialized };
    (unsafe { initialized = initialized.wrapping_add(1) });
    if fresh0 != 0 {
        return CURLE_OK;
    }
    if memoryfuncs {
        (unsafe { Curl_cmalloc = ::std::mem::transmute::<
            Option<unsafe extern "C" fn(u64) -> *mut libc::c_void>,
            curl_malloc_callback,
        >(Some(
            malloc as unsafe extern "C" fn(u64) -> *mut libc::c_void,
        )) });
        (unsafe { Curl_cfree = ::std::mem::transmute::<
            Option<unsafe extern "C" fn(*mut libc::c_void) -> ()>,
            curl_free_callback,
        >(Some(free as unsafe extern "C" fn(*mut libc::c_void) -> ())) });
        (unsafe { Curl_crealloc = ::std::mem::transmute::<
            Option<unsafe extern "C" fn(*mut libc::c_void, u64) -> *mut libc::c_void>,
            curl_realloc_callback,
        >(Some(
            realloc as unsafe extern "C" fn(*mut libc::c_void, u64) -> *mut libc::c_void,
        )) });
        (unsafe { Curl_cstrdup = ::std::mem::transmute::<
            Option<unsafe extern "C" fn(*const i8) -> *mut i8>,
            curl_strdup_callback,
        >(Some(strdup as unsafe extern "C" fn(*const i8) -> *mut i8)) });
        (unsafe { Curl_ccalloc = ::std::mem::transmute::<
            Option<unsafe extern "C" fn(u64, u64) -> *mut libc::c_void>,
            curl_calloc_callback,
        >(Some(
            calloc as unsafe extern "C" fn(u64, u64) -> *mut libc::c_void,
    ...
    if !((unsafe { Curl_ssl_init() }) == 0) {
        if !((unsafe { Curl_resolver_global_init() }) != 0) {
            (unsafe { init_flags = flags });
            return CURLE_OK;
        }
    }
    (unsafe { initialized = initialized.wrapping_sub(1) });
    return CURLE_FAILED_INIT;
}
#[no_mangle]
pub extern "C" fn curl_global_init(mut flags: i64) -> CURLcode {
...
pub extern "C" fn curl_global_init_mem(
    ...
    (unsafe { Curl_cmalloc = m });
    (unsafe { Curl_cfree = f });
    (unsafe { Curl_cstrdup = s });
    (unsafe { Curl_crealloc = r });
    (unsafe { Curl_ccalloc = c });
...
pub extern "C" fn curl_global_cleanup() {
    ...
    (unsafe { Curl_ssl_cleanup() });
    (unsafe { Curl_resolver_global_cleanup() });
    (unsafe { init_flags = 0 as i32 as i64 });
}
#[no_mangle]
pub extern "C" fn curl_easy_init() -> *mut CURL {
    ...
    if (unsafe { initialized }) == 0 {
        result = curl_global_init(((1 as i32) << 0 as i32 | (1 as i32) << 1 as i32) as i64);
        if result as u64 != 0 {
            return 0 as *mut CURL;
        }
    }
    result = unsafe { Curl_open(&mut data) };
    if result as u64 != 0 {
        return 0 as *mut CURL;
    }
    return data;
}
extern "C" fn easy_transfer(mut multi: *mut Curl_multi) -> CURLcode {
    ...
    while !done && mcode as u64 == 0 {
        ...
        if mcode as u64 == 0 && still_running == 0 {
            let mut rc: i32 = 0;
            let mut msg: *mut CURLMsg = unsafe { curl_multi_info_read(multi, &mut rc) };
            if !msg.is_null() {
...
extern "C" fn easy_perform(mut data: *mut Curl_easy, mut events: bool) -> CURLcode {
    ...
    if !(unsafe { (*data).multi }).is_null() {
        (unsafe { Curl_failf(
            data,
            b"easy handle already used in multi handle\0" as *const u8 as *const i8,
        ) });
        return CURLE_FAILED_INIT;
    }
    if !(unsafe { (*data).multi_easy }).is_null() {
        multi = unsafe { (*data).multi_easy };
    } else {
        multi = unsafe { Curl_multi_handle(1 as i32, 3 as i32) };
        if multi.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        let fresh1 = unsafe { &mut ((*data).multi_easy) };
        *fresh1 = multi;
    }
    if unsafe { (*multi).in_callback } {
        return CURLE_RECURSIVE_API_CALL;
    }
    (unsafe { curl_multi_setopt(multi, CURLMOPT_MAXCONNECTS, (*data).set.maxconnects) });
    mcode = unsafe { curl_multi_add_handle(multi, data) };
    if mcode as u64 != 0 {
        (unsafe { curl_multi_cleanup(multi) });
        let fresh2 = unsafe { &mut ((*data).multi_easy) };
    ...
    (unsafe { curl_multi_remove_handle(multi, data) });
    sigpipe_restore(&mut pipe_st);
    return result;
}
#[no_mangle]
pub extern "C" fn curl_easy_perform(mut data: *mut Curl_easy) -> CURLcode {
    return easy_perform(data, 0 as i32 != 0);
}
#[no_mangle]
pub extern "C" fn curl_easy_cleanup(mut data: *mut Curl_easy) {
    let mut pipe_st: sigpipe_ignore = sigpipe_ignore {
        old_pipe_act: sigaction {
            __sigaction_handler: C2RustUnnamed_6 { sa_handler: None },
            sa_mask: __sigset_t { __val: [0; 16] },
...
extern "C" fn dupset(mut dst: *mut Curl_easy, mut src: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut i: dupstring = STRING_CERT;
    let mut j: dupblob = BLOB_CERT;
    (unsafe { (*dst).set = (*src).set });
    (unsafe { Curl_mime_initpart(&mut (*dst).set.mimepost, dst) });
    (unsafe { memset(
        ((*dst).set.str_0).as_mut_ptr() as *mut libc::c_void,
        0 as i32,
        (STRING_LAST as i32 as u64).wrapping_mul(::std::mem::size_of::<*mut i8>() as u64),
    ) });
    i = STRING_CERT;
    ...
    (unsafe { memset(
        ...
        0 as i32,
        (BLOB_LAST as i32 as u64).wrapping_mul(::std::mem::size_of::<*mut curl_blob>() as u64),
    ) });
    j = BLOB_CERT;
    while (j as u32) < BLOB_LAST as i32 as u32 {
        result = unsafe { Curl_setblobopt(
    ...
    if (unsafe { (*src).set.postfieldsize }) != 0 && !(unsafe { (*src).set.str_0[i as usize] }).is_null() {
        let fresh3 = unsafe { &mut ((*dst).set.str_0[i as usize]) };
        *fresh3 = (unsafe { Curl_memdup(
            (*src).set.str_0[i as usize] as *const libc::c_void,
            curlx_sotouz((*src).set.postfieldsize),
        ) }) as *mut i8;
        if (unsafe { (*dst).set.str_0[i as usize] }).is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        let fresh4 = unsafe { &mut ((*dst).set.postfields) };
        *fresh4 = (unsafe { (*dst).set.str_0[i as usize] }) as *mut libc::c_void;
    }
    result = unsafe { Curl_mime_duppart(&mut (*dst).set.mimepost, &mut (*src).set.mimepost) };
    if !(unsafe { (*src).set.resolve }).is_null() {
        let fresh5 = unsafe { &mut ((*dst).state.resolve) };
        *fresh5 = unsafe { (*dst).set.resolve };
    }
    return result;
}
#[no_mangle]
pub extern "C" fn curl_easy_duphandle(mut data: *mut Curl_easy) -> *mut CURL {
    let mut current_block: u64;
    let mut outcurl: *mut Curl_easy = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<Curl_easy>() as u64,
    ) }) as *mut Curl_easy;
    if !outcurl.is_null() {
        (unsafe { (*outcurl).set.buffer_size = (*data).set.buffer_size });
        if !(dupset(outcurl, data) as u64 != 0) {
            (unsafe { Curl_dyn_init(
                &mut (*outcurl).state.headerb,
                (100 as i32 * 1024 as i32) as size_t,
            ) });
            let fresh6 = unsafe { &mut ((*outcurl).state.conn_cache) };
            *fresh6 = 0 as *mut conncache;
            (unsafe { (*outcurl).state.lastconnect_id = -(1 as i32) as i64 });
            (unsafe { (*outcurl).progress.flags = (*data).progress.flags });
            let fresh7 = unsafe { &mut ((*outcurl).progress) };
            (*fresh7).set_callback(unsafe { ((*data).progress).callback() });
            if !(unsafe { (*data).cookies }).is_null() {
                let fresh8 = unsafe { &mut ((*outcurl).cookies) };
                *fresh8 = unsafe { Curl_cookie_init(
                    data,
                    (*(*data).cookies).filename,
                    (*outcurl).cookies,
                    ((*data).set).cookiesession() != 0,
                ) };
                if (unsafe { (*outcurl).cookies }).is_null() {
            ...
            match current_block {
                ...
                _ => {
                    if !(unsafe { (*data).state.cookielist }).is_null() {
                        let fresh9 = unsafe { &mut ((*outcurl).state.cookielist) };
                        *fresh9 = unsafe { Curl_slist_duplicate((*data).state.cookielist) };
                        if (unsafe { (*outcurl).state.cookielist }).is_null() {
                            current_block = 12715656406826180841;
                        } else {
                    ...
                    match current_block {
                        ...
                        _ => {
                            if !(unsafe { (*data).state.url }).is_null() {
                                let fresh10 = unsafe { &mut ((*outcurl).state.url) };
                                *fresh10 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                                    (*data).state.url,
                                ) };
                                if (unsafe { (*outcurl).state.url }).is_null() {
                                    current_block = 12715656406826180841;
                                } else {
                                    let fresh11 = unsafe { &mut ((*outcurl).state) };
                            ...
                            match current_block {
                                ...
                                _ => {
                                    if !(unsafe { (*data).state.referer }).is_null() {
                                        let fresh12 = unsafe { &mut ((*outcurl).state.referer) };
                                        *fresh12 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                                            (*data).state.referer,
                                        ) };
                                        if (unsafe { (*outcurl).state.referer }).is_null() {
                                            current_block = 12715656406826180841;
                                        } else {
                                            let fresh13 = unsafe { &mut ((*outcurl).state) };
                                    ...
                                    match current_block {
                                        12715656406826180841 => {}
                                        _ => {
                                            if !(unsafe { (*outcurl).set.str_0
                                                [STRING_SSL_ENGINE as i32 as usize] })
                                                .is_null()
                                            {
                                                if (unsafe { Curl_ssl_set_engine(
                                                    outcurl,
                                                    (*outcurl).set.str_0
                                                        [STRING_SSL_ENGINE as i32 as usize],
                                                ) })
                                            ...
                                            match current_block {
                                                ...
                                                _ => {
                                                    if !(unsafe { (*data).hsts }).is_null() {
                                                        let fresh14 = unsafe { &mut ((*outcurl).hsts) };
                                                        *fresh14 = unsafe { Curl_hsts_init() };
                                                        if (unsafe { (*outcurl).hsts }).is_null() {
                                                            current_block = 12715656406826180841;
                                                        } else {
                                                            if !(unsafe { (*outcurl).set.str_0
                                                                [STRING_HSTS as i32 as usize] })
                                                                .is_null()
                                                            {
                                                                (unsafe { Curl_hsts_loadfile(
                                                                    outcurl,
                                                                    (*outcurl).hsts,
                                                                    (*outcurl).set.str_0[STRING_HSTS
                                                                        as i32
                                                                        as usize],
                                                                ) });
                                                            }
                                                            (unsafe { Curl_hsts_loadcb(
                                                                outcurl,
                                                                (*outcurl).hsts,
                                                            ) });
                                                    ...
                                                    match current_block {
                                                        12715656406826180841 => {}
                                                        _ => {
                                                            if !((unsafe { Curl_resolver_duphandle(
                                                                outcurl,
                                                                &mut (*outcurl)
                                                                    .state
                                                                    .async_0
                                                                ...
                                                                != 0)
                                                            {
                                                                (unsafe { Curl_initinfo(outcurl) });
                                                                (unsafe { (*outcurl).magic =
                                                                    0xc0dedbad as u32 });
                                                                return outcurl;
                                                            }
                                                        }
    ...
    if !outcurl.is_null() {
        (unsafe { curl_slist_free_all((*outcurl).state.cookielist) });
        let fresh15 = unsafe { &mut ((*outcurl).state.cookielist) };
        *fresh15 = 0 as *mut curl_slist;
        (unsafe { Curl_cfree.expect("non-null function pointer")(
            (*outcurl).state.buffer as *mut libc::c_void,
        ) });
        let fresh16 = unsafe { &mut ((*outcurl).state.buffer) };
        *fresh16 = 0 as *mut i8;
        (unsafe { Curl_dyn_free(&mut (*outcurl).state.headerb) });
        (unsafe { Curl_cfree.expect("non-null function pointer")((*outcurl).state.url as *mut libc::c_void) });
        let fresh17 = unsafe { &mut ((*outcurl).state.url) };
        *fresh17 = 0 as *mut i8;
        (unsafe { Curl_cfree.expect("non-null function pointer")(
            (*outcurl).state.referer as *mut libc::c_void,
        ) });
        let fresh18 = unsafe { &mut ((*outcurl).state.referer) };
        *fresh18 = 0 as *mut i8;
        (unsafe { Curl_altsvc_cleanup(&mut (*outcurl).asi) });
        (unsafe { Curl_hsts_cleanup(&mut (*outcurl).hsts) });
        (unsafe { Curl_freeset(outcurl) });
        (unsafe { Curl_cfree.expect("non-null function pointer")(outcurl as *mut libc::c_void) });
    }
    return 0 as *mut CURL;
}
#[no_mangle]
pub extern "C" fn curl_easy_reset(mut data: *mut Curl_easy) {
    (unsafe { Curl_free_request_state(data) });
    (unsafe { Curl_freeset(data) });
    (unsafe { memset(
        &mut (*data).set as *mut UserDefined as *mut libc::c_void,
        0 as i32,
        ::std::mem::size_of::<UserDefined>() as u64,
    ) });
    (unsafe { Curl_init_userdefined(data) });
    (unsafe { memset(
...
#[no_mangle]
pub extern "C" fn curl_easy_pause(mut data: *mut Curl_easy, mut action: i32) -> CURLcode {
    let mut k: *mut SingleRequest = 0 as *mut SingleRequest;
    let mut result: CURLcode = CURLE_OK;
    let mut oldstate: i32 = 0;
    let mut newstate: i32 = 0;
    if !(!data.is_null() && (unsafe { (*data).magic }) == 0xc0dedbad as u32) || (unsafe { (*data).conn }).is_null() {
        return CURLE_BAD_FUNCTION_ARGUMENT;
    }
    k = unsafe { &mut (*data).req };
    oldstate = (unsafe { (*k).keepon }) & ((1 as i32) << 4 as i32 | (1 as i32) << 5 as i32);
    newstate = (unsafe { (*k).keepon }) & !((1 as i32) << 4 as i32 | (1 as i32) << 5 as i32)
    ...
    if newstate & ((1 as i32) << 4 as i32 | (1 as i32) << 5 as i32) == oldstate {
        return CURLE_OK;
    }
    if (unsafe { (*k).keepon }) & !newstate & (1 as i32) << 5 as i32 != 0
        && ((unsafe { (*data).mstate }) as u32 == MSTATE_PERFORMING as i32 as u32
            || (unsafe { (*data).mstate }) as u32 == MSTATE_RATELIMITING as i32 as u32)
    ...
    {
        (unsafe { Curl_mime_unpause((*data).state.in_0 as *mut curl_mimepart) });
    }
    (unsafe { (*k).keepon = newstate });
    if newstate & (1 as i32) << 4 as i32 == 0 {
        (unsafe { Curl_http2_stream_pause(data, 0 as i32 != 0) });
        if (unsafe { (*data).state.tempcount }) != 0 {
            let mut i: u32 = 0;
            let mut count: u32 = unsafe { (*data).state.tempcount };
            ...
            while i < (unsafe { (*data).state.tempcount }) {
                writebuf[i as usize] = unsafe { (*data).state.tempwrite[i as usize] };
                (unsafe { Curl_dyn_init(
            ...
            while i < count {
                ...
                (unsafe { Curl_dyn_free(&mut (*writebuf.as_mut_ptr().offset(i as isize)).b) });
                i = i.wrapping_add(1);
            }
            if result as u64 != 0 {
                return result;
    ...
    if newstate & ((1 as i32) << 4 as i32 | (1 as i32) << 5 as i32)
        != (1 as i32) << 4 as i32 | (1 as i32) << 5 as i32
    {
        (unsafe { Curl_expire(data, 0 as i32 as timediff_t, EXPIRE_RUN_NOW) });
        (unsafe { (*data).state.keeps_speed.tv_sec = 0 as i32 as time_t });
        if (unsafe { (*data).state.tempcount }) == 0 {
            (unsafe { (*(*data).conn).cselect_bits = 0x1 as i32 | 0x2 as i32 });
        }
        if !(unsafe { (*data).multi }).is_null() {
            (unsafe { Curl_update_timer((*data).multi) });
        }
    }
    if (unsafe { ((*data).state).done() }) == 0 {
        (unsafe { Curl_updatesocket(data) });
    }
    return result;
}
extern "C" fn easy_connection(
    mut data: *mut Curl_easy,
    ...
    (unsafe { *sfd = Curl_getconnectinfo(data, connp) });
    if (unsafe { *sfd }) == -(1 as i32) {
        (unsafe { Curl_failf(
            data,
...
#[no_mangle]
pub extern "C" fn curl_easy_recv(
    mut data: *mut Curl_easy,
    ...
    if unsafe { Curl_is_in_callback(data) } {
        return CURLE_RECURSIVE_API_CALL;
    }
    ...
    if (unsafe { (*data).conn }).is_null() {
        (unsafe { Curl_attach_connnection(data, c) });
    }
    (unsafe { *n = 0 as i32 as size_t });
    result = unsafe { Curl_read(data, sfd, buffer as *mut i8, buflen, &mut n1) };
    if result as u64 != 0 {
        return result;
    }
    (unsafe { *n = n1 as size_t });
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn curl_easy_send(
    mut data: *mut Curl_easy,
    ...
    (unsafe { *n = 0 as i32 as size_t });
    sigpipe_ignore(data, &mut pipe_st);
    result = unsafe { Curl_write(data, sfd, buffer, buflen, &mut n1) };
    sigpipe_restore(&mut pipe_st);
...
extern "C" fn conn_upkeep(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut _param: *mut libc::c_void,
) -> i32 {
    if unsafe { ((*(*conn).handler).connection_check).is_some() } {
        (unsafe { Curl_attach_connnection(data, conn) });
        (unsafe { ((*(*conn).handler).connection_check).expect("non-null function pointer")(
            data,
            conn,
...
extern "C" fn upkeep(mut conn_cache: *mut conncache, mut data: *mut libc::c_void) -> CURLcode {
    (unsafe { Curl_conncache_foreach(
        ...
        Some(
            conn_upkeep
                as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata, *mut libc::c_void) -> i32,
        ),
    ) });
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn curl_easy_upkeep(mut data: *mut Curl_easy) -> CURLcode {
