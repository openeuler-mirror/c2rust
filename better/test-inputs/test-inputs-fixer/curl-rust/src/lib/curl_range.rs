...
#[no_mangle]
pub extern "C" fn Curl_range(mut data: *mut Curl_easy) -> CURLcode {
    let mut from: curl_off_t = 0;
    let mut to: curl_off_t = 0;
    let mut ptr: *mut i8 = 0 as *mut i8;
    let mut ptr2: *mut i8 = 0 as *mut i8;
    if (unsafe { ((*data).state).use_range() }) as i32 != 0 && !(unsafe { (*data).state.range }).is_null() {
        let mut from_t: CURLofft = CURL_OFFT_OK;
        let mut to_t: CURLofft = CURL_OFFT_OK;
        from_t = unsafe { curlx_strtoofft((*data).state.range, &mut ptr, 0 as i32, &mut from) };
        if from_t as u32 == CURL_OFFT_FLOW as i32 as u32 {
            return CURLE_RANGE_ERROR;
        }
        while (unsafe { *ptr }) as i32 != 0
            && ((unsafe { Curl_isspace(*ptr as u8 as i32) }) != 0 || (unsafe { *ptr }) as i32 == '-' as i32)
        {
            ptr = unsafe { ptr.offset(1) };
        }
        to_t = unsafe { curlx_strtoofft(ptr, &mut ptr2, 0 as i32, &mut to) };
        if to_t as u32 == CURL_OFFT_FLOW as i32 as u32 {
            return CURLE_RANGE_ERROR;
        }
        if to_t as u32 == CURL_OFFT_INVAL as i32 as u32 && from_t as u64 == 0 {
            (unsafe { (*data).state.resume_from = from });
        } else if from_t as u32 == CURL_OFFT_INVAL as i32 as u32 && to_t as u64 == 0 {
            (unsafe { (*data).req.maxdownload = to });
            (unsafe { (*data).state.resume_from = -to });
        } else {
            ...
            (unsafe { (*data).req.maxdownload = totalsize + 1 as i32 as i64 });
            (unsafe { (*data).state.resume_from = from });
        }
    } else {
