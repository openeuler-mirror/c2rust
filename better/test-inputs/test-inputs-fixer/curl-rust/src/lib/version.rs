...
extern "C" {
    ...
    fn Curl_ssl_version(buffer: *mut i8, size: size_t);
    fn Curl_http2_ver(p: *mut i8, len: size_t);
...
#[no_mangle]
pub extern "C" fn curl_version() -> *mut i8 {
    ...
    if (unsafe { ldap_get_option(
        0 as *mut LDAP,
        0 as i32,
        &mut api as *mut LDAPAPIInfo as *mut libc::c_void,
    ) }) == 0 as i32
    {
        let mut patch: u32 = (api.ldapai_vendor_version % 100 as i32) as u32;
        let mut major: u32 = (api.ldapai_vendor_version / 10000 as i32) as u32;
        ...
        (unsafe { ldap_memfree(api.ldapai_vendor_name as *mut libc::c_void) });
        (unsafe { ber_memvfree(api.ldapai_extensions as *mut *mut libc::c_void) });
    }
    outp = (unsafe { &mut *out.as_mut_ptr().offset(0 as i32 as isize) }) as *mut i8;
    outlen = ::std::mem::size_of::<[i8; 300]>() as u64;
    j = 0 as i32;
    while j < i {
        let mut n: size_t = unsafe { strlen(src[j as usize]) };
        ...
        (unsafe { memcpy(
            ...
            src[j as usize] as *const libc::c_void,
            n,
        ) });
        outp = unsafe { outp.offset(n as isize) };
        outlen = (outlen as u64).wrapping_sub(n) as size_t as size_t;
        j += 1;
    }
    (unsafe { *outp = 0 as i32 as i8 });
    return unsafe { out.as_mut_ptr() };
}
...
static mut version_info: curl_version_info_data = unsafe {
    {
        let mut init = curl_version_info_data {
            ...
            cainfo: b"/etc/ssl/certs/ca-certificates.crt\0" as *const u8 as *const i8,
...
#[no_mangle]
pub extern "C" fn curl_version_info(mut _stamp: CURLversion) -> *mut curl_version_info_data {
    static mut ssl_buffer: [i8; 80] = [0; 80];
    (unsafe { Curl_ssl_version(
        ssl_buffer.as_mut_ptr(),
        ::std::mem::size_of::<[i8; 80]>() as u64,
    ) });
    (unsafe { version_info.ssl_version = ssl_buffer.as_mut_ptr() });
    if (unsafe { (*Curl_ssl).supports }) & ((1 as i32) << 4 as i32) as u32 != 0 {
        (unsafe { version_info.features |= (1 as i32) << 21 as i32 });
    } else {
        (unsafe { version_info.features &= !((1 as i32) << 21 as i32) });
    }
    (unsafe { version_info.libz_version = zlibVersion() });
    let mut h2: *mut nghttp2_info = unsafe { nghttp2_version(0 as i32) };
    (unsafe { version_info.nghttp2_ver_num = (*h2).version_num as u32 });
    (unsafe { version_info.nghttp2_version = (*h2).version_str });
    (unsafe { version_info.gsasl_version = gsasl_check_version(0 as *const i8) });
    return unsafe { &mut version_info };
}
