...
#[no_mangle]
pub static mut Curl_handler_file: Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_it: Some(file_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: Some(
                file_done as unsafe extern "C" fn(*mut Curl_easy, CURLcode, bool) -> CURLcode,
            ),
            do_more: None,
            connect_it: Some(
                file_connect as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            connecting: None,
            doing: None,
            proto_getsock: None,
            ...
            flags: ((1 as i32) << 4 as i32 | (1 as i32) << 6 as i32) as u32,
        };
        init
    }
};
extern "C" fn file_setup_connection(
    mut data: *mut Curl_easy,
    mut _conn: *mut connectdata,
) -> CURLcode {
    let fresh0 = unsafe { &mut ((*data).req.p.file) };
    *fresh0 = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<FILEPROTO>() as u64,
    ) }) as *mut FILEPROTO;
    if (unsafe { (*data).req.p.file }).is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    return CURLE_OK;
}
extern "C" fn file_connect(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut real_path: *mut i8 = 0 as *mut i8;
    let mut file: *mut FILEPROTO = unsafe { (*data).req.p.file };
    let mut fd: i32 = 0;
    ...
    if !(unsafe { memchr(real_path as *const libc::c_void, 0 as i32, real_path_len) }).is_null() {
        (unsafe { Curl_cfree.expect("non-null function pointer")(real_path as *mut libc::c_void) });
        real_path = 0 as *mut i8;
        return CURLE_URL_MALFORMAT;
    }
    fd = unsafe { open(real_path, 0 as i32) };
    let fresh1 = unsafe { &mut ((*file).path) };
    *fresh1 = real_path;
    let fresh2 = unsafe { &mut ((*file).freepath) };
    *fresh2 = real_path;
    (unsafe { (*file).fd = fd });
    if (unsafe { ((*data).set).upload() }) == 0 && fd == -(1 as i32) {
        (unsafe { Curl_failf(
            data,
...
extern "C" fn file_done(
    ...
    let mut file: *mut FILEPROTO = unsafe { (*data).req.p.file };
    if !file.is_null() {
        (unsafe { Curl_cfree.expect("non-null function pointer")((*file).freepath as *mut libc::c_void) });
        let fresh3 = unsafe { &mut ((*file).freepath) };
        *fresh3 = 0 as *mut i8;
        let fresh4 = unsafe { &mut ((*file).path) };
        *fresh4 = 0 as *mut i8;
        if (unsafe { (*file).fd }) != -(1 as i32) {
            (unsafe { close((*file).fd) });
        }
        (unsafe { (*file).fd = -(1 as i32) });
    }
    return CURLE_OK;
}
extern "C" fn file_disconnect(
    mut data: *mut Curl_easy,
    mut _conn: *mut connectdata,
...
extern "C" fn file_upload(mut data: *mut Curl_easy) -> CURLcode {
    let mut file: *mut FILEPROTO = unsafe { (*data).req.p.file };
    let mut dir: *const i8 = unsafe { strchr((*file).path, '/' as i32) };
    let mut fd: i32 = 0;
    let mut mode: i32 = 0;
    ...
    let fresh5 = unsafe { &mut ((*data).req.upload_fromhere) };
    *fresh5 = buf;
    if dir.is_null() {
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    if (unsafe { *dir.offset(1 as i32 as isize) }) == 0 {
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    ...
    fd = unsafe { open((*file).path, mode, (*data).set.new_file_perms) };
    if fd < 0 as i32 {
        (unsafe { Curl_failf(
            data,
    ...
    if (unsafe { (*data).state.resume_from }) < 0 as i32 as i64 {
        ...
        (unsafe { (*data).state.resume_from = file_stat.st_size });
    }
    while result as u64 == 0 {
        let mut nread: size_t = 0;
        let mut nwrite: size_t = 0;
        let mut readcount: size_t = 0;
        result = unsafe { Curl_fillreadbuffer(data, (*data).set.buffer_size as size_t, &mut readcount) };
        if result as u64 != 0 {
            break;
        }
        if readcount == 0 {
        ...
        nread = readcount;
        if (unsafe { (*data).state.resume_from }) != 0 {
            if nread as curl_off_t <= (unsafe { (*data).state.resume_from }) {
                let fresh6 = unsafe { &mut ((*data).state.resume_from) };
                *fresh6 = (*fresh6 as u64).wrapping_sub(nread) as curl_off_t as curl_off_t;
                nread = 0 as i32 as size_t;
                buf2 = buf;
            } else {
                buf2 = unsafe { buf.offset((*data).state.resume_from as isize) };
                nread = (nread as u64).wrapping_sub((unsafe { (*data).state.resume_from }) as size_t) as size_t
                    as size_t;
        ...
        nwrite = (unsafe { write(fd, buf2 as *const libc::c_void, nread) }) as size_t;
        if nwrite != nread {
            result = CURLE_SEND_ERROR;
            break;
        } else {
            bytecount = (bytecount as u64).wrapping_add(nread) as curl_off_t as curl_off_t;
            (unsafe { Curl_pgrsSetUploadCounter(data, bytecount) });
    ...
    if result as u64 == 0 && (unsafe { Curl_pgrsUpdate(data) }) != 0 {
        result = CURLE_ABORTED_BY_CALLBACK;
    }
    (unsafe { close(fd) });
    return result;
}
extern "C" fn file_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    ...
    file = unsafe { (*data).req.p.file };
    fd = unsafe { (*file).fd };
    if -(1 as i32) != fstat(fd, &mut statbuf) {
        if !(statbuf.st_mode & 0o170000 as i32 as u32 == 0o40000 as i32 as u32) {
            expected_size = statbuf.st_size;
        }
        (unsafe { (*data).info.filetime = statbuf.st_mtim.tv_sec });
        fstated = 1 as i32 != 0;
    }
    if fstated as i32 != 0
        && (unsafe { (*data).state.range }).is_null()
        && (unsafe { (*data).set.timecondition }) as u32 != 0
    {
        if !(unsafe { Curl_meets_timecondition(data, (*data).info.filetime) }) {
            (unsafe { *done = 1 as i32 != 0 });
            return CURLE_OK;
        }
    }
    if fstated {
        ...
        let mut accept_ranges: [i8; 24] =
            *(unsafe { ::std::mem::transmute::<&[u8; 24], &mut [i8; 24]>(b"Accept-ranges: bytes\r\n\0\0") });
        if expected_size >= 0 as i32 as i64 {
            headerlen = unsafe { curl_msnprintf(
                header.as_mut_ptr(),
        ...
        (unsafe { Curl_pgrsSetDownloadSize(data, expected_size) });
    ...
    if (unsafe { (*data).state.resume_from }) < 0 as i32 as i64 {
        ...
        let fresh7 = unsafe { &mut ((*data).state.resume_from) };
        *fresh7 += statbuf.st_size;
    }
    if (unsafe { (*data).state.resume_from }) > 0 as i32 as i64 {
        if (unsafe { (*data).state.resume_from }) <= expected_size {
            expected_size -= unsafe { (*data).state.resume_from };
        } else {
            (unsafe { Curl_failf(
                data,
                b"failed to resume file:// transfer\0" as *const u8 as *const i8,
            ) });
            return CURLE_BAD_DOWNLOAD_RESUME;
        }
    }
    if (unsafe { (*data).req.maxdownload }) > 0 as i32 as i64 {
        expected_size = unsafe { (*data).req.maxdownload };
    }
    ...
    if (unsafe { (*data).state.resume_from }) != 0 {
        if (unsafe { (*data).state.resume_from }) != (unsafe { lseek(fd, (*data).state.resume_from, 0 as i32) }) {
            return CURLE_BAD_DOWNLOAD_RESUME;
        }
    }
    ...
    while result as u64 == 0 {
        ...
        if size_known {
            bytestoread = if expected_size < (unsafe { (*data).set.buffer_size }) {
                unsafe { curlx_sotouz(expected_size) }
            } else {
                (unsafe { (*data).set.buffer_size }) as size_t
            };
        } else {
            bytestoread = ((unsafe { (*data).set.buffer_size }) - 1 as i32 as i64) as size_t;
        }
        nread = unsafe { read(fd, buf as *mut libc::c_void, bytestoread) };
        if nread > 0 as i32 as i64 {
            (unsafe { *buf.offset(nread as isize) = 0 as i32 as i8 });
        }
        if nread <= 0 as i32 as i64 || size_known as i32 != 0 && expected_size == 0 as i32 as i64 {
            break;
        }
        bytecount += nread;
        if size_known {
        ...
        result = unsafe { Curl_client_write(data, (1 as i32) << 0 as i32, buf, nread as size_t) };
        if result as u64 != 0 {
            return result;
        }
        (unsafe { Curl_pgrsSetDownloadCounter(data, bytecount) });
        if (unsafe { Curl_pgrsUpdate(data) }) != 0 {
            result = CURLE_ABORTED_BY_CALLBACK;
        } else {
            result = unsafe { Curl_speedcheck(data, Curl_now()) };
