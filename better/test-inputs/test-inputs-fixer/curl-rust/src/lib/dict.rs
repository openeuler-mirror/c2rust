...
#[no_mangle]
pub static mut Curl_handler_dict: Curl_handler =  {
    {
        let mut init = Curl_handler {
            scheme: b"DICT\0" as *const u8 as *const i8,
            setup_connection: None,
            do_it: Some(dict_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode),
            done: None,
            do_more: None,
            connect_it: None,
            connecting: None,
...
extern "C" fn unescape_word(mut data: *mut Curl_easy, mut inputbuff: *const i8) -> *mut i8 {
    ...
    dictp = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
    ...
    if !dictp.is_null() {
        ...
        loop {
                ...
                || ch as i32 == '\\' as i32
            {
                let fresh0 = olen;
                olen = olen + 1;
                (unsafe { *dictp.offset(fresh0 as isize) = '\\' as i32 as i8 });
            }
            let fresh1 = olen;
            olen = olen + 1;
            (unsafe { *dictp.offset(fresh1 as isize) = ch });
            ptr = unsafe { ptr.offset(1) };
        }
        (unsafe { *dictp.offset(olen as isize) = 0 as i32 as i8 });
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")(newp as *mut libc::c_void) });
    return dictp;
}
unsafe extern "C" fn sendf(
    ...
    loop {
        ...
        write_len = (write_len as u64).wrapping_sub(bytes_written as u64) as size_t as size_t;
        sptr = sptr.offset(bytes_written as isize);
    }
    Curl_cfree.expect("non-null function pointer")(s as *mut libc::c_void);
    return result;
}
extern "C" fn dict_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    ...
    let _ = (unsafe { ((*conn).bits).user_passwd() }) != 0;
    if (unsafe { Curl_strncasecompare(
        path,
        b"/MATCH:\0" as *const u8 as *const i8,
        (::std::mem::size_of::<[i8; 8]>() as u64).wrapping_sub(1 as i32 as u64),
    ) }) != 0
        || (unsafe { Curl_strncasecompare(
            path,
            b"/M:\0" as *const u8 as *const i8,
            (::std::mem::size_of::<[i8; 4]>() as u64).wrapping_sub(1 as i32 as u64),
        ) }) != 0
        || (unsafe { Curl_strncasecompare(
            path,
            b"/FIND:\0" as *const u8 as *const i8,
            (::std::mem::size_of::<[i8; 7]>() as u64).wrapping_sub(1 as i32 as u64),
        ) }) != 0
    {
        word = unsafe { strchr(path, ':' as i32) };
        if !word.is_null() {
            word = unsafe { word.offset(1) };
            database = unsafe { strchr(word, ':' as i32) };
            if !database.is_null() {
                let fresh2 = database;
                database = unsafe { database.offset(1) };
                (unsafe { *fresh2 = 0 as i32 as i8 });
                strategy = unsafe { strchr(database, ':' as i32) };
                if !strategy.is_null() {
                    let fresh3 = strategy;
                    strategy = unsafe { strategy.offset(1) };
                    (unsafe { *fresh3 = 0 as i32 as i8 });
                    nthdef = unsafe { strchr(strategy, ':' as i32) };
                    if !nthdef.is_null() {
                        (unsafe { *nthdef = 0 as i32 as i8 });
                    }
                }
            }
        }
        if word.is_null() || (unsafe { *word }) as i32 == 0 as i32 as i8 as i32 {
            (unsafe { Curl_infof(data, b"lookup word is missing\0" as *const u8 as *const i8) });
            word = b"default\0" as *const u8 as *const i8 as *mut i8;
        }
        if database.is_null() || (unsafe { *database }) as i32 == 0 as i32 as i8 as i32 {
            database = b"!\0" as *const u8 as *const i8 as *mut i8;
        }
        if strategy.is_null() || (unsafe { *strategy }) as i32 == 0 as i32 as i8 as i32 {
        ...
        (unsafe { Curl_cfree.expect("non-null function pointer")(eword as *mut libc::c_void) });
        if result as u64 != 0 {
            (unsafe { Curl_failf(
                data,
        ...
        ) }) != 0
    {
        ...
        if !word.is_null() {
            ...
            if !database.is_null() {
                ...
                nthdef = unsafe { strchr(database, ':' as i32) };
                if !nthdef.is_null() {
                    (unsafe { *nthdef = 0 as i32 as i8 });
                }
            }
        ...
        ppath = unsafe { strchr(path, '/' as i32) };
        if !ppath.is_null() {
            let mut i: i32 = 0;
            ppath = unsafe { ppath.offset(1) };
            i = 0 as i32;
            while (unsafe { *ppath.offset(i as isize) }) != 0 {
                if (unsafe { *ppath.offset(i as isize) }) as i32 == ':' as i32 {
                    (unsafe { *ppath.offset(i as isize) = ' ' as i32 as i8 });
                }
                i += 1;
            }
            result = unsafe { sendf(
