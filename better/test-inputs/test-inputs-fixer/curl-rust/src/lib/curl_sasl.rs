...
extern "C" {
    ...
    fn Curl_auth_create_login_message(value: *const i8, out: *mut bufref) -> CURLcode;
    fn Curl_auth_create_external_message(user: *const i8, out: *mut bufref) -> CURLcode;
...
#[no_mangle]
pub extern "C" fn Curl_sasl_cleanup(mut conn: *mut connectdata, mut authused: u32) {
    if authused & ((1 as i32) << 9 as i32 | (1 as i32) << 10 as i32) as u32 != 0 {
        (unsafe { Curl_auth_gsasl_cleanup(&mut (*conn).gsasl) });
    }
    if authused == ((1 as i32) << 6 as i32) as u32 {
        (unsafe { Curl_auth_cleanup_ntlm(&mut (*conn).ntlm) });
    }
}
#[no_mangle]
pub extern "C" fn Curl_sasl_decode_mech(
    mut ptr: *const i8,
    mut maxlen: size_t,
    mut len: *mut size_t,
) -> u16 {
    ...
    let mut c: i8 = 0;
    i = 0 as i32 as u32;
    while !(unsafe { mechtable[i as usize].name }).is_null() {
        if maxlen >= (unsafe { mechtable[i as usize].len })
            && (unsafe { memcmp(
        ...
        {
            if !len.is_null() {
                (unsafe { *len = mechtable[i as usize].len });
            }
            if maxlen == (unsafe { mechtable[i as usize].len }) {
                return unsafe { mechtable[i as usize].bit };
            }
            c = unsafe { *ptr.offset(mechtable[i as usize].len as isize) };
            if (unsafe { Curl_isupper(c as u8 as i32) }) == 0
                && (unsafe { Curl_isdigit(c as u8 as i32) }) == 0
                && c as i32 != '-' as i32
                && c as i32 != '_' as i32
            {
                return unsafe { mechtable[i as usize].bit };
...
#[no_mangle]
pub extern "C" fn Curl_sasl_parse_url_auth_option(
    mut sasl: *mut SASL,
    mut value: *const i8,
    mut len: size_t,
) -> CURLcode {
    ...
    if unsafe { (*sasl).resetprefs } {
        (unsafe { (*sasl).resetprefs = 0 as i32 != 0 });
        (unsafe { (*sasl).prefmech = 0 as i32 as u16 });
    }
    if (unsafe { strncmp(value, b"*\0" as *const u8 as *const i8, len) }) == 0 {
        (unsafe { (*sasl).prefmech = (0xffff as i32 & !((1 as i32) << 5 as i32)) as u16 });
    } else {
        let mut mechbit: u16 = Curl_sasl_decode_mech(value, len, &mut mechlen);
        if mechbit as i32 != 0 && mechlen == len {
            let fresh0 = unsafe { &mut ((*sasl).prefmech) };
            *fresh0 = (*fresh0 as i32 | mechbit as i32) as u16;
        } else {
...
#[no_mangle]
pub extern "C" fn Curl_sasl_init(mut sasl: *mut SASL, mut params: *const SASLproto) {
    let fresh1 = unsafe { &mut ((*sasl).params) };
    *fresh1 = params;
    (unsafe { (*sasl).state = SASL_STOP });
    (unsafe { (*sasl).authmechs = 0 as i32 as u16 });
    (unsafe { (*sasl).prefmech = (0xffff as i32 & !((1 as i32) << 5 as i32)) as u16 });
    (unsafe { (*sasl).authused = 0 as i32 as u16 });
    (unsafe { (*sasl).resetprefs = 1 as i32 != 0 });
    (unsafe { (*sasl).mutual_auth = 0 as i32 != 0 });
    (unsafe { (*sasl).force_ir = 0 as i32 != 0 });
}
extern "C" fn state(mut sasl: *mut SASL, mut _data: *mut Curl_easy, mut newstate: saslstate) {
    (unsafe { (*sasl).state = newstate });
}
extern "C" fn get_server_message(
    mut sasl: *mut SASL,
    ...
    let mut result: CURLcode = CURLE_OK;
    (unsafe { ((*(*sasl).params).getmessage).expect("non-null function pointer")(
        (*data).state.buffer,
        &mut serverdata,
    ) });
    if serverdata.is_null() {
        result = CURLE_BAD_CONTENT_ENCODING;
    } else if (unsafe { *serverdata }) == 0 || (unsafe { *serverdata }) as i32 == '=' as i32 {
        (unsafe { Curl_bufref_set(out, 0 as *const libc::c_void, 0 as i32 as size_t, None) });
    } else {
        result = unsafe { Curl_base64_decode(serverdata, &mut msg, &mut msglen) };
        if result as u64 == 0 {
...
extern "C" fn build_message(mut data: *mut Curl_easy, mut msg: *mut bufref) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut base64: *mut i8 = 0 as *mut i8;
    let mut base64len: size_t = 0;
    if (unsafe { Curl_bufref_ptr(msg) }).is_null() {
        (unsafe { Curl_bufref_set(
            msg,
    ...
    } else if (unsafe { Curl_bufref_len(msg) }) == 0 {
        (unsafe { Curl_bufref_set(
            msg,
            b"=\0" as *const u8 as *const i8 as *const libc::c_void,
            1 as i32 as size_t,
...
#[no_mangle]
pub extern "C" fn Curl_sasl_can_authenticate(
    mut sasl: *mut SASL,
    ...
    if (unsafe { (*sasl).authmechs }) as i32 & (unsafe { (*sasl).prefmech }) as i32 & (1 as i32) << 5 as i32 != 0 {
        return 1 as i32 != 0;
    }
    return 0 as i32 != 0;
}
#[no_mangle]
pub extern "C" fn Curl_sasl_start(
    mut sasl: *mut SASL,
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut force_ir: bool,
    ...
    let port: i64 = (if CURLPROXY_HTTPS as i32 as u32 == (unsafe { (*conn).http_proxy.proxytype }) as u32
        && ssl_connection_complete as i32 as u32
            != (unsafe { (*conn).proxy_ssl[(if (*conn).sock[1 as i32 as usize] == -(1 as i32) {
                0 as i32
            } else {
    ...
    let mut service: *const i8 =
        if !(unsafe { (*data).set.str_0[STRING_SERVICE_NAME as i32 as usize] }).is_null() {
            (unsafe { (*data).set.str_0[STRING_SERVICE_NAME as i32 as usize] }) as *const i8
        } else {
            unsafe { (*(*sasl).params).service }
        };
    let mut oauth_bearer: *const i8 = unsafe { (*data).set.str_0[STRING_BEARER as i32 as usize] };
    let mut nullmsg: bufref = bufref {
        dtor: None,
        ptr: 0 as *const u8,
        len: 0,
    };
    (unsafe { Curl_bufref_init(&mut nullmsg) });
    (unsafe { Curl_bufref_init(&mut resp) });
    (unsafe { (*sasl).force_ir = force_ir });
    (unsafe { (*sasl).authused = 0 as i32 as u16 });
    enabledmechs = ((unsafe { (*sasl).authmechs }) as i32 & (unsafe { (*sasl).prefmech }) as i32) as u32;
    (unsafe { *progress = SASL_IDLE });
    if enabledmechs & ((1 as i32) << 5 as i32) as u32 != 0
        && (unsafe { *((*conn).passwd).offset(0 as i32 as isize) }) == 0
    {
        mech = b"EXTERNAL\0" as *const u8 as *const i8;
        state1 = SASL_EXTERNAL;
        (unsafe { (*sasl).authused = ((1 as i32) << 5 as i32) as u16 });
        if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
            result = unsafe { Curl_auth_create_external_message((*conn).user, &mut resp) };
        }
    } else if (unsafe { ((*conn).bits).user_passwd() }) != 0 {
        if enabledmechs & ((1 as i32) << 10 as i32) as u32 != 0
            && (unsafe { Curl_auth_gsasl_is_supported(
                data,
                ...
                != 0
        {
            ...
            (unsafe { (*sasl).authused = ((1 as i32) << 10 as i32) as u16 });
            state1 = SASL_GSASL;
            state2 = SASL_GSASL;
            result = unsafe { Curl_auth_gsasl_start(data, (*conn).user, (*conn).passwd, &mut (*conn).gsasl) };
            if result as u32 == CURLE_OK as i32 as u32
                && (force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0)
            {
                result = unsafe { Curl_auth_gsasl_token(data, &mut nullmsg, &mut (*conn).gsasl, &mut resp) };
            }
        } else if enabledmechs & ((1 as i32) << 9 as i32) as u32 != 0
            && (unsafe { Curl_auth_gsasl_is_supported(
                data,
                ...
                != 0
        {
            ...
            (unsafe { (*sasl).authused = ((1 as i32) << 9 as i32) as u16 });
            state1 = SASL_GSASL;
            state2 = SASL_GSASL;
            result = unsafe { Curl_auth_gsasl_start(data, (*conn).user, (*conn).passwd, &mut (*conn).gsasl) };
            if result as u32 == CURLE_OK as i32 as u32
        ...
        } else if enabledmechs & ((1 as i32) << 3 as i32) as u32 != 0
            && (unsafe { Curl_auth_is_digest_supported() }) as i32 != 0
        {
            mech = b"DIGEST-MD5\0" as *const u8 as *const i8;
            state1 = SASL_DIGESTMD5;
            (unsafe { (*sasl).authused = ((1 as i32) << 3 as i32) as u16 });
        } else if enabledmechs & ((1 as i32) << 2 as i32) as u32 != 0 {
            mech = b"CRAM-MD5\0" as *const u8 as *const i8;
            state1 = SASL_CRAMMD5;
            (unsafe { (*sasl).authused = ((1 as i32) << 2 as i32) as u16 });
        } else if enabledmechs & ((1 as i32) << 6 as i32) as u32 != 0
            && (unsafe { Curl_auth_is_ntlm_supported() }) as i32 != 0
        {
            mech = b"NTLM\0" as *const u8 as *const i8;
            state1 = SASL_NTLM;
            state2 = SASL_NTLM_TYPE2MSG;
            (unsafe { (*sasl).authused = ((1 as i32) << 6 as i32) as u16 });
            if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
        ...
        } else if enabledmechs & ((1 as i32) << 8 as i32) as u32 != 0 && !oauth_bearer.is_null() {
            mech = b"OAUTHBEARER\0" as *const u8 as *const i8;
            state1 = SASL_OAUTH2;
            state2 = SASL_OAUTH2_RESP;
            (unsafe { (*sasl).authused = ((1 as i32) << 8 as i32) as u16 });
            if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
                result = unsafe { Curl_auth_create_oauth_bearer_message(
                    (*conn).user,
                    hostname,
                    port,
                    oauth_bearer,
        ...
        } else if enabledmechs & ((1 as i32) << 7 as i32) as u32 != 0 && !oauth_bearer.is_null() {
            mech = b"XOAUTH2\0" as *const u8 as *const i8;
            state1 = SASL_OAUTH2;
            (unsafe { (*sasl).authused = ((1 as i32) << 7 as i32) as u16 });
            if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
                result =
                    unsafe { Curl_auth_create_xoauth_bearer_message((*conn).user, oauth_bearer, &mut resp) };
            }
        } else if enabledmechs & ((1 as i32) << 1 as i32) as u32 != 0 {
            mech = b"PLAIN\0" as *const u8 as *const i8;
            state1 = SASL_PLAIN;
            (unsafe { (*sasl).authused = ((1 as i32) << 1 as i32) as u16 });
            if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
                result = unsafe { Curl_auth_create_plain_message(
                    (*conn).sasl_authzid,
                    (*conn).user,
                    (*conn).passwd,
                    &mut resp,
        ...
        } else if enabledmechs & ((1 as i32) << 0 as i32) as u32 != 0 {
            mech = b"LOGIN\0" as *const u8 as *const i8;
            state1 = SASL_LOGIN;
            state2 = SASL_LOGIN_PASSWD;
            (unsafe { (*sasl).authused = ((1 as i32) << 0 as i32) as u16 });
            if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
                result = unsafe { Curl_auth_create_login_message((*conn).user, &mut resp) };
            }
        }
    }
    if result as u64 == 0 && !mech.is_null() {
        if !(unsafe { Curl_bufref_ptr(&mut resp) }).is_null() {
            result = build_message(data, &mut resp);
        }
        if (unsafe { (*(*sasl).params).maxirlen }) != 0
            && (unsafe { strlen(mech) }).wrapping_add(unsafe { Curl_bufref_len(&mut resp) }) > (unsafe { (*(*sasl).params).maxirlen })
        {
            (unsafe { Curl_bufref_free(&mut resp) });
        }
        if result as u64 == 0 {
            result = unsafe { ((*(*sasl).params).sendauth).expect("non-null function pointer")(
                data,
                conn,
                mech,
                Curl_bufref_ptr(&mut resp) as *const i8,
            ) };
        }
        if result as u64 == 0 {
            (unsafe { *progress = SASL_INPROGRESS });
            state(
                sasl,
                data,
                (if !(unsafe { Curl_bufref_ptr(&mut resp) }).is_null() {
...
#[no_mangle]
pub extern "C" fn Curl_sasl_continue(
    mut sasl: *mut SASL,
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut code: i32,
    ...
    (unsafe { Curl_bufref_init(&mut serverdata) });
    (unsafe { Curl_bufref_init(&mut resp) });
    (unsafe { *progress = SASL_INPROGRESS });
    if (unsafe { (*sasl).state }) as u32 == SASL_FINAL as i32 as u32 {
        if code != (unsafe { (*(*sasl).params).finalcode }) {
            result = CURLE_LOGIN_DENIED;
        }
        (unsafe { *progress = SASL_DONE });
        state(sasl, data, SASL_STOP);
        return result;
    }
    if (unsafe { (*sasl).state }) as u32 != SASL_CANCEL as i32 as u32
        && (unsafe { (*sasl).state }) as u32 != SASL_OAUTH2_RESP as i32 as u32
        && code != (unsafe { (*(*sasl).params).contcode })
    {
        (unsafe { *progress = SASL_DONE });
        state(sasl, data, SASL_STOP);
        return CURLE_LOGIN_DENIED;
    }
    match (unsafe { (*sasl).state }) as u32 {
        0 => {
            (unsafe { *progress = SASL_DONE });
            return result;
        }
        ...
        3 => {
            result = unsafe { Curl_auth_create_login_message((*conn).passwd, &mut resp) };
        }
        4 => {
            result = unsafe { Curl_auth_create_external_message((*conn).user, &mut resp) };
        }
        15 => {
            ...
            if result as u64 == 0 {
                result =
                    unsafe { Curl_auth_gsasl_token(data, &mut serverdata, &mut (*conn).gsasl, &mut resp) };
            }
            if result as u64 == 0 && (unsafe { Curl_bufref_len(&mut resp) }) > 0 as i32 as u64 {
                newstate = SASL_GSASL;
            }
        }
        5 => {
            result = get_server_message(sasl, data, &mut serverdata);
            if result as u64 == 0 {
                result = unsafe { Curl_auth_create_cram_md5_message(
                    &mut serverdata,
                    (*conn).user,
                    (*conn).passwd,
                    &mut resp,
        ...
        6 => {
            result = get_server_message(sasl, data, &mut serverdata);
            if result as u64 == 0 {
                result = unsafe { Curl_auth_create_digest_md5_message(
                    data,
                    &mut serverdata,
                    (*conn).user,
                    (*conn).passwd,
        ...
        9 => {
            ...
            if result as u64 == 0 {
                result =
                    unsafe { Curl_auth_decode_ntlm_type2_message(data, &mut serverdata, &mut (*conn).ntlm) };
            }
            if result as u64 == 0 {
                result = unsafe { Curl_auth_create_ntlm_type3_message(
                    data,
                    (*conn).user,
                    (*conn).passwd,
                    &mut (*conn).ntlm,
        ...
        13 => {
            if (unsafe { (*sasl).authused }) as i32 == (1 as i32) << 8 as i32 {
                result = unsafe { Curl_auth_create_oauth_bearer_message(
                    (*conn).user,
                    hostname,
                    port,
        ...
        14 => {
            if code == (unsafe { (*(*sasl).params).finalcode }) {
                ...
                state(sasl, data, SASL_STOP);
                return result;
            } else if code == (unsafe { (*(*sasl).params).contcode }) {
                (unsafe { Curl_bufref_set(
                    &mut resp,
        ...
        16 => {
            let fresh2 = unsafe { &mut ((*sasl).authmechs) };
            *fresh2 = (*fresh2 as i32 ^ (unsafe { (*sasl).authused }) as i32) as u16;
            return Curl_sasl_start(sasl, data, conn, unsafe { (*sasl).force_ir }, progress);
        }
        _ => {
            (unsafe { Curl_failf(
                data,
                b"Unsupported SASL authentication mechanism\0" as *const u8 as *const i8,
            ) });
            result = CURLE_UNSUPPORTED_PROTOCOL;
        }
    }
    (unsafe { Curl_bufref_free(&mut serverdata) });
    match result as u32 {
        61 => {
            result = unsafe { ((*(*sasl).params).sendcont).expect("non-null function pointer")(
                data,
                conn,
