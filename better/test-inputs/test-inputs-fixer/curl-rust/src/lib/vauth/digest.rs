...
extern "C" {
    ...
    fn Curl_auth_build_spn(service: *const i8, host: *const i8, realm: *const i8) -> *mut i8;
    ...
    fn Curl_bufref_ptr(br: *const bufref) -> *const u8;
    ...
    fn Curl_md5it(output: *mut u8, input: *const u8, len: size_t);
    ...
    fn Curl_sha256it(outbuffer: *mut u8, input: *const u8, len: size_t);
...
#[no_mangle]
pub extern "C" fn Curl_auth_digest_get_pair(
    mut str: *const i8,
    mut value: *mut i8,
    mut content: *mut i8,
    ...
    while (unsafe { *str }) as i32 != 0 && (unsafe { *str }) as i32 != '=' as i32 && {
        let fresh0 = c;
        c = c - 1;
        fresh0 != 0
    } {
    ...
    (unsafe { *value = 0 as i32 as i8 });
    let fresh3 = str;
    str = unsafe { str.offset(1) };
    if '=' as i32 != (unsafe { *fresh3 }) as i32 {
        return 0 as i32 != 0;
    }
    ...
    while (unsafe { *str }) as i32 != 0 && {
        let fresh4 = c;
        c = c - 1;
        fresh4 != 0
    } {
        match (unsafe { *str }) as i32 {
            92 => {
                if !escape {
                    escape = 1 as i32 != 0;
                    let fresh5 = content;
    ...
    (unsafe { *content = 0 as i32 as i8 });
    (unsafe { *endptr = str });
    return 1 as i32 != 0;
}
extern "C" fn auth_digest_md5_to_ascii(mut source: *mut u8, mut dest: *mut u8) {
    let mut i: i32 = 0;
    i = 0 as i32;
    while i < 16 as i32 {
        (unsafe { curl_msnprintf(
            &mut *dest.offset((i * 2 as i32) as isize) as *mut u8 as *mut i8,
            3 as i32 as size_t,
...
extern "C" fn auth_digest_sha256_to_ascii(mut source: *mut u8, mut dest: *mut u8) {
    let mut i: i32 = 0;
    i = 0 as i32;
    while i < 32 as i32 {
        (unsafe { curl_msnprintf(
...
extern "C" fn auth_digest_string_quoted(mut source: *const i8) -> *mut i8 {
    let mut dest: *mut i8 = 0 as *mut i8;
    let mut s: *const i8 = source;
    let mut n: size_t = 1 as i32 as size_t;
    while (unsafe { *s }) != 0 {
        n = n.wrapping_add(1);
        if (unsafe { *s }) as i32 == '"' as i32 || (unsafe { *s }) as i32 == '\\' as i32 {
            n = n.wrapping_add(1);
        }
        s = unsafe { s.offset(1) };
    }
    dest = (unsafe { Curl_cmalloc.expect("non-null function pointer")(n) }) as *mut i8;
    if !dest.is_null() {
...
extern "C" fn auth_digest_get_key_value(
    mut chlg: *const i8,
    mut key: *const i8,
    mut value: *mut i8,
    ...
    let mut i: size_t = 0;
    find_pos = unsafe { strstr(chlg, key) };
    if find_pos.is_null() {
        return 0 as i32 != 0;
    }
    find_pos = unsafe { find_pos.offset(strlen(key) as isize) };
    i = 0 as i32 as size_t;
    while (unsafe { *find_pos }) as i32 != 0
        && (unsafe { *find_pos }) as i32 != end_char as i32
        && i < max_val_len.wrapping_sub(1 as i32 as u64)
    {
        let fresh10 = find_pos;
        find_pos = unsafe { find_pos.offset(1) };
        (unsafe { *value.offset(i as isize) = *fresh10 });
        i = i.wrapping_add(1);
    }
    (unsafe { *value.offset(i as isize) = '\u{0}' as i32 as i8 });
    return 1 as i32 != 0;
}
extern "C" fn auth_digest_get_qop_values(mut options: *const i8, mut value: *mut i32) -> CURLcode {
    let mut tmp: *mut i8 = 0 as *mut i8;
    let mut token: *mut i8 = 0 as *mut i8;
    let mut tok_buf: *mut i8 = 0 as *mut i8;
    (unsafe { *value = 0 as i32 });
    tmp = unsafe { Curl_cstrdup.expect("non-null function pointer")(options) };
    if tmp.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    token = unsafe { strtok_r(tmp, b",\0" as *const u8 as *const i8, &mut tok_buf) };
    while !token.is_null() {
        if (unsafe { Curl_strcasecompare(token, b"auth\0" as *const u8 as *const i8) }) != 0 {
            (unsafe { *value |= (1 as i32) << 0 as i32 });
        } else if (unsafe { Curl_strcasecompare(token, b"auth-int\0" as *const u8 as *const i8) }) != 0 {
            (unsafe { *value |= (1 as i32) << 1 as i32 });
        } else if (unsafe { Curl_strcasecompare(token, b"auth-conf\0" as *const u8 as *const i8) }) != 0 {
            (unsafe { *value |= (1 as i32) << 2 as i32 });
        }
        token = unsafe { strtok_r(0 as *mut i8, b",\0" as *const u8 as *const i8, &mut tok_buf) };
    }
    (unsafe { Curl_cfree.expect("non-null function pointer")(tmp as *mut libc::c_void) });
    return CURLE_OK;
}
extern "C" fn auth_decode_digest_md5_message(
    ...
    let mut chlg: *const i8 = (unsafe { Curl_bufref_ptr(chlgref) }) as *const i8;
    if (unsafe { Curl_bufref_len(chlgref) }) == 0 {
    ...
    if !auth_digest_get_key_value(
        ...
        (unsafe { strcpy(realm, b"\0" as *const u8 as *const i8) });
    }
    if !auth_digest_get_key_value(
        chlg,
...
#[no_mangle]
pub extern "C" fn Curl_auth_create_digest_md5_message(
    ...
    let mut nonceCount: [i8; 9] = *(unsafe { ::std::mem::transmute::<&[u8; 9], &mut [i8; 9]>(b"00000001\0") });
    let mut method: [i8; 13] =
        *(unsafe { ::std::mem::transmute::<&[u8; 13], &mut [i8; 13]>(b"AUTHENTICATE\0") });
    let mut qop: [i8; 5] = *(unsafe { ::std::mem::transmute::<&[u8; 5], &mut [i8; 5]>(b"auth\0") });
    ...
    (unsafe { Curl_MD5_update(ctxt, userp as *const u8, curlx_uztoui(strlen(userp))) });
    ...
    (unsafe { Curl_MD5_update(ctxt, passwdp as *const u8, curlx_uztoui(strlen(passwdp))) });
    (unsafe { Curl_MD5_final(ctxt, digest.as_mut_ptr()) });
    ctxt = unsafe { Curl_MD5_init(Curl_DIGEST_MD5.as_ptr()) };
    if ctxt.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { Curl_MD5_update(ctxt, digest.as_mut_ptr() as *const u8, 16 as i32 as u32) });
    (unsafe { Curl_MD5_update(
        ctxt,
        b":\0" as *const u8 as *const i8 as *const u8,
        1 as i32 as u32,
    ...
    spn = unsafe { Curl_auth_build_spn(service, realm.as_mut_ptr(), 0 as *const i8) };
    if spn.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    ctxt = unsafe { Curl_MD5_init(Curl_DIGEST_MD5.as_ptr()) };
    if ctxt.is_null() {
        (unsafe { Curl_cfree.expect("non-null function pointer")(spn as *mut libc::c_void) });
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { Curl_MD5_update(
        ctxt,
    ...
    (unsafe { Curl_MD5_update(ctxt, spn as *const u8, curlx_uztoui(strlen(spn))) });
    (unsafe { Curl_MD5_final(ctxt, digest.as_mut_ptr()) });
    i = 0 as i32 as size_t;
    while i < 16 as i32 as u64 {
        (unsafe { curl_msnprintf(
    ...
    response = unsafe { curl_maprintf (b"username=\"%s\",realm=\"%s\",nonce=\"%s\",cnonce=\"%s\",nc=\"%s\",digest-uri=\"%s\",response=%s,qop=%s\0" as * const u8 as * const i8 , userp , realm . as_mut_ptr () , nonce . as_mut_ptr () , cnonce . as_mut_ptr () , nonceCount . as_mut_ptr () , spn , resp_hash_hex . as_mut_ptr () , qop . as_mut_ptr () ,) } ;
    (unsafe { Curl_cfree.expect("non-null function pointer")(spn as *mut libc::c_void) });
    if response.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { Curl_bufref_set(
        out,
...
#[no_mangle]
pub extern "C" fn Curl_auth_decode_digest_http_message(
    mut chlg: *const i8,
    ...
    let mut token: *mut i8 = 0 as *mut i8;
    let mut tmp: *mut i8 = 0 as *mut i8;
    if !(unsafe { (*digest).nonce }).is_null() {
        before = 1 as i32 != 0;
    }
    ...
    loop {
        ...
        while (unsafe { *chlg }) as i32 != 0 && (unsafe { Curl_isspace(*chlg as u8 as i32) }) != 0 {
            chlg = unsafe { chlg.offset(1) };
        }
        if !Curl_auth_digest_get_pair(chlg, value.as_mut_ptr(), content.as_mut_ptr(), &mut chlg) {
            break;
        }
        if (unsafe { Curl_strcasecompare(value.as_mut_ptr(), b"nonce\0" as *const u8 as *const i8) }) != 0 {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).nonce as *mut libc::c_void) });
            let fresh11 = unsafe { &mut ((*digest).nonce) };
            *fresh11 = unsafe { Curl_cstrdup.expect("non-null function pointer")(content.as_mut_ptr()) };
            if (unsafe { (*digest).nonce }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
        } else if (unsafe { Curl_strcasecompare(value.as_mut_ptr(), b"stale\0" as *const u8 as *const i8) }) != 0
        {
            if (unsafe { Curl_strcasecompare(content.as_mut_ptr(), b"true\0" as *const u8 as *const i8) }) != 0 {
                (unsafe { (*digest).set_stale(1 as i32 as bit) });
                (unsafe { (*digest).nc = 1 as i32 });
            }
        } else if (unsafe { Curl_strcasecompare(value.as_mut_ptr(), b"realm\0" as *const u8 as *const i8) }) != 0
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).realm as *mut libc::c_void) });
            let fresh12 = unsafe { &mut ((*digest).realm) };
            *fresh12 = unsafe { Curl_cstrdup.expect("non-null function pointer")(content.as_mut_ptr()) };
            if (unsafe { (*digest).realm }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
        } else if (unsafe { Curl_strcasecompare(value.as_mut_ptr(), b"opaque\0" as *const u8 as *const i8) })
            != 0
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).opaque as *mut libc::c_void) });
            let fresh13 = unsafe { &mut ((*digest).opaque) };
            *fresh13 = unsafe { Curl_cstrdup.expect("non-null function pointer")(content.as_mut_ptr()) };
            if (unsafe { (*digest).opaque }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
        } else if (unsafe { Curl_strcasecompare(value.as_mut_ptr(), b"qop\0" as *const u8 as *const i8) }) != 0 {
            let mut tok_buf: *mut i8 = 0 as *mut i8;
            tmp = unsafe { Curl_cstrdup.expect("non-null function pointer")(content.as_mut_ptr()) };
            if tmp.is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
            token = unsafe { strtok_r(tmp, b",\0" as *const u8 as *const i8, &mut tok_buf) };
            ...
            (unsafe { Curl_cfree.expect("non-null function pointer")(tmp as *mut libc::c_void) });
            if foundAuth {
                (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).qop as *mut libc::c_void) });
                let fresh14 = unsafe { &mut ((*digest).qop) };
                *fresh14 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                    b"auth\0" as *const u8 as *const i8,
                ) };
                if (unsafe { (*digest).qop }).is_null() {
                    return CURLE_OUT_OF_MEMORY;
                }
            } else if foundAuthInt {
                (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).qop as *mut libc::c_void) });
                let fresh15 = unsafe { &mut ((*digest).qop) };
                *fresh15 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
        ...
        } else if (unsafe { Curl_strcasecompare(value.as_mut_ptr(), b"algorithm\0" as *const u8 as *const i8) })
            != 0
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")(
                (*digest).algorithm as *mut libc::c_void,
            ) });
            let fresh16 = unsafe { &mut ((*digest).algorithm) };
            *fresh16 = unsafe { Curl_cstrdup.expect("non-null function pointer")(content.as_mut_ptr()) };
            if (unsafe { (*digest).algorithm }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
            ...
            {
                (unsafe { (*digest).algo = CURLDIGESTALGO_MD5SESS as i32 });
            } else if (unsafe { Curl_strcasecompare(content.as_mut_ptr(), b"MD5\0" as *const u8 as *const i8) })
                != 0
            {
                (unsafe { (*digest).algo = CURLDIGESTALGO_MD5 as i32 });
            } else if (unsafe { Curl_strcasecompare(
                content.as_mut_ptr(),
                b"SHA-256\0" as *const u8 as *const i8,
            ) }) != 0
            {
                (unsafe { (*digest).algo = CURLDIGESTALGO_SHA256 as i32 });
            } else if (unsafe { Curl_strcasecompare(
                content.as_mut_ptr(),
                b"SHA-256-SESS\0" as *const u8 as *const i8,
            ) }) != 0
            {
                (unsafe { (*digest).algo = CURLDIGESTALGO_SHA256SESS as i32 });
            } else if (unsafe { Curl_strcasecompare(
                content.as_mut_ptr(),
                b"SHA-512-256\0" as *const u8 as *const i8,
            ) }) != 0
            {
                (unsafe { (*digest).algo = CURLDIGESTALGO_SHA512_256 as i32 });
            } else if (unsafe { Curl_strcasecompare(
                content.as_mut_ptr(),
                b"SHA-512-256-SESS\0" as *const u8 as *const i8,
            ) }) != 0
            {
                (unsafe { (*digest).algo = CURLDIGESTALGO_SHA512_256SESS as i32 });
            } else {
                return CURLE_BAD_CONTENT_ENCODING;
            }
        } else if (unsafe { Curl_strcasecompare(value.as_mut_ptr(), b"userhash\0" as *const u8 as *const i8) })
            != 0
        {
            if (unsafe { Curl_strcasecompare(content.as_mut_ptr(), b"true\0" as *const u8 as *const i8) }) != 0 {
                (unsafe { (*digest).set_userhash(1 as i32 as bit) });
            }
        }
        while (unsafe { *chlg }) as i32 != 0 && (unsafe { Curl_isspace(*chlg as u8 as i32) }) != 0 {
            chlg = unsafe { chlg.offset(1) };
        }
        if ',' as i32 == (unsafe { *chlg }) as i32 {
            chlg = unsafe { chlg.offset(1) };
        }
    }
    if before as i32 != 0 && (unsafe { (*digest).stale() }) == 0 {
        return CURLE_BAD_CONTENT_ENCODING;
    }
    if (unsafe { (*digest).nonce }).is_null() {
        return CURLE_BAD_CONTENT_ENCODING;
...
extern "C" fn auth_create_digest_http_message(
    mut data: *mut Curl_easy,
    mut userp: *const i8,
    mut passwdp: *const i8,
    ...
    mut convert_to_ascii: Option<unsafe extern "C" fn(*mut u8, *mut u8) -> ()>,
    mut hash: Option<unsafe extern "C" fn(*mut u8, *const u8, size_t) -> ()>,
) -> CURLcode {
    ...
    let mut tmp: *mut i8 = 0 as *mut i8;
    if (unsafe { (*digest).nc }) == 0 {
        (unsafe { (*digest).nc = 1 as i32 });
    }
    if (unsafe { (*digest).cnonce }).is_null() {
        ...
        let fresh17 = unsafe { &mut ((*digest).cnonce) };
        *fresh17 = cnonce;
    }
    if (unsafe { (*digest).userhash() }) != 0 {
        hashthis = unsafe { curl_maprintf(b"%s:%s\0" as *const u8 as *const i8, userp, (*digest).realm) };
        if hashthis.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        result = CURLE_OK as i32 as CURLcode;
        if result as u64 != 0 {
            (unsafe { Curl_cfree.expect("non-null function pointer")(hashthis as *mut libc::c_void) });
            return result;
        }
        (unsafe { hash.expect("non-null function pointer")(
            hashbuf.as_mut_ptr(),
            hashthis as *mut u8,
            strlen(hashthis),
        ) });
        (unsafe { Curl_cfree.expect("non-null function pointer")(hashthis as *mut libc::c_void) });
        (unsafe { convert_to_ascii.expect("non-null function pointer")(
            hashbuf.as_mut_ptr(),
            userh.as_mut_ptr() as *mut u8,
        ) });
    }
    hashthis = unsafe { curl_maprintf(
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(hashthis as *mut libc::c_void) });
    (unsafe { convert_to_ascii.expect("non-null function pointer")(hashbuf.as_mut_ptr(), ha1.as_mut_ptr()) });
    if (unsafe { (*digest).algo }) == CURLDIGESTALGO_MD5SESS as i32
        || (unsafe { (*digest).algo }) == CURLDIGESTALGO_SHA256SESS as i32
        || (unsafe { (*digest).algo }) == CURLDIGESTALGO_SHA512_256SESS as i32
    {
        tmp = unsafe { curl_maprintf(
            b"%s:%s:%s\0" as *const u8 as *const i8,
            ha1.as_mut_ptr(),
            (*digest).nonce,
            (*digest).cnonce,
        ...
        (unsafe { hash.expect("non-null function pointer")(hashbuf.as_mut_ptr(), tmp as *mut u8, strlen(tmp)) });
        (unsafe { Curl_cfree.expect("non-null function pointer")(tmp as *mut libc::c_void) });
        (unsafe { convert_to_ascii.expect("non-null function pointer")(
            hashbuf.as_mut_ptr(),
            ha1.as_mut_ptr(),
    ...
    hashthis = unsafe { curl_maprintf(b"%s:%s\0" as *const u8 as *const i8, request, uripath) };
    if hashthis.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    if !(unsafe { (*digest).qop }).is_null()
        && (unsafe { Curl_strcasecompare((*digest).qop, b"auth-int\0" as *const u8 as *const i8) }) != 0
    {
        ...
        (unsafe { convert_to_ascii.expect("non-null function pointer")(
            hashbuf.as_mut_ptr(),
            hashed.as_mut_ptr() as *mut u8,
        ) });
        hashthis2 = unsafe { curl_maprintf(
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(hashthis as *mut libc::c_void) });
    (unsafe { convert_to_ascii.expect("non-null function pointer")(hashbuf.as_mut_ptr(), ha2.as_mut_ptr()) });
    if !(unsafe { (*digest).qop }).is_null() {
        hashthis = unsafe { curl_maprintf(
    ...
    userp_quoted = auth_digest_string_quoted(if (unsafe { (*digest).userhash() }) as i32 != 0 {
        userh.as_mut_ptr() as *const i8
    } else {
        userp
    });
    ...
    if !(unsafe { (*digest).qop }).is_null() {
        response = unsafe { curl_maprintf (b"username=\"%s\", realm=\"%s\", nonce=\"%s\", uri=\"%s\", cnonce=\"%s\", nc=%08x, qop=%s, response=\"%s\"\0" as * const u8 as * const i8 , userp_quoted , (* digest) . realm , (* digest) . nonce , uripath , (* digest) . cnonce , (* digest) . nc , (* digest) . qop , request_digest . as_mut_ptr () ,) } ;
        if (unsafe { Curl_strcasecompare((*digest).qop, b"auth\0" as *const u8 as *const i8) }) != 0 {
            let fresh18 = unsafe { &mut ((*digest).nc) };
            *fresh18 += 1;
        }
    } else {
        response = unsafe { curl_maprintf(
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(userp_quoted as *mut libc::c_void) });
    if response.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    if !(unsafe { (*digest).opaque }).is_null() {
        ...
        (unsafe { Curl_cfree.expect("non-null function pointer")(response as *mut libc::c_void) });
        if tmp.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        response = tmp;
    ...
    if (unsafe { (*digest).userhash() }) != 0 {
        tmp = unsafe { curl_maprintf(b"%s, userhash=true\0" as *const u8 as *const i8, response) };
        (unsafe { Curl_cfree.expect("non-null function pointer")(response as *mut libc::c_void) });
        if tmp.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
    ...
    (unsafe { *outptr = response });
    (unsafe { *outlen = strlen(response) });
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_auth_create_digest_http_message(
    mut data: *mut Curl_easy,
    mut userp: *const i8,
    mut passwdp: *const i8,
    mut request: *const u8,
    ...
    match unsafe { (*digest).algo } {
        0 | 1 => {
            return auth_create_digest_http_message(
                ...
                outptr,
                outlen,
                Some(auth_digest_md5_to_ascii as unsafe extern "C" fn(*mut u8, *mut u8) -> ()),
                Some(Curl_md5it as unsafe extern "C" fn(*mut u8, *const u8, size_t) -> ()),
            );
        }
        2 | 3 | 4 | 5 => {
            return auth_create_digest_http_message(
                ...
                outptr,
                outlen,
                Some(auth_digest_sha256_to_ascii as unsafe extern "C" fn(*mut u8, *mut u8) -> ()),
                Some(Curl_sha256it as unsafe extern "C" fn(*mut u8, *const u8, size_t) -> ()),
            );
        }
...
#[no_mangle]
pub extern "C" fn Curl_auth_digest_cleanup(mut digest: *mut digestdata) {
    (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).nonce as *mut libc::c_void) });
    let fresh19 = unsafe { &mut ((*digest).nonce) };
    *fresh19 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).cnonce as *mut libc::c_void) });
    let fresh20 = unsafe { &mut ((*digest).cnonce) };
    *fresh20 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).realm as *mut libc::c_void) });
    let fresh21 = unsafe { &mut ((*digest).realm) };
    *fresh21 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).opaque as *mut libc::c_void) });
    let fresh22 = unsafe { &mut ((*digest).opaque) };
    *fresh22 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).qop as *mut libc::c_void) });
    let fresh23 = unsafe { &mut ((*digest).qop) };
    *fresh23 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")((*digest).algorithm as *mut libc::c_void) });
    let fresh24 = unsafe { &mut ((*digest).algorithm) };
    *fresh24 = 0 as *mut i8;
    (unsafe { (*digest).nc = 0 as i32 });
    (unsafe { (*digest).algo = CURLDIGESTALGO_MD5 as i32 });
    (unsafe { (*digest).set_stale(0 as i32 as bit) });
    (unsafe { (*digest).set_userhash(0 as i32 as bit) });
}
