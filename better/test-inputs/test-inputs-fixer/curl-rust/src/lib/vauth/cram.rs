...
extern "C" {
    ...
    fn Curl_HMAC_update(context: *mut HMAC_context, data: *const u8, len: u32) -> i32;
    fn Curl_HMAC_final(context: *mut HMAC_context, result: *mut u8) -> i32;
...
#[no_mangle]
pub extern "C" fn Curl_auth_create_cram_md5_message(
    ...
    if (unsafe { Curl_bufref_len(chlg) }) != 0 {
        (unsafe { Curl_HMAC_update(
            ctxt,
            Curl_bufref_ptr(chlg),
            curlx_uztoui(Curl_bufref_len(chlg)),
        ) });
    }
    (unsafe { Curl_HMAC_final(ctxt, digest.as_mut_ptr()) });
