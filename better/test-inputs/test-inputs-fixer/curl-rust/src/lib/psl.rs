...
#[no_mangle]
pub extern "C" fn Curl_psl_destroy(mut pslcache: *mut PslCache) {
    if !(unsafe { (*pslcache).psl }).is_null() {
        if unsafe { (*pslcache).dynamic } {
            (unsafe { psl_free((*pslcache).psl as *mut psl_ctx_t) });
        }
        let fresh0 = unsafe { &mut ((*pslcache).psl) };
        *fresh0 = 0 as *const psl_ctx_t;
        (unsafe { (*pslcache).dynamic = 0 as i32 != 0 });
    }
}
extern "C" fn now_seconds() -> time_t {
    let mut now: curltime = unsafe { Curl_now() };
    return now.tv_sec;
}
#[no_mangle]
pub extern "C" fn Curl_psl_use(mut easy: *mut Curl_easy) -> *const psl_ctx_t {
    let mut pslcache: *mut PslCache = unsafe { (*easy).psl };
    let mut psl: *const psl_ctx_t = 0 as *const psl_ctx_t;
    let mut now: time_t = 0;
    if pslcache.is_null() {
        return 0 as *const psl_ctx_t;
    }
    (unsafe { Curl_share_lock(easy, CURL_LOCK_DATA_PSL, CURL_LOCK_ACCESS_SHARED) });
    now = now_seconds();
    if (unsafe { (*pslcache).psl }).is_null() || (unsafe { (*pslcache).expires }) <= now {
        (unsafe { Curl_share_unlock(easy, CURL_LOCK_DATA_PSL) });
        (unsafe { Curl_share_lock(easy, CURL_LOCK_DATA_PSL, CURL_LOCK_ACCESS_SINGLE) });
        now = now_seconds();
        if (unsafe { (*pslcache).psl }).is_null() || (unsafe { (*pslcache).expires }) <= now {
            let mut dynamic: bool = 0 as i32 != 0;
            let mut expires: time_t = 0x7fffffffffffffff as i64;
            psl = unsafe { psl_latest(0 as *const i8) };
            dynamic = !psl.is_null();
            expires = if now < 0x7fffffffffffffff as i64 - (72 as i32 * 3600 as i32) as i64 {
                now + (72 as i32 * 3600 as i32) as i64
            } else {
                0x7fffffffffffffff as i64
            };
            if psl.is_null() && !(unsafe { (*pslcache).dynamic }) {
                psl = unsafe { psl_builtin() };
            }
            if !psl.is_null() {
                Curl_psl_destroy(pslcache);
                let fresh1 = unsafe { &mut ((*pslcache).psl) };
                *fresh1 = psl;
                (unsafe { (*pslcache).dynamic = dynamic });
                (unsafe { (*pslcache).expires = expires });
            }
        }
        (unsafe { Curl_share_unlock(easy, CURL_LOCK_DATA_PSL) });
        (unsafe { Curl_share_lock(easy, CURL_LOCK_DATA_PSL, CURL_LOCK_ACCESS_SHARED) });
    }
    psl = unsafe { (*pslcache).psl };
    if psl.is_null() {
        (unsafe { Curl_share_unlock(easy, CURL_LOCK_DATA_PSL) });
    }
    return psl;
...
#[no_mangle]
pub extern "C" fn Curl_psl_release(mut easy: *mut Curl_easy) {
    (unsafe { Curl_share_unlock(easy, CURL_LOCK_DATA_PSL) });
}
