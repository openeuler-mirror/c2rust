...
extern "C" fn conn_llist_dtor(mut _user: *mut libc::c_void, mut element: *mut libc::c_void) {
    let mut conn: *mut connectdata = element as *mut connectdata;
    let fresh0 = unsafe { &mut ((*conn).bundle) };
    *fresh0 = 0 as *mut connectbundle;
}
extern "C" fn bundle_create(mut bundlep: *mut *mut connectbundle) -> CURLcode {
    (unsafe { *bundlep = Curl_cmalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<connectbundle>() as u64,
    ) as *mut connectbundle });
    if (unsafe { *bundlep }).is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { (**bundlep).num_connections = 0 as i32 as size_t });
    (unsafe { (**bundlep).multiuse = 0 as i32 });
    (unsafe { Curl_llist_init(
        ...
        ::std::mem::transmute::<
            ...
            Curl_llist_dtor,
        >(Some(
            conn_llist_dtor as unsafe extern "C" fn(*mut libc::c_void, *mut libc::c_void) -> (),
        )),
    ) });
    return CURLE_OK;
}
extern "C" fn bundle_destroy(mut bundle: *mut connectbundle) {
    if bundle.is_null() {
        return;
    }
    (unsafe { Curl_llist_destroy(&mut (*bundle).conn_list, 0 as *mut libc::c_void) });
    (unsafe { Curl_cfree.expect("non-null function pointer")(bundle as *mut libc::c_void) });
}
extern "C" fn bundle_add_conn(mut bundle: *mut connectbundle, mut conn: *mut connectdata) {
    ...
    let fresh1 = unsafe { &mut ((*conn).bundle) };
    *fresh1 = bundle;
    let fresh2 = unsafe { &mut ((*bundle).num_connections) };
    *fresh2 = (*fresh2).wrapping_add(1);
}
extern "C" fn bundle_remove_conn(
    ...
    while !curr.is_null() {
        if (unsafe { (*curr).ptr }) == conn as *mut libc::c_void {
            (unsafe { Curl_llist_remove(&mut (*bundle).conn_list, curr, 0 as *mut libc::c_void) });
            let fresh3 = unsafe { &mut ((*bundle).num_connections) };
            *fresh3 = (*fresh3).wrapping_sub(1);
            let fresh4 = unsafe { &mut ((*conn).bundle) };
...
extern "C" fn free_bundle_hash_entry(mut freethis: *mut libc::c_void) {
    let mut b: *mut connectbundle = freethis as *mut connectbundle;
    bundle_destroy(b);
}
#[no_mangle]
pub extern "C" fn Curl_conncache_init(mut connc: *mut conncache, mut size: i32) -> i32 {
    let mut rc: i32 = 0;
    let fresh5 = unsafe { &mut ((*connc).closure_handle) };
    *fresh5 = unsafe { curl_easy_init() };
    if (unsafe { (*connc).closure_handle }).is_null() {
        return 1 as i32;
    }
    rc = unsafe { Curl_hash_init(
        &mut (*connc).hash,
        size,
    ...
    if rc != 0 {
        (unsafe { Curl_close(&mut (*connc).closure_handle) });
    } else {
        let fresh6 = unsafe { &mut ((*(*connc).closure_handle).state.conn_cache) };
        *fresh6 = connc;
    }
    return rc;
}
#[no_mangle]
pub extern "C" fn Curl_conncache_destroy(mut connc: *mut conncache) {
    if !connc.is_null() {
        (unsafe { Curl_hash_destroy(&mut (*connc).hash) });
    }
}
extern "C" fn hashkey(
    mut conn: *mut connectdata,
    ...
    (unsafe { Curl_strntolower(buf, buf, len) });
}
#[no_mangle]
pub extern "C" fn Curl_conncache_size(mut data: *mut Curl_easy) -> size_t {
    ...
    num = unsafe { (*(*data).state.conn_cache).num_conn };
    if !(unsafe { (*data).share }).is_null() {
...
#[no_mangle]
pub extern "C" fn Curl_conncache_find_bundle(
...
extern "C" fn conncache_add_bundle(
    mut connc: *mut conncache,
    mut key: *mut i8,
    mut bundle: *mut connectbundle,
) -> bool {
    let mut p: *mut libc::c_void = unsafe { Curl_hash_add(
        &mut (*connc).hash,
...
extern "C" fn conncache_remove_bundle(mut connc: *mut conncache, mut bundle: *mut connectbundle) {
    ...
    (unsafe { Curl_hash_start_iterate(&mut (*connc).hash, &mut iter) });
    he = unsafe { Curl_hash_next_element(&mut iter) };
    while !he.is_null() {
        if (unsafe { (*he).ptr }) == bundle as *mut libc::c_void {
...
#[no_mangle]
pub extern "C" fn Curl_conncache_add_conn(mut data: *mut Curl_easy) -> CURLcode {
    let mut current_block: u64;
    let mut result: CURLcode = CURLE_OK;
    let mut bundle: *mut connectbundle = 0 as *mut connectbundle;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut connc: *mut conncache = unsafe { (*data).state.conn_cache };
    bundle = Curl_conncache_find_bundle(data, conn, unsafe { (*data).state.conn_cache }, 0 as *mut *const i8);
    if bundle.is_null() {
        let mut rc: i32 = 0;
        let mut key: [i8; 128] = [0; 128];
        ...
        if result as u64 != 0 {
            ...
            rc = conncache_add_bundle(unsafe { (*data).state.conn_cache }, key.as_mut_ptr(), bundle) as i32;
            if rc == 0 {
    ...
    match current_block {
        17407779659766490442 => {
            ...
            let fresh7 = unsafe { &mut ((*connc).next_connection_id) };
            let fresh8 = *fresh7;
            *fresh7 = *fresh7 + 1;
            (unsafe { (*conn).connection_id = fresh8 });
            let fresh9 = unsafe { &mut ((*connc).num_conn) };
...
#[no_mangle]
pub extern "C" fn Curl_conncache_remove_conn(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut lock: bool,
) {
    let mut bundle: *mut connectbundle = unsafe { (*conn).bundle };
    let mut connc: *mut conncache = unsafe { (*data).state.conn_cache };
    if !bundle.is_null() {
        if lock {
            if !(unsafe { (*data).share }).is_null() {
        ...
        if (unsafe { (*bundle).num_connections }) == 0 as i32 as u64 {
            conncache_remove_bundle(connc, bundle);
        }
        let fresh10 = unsafe { &mut ((*conn).bundle) };
        *fresh10 = 0 as *mut connectbundle;
        if !connc.is_null() {
            let fresh11 = unsafe { &mut ((*connc).num_conn) };
...
#[no_mangle]
pub extern "C" fn Curl_conncache_foreach(
    mut data: *mut Curl_easy,
    mut connc: *mut conncache,
    ...
    while !he.is_null() {
        let mut bundle: *mut connectbundle = 0 as *mut connectbundle;
        bundle = (unsafe { (*he).ptr }) as *mut connectbundle;
        he = unsafe { Curl_hash_next_element(&mut iter) };
        curr = unsafe { (*bundle).conn_list.head };
        while !curr.is_null() {
            let mut conn: *mut connectdata = (unsafe { (*curr).ptr }) as *mut connectdata;
            curr = unsafe { (*curr).next };
            if 1 as i32 == (unsafe { func.expect("non-null function pointer")(data, conn, param) }) {
                if !(unsafe { (*data).share }).is_null() {
                    (unsafe { Curl_share_unlock(data, CURL_LOCK_DATA_CONNECT) });
                }
                return 1 as i32 != 0;
...
extern "C" fn conncache_find_first_connection(mut connc: *mut conncache) -> *mut connectdata {
    let mut iter: Curl_hash_iterator = Curl_hash_iterator {
        hash: 0 as *mut Curl_hash,
        slot_index: 0,
        current_element: 0 as *mut Curl_llist_element,
    ...
    while !he.is_null() {
        ...
        curr = unsafe { (*bundle).conn_list.head };
        if !curr.is_null() {
            return (unsafe { (*curr).ptr }) as *mut connectdata;
        }
        he = unsafe { Curl_hash_next_element(&mut iter) };
    }
...
#[no_mangle]
pub extern "C" fn Curl_conncache_return_conn(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
) -> bool {
    let mut maxconnects: size_t = (if (unsafe { (*(*data).multi).maxconnects }) < 0 as i32 as i64 {
        ((unsafe { (*(*data).multi).num_easy }) * 4 as i32) as i64
    } else {
        unsafe { (*(*data).multi).maxconnects }
    }) as size_t;
    let mut conn_candidate: *mut connectdata = 0 as *mut connectdata;
    (unsafe { (*conn).lastused = Curl_now() });
    if maxconnects > 0 as i32 as u64 && Curl_conncache_size(data) > maxconnects {
        (unsafe { Curl_infof(
            data,
            b"Connection cache is full, closing the oldest one\0" as *const u8 as *const i8,
        ) });
        conn_candidate = Curl_conncache_extract_oldest(data);
        if !conn_candidate.is_null() {
            (unsafe { Curl_disconnect(data, conn_candidate, 0 as i32 != 0) });
        }
    }
    return if conn_candidate == conn {
        0 as i32
    } else {
        1 as i32
    } != 0;
...
#[no_mangle]
pub extern "C" fn Curl_conncache_extract_bundle(
    mut data: *mut Curl_easy,
    mut bundle: *mut connectbundle,
) -> *mut connectdata {
    let mut curr: *mut Curl_llist_element = 0 as *mut Curl_llist_element;
    ...
    curr = unsafe { (*bundle).conn_list.head };
    while !curr.is_null() {
        conn = (unsafe { (*curr).ptr }) as *mut connectdata;
        if (unsafe { (*conn).easyq.size }) == 0 {
            score = unsafe { Curl_timediff(now, (*conn).lastused) };
            if score > highscore {
                highscore = score;
                conn_candidate = conn;
            }
    ...
    if !conn_candidate.is_null() {
        ...
        let fresh12 = unsafe { &mut ((*(*data).state.conn_cache).num_conn) };
        *fresh12 = (*fresh12).wrapping_sub(1);
    }
    return conn_candidate;
}
#[no_mangle]
pub extern "C" fn Curl_conncache_extract_oldest(mut data: *mut Curl_easy) -> *mut connectdata {
    let mut connc: *mut conncache = unsafe { (*data).state.conn_cache };
    let mut iter: Curl_hash_iterator = Curl_hash_iterator {
        hash: 0 as *mut Curl_hash,
        slot_index: 0,
    ...
    while !he.is_null() {
        ...
        while !curr.is_null() {
            ...
            if (unsafe { (*conn).easyq.size }) == 0
                ...
                && (unsafe { ((*conn).bits).connect_only() }) == 0
            {
                score = unsafe { Curl_timediff(now, (*conn).lastused) };
                if score > highscore {
                    highscore = score;
    ...
    if !conn_candidate.is_null() {
        ...
        let fresh13 = unsafe { &mut ((*connc).num_conn) };
...
#[no_mangle]
pub extern "C" fn Curl_conncache_close_all_connections(mut connc: *mut conncache) {
    let mut conn: *mut connectdata = 0 as *mut connectdata;
    let mut buffer: [i8; 1025] = [0; 1025];
    if (unsafe { (*connc).closure_handle }).is_null() {
        return;
    }
    let fresh14 = unsafe { &mut ((*(*connc).closure_handle).state.buffer) };
    *fresh14 = buffer.as_mut_ptr();
    (unsafe { (*(*connc).closure_handle).set.buffer_size = 1024 as i32 as i64 });
    ...
    while !conn.is_null() {
        ...
        sigpipe_ignore(unsafe { (*connc).closure_handle }, &mut pipe_st);
        (unsafe { Curl_conncontrol(conn, 1 as i32) });
        Curl_conncache_remove_conn(unsafe { (*connc).closure_handle }, conn, 1 as i32 != 0);
        (unsafe { Curl_disconnect((*connc).closure_handle, conn, 0 as i32 != 0) });
        sigpipe_restore(&mut pipe_st);
        conn = conncache_find_first_connection(connc);
    }
    let fresh15 = unsafe { &mut ((*(*connc).closure_handle).state.buffer) };
    *fresh15 = 0 as *mut i8;
    if !(unsafe { (*connc).closure_handle }).is_null() {
        ...
        sigpipe_ignore(unsafe { (*connc).closure_handle }, &mut pipe_st_0);
        (unsafe { Curl_hostcache_clean(
