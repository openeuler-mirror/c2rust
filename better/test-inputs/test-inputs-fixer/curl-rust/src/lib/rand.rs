...
extern "C" {
    ...
    fn Curl_ssl_random(data: *mut Curl_easy, buffer: *mut u8, length: size_t) -> CURLcode;
...
extern "C" fn randit(mut data: *mut Curl_easy, mut rnd: *mut u32) -> CURLcode {
    let mut r: u32 = 0;
    let mut result: CURLcode = CURLE_OK;
    static mut randseed: u32 = 0;
    static mut seeded: bool = 0 as i32 != 0;
    result = unsafe { Curl_ssl_random(data, rnd as *mut u8, ::std::mem::size_of::<u32>() as u64) };
    if result as u32 != CURLE_NOT_BUILT_IN as i32 as u32 {
        return result;
    }
    if !(unsafe { seeded }) {
        let mut fd: i32 = unsafe { open(b"/dev/urandom\0" as *const u8 as *const i8, 0 as i32) };
        if fd > -(1 as i32) {
            ...
            if nread as u64 == ::std::mem::size_of::<u32>() as u64 {
                (unsafe { seeded = 1 as i32 != 0 });
            }
            (unsafe { close(fd) });
        }
    }
    if !(unsafe { seeded }) {
        ...
        (unsafe { randseed = randseed.wrapping_add((now.tv_usec as u32).wrapping_add(now.tv_sec as u32)) });
        (unsafe { randseed = randseed
    ...
    (unsafe { randseed = randseed
        ...
        .wrapping_add(12345 as i32 as u32) });
    r = unsafe { randseed };
    (unsafe { *rnd = r << 16 as i32 | r >> 16 as i32 & 0xffff as i32 as u32 });
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_rand(
    ...
    while num != 0 {
        let mut r: u32 = 0;
        let mut left: size_t = if num < ::std::mem::size_of::<u32>() as u64 {
            num
        } else {
            ::std::mem::size_of::<u32>() as u64
        };
        ...
        while left != 0 {
            let fresh0 = rnd;
            rnd = unsafe { rnd.offset(1) };
            (unsafe { *fresh0 = (r & 0xff as i32 as u32) as u8 });
            r >>= 8 as i32;
...
pub extern "C" fn Curl_rand_hex(
    ...
    if num.wrapping_div(2 as i32 as u64) >= ::std::mem::size_of::<[u8; 128]>() as u64
        || num & 1 as i32 as u64 == 0
    {
        return CURLE_BAD_FUNCTION_ARGUMENT;
    }
    num = num.wrapping_sub(1);
    result = Curl_rand(data, buffer.as_mut_ptr(), num.wrapping_div(2 as i32 as u64));
    if result as u64 != 0 {
        return result;
    }
    while num != 0 {
        let fresh1 = rnd;
        rnd = unsafe { rnd.offset(1) };
        (unsafe { *fresh1 = *hex.offset(((*bufp as i32 & 0xf0 as i32) >> 4 as i32) as isize) as u8 });
        let fresh2 = rnd;
        rnd = unsafe { rnd.offset(1) };
        (unsafe { *fresh2 = *hex.offset((*bufp as i32 & 0xf as i32) as isize) as u8 });
        bufp = unsafe { bufp.offset(1) };
        num = (num as u64).wrapping_sub(2 as i32 as u64) as size_t as size_t;
    }
    (unsafe { *rnd = 0 as i32 as u8 });
    return result;
}
