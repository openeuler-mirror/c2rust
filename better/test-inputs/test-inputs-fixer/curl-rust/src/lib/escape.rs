...
pub extern "C" fn Curl_isunreserved(mut in_0: u8) -> bool {
    match in_0 as i32 {
        ...
        | 76 | 77 | 78 | 79 | 80 | 81 | 82 | 83 | 84 | 85 | 86 | 87 | 88 | 89 | 90 | 45 | 46
        | 95 | 126 => return 1 as i32 != 0,
        _ => {}
    }
    return 0 as i32 != 0;
}
#[no_mangle]
pub extern "C" fn curl_escape(mut string: *const i8, mut inlength: i32) -> *mut i8 {
    return curl_easy_escape(0 as *mut CURL, string, inlength);
}
#[no_mangle]
pub extern "C" fn curl_unescape(mut string: *const i8, mut length: i32) -> *mut i8 {
    return curl_easy_unescape(0 as *mut CURL, string, length, 0 as *mut i32);
}
#[no_mangle]
pub extern "C" fn curl_easy_escape(
    ...
    (unsafe { Curl_dyn_init(&mut d, (8000000 as i32 * 3 as i32) as size_t) });
    length = if inlength != 0 {
        inlength as size_t
    } else {
        unsafe { strlen(string) }
    };
    if length == 0 {
    ...
    loop {
        ...
        let mut in_0: u8 = (unsafe { *string }) as u8;
        if Curl_isunreserved(in_0) {
            ...
            if (unsafe { Curl_dyn_add(&mut d, encoded.as_mut_ptr()) }) as u64 != 0 {
                return 0 as *mut i8;
            }
        }
        string = unsafe { string.offset(1) };
    }
    return unsafe { Curl_dyn_ptr(&mut d) };
}
#[no_mangle]
pub extern "C" fn Curl_urldecode(
    ...
    let mut hex: u64 = 0;
    let mut result: CURLcode = CURLE_OK;
    alloc = (if length != 0 { length } else { unsafe { strlen(string) } }).wrapping_add(1 as i32 as u64);
    ns = (unsafe { Curl_cmalloc.expect("non-null function pointer")(alloc) }) as *mut i8;
    if ns.is_null() {
    ...
    loop {
        ...
        let mut in_0: u8 = (unsafe { *string }) as u8;
        if '%' as i32 == in_0 as i32
            && alloc > 2 as i32 as u64
            && (unsafe { Curl_isxdigit(*string.offset(1 as i32 as isize) as u8 as i32) }) != 0
            && (unsafe { Curl_isxdigit(*string.offset(2 as i32 as isize) as u8 as i32) }) != 0
        {
            let mut hexstr: [i8; 3] = [0; 3];
            let mut ptr: *mut i8 = 0 as *mut i8;
            hexstr[0 as i32 as usize] = unsafe { *string.offset(1 as i32 as isize) };
            hexstr[1 as i32 as usize] = unsafe { *string.offset(2 as i32 as isize) };
            hexstr[2 as i32 as usize] = 0 as i32 as i8;
            hex = unsafe { strtoul(hexstr.as_mut_ptr(), &mut ptr, 16 as i32) };
            in_0 = unsafe { curlx_ultouc(hex) };
            if !data.is_null() {
                result = CURLE_OK as i32 as CURLcode;
                if result as u64 != 0 {
                    (unsafe { Curl_cfree.expect("non-null function pointer")(ns as *mut libc::c_void) });
                    return result;
                }
            }
            string = unsafe { string.offset(2 as i32 as isize) };
            alloc = (alloc as u64).wrapping_sub(2 as i32 as u64) as size_t as size_t;
        }
        if ctrl as u32 == REJECT_CTRL as i32 as u32 && (in_0 as i32) < 0x20 as i32
            || ctrl as u32 == REJECT_ZERO as i32 as u32 && in_0 as i32 == 0 as i32
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")(ns as *mut libc::c_void) });
        ...
        (unsafe { *ns.offset(fresh1 as isize) = in_0 as i8 });
        string = unsafe { string.offset(1) };
    }
    (unsafe { *ns.offset(strindex as isize) = 0 as i32 as i8 });
    if !olen.is_null() {
        (unsafe { *olen = strindex });
    }
    (unsafe { *ostring = ns });
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn curl_easy_unescape(
    mut data: *mut Curl_easy,
    mut string: *const i8,
    mut length: i32,
    mut olen: *mut i32,
    ...
    if length >= 0 as i32 {
        ...
        if !olen.is_null() {
            if outputlen <= 2147483647 as i32 as size_t {
                (unsafe { *olen = curlx_uztosi(outputlen) });
            } else {
...
#[no_mangle]
pub extern "C" fn curl_free(mut p: *mut libc::c_void) {
    (unsafe { Curl_cfree.expect("non-null function pointer")(p) });
}
