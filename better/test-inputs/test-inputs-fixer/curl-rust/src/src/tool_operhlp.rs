...
#[no_mangle]
pub extern "C" fn clean_getout(mut config: *mut OperationConfig) {
    if !config.is_null() {
        let mut next: *mut getout = 0 as *mut getout;
        let mut node: *mut getout = unsafe { (*config).url_list };
        while !node.is_null() {
            next = unsafe { (*node).next };
            (unsafe { free((*node).url as *mut libc::c_void) });
            let fresh0 = unsafe { &mut ((*node).url) };
            *fresh0 = 0 as *mut i8;
            (unsafe { free((*node).outfile as *mut libc::c_void) });
            let fresh1 = unsafe { &mut ((*node).outfile) };
            *fresh1 = 0 as *mut i8;
            (unsafe { free((*node).infile as *mut libc::c_void) });
            let fresh2 = unsafe { &mut ((*node).infile) };
            *fresh2 = 0 as *mut i8;
            (unsafe { free(node as *mut libc::c_void) });
            node = 0 as *mut getout;
            node = next;
        }
        let fresh3 = unsafe { &mut ((*config).url_list) };
...
pub extern "C" fn add_file_name_to_url(mut url: *mut i8, mut filename: *const i8) -> *mut i8 {
    ...
    if ptr.is_null() || {
        ...
        encfile = unsafe { curl_easy_escape(curl, filep, 0 as i32) };
        if !encfile.is_null() {
            ...
            (unsafe { curl_free(encfile as *mut libc::c_void) });
...
#[no_mangle]
pub extern "C" fn get_url_file_name(mut filename: *mut *mut i8, mut url: *const i8) -> CURLcode {
    let mut pc: *const i8 = 0 as *const i8;
    let mut pc2: *const i8 = 0 as *const i8;
    (unsafe { *filename = 0 as *mut i8 });
