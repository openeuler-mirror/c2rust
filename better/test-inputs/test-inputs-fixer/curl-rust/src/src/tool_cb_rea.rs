...
pub extern "C" fn tool_read_cb(
    ...
    rc = unsafe { read(
        ...
        sz.wrapping_mul(nmemb),
    ) };
    if rc < 0 as i32 as i64 {
        if (unsafe { *__errno_location() }) == 11 as i32 {
            (unsafe { *__errno_location() = 0 as i32 });
            (unsafe { (*(*in_0).config).readbusy = 1 as i32 != 0 });
            return 0x10000001 as i32 as size_t;
        }
        rc = 0 as i32 as ssize_t;
    }
    (unsafe { (*(*in_0).config).readbusy = 0 as i32 != 0 });
    return rc as size_t;
}
#[no_mangle]
pub extern "C" fn tool_readbusy_cb(
    mut clientp: *mut libc::c_void,
    mut _dltotal: curl_off_t,
    mut _dlnow: curl_off_t,
    mut _ultotal: curl_off_t,
    mut _ulnow: curl_off_t,
) -> i32 {
    ...
    return if (unsafe { (*per).noprogress }) as i32 != 0 {
        0 as i32
    } else {
        0x10000001 as i32
    };
