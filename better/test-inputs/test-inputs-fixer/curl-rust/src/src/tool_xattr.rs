...
extern "C" fn stripcredentials(mut url: *mut *mut i8) -> bool {
    let mut u: *mut CURLU = 0 as *mut CURLU;
    let mut uc: CURLUcode = CURLUE_OK;
    let mut nurl: *mut i8 = 0 as *mut i8;
    u = unsafe { curl_url() };
    if !u.is_null() {
        uc = unsafe { curl_url_set(u, CURLUPART_URL, *url, 0 as i32 as u32) };
        if !(uc as u64 != 0) {
            ...
            if !(uc as u64 != 0) {
                ...
                if !(uc as u64 != 0) {
                    uc = unsafe { curl_url_get(u, CURLUPART_URL, &mut nurl, 0 as i32 as u32) };
                    if !(uc as u64 != 0) {
...
#[no_mangle]
pub extern "C" fn fwrite_xattr(mut curl: *mut CURL, mut fd: i32) -> i32 {
    let mut i: i32 = 0 as i32;
    let mut err: i32 = 0 as i32;
    while err == 0 as i32 && !(unsafe { mappings[i as usize].attr }).is_null() {
        let mut value: *mut i8 = 0 as *mut i8;
        let mut result: CURLcode =
            unsafe { curl_easy_getinfo(curl, mappings[i as usize].info, &mut value as *mut *mut i8) };
        if result as u64 == 0 && !value.is_null() {
            let mut freeptr: bool = 0 as i32 != 0;
            if CURLINFO_EFFECTIVE_URL as i32 as u32 == (unsafe { mappings[i as usize].info }) as u32 {
                freeptr = stripcredentials(&mut value);
            }
            if !value.is_null() {
                err = unsafe { fsetxattr(
                    fd,
                ...
                if freeptr {
                    (unsafe { curl_free(value as *mut libc::c_void) });
                }
            }
        }
        i += 1;
