...
#[no_mangle]
pub static mut curlinfo: *mut curl_version_info_data =
    0 as *const curl_version_info_data as *mut curl_version_info_data;
#[no_mangle]
pub static mut built_in_protos: i64 = 0 as i32 as i64;
#[no_mangle]
pub extern "C" fn get_libcurl_info() -> CURLcode {
    ...
    if !(unsafe { (*curlinfo).protocols }).is_null() {
        ...
        while !(unsafe { *proto }).is_null() {
            ...
            while !(unsafe { (*p).proto_name }).is_null() {
                if (unsafe { curl_strequal(*proto, (*p).proto_name) }) != 0 {
