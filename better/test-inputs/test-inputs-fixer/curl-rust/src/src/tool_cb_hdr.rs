...
pub extern "C" fn tool_header_cb(
    ...
    let mut hdrcbdata: *mut HdrCbData = unsafe { &mut (*per).hdrcbdata };
    let mut outs: *mut OutStruct = unsafe { &mut (*per).outs };
    let mut heads: *mut OutStruct = unsafe { &mut (*per).heads };
    let mut etag_save: *mut OutStruct = unsafe { &mut (*per).etag_save };
    ...
    if !(unsafe { (*(*per).config).headerfile }).is_null() && !(unsafe { (*heads).stream }).is_null() {
        let mut rc: size_t = unsafe { fwrite(ptr as *const libc::c_void, size, nmemb, (*heads).stream) };
        if rc != cb {
            return rc;
        }
        (unsafe { fflush((*heads).stream) });
    }
    if !(unsafe { (*(*per).config).etag_save_file }).is_null() && !(unsafe { (*etag_save).stream }).is_null() {
        ...
        ) }) != 0
        {
            ...
            if (unsafe { *eot }) as i32 == '\n' as i32 {
                while (unsafe { Curl_isspace(*etag_h as u8 as i32) }) != 0 && etag_h < eot {
                    etag_h = unsafe { etag_h.offset(1) };
                }
                while (unsafe { Curl_isspace(*eot as u8 as i32) }) != 0 {
                    eot = unsafe { eot.offset(-1) };
                }
                if eot >= etag_h {
                    let mut etag_length: size_t =
                    ...
                    (unsafe { fflush((*etag_save).stream) });
                }
            }
        }
    }
    (unsafe { curl_easy_getinfo((*per).curl, CURLINFO_PROTOCOL, &mut protocol as *mut i64) });
    if (unsafe { (*hdrcbdata).honor_cd_filename }) as i32 != 0
        && cb > 20 as i32 as u64
        ...
        && protocol & ((1 as i32) << 1 as i32 | (1 as i32) << 0 as i32) as i64 != 0
    {
        ...
        loop {
            ...
            while (unsafe { *p }) as i32 != 0 && p < end && (unsafe { Curl_isalpha(*p as u8 as i32) }) == 0 {
                p = unsafe { p.offset(1) };
            }
            ...
            ) }) != 0
            {
                ...
                if !filename.is_null() {
                    if !(unsafe { (*outs).stream }).is_null() {
                        (unsafe { free(filename as *mut libc::c_void) });
                        return failure;
                    }
                    (unsafe { (*outs).is_cd_filename = 1 as i32 != 0 });
                    (unsafe { (*outs).s_isreg = 1 as i32 != 0 });
                    (unsafe { (*outs).fopened = 0 as i32 != 0 });
                    let fresh0 = unsafe { &mut ((*outs).filename) };
                    *fresh0 = filename;
                    (unsafe { (*outs).alloc_filename = 1 as i32 != 0 });
                    (unsafe { (*hdrcbdata).honor_cd_filename = 0 as i32 != 0 });
                    if !(unsafe { tool_create_output_file(outs, (*per).config) }) {
                        return failure;
                    }
                }
                break;
        ...
        if (unsafe { (*outs).stream }).is_null() && !(unsafe { tool_create_output_file(outs, (*per).config) }) {
            return failure;
        }
    }
    if !(unsafe { (*(*hdrcbdata).config).writeout }).is_null() {
        ...
        if !value.is_null() {
            ...
            let fresh1 = unsafe { &mut ((*per).num_headers) };
            *fresh1 += 1;
    ...
    if (unsafe { (*(*hdrcbdata).config).show_headers }) as i32 != 0
        && protocol
            & ((1 as i32) << 0 as i32
                | (1 as i32) << 1 as i32
                | (1 as i32) << 18 as i32
            ...
            != 0
    {
        ...
        if (unsafe { (*(*hdrcbdata).global).isatty }) as i32 != 0
            && (unsafe { (*(*hdrcbdata).global).styled_output }) as i32 != 0
        {
            value_0 = (unsafe { memchr(ptr as *const libc::c_void, ':' as i32, cb) }) as *mut i8;
        }
        if !value_0.is_null() {
            let mut namelen: size_t = (unsafe { value_0.offset_from(ptr) }) as i64 as size_t;
...
extern "C" fn parse_filename(mut ptr: *const i8, mut len: size_t) -> *mut i8 {
