...
#[no_mangle]
pub extern "C" fn htmlGetMetaEncoding(mut doc: htmlDocPtr) -> *const xmlChar {
    ...
    's_121: loop {
        ...
        if (unsafe { (*cur).type_0 }) as u32 == XML_ELEMENT_NODE as i32 as u32 && !(unsafe { (*cur).name }).is_null() {
            ...
            {
                let mut attr: xmlAttrPtr = unsafe { (*cur).properties };
                let mut http: i32 = 0;
...
#[no_mangle]
pub extern "C" fn htmlSetMetaEncoding(mut doc: htmlDocPtr, mut encoding: *const xmlChar) -> i32 {
    let mut current_block: u64;
    let mut cur: htmlNodePtr = 0 as *mut xmlNode;
...
#[no_mangle]
pub extern "C" fn htmlIsBooleanAttr(mut name: *const xmlChar) -> i32 {
...
extern "C" fn htmlSaveErr(mut code: i32, mut node: xmlNodePtr, mut extra: *const i8) {
...
#[no_mangle]
pub extern "C" fn htmlNodeDumpFile(mut out: *mut FILE, mut doc: xmlDocPtr, mut cur: xmlNodePtr) {
    htmlNodeDumpFileFormat(out, doc, cur, 0 as *const i8, 1 as i32);
}
#[no_mangle]
pub extern "C" fn htmlDocDumpMemoryFormat(
    mut cur: xmlDocPtr,
    ...
    if cur.is_null() {
        (unsafe { *mem = 0 as *mut xmlChar });
        (unsafe { *size = 0 as i32 });
        return;
    }
    ...
    if !(unsafe { (*buf).conv }).is_null() {
        (unsafe { *size = xmlBufUse((*buf).conv) as i32 });
        (unsafe { *mem = xmlStrndup(xmlBufContent((*buf).conv as *const xmlBuf), *size) });
    } else {
        (unsafe { *size = xmlBufUse((*buf).buffer) as i32 });
        (unsafe { *mem = xmlStrndup(xmlBufContent((*buf).buffer as *const xmlBuf), *size) });
    }
    (unsafe { xmlOutputBufferClose(buf) });
}
#[no_mangle]
pub extern "C" fn htmlDocDumpMemory(
    mut cur: xmlDocPtr,
    mut mem: *mut *mut xmlChar,
    mut size: *mut i32,
) {
...
extern "C" fn htmlDtdDumpOutput(
    mut buf: xmlOutputBufferPtr,
    mut doc: xmlDocPtr,
    mut _encoding: *const i8,
) {
    let mut cur: xmlDtdPtr = unsafe { (*doc).intSubset };
    if cur.is_null() {
        htmlSaveErr(
    ...
    if !(unsafe { (*cur).ExternalID }).is_null() {
        ...
        (unsafe { xmlBufWriteQuotedString((*buf).buffer, (*cur).ExternalID) });
        if !(unsafe { (*cur).SystemID }).is_null() {
            (unsafe { xmlOutputBufferWriteString(buf, b" \0" as *const u8 as *const i8) });
            (unsafe { xmlBufWriteQuotedString((*buf).buffer, (*cur).SystemID) });
        }
    ...
    if !(unsafe { (*cur).children }).is_null() && htmlIsBooleanAttr(unsafe { (*cur).name }) == 0 {
        ...
        if !value.is_null() {
                        ...
                        ) }) == 0)
            {
                ...
                if !escaped.is_null() {
                    (unsafe { xmlBufWriteQuotedString((*buf).buffer, escaped) });
                    (unsafe { xmlFree.expect("non-null function pointer")(escaped as *mut libc::c_void) });
                } else {
                    (unsafe { xmlBufWriteQuotedString((*buf).buffer, value) });
                }
            } else {
                (unsafe { xmlBufWriteQuotedString((*buf).buffer, value) });
            }
...
#[no_mangle]
pub extern "C" fn htmlNodeDumpFormatOutput(
    mut buf: xmlOutputBufferPtr,
    mut doc: xmlDocPtr,
    mut cur: xmlNodePtr,
    mut encoding: *const i8,
        ...
        match (unsafe { (*cur).type_0 }) as u32 {
            ...
            1 => {
                if (unsafe { (*cur).parent }) != parent && !(unsafe { (*cur).children }).is_null() {
                    ...
                    if (unsafe { (*cur).ns }).is_null() {
                        info = unsafe { htmlTagLookup((*cur).name) };
                    } else {
            ...
            3 => {
                if !(unsafe { (*cur).content }).is_null() {
                                ...
                                ) }) != 0)
                    {
                        ...
                        buffer = unsafe { xmlEncodeEntitiesReentrant(doc, (*cur).content) };
                        if !buffer.is_null() {
...
#[no_mangle]
pub extern "C" fn htmlNodeDumpOutput(
    mut buf: xmlOutputBufferPtr,
    mut doc: xmlDocPtr,
    mut cur: xmlNodePtr,
    mut _encoding: *const i8,
...
pub extern "C" fn htmlDocContentDumpFormatOutput(
    ...
    htmlNodeDumpFormatOutput(buf, cur, cur as xmlNodePtr, 0 as *const i8, format);
    if !cur.is_null() {
        (unsafe { (*cur).type_0 = type_0 as xmlElementType });
    }
}
#[no_mangle]
pub extern "C" fn htmlDocContentDumpOutput(
    mut buf: xmlOutputBufferPtr,
    mut cur: xmlDocPtr,
    mut _encoding: *const i8,
) {
    htmlNodeDumpFormatOutput(buf, cur, cur as xmlNodePtr, 0 as *const i8, 1 as i32);
}
#[no_mangle]
pub extern "C" fn htmlDocDump(mut f: *mut FILE, mut cur: xmlDocPtr) -> i32 {
    let mut buf: xmlOutputBufferPtr = 0 as *mut xmlOutputBuffer;
    let mut handler: xmlCharEncodingHandlerPtr = 0 as xmlCharEncodingHandlerPtr;
    let mut encoding: *const i8 = 0 as *const i8;
    let mut ret: i32 = 0;
...
#[no_mangle]
pub extern "C" fn htmlSaveFile(mut filename: *const i8, mut cur: xmlDocPtr) -> i32 {
    let mut buf: xmlOutputBufferPtr = 0 as *mut xmlOutputBuffer;
    let mut handler: xmlCharEncodingHandlerPtr = 0 as xmlCharEncodingHandlerPtr;
    let mut encoding: *const i8 = 0 as *const i8;
    let mut ret: i32 = 0;
