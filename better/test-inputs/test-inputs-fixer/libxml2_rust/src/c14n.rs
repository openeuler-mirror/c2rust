...
pub type xmlC14NIsVisibleCallback =
    Option<unsafe extern "C" fn(*mut libc::c_void, xmlNodePtr, xmlNodePtr) -> i32>;
...
extern "C" fn xmlC14NVisibleNsStackCreate() -> xmlC14NVisibleNsStackPtr {
...
extern "C" fn xmlC14NVisibleNsStackDestroy(mut cur: xmlC14NVisibleNsStackPtr) {
    if cur.is_null() {
...
extern "C" fn xmlC14NVisibleNsStackSave(
    mut cur: xmlC14NVisibleNsStackPtr,
    ...
    (unsafe { (*state).nsCurEnd = (*cur).nsCurEnd });
    (unsafe { (*state).nsPrevStart = (*cur).nsPrevStart });
    (unsafe { (*state).nsPrevEnd = (*cur).nsPrevEnd });
}
extern "C" fn xmlC14NVisibleNsStackRestore(
    mut cur: xmlC14NVisibleNsStackPtr,
    mut state: xmlC14NVisibleNsStackPtr,
) {
    if cur.is_null() || state.is_null() {
    ...
    (unsafe { (*cur).nsCurEnd = (*state).nsCurEnd });
    (unsafe { (*cur).nsPrevStart = (*state).nsPrevStart });
    (unsafe { (*cur).nsPrevEnd = (*state).nsPrevEnd });
}
extern "C" fn xmlC14NVisibleNsStackShift(mut cur: xmlC14NVisibleNsStackPtr) {
    if cur.is_null() {
...
extern "C" fn xmlC14NStrEqual(mut str1: *const xmlChar, mut str2: *const xmlChar) -> i32 {
    if str1 == str2 {
        return 1 as i32;
    }
    if str1.is_null() {
...
extern "C" fn xmlC14NVisibleNsStackFind(
    ...
    if !(unsafe { (*cur).nsTab }).is_null() {
        ...
        while i >= start {
            let mut ns1: xmlNsPtr = unsafe { *((*cur).nsTab).offset(i as isize) };
            if xmlC14NStrEqual(
                prefix,
                if !ns1.is_null() {
                    unsafe { (*ns1).prefix }
...
extern "C" fn xmlExcC14NVisibleNsStackFind(
    mut cur: xmlC14NVisibleNsStackPtr,
    mut ns: xmlNsPtr,
...
extern "C" fn xmlC14NIsXmlNs(mut ns: xmlNsPtr) -> i32 {
    return (!ns.is_null()
        && (unsafe { xmlStrEqual(
            (*ns).prefix,
...
extern "C" fn xmlC14NPrintNamespaces(ns: xmlNsPtr, mut ctx: xmlC14NCtxPtr) -> i32 {
    ...
    if !(unsafe { (*ns).href }).is_null() {
        (unsafe { xmlBufWriteQuotedString((*(*ctx).buf).buffer, (*ns).href) });
    } else {
        (unsafe { xmlOutputBufferWriteString((*ctx).buf, b"\"\"\0" as *const u8 as *const i8) });
    }
    return 1 as i32;
...
extern "C" fn xmlC14NProcessNamespacesAxis(
    mut ctx: xmlC14NCtxPtr,
    mut cur: xmlNodePtr,
    mut visible: i32,
) -> i32 {
        ...
        while !ns.is_null() {
                ...
                }) != 0
            {
                ...
                if already_rendered == 0 {
                    (unsafe { xmlListInsert(list, ns as *mut libc::c_void) });
                }
    ...
    (unsafe { xmlListDelete(list) });
    return 0 as i32;
}
extern "C" fn xmlExcC14NProcessNamespacesAxis(
    mut ctx: xmlC14NCtxPtr,
    mut cur: xmlNodePtr,
    mut visible: i32,
) -> i32 {
    ...
    if !(unsafe { (*ctx).inclusive_ns_prefixes }).is_null() {
        let mut prefix: *mut xmlChar = 0 as *mut xmlChar;
        let mut i: i32 = 0;
        i = 0 as i32;
        while !(unsafe { *((*ctx).inclusive_ns_prefixes).offset(i as isize) }).is_null() {
            prefix = unsafe { *((*ctx).inclusive_ns_prefixes).offset(i as isize) };
    ...
    while !attr.is_null() {
            ...
            }) != 0
        {
            ...
            if already_rendered == 0 && visible != 0 {
                (unsafe { xmlListInsert(list, (*attr).ns as *mut libc::c_void) });
            }
...
extern "C" fn xmlC14NIsXmlAttr(mut attr: xmlAttrPtr) -> i32 {
...
extern "C" fn xmlC14NPrintAttrs(mut data: *const libc::c_void, mut user: *mut libc::c_void) -> i32 {
...
extern "C" fn xmlC14NFindHiddenParentAttr(
    mut ctx: xmlC14NCtxPtr,
    mut cur: xmlNodePtr,
...
extern "C" fn xmlC14NFixupBaseAttr(
    mut ctx: xmlC14NCtxPtr,
...
extern "C" fn xmlC14NProcessAttrsAxis(
    mut ctx: xmlC14NCtxPtr,
    mut cur: xmlNodePtr,
    ...
    match (unsafe { (*ctx).mode }) as u32 {
        0 => {
            ...
            while !attr.is_null() {
                ...
                {
                    (unsafe { xmlListInsert(list, attr as *mut libc::c_void) });
                }
                attr = unsafe { (*attr).next };
            }
            if parent_visible != 0
                ...
                while !tmp.is_null() {
                    ...
                    while !attr.is_null() {
                        if xmlC14NIsXmlAttr(attr) != 0 as i32 {
                            if (unsafe { xmlListSearch(list, attr as *mut libc::c_void) }).is_null() {
                                (unsafe { xmlListInsert(list, attr as *mut libc::c_void) });
                            }
                        }
                        attr = unsafe { (*attr).next };
        ...
        2 => {
            ...
            if parent_visible != 0 {
                ...
                if !xml_lang_attr.is_null() {
                    (unsafe { xmlListInsert(list, xml_lang_attr as *mut libc::c_void) });
                }
                ...
                if !xml_space_attr.is_null() {
                    (unsafe { xmlListInsert(list, xml_space_attr as *mut libc::c_void) });
                }
                ...
                if !xml_base_attr.is_null() {
                    ...
                    if !xml_base_attr.is_null() {
                        (unsafe { xmlListInsert(list, xml_base_attr as *mut libc::c_void) });
...
extern "C" fn xmlC14NCheckForRelativeNamespaces(
    mut ctx: xmlC14NCtxPtr,
    mut cur: xmlNodePtr,
) -> i32 {
    let mut ns: xmlNsPtr = 0 as *mut xmlNs;
...
extern "C" fn xmlC14NProcessElementNode(
    mut ctx: xmlC14NCtxPtr,
    mut cur: xmlNodePtr,
    mut visible: i32,
) -> i32 {
    ...
    xmlC14NVisibleNsStackSave(unsafe { (*ctx).ns_rendered }, &mut state);
    if visible != 0 {
    ...
    xmlC14NVisibleNsStackRestore(unsafe { (*ctx).ns_rendered }, &mut state);
    return 0 as i32;
}
extern "C" fn xmlC14NProcessNode(mut ctx: xmlC14NCtxPtr, mut cur: xmlNodePtr) -> i32 {
    let mut ret: i32 = 0 as i32;
...
extern "C" fn xmlC14NProcessNodeList(mut ctx: xmlC14NCtxPtr, mut cur: xmlNodePtr) -> i32 {
    let mut ret: i32 = 0;
    if ctx.is_null() {
...
extern "C" fn xmlC14NFreeCtx(mut ctx: xmlC14NCtxPtr) {
    if ctx.is_null() {
...
extern "C" fn xmlC14NNewCtx(
    ...
    *fresh14 = xmlC14NVisibleNsStackCreate();
    ...
    if (unsafe { (*ctx).mode }) as u32 == XML_C14N_EXCLUSIVE_1_0 as i32 as u32 {
        let fresh15 = unsafe { &mut ((*ctx).inclusive_ns_prefixes) };
        *fresh15 = inclusive_ns_prefixes;
    }
    return ctx;
}
#[no_mangle]
pub extern "C" fn xmlC14NExecute(
    mut doc: xmlDocPtr,
    mut is_visible_callback: xmlC14NIsVisibleCallback,
    mut user_data: *mut libc::c_void,
    mut mode: i32,
...
#[no_mangle]
pub extern "C" fn xmlC14NDocSaveTo(
    mut doc: xmlDocPtr,
    ...
    return xmlC14NExecute(
        ...
        Some(
            xmlC14NIsNodeInNodeset
                as unsafe extern "C" fn(*mut libc::c_void, xmlNodePtr, xmlNodePtr) -> i32,
        ),
...
#[no_mangle]
pub extern "C" fn xmlC14NDocDumpMemory(
    mut doc: xmlDocPtr,
    mut nodes: xmlNodeSetPtr,
    mut mode: i32,
    mut inclusive_ns_prefixes: *mut *mut xmlChar,
    ...
    ret = (unsafe { xmlBufUse((*buf).buffer) }) as i32;
    if ret >= 0 as i32 {
        (unsafe { *doc_txt_ptr = xmlStrndup(xmlBufContent((*buf).buffer as *const xmlBuf), ret) });
    }
    (unsafe { xmlOutputBufferClose(buf) });
    if (unsafe { *doc_txt_ptr }).is_null() && ret >= 0 as i32 {
...
#[no_mangle]
pub extern "C" fn xmlC14NDocSave(
    mut doc: xmlDocPtr,
    mut nodes: xmlNodeSetPtr,
    mut mode: i32,
    mut inclusive_ns_prefixes: *mut *mut xmlChar,
...
extern "C" fn xmlC11NNormalizeString(
