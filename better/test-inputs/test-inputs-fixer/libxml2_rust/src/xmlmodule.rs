...
extern "C" fn xmlModuleErrMemory(mut module: xmlModulePtr, mut extra: *const i8) {
    let mut name: *const i8 = 0 as *const i8;
    if !module.is_null() {
        name = (unsafe { (*module).name }) as *const i8;
    }
...
#[no_mangle]
pub extern "C" fn xmlModuleOpen(mut name: *const i8, mut _options: i32) -> xmlModulePtr {
    ...
    *fresh1 = unsafe { xmlStrdup(name as *const xmlChar) };
    return module;
}
#[no_mangle]
pub extern "C" fn xmlModuleSymbol(
    ...
    rc = xmlModulePlatformSymbol(unsafe { (*module).handle }, name, symbol);
    if rc == -(1 as i32) {
        (unsafe { __xmlRaiseError(
            None,
            None,
...
#[no_mangle]
pub extern "C" fn xmlModuleClose(mut module: xmlModulePtr) -> i32 {
    let mut rc: i32 = 0;
    if module.is_null() {
        (unsafe { __xmlRaiseError(
            None,
...
#[no_mangle]
pub extern "C" fn xmlModuleFree(mut module: xmlModulePtr) -> i32 {
    if module.is_null() {
        (unsafe { __xmlRaiseError(
            None,
            None,
...
extern "C" fn xmlModulePlatformOpen(mut name: *const i8) -> *mut libc::c_void {
    return unsafe { dlopen(name, 0x100 as i32 | 0x2 as i32) };
}
extern "C" fn xmlModulePlatformClose(mut handle: *mut libc::c_void) -> i32 {
    return unsafe { dlclose(handle) };
}
extern "C" fn xmlModulePlatformSymbol(
    mut handle: *mut libc::c_void,
    mut name: *const i8,
    mut symbol: *mut *mut libc::c_void,
) -> i32 {
    (unsafe { *symbol = dlsym(handle, name) });
