...
extern "C" {
    ...
    fn __xmlLoaderErr(ctx: *mut libc::c_void, msg: *const i8, filename: *const i8);
...
#[no_mangle]
pub extern "C" fn xmlErrMemory(mut ctxt: xmlParserCtxtPtr, mut extra: *const i8) {
    if !ctxt.is_null()
...
#[no_mangle]
pub extern "C" fn xmlParserInputRead(mut _in_0: xmlParserInputPtr, mut _len: i32) -> i32 {
    return -(1 as i32);
}
#[no_mangle]
pub extern "C" fn xmlParserInputGrow(mut in_0: xmlParserInputPtr, mut len: i32) -> i32 {
    let mut ret: i32 = 0;
    ...
    if (unsafe { xmlBufUse((*(*in_0).buf).buffer) }) > (indx as u32).wrapping_add(250 as i32 as u32) as u64 {
        return 0 as i32;
    }
    ...
    *fresh0 = unsafe { xmlBufContent((*(*in_0).buf).buffer as *const xmlBuf) };
    let fresh1 = unsafe { &mut ((*in_0).cur) };
    *fresh1 = unsafe { ((*in_0).base).offset(indx as isize) };
    let fresh2 = unsafe { &mut ((*in_0).end) };
    *fresh2 = unsafe { xmlBufEnd((*(*in_0).buf).buffer) };
    return ret;
}
#[no_mangle]
pub extern "C" fn xmlParserInputShrink(mut in_0: xmlParserInputPtr) {
    ...
    if used > 250 as i32 as u64 {
        ret = unsafe { xmlBufShrink((*(*in_0).buf).buffer, used.wrapping_sub(80 as i32 as u64)) };
    ...
    if (unsafe { xmlBufUse((*(*in_0).buf).buffer) }) <= 250 as i32 as u64 {
        (unsafe { xmlParserInputBufferRead((*in_0).buf, 2 as i32 * 250 as i32) });
    }
    let fresh4 = unsafe { &mut ((*in_0).base) };
    *fresh4 = unsafe { xmlBufContent((*(*in_0).buf).buffer as *const xmlBuf) };
    let fresh5 = unsafe { &mut ((*in_0).cur) };
    *fresh5 = unsafe { ((*in_0).base).offset(used as isize) };
    let fresh6 = unsafe { &mut ((*in_0).end) };
    *fresh6 = unsafe { xmlBufEnd((*(*in_0).buf).buffer) };
}
#[no_mangle]
pub extern "C" fn xmlNextChar(mut ctxt: xmlParserCtxtPtr) {
...
#[no_mangle]
pub extern "C" fn xmlCurrentChar(mut ctxt: xmlParserCtxtPtr, mut len: *mut i32) -> i32 {
    let mut current_block: u64;
    if ctxt.is_null() || len.is_null() || (unsafe { (*ctxt).input }).is_null() {
        return 0 as i32;
    }
...
#[no_mangle]
pub extern "C" fn xmlStringCurrentChar(
    mut ctxt: xmlParserCtxtPtr,
...
#[no_mangle]
pub extern "C" fn xmlCopyCharMultiByte(mut out: *mut xmlChar, mut val: i32) -> i32 {
    if out.is_null() {
        return 0 as i32;
    }
    if val >= 0x80 as i32 {
...
#[no_mangle]
pub extern "C" fn xmlCopyChar(mut _len: i32, mut out: *mut xmlChar, mut val: i32) -> i32 {
    if out.is_null() {
        return 0 as i32;
    }
    if val >= 0x80 as i32 {
...
#[no_mangle]
pub extern "C" fn xmlSwitchEncoding(mut ctxt: xmlParserCtxtPtr, mut enc: xmlCharEncoding) -> i32 {
    ...
    handler = unsafe { xmlGetCharEncodingHandler(enc) };
    if handler.is_null() {
        match enc as i32 {
            22 => {
                (unsafe { (*ctxt).charset = XML_CHAR_ENCODING_UTF8 as i32 });
            ...
            _ => {
                __xmlErrEncoding(
                    ...
                    (unsafe { xmlGetCharEncodingName(enc) }) as *mut xmlChar,
                    0 as *const xmlChar,
                );
                (unsafe { xmlStopParser(ctxt) });
                return -(1 as i32);
...
extern "C" fn xmlSwitchInputEncodingInt(
    ...
    if !(unsafe { (*input).buf }).is_null() {
        ...
        if !(unsafe { (*(*input).buf).encoder }).is_null() {
            ...
            (unsafe { xmlCharEncCloseFunc((*(*input).buf).encoder) });
        ...
        *fresh27 = handler;
        if (unsafe { xmlBufIsEmpty((*(*input).buf).buffer) }) == 0 as i32 {
            ...
            (unsafe { xmlBufShrink((*(*input).buf).buffer, processed as size_t) });
            let fresh31 = unsafe { &mut ((*(*input).buf).raw) };
            *fresh31 = unsafe { (*(*input).buf).buffer };
            let fresh32 = unsafe { &mut ((*(*input).buf).buffer) };
            *fresh32 = unsafe { xmlBufCreate() };
            (unsafe { (*(*input).buf).rawconsumed = processed as u64 });
            use_0 = (unsafe { xmlBufUse((*(*input).buf).raw) }) as u32;
            if (unsafe { (*ctxt).html }) != 0 {
                nbchars = unsafe { xmlCharEncInput((*input).buf, 1 as i32) };
            } else {
                nbchars = unsafe { xmlCharEncFirstLineInput((*input).buf, len) };
            }
            ...
            *fresh33 = (*fresh33)
                .wrapping_add((use_0 as u64).wrapping_sub(unsafe { xmlBufUse((*(*input).buf).raw) }));
        }
        return 0 as i32;
    } else {
        xmlErrInternal(
        ...
        (unsafe { xmlCharEncCloseFunc(handler) });
        return -(1 as i32);
    };
}
#[no_mangle]
...
#[no_mangle]
pub extern "C" fn xmlFreeInputStream(mut input: xmlParserInputPtr) {
    if input.is_null() {
        return;
    }
...
#[no_mangle]
pub extern "C" fn xmlNewInputStream(mut ctxt: xmlParserCtxtPtr) -> xmlParserInputPtr {
...
#[no_mangle]
pub extern "C" fn xmlInitParserCtxt(mut ctxt: xmlParserCtxtPtr) -> i32 {
    let mut input: xmlParserInputPtr = 0 as *mut xmlParserInput;
    if ctxt.is_null() {
        xmlErrInternal(
            0 as xmlParserCtxtPtr,
    ...
    (unsafe { xmlDictSetLimit((*ctxt).dict, 10000000 as i32 as size_t) });
    ...
    if (unsafe { (*ctxt).sax }).is_null() {
        ...
        (unsafe { xmlSAXVersion((*ctxt).sax, 2 as i32) });
    }
    ...
    *fresh74 = 0 as xmlDocPtr;
    ...
    (unsafe { (*ctxt).loadsubset = *__xmlLoadExtDtdDefaultValue() });
    if (unsafe { (*ctxt).loadsubset }) != 0 {
        (unsafe { (*ctxt).options |= XML_PARSE_DTDLOAD as i32 });
    }
    (unsafe { (*ctxt).validate = *__xmlDoValidityCheckingDefaultValue() });
    (unsafe { (*ctxt).pedantic = *__xmlPedanticParserDefaultValue() });
    ...
    *fresh78 = Some(
    ...
    if (unsafe { (*ctxt).validate }) != 0 {
        if (unsafe { *__xmlGetWarningsDefaultValue() }) == 0 as i32 {
    ...
    (unsafe { (*ctxt).replaceEntities = *__xmlSubstituteEntitiesDefaultValue() });
    ...
    *fresh81 = 0 as *mut libc::c_void;
    ...
    xmlInitNodeInfoSeq(unsafe { &mut (*ctxt).node_seq });
    return 0 as i32;
}
#[no_mangle]
pub extern "C" fn xmlFreeParserCtxt(mut ctxt: xmlParserCtxtPtr) {
    let mut input: xmlParserInputPtr = 0 as *mut xmlParserInput;
    if ctxt.is_null() {
        return;
    }
    ...
    if !(unsafe { (*ctxt).sax }).is_null() && (unsafe { (*ctxt).sax }) != (unsafe { __xmlDefaultSAXHandler() }) as xmlSAXHandlerPtr {
...
#[no_mangle]
pub extern "C" fn xmlNewParserCtxt() -> xmlParserCtxtPtr {
...
#[no_mangle]
pub extern "C" fn xmlClearParserCtxt(mut ctxt: xmlParserCtxtPtr) {
    if ctxt.is_null() {
        return;
    }
    xmlClearNodeInfoSeq(unsafe { &mut (*ctxt).node_seq });
    (unsafe { xmlCtxtReset(ctxt) });
}
#[no_mangle]
pub extern "C" fn xmlParserFindNodeInfo(
    ...
    pos = xmlParserFindNodeInfoIndex(unsafe { &mut (*ctx).node_seq }, node);
    if pos < (unsafe { (*ctx).node_seq.length })
        && (unsafe { (*((*ctx).node_seq.buffer).offset(pos as isize)).node }) == node as *const _xmlNode
    {
        return (unsafe { &mut *((*ctx).node_seq.buffer).offset(pos as isize) }) as *mut xmlParserNodeInfo;
    } else {
        return 0 as *const xmlParserNodeInfo;
    };
}
#[no_mangle]
pub extern "C" fn xmlInitNodeInfoSeq(mut seq: xmlParserNodeInfoSeqPtr) {
    if seq.is_null() {
        return;
    }
    (unsafe { (*seq).length = 0 as i32 as u64 });
    (unsafe { (*seq).maximum = 0 as i32 as u64 });
    let fresh82 = unsafe { &mut ((*seq).buffer) };
    *fresh82 = 0 as *mut xmlParserNodeInfo;
}
#[no_mangle]
pub extern "C" fn xmlClearNodeInfoSeq(mut seq: xmlParserNodeInfoSeqPtr) {
    if seq.is_null() {
        return;
    }
    if !(unsafe { (*seq).buffer }).is_null() {
        (unsafe { xmlFree.expect("non-null function pointer")((*seq).buffer as *mut libc::c_void) });
    }
    xmlInitNodeInfoSeq(seq);
}
#[no_mangle]
pub extern "C" fn xmlParserFindNodeInfoIndex(
    ...
    while lower <= upper && found == 0 {
        ...
        if node
            == (unsafe { (*((*seq).buffer).offset(middle.wrapping_sub(1 as i32 as u64) as isize)).node })
                as xmlNodePtr
        {
            found = 1 as i32;
        } else if node
...
#[no_mangle]
pub extern "C" fn xmlParserAddNodeInfo(mut ctxt: xmlParserCtxtPtr, info: xmlParserNodeInfoPtr) {
    let mut pos: u64 = 0;
    if ctxt.is_null() || info.is_null() {
        return;
    }
    pos = xmlParserFindNodeInfoIndex(unsafe { &mut (*ctxt).node_seq }, (unsafe { (*info).node }) as xmlNodePtr);
...
pub extern "C" fn xmlPedanticParserDefault(mut val: i32) -> i32 {
    let mut old: i32 = unsafe { *__xmlPedanticParserDefaultValue() };
    (unsafe { *__xmlPedanticParserDefaultValue() = val });
    return old;
}
#[no_mangle]
pub extern "C" fn xmlLineNumbersDefault(mut val: i32) -> i32 {
    let mut old: i32 = unsafe { *__xmlLineNumbersDefaultValue() };
    (unsafe { *__xmlLineNumbersDefaultValue() = val });
    return old;
}
#[no_mangle]
pub extern "C" fn xmlSubstituteEntitiesDefault(mut val: i32) -> i32 {
    let mut old: i32 = unsafe { *__xmlSubstituteEntitiesDefaultValue() };
    (unsafe { *__xmlSubstituteEntitiesDefaultValue() = val });
    return old;
}
#[no_mangle]
pub extern "C" fn xmlKeepBlanksDefault(mut val: i32) -> i32 {
    let mut old: i32 = unsafe { *__xmlKeepBlanksDefaultValue() };
    (unsafe { *__xmlKeepBlanksDefaultValue() = val });
    if val == 0 {
        (unsafe { *__xmlIndentTreeOutput() = 1 as i32 });
    }
    return old;
}
