...
extern "C" fn xmlEntitiesErr(mut code: xmlParserErrors, mut msg: *const i8) {
    (unsafe { __xmlSimpleError(
        XML_FROM_TREE as i32,
        code as i32,
        0 as xmlNodePtr,
...
extern "C" fn xmlFreeEntity(mut entity: xmlEntityPtr) {
    ...
    if !dict.is_null() {
        if !(unsafe { (*entity).name }).is_null() && (unsafe { xmlDictOwns(dict, (*entity).name) }) == 0 {
            (unsafe { xmlFree.expect("non-null function pointer")(
                (*entity).name as *mut i8 as *mut libc::c_void,
            ) });
        }
        if !(unsafe { (*entity).ExternalID }).is_null() && (unsafe { xmlDictOwns(dict, (*entity).ExternalID) }) == 0 {
            (unsafe { xmlFree.expect("non-null function pointer")(
                (*entity).ExternalID as *mut i8 as *mut libc::c_void,
            ) });
        }
        if !(unsafe { (*entity).SystemID }).is_null() && (unsafe { xmlDictOwns(dict, (*entity).SystemID) }) == 0 {
            (unsafe { xmlFree.expect("non-null function pointer")(
                (*entity).SystemID as *mut i8 as *mut libc::c_void,
            ) });
        }
        if !(unsafe { (*entity).URI }).is_null() && (unsafe { xmlDictOwns(dict, (*entity).URI) }) == 0 {
            (unsafe { xmlFree.expect("non-null function pointer")(
                (*entity).URI as *mut i8 as *mut libc::c_void,
            ) });
        }
        if !(unsafe { (*entity).content }).is_null() && (unsafe { xmlDictOwns(dict, (*entity).content) }) == 0 {
            (unsafe { xmlFree.expect("non-null function pointer")(
                (*entity).content as *mut i8 as *mut libc::c_void,
            ) });
        }
        if !(unsafe { (*entity).orig }).is_null() && (unsafe { xmlDictOwns(dict, (*entity).orig) }) == 0 {
            (unsafe { xmlFree.expect("non-null function pointer")(
...
extern "C" fn xmlCreateEntity(
    ...
    if dict.is_null() {
        ...
        *fresh3 = unsafe { xmlDictLookup(dict, name, -(1 as i32)) };
        if !ExternalID.is_null() {
            let fresh4 = unsafe { &mut ((*ret).ExternalID) };
            *fresh4 = unsafe { xmlDictLookup(dict, ExternalID, -(1 as i32)) };
        }
        if !SystemID.is_null() {
            let fresh5 = unsafe { &mut ((*ret).SystemID) };
            *fresh5 = unsafe { xmlDictLookup(dict, SystemID, -(1 as i32)) };
        }
    }
    if !content.is_null() {
        ...
        if !dict.is_null() && (unsafe { (*ret).length }) < 5 as i32 {
            ...
            *fresh6 = (unsafe { xmlDictLookup(dict, content, (*ret).length) }) as *mut xmlChar;
        } else {
    ...
    match type_0 {
        1 | 2 | 3 => {
            ...
            if (unsafe { (*dtd).entities }).is_null() {
                ...
                *fresh11 = (unsafe { xmlHashCreateDict(0 as i32, dict) }) as *mut libc::c_void;
            }
        ...
        4 | 5 => {
            if (unsafe { (*dtd).pentities }).is_null() {
                ...
                *fresh12 = (unsafe { xmlHashCreateDict(0 as i32, dict) }) as *mut libc::c_void;
            }
...
#[no_mangle]
pub extern "C" fn xmlGetPredefinedEntity(mut name: *const xmlChar) -> xmlEntityPtr {
    if name.is_null() {
        return 0 as xmlEntityPtr;
    }
...
#[no_mangle]
pub extern "C" fn xmlGetDtdEntity(mut doc: xmlDocPtr, mut name: *const xmlChar) -> xmlEntityPtr {
    let mut table: xmlEntitiesTablePtr = 0 as *mut xmlEntitiesTable;
    if doc.is_null() {
        return 0 as xmlEntityPtr;
    }
...
extern "C" fn xmlEncodeEntitiesInternal(
    ...
    's_62: loop {
        ...
        let mut indx: size_t = (unsafe { out.offset_from(buffer) }) as i64 as size_t;
        if indx.wrapping_add(100 as i32 as u64) > buffer_size {
            let mut tmp: *mut xmlChar = 0 as *mut xmlChar;
            let mut new_size: size_t = buffer_size.wrapping_mul(2 as i32 as u64);
        ...
        if (unsafe { *cur }) as i32 == '<' as i32 {
                ...
                }
            {
                while cur != end {
                    ...
                    let mut new_size_0: size_t = buffer_size.wrapping_mul(2 as i32 as u64);
                ...
                && !(unsafe { strchr(cur as *const i8, '}' as i32) }).is_null()
            {
                while (unsafe { *cur }) as i32 != '}' as i32 {
                    ...
                    let mut new_size_1: size_t = buffer_size.wrapping_mul(2 as i32 as u64);
...
#[no_mangle]
pub extern "C" fn xmlEncodeSpecialChars(
    mut _doc: *const xmlDoc,
    mut input: *const xmlChar,
) -> *mut xmlChar {
    let mut current_block: u64;
    let mut cur: *const xmlChar = input;
...
#[no_mangle]
pub extern "C" fn xmlCreateEntitiesTable() -> xmlEntitiesTablePtr {
    return (unsafe { xmlHashCreate(0 as i32) }) as xmlEntitiesTablePtr;
}
extern "C" fn xmlFreeEntityWrapper(mut entity: *mut libc::c_void, mut _name: *const xmlChar) {
    if !entity.is_null() {
        xmlFreeEntity(entity as xmlEntityPtr);
    }
}
#[no_mangle]
pub extern "C" fn xmlFreeEntitiesTable(mut table: xmlEntitiesTablePtr) {
    (unsafe { xmlHashFree(
        table,
        Some(xmlFreeEntityWrapper as unsafe extern "C" fn(*mut libc::c_void, *const xmlChar) -> ()),
    ) });
}
...
#[no_mangle]
pub extern "C" fn xmlCopyEntitiesTable(mut table: xmlEntitiesTablePtr) -> xmlEntitiesTablePtr {
...
extern "C" fn xmlDumpEntityContent(mut buf: xmlBufferPtr, mut content: *const xmlChar) {
...
#[no_mangle]
pub extern "C" fn xmlDumpEntityDecl(mut buf: xmlBufferPtr, mut ent: xmlEntityPtr) {
...
#[no_mangle]
pub extern "C" fn xmlDumpEntitiesTable(mut buf: xmlBufferPtr, mut table: xmlEntitiesTablePtr) {
    (unsafe { xmlHashScan(
        table,
        Some(
