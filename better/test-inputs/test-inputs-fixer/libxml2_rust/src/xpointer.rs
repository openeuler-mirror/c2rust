...
extern "C" fn xmlXPtrErr(
    ...
    *fresh0 = (unsafe { xmlStrdup((*ctxt).base) }) as *mut i8;
...
extern "C" fn xmlXPtrGetNthChild(mut cur: xmlNodePtr, mut no: i32) -> xmlNodePtr {
    let mut i: i32 = 0;
...
extern "C" fn xmlXPtrGetChildNo(mut ctxt: xmlXPathParserContextPtr, mut indx: i32) {
...
extern "C" fn xmlXPtrEvalXPtrPart(mut ctxt: xmlXPathParserContextPtr, mut name: *mut xmlChar) {
    ...
    if (unsafe { xmlStrEqual(
        ...
        || (unsafe { xmlStrEqual(name, b"xpath1\0" as *const u8 as *const i8 as *mut xmlChar) }) != 0
    {
        let mut oldBase: *const xmlChar = unsafe { (*ctxt).base };
        let mut oldCur: *const xmlChar = unsafe { (*ctxt).cur };
        ...
        *fresh12 = oldCur;
    } else if (unsafe { xmlStrEqual(name, b"element\0" as *const u8 as *const i8 as *mut xmlChar) }) != 0 {
        let mut oldBase_0: *const xmlChar = unsafe { (*ctxt).base };
        let mut oldCur_0: *const xmlChar = unsafe { (*ctxt).cur };
        ...
        *fresh18 = oldCur_0;
    } else if (unsafe { xmlStrEqual(name, b"xmlns\0" as *const u8 as *const i8 as *mut xmlChar) }) != 0 {
        let mut oldBase_1: *const xmlChar = unsafe { (*ctxt).base };
        let mut oldCur_1: *const xmlChar = unsafe { (*ctxt).cur };
...
extern "C" fn xmlXPtrEvalFullXPtr(mut ctxt: xmlXPathParserContextPtr, mut name: *mut xmlChar) {
    if name.is_null() {
        name = unsafe { xmlXPathParseName(ctxt) };
    }
    if name.is_null() {
    ...
    while !name.is_null() {
        ...
        if !(unsafe { (*ctxt).value }).is_null() {
            let mut obj: xmlXPathObjectPtr = unsafe { (*ctxt).value };
            match (unsafe { (*obj).type_0 }) as u32 {
                1 => {
                    let mut loc: xmlNodeSetPtr = unsafe { (*(*ctxt).value).nodesetval };
...
extern "C" fn xmlXPtrEvalChildSeq(mut ctxt: xmlXPathParserContextPtr, mut name: *mut xmlChar) {
    if name.is_null()
...
extern "C" fn xmlXPtrEvalXPointer(mut ctxt: xmlXPathParserContextPtr) {
...
#[no_mangle]
pub extern "C" fn xmlXPtrNewContext(
    mut doc: xmlDocPtr,
    mut _here: xmlNodePtr,
    mut _origin: xmlNodePtr,
) -> xmlXPathContextPtr {
