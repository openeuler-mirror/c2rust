...
pub extern "C" fn xmlInitializeGlobalState<'a1>(
    ...
    (*(borrow_mut(&mut gs)).unwrap()).xmlBufferAllocScheme =
        unsafe { xmlBufferAllocSchemeThrDef };
    (*(borrow_mut(&mut gs)).unwrap()).xmlDefaultBufferSize =
        unsafe { xmlDefaultBufferSizeThrDef };
    ...
    *fresh8 = unsafe {
    ...
    (*(borrow_mut(&mut gs)).unwrap()).xmlGetWarningsDefaultValue =
        unsafe { xmlGetWarningsDefaultValueThrDef };
    ...
    *fresh9 = unsafe { xmlTreeIndentStringThrDef };
    (*(borrow_mut(&mut gs)).unwrap()).xmlKeepBlanksDefaultValue =
        unsafe { xmlKeepBlanksDefaultValueThrDef };
    (*(borrow_mut(&mut gs)).unwrap()).xmlLineNumbersDefaultValue =
        unsafe { xmlLineNumbersDefaultValueThrDef };
    (*(borrow_mut(&mut gs)).unwrap()).xmlLoadExtDtdDefaultValue =
        unsafe { xmlLoadExtDtdDefaultValueThrDef };
    (*(borrow_mut(&mut gs)).unwrap()).xmlParserDebugEntities =
        unsafe { xmlParserDebugEntitiesThrDef };
    ...
    *fresh18 = unsafe { xmlOutputBufferCreateFilenameValueThrDef };
    (unsafe {
        memset(
            &mut (*(borrow_mut(&mut gs)).unwrap()).xmlLastError as *mut xmlError
                as *mut libc::c_void,
            0 as i32,
...
> {
    if (unsafe { xmlIsMainThread() }) != 0 {
        ...
        return Some(
            unsafe { &mut (*(xmlGetGlobalState)()).xmlParserInputBufferCreateFilenameValue },
        );
    };
}
#[no_mangle]
