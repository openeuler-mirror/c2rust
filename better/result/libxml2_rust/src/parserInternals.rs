...
pub extern "C" fn xmlParserInputGrow(
    ...
    if xmlBufUse(unsafe { (*(*in_0).buf).buffer })
        > (indx as u32).wrapping_add(250 as i32 as u32) as u64
    {
        return 0 as i32;
    }
...
pub extern "C" fn xmlCurrentChar<'a1>(
    ...
    if (unsafe { *(*(*ctxt).input).cur }) as i32 >= 0x20 as i32
        && (unsafe { *(*(*ctxt).input).cur }) as i32 <= 0x7f as i32
    {
    ...
    if (unsafe { (*ctxt).charset }) == XML_CHAR_ENCODING_UTF8 as i32 {
        ...
        if c as i32 & 0x80 as i32 != 0 {
            if !(c as i32 & 0x40 as i32 == 0 as i32 || c as i32 == 0xc0 as i32) {
                ...
                if !((unsafe { *cur.offset(1 as i32 as isize) }) as i32 & 0xc0 as i32
                    != 0x80 as i32)
                {
    ...
    match enc as i32 {
        ...
        1 => {
            ...
            if !(unsafe { (*ctxt).input }).is_null()
                && (unsafe { *((*(*ctxt).input).cur).offset(0 as i32 as isize) }) as i32
                    == 0xef as i32
        ...
        *fresh27 = handler;
        if xmlBufIsEmpty(unsafe { (*(*input).buf).buffer }) == 0 as i32 {
            ...
            if !(unsafe { (*handler).name }).is_null()
                && ((unsafe { strcmp((*handler).name, b"UTF-16LE\0" as *const u8 as *const i8) })
                    == 0
                    || (unsafe { strcmp((*handler).name, b"UTF-16\0" as *const u8 as *const i8) })
                        == 0)
            ...
            if !(unsafe { (*handler).name }).is_null()
                && (unsafe { strcmp((*handler).name, b"UTF-16BE\0" as *const u8 as *const i8) })
                    == 0
            ...
            *fresh33 = (*fresh33).wrapping_add(
                (use_0 as u64).wrapping_sub(xmlBufUse(unsafe { (*(*input).buf).raw })),
            );
        }
        return 0 as i32;
    } else {
...
pub extern "C" fn xmlNewInputStream(
    ...
    input = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlParserInput>() as u64)
...
pub extern "C" fn xmlNewEntityInputStream(
    ...
    *fresh41 =
        (unsafe { &mut *((*entity).content).offset((*input).length as isize) }) as *mut xmlChar;
    return input;
}
#[no_mangle]
...
pub extern "C" fn xmlNewInputFromFile(
    ...
    *fresh46 = (unsafe { xmlCanonicPath(URI as *const xmlChar) }) as *mut i8;
    if !URI.is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")(URI as *mut i8 as *mut libc::c_void)
        });
    }
...
pub extern "C" fn xmlFreeParserCtxt(mut ctxt: *mut crate::src::HTMLparser::_xmlParserCtxt) {
    ...
    if !(unsafe { (*ctxt).sax }).is_null()
        && (unsafe { (*ctxt).sax }) != __xmlDefaultSAXHandler() as xmlSAXHandlerPtr
    {
...
pub extern "C" fn xmlParserFindNodeInfo<'a1, 'a2>(
    ...
    {
        return Some(
            unsafe { &mut *((*(borrow(&ctx)).unwrap()).node_seq.buffer).offset(pos as isize) },
        );
    } else {
        return Option::<&'_ crate::src::HTMLparser::_xmlParserNodeInfo>::None;
    };
...
pub extern "C" fn xmlParserAddNodeInfo(
    ...
    pos = xmlParserFindNodeInfoIndex(
        Some(unsafe { &mut (*ctxt).node_seq }),
        (unsafe { (*info).node }) as xmlNodePtr,
    );
    if pos < (unsafe { (*ctxt).node_seq.length })
        && !(unsafe { (*ctxt).node_seq.buffer }).is_null()
        && (unsafe { (*((*ctxt).node_seq.buffer).offset(pos as isize)).node })
            == (unsafe { (*info).node })
    {
        (unsafe { *((*ctxt).node_seq.buffer).offset(pos as isize) = *info });
    } else {
        if (unsafe { (*ctxt).node_seq.length }).wrapping_add(1 as i32 as u64)
            > (unsafe { (*ctxt).node_seq.maximum })
            || (unsafe { (*ctxt).node_seq.buffer }).is_null()
        {
            ...
            if (unsafe { (*ctxt).node_seq.buffer }).is_null() {
                tmp_buffer =
                    (unsafe { xmlMalloc.expect("non-null function pointer")(byte_size as size_t) })
