...
extern "C" fn xmlRegEpxFromParse(
    ...
    ret = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlRegexp>() as u64)
    }) as xmlRegexpPtr;
    if ret.is_null() {
            ...
            == XML_REGEXP_STRING as i32 as u32
    {
        ...
        while i < (unsafe { (*ret).nbAtoms }) {
                    ...
                    == XML_REGEXP_QUANT_ONCE as i32 as u32
            {
                ...
                (unsafe {
                    xmlFree.expect("non-null function pointer")(stringRemap as *mut libc::c_void)
                });
                i = 0 as i32;
                while i < nbatoms {
                    (unsafe {
        ...
        's_444: loop {
            ...
            if !(stateno == -(1 as i32)) {
                ...
                (unsafe {
                    *transitions.offset((stateno * (nbatoms + 1 as i32)) as isize) =
                        (*state).type_0 as i32
                });
                j = 0 as i32;
                while j < (unsafe { (*state).nbTrans }) {
                    ...
                    {
                        atomno = unsafe {
                            *stringRemap.offset((*(*(borrow(&trans)).unwrap()).atom).no as isize)
                        };
                        if !(unsafe { (*(*(borrow_mut(&mut trans)).unwrap()).atom).data })
                            .is_null()
                            && transdata.is_null()
                        {
                        ...
                        targetno = unsafe {
                            *stateRemap.offset((*(borrow(&trans)).unwrap()).to as isize)
                        };
                        prev = unsafe {
                            *transitions.offset(
                        ...
                        if prev != 0 as i32 {
                            ...
                            if !transdata.is_null() {
                                let fresh5 = unsafe {
                                    &mut (*transdata.offset((stateno * nbatoms + atomno) as isize))
                                };
                                *fresh5 =
                                    unsafe { (*(*(borrow_mut(&mut trans)).unwrap()).atom).data };
                            }
                        }
                    }
                    j += 1;
...
extern "C" fn xmlRegNewRange(
    ...
    ret = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlRegRange>() as u64)
    }) as xmlRegRangePtr;
    if ret.is_null() {
...
extern "C" fn xmlRegFreeRange(mut range: *mut crate::src::xmlregexp::_xmlRegRange) {
    ...
    if !(unsafe { (*range).blockName }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*range).blockName as *mut libc::c_void)
        });
    }
...
extern "C" fn xmlRegNewAtom(
    ...
    ret = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlRegAtom>() as u64)
    }) as xmlRegAtomPtr;
    if ret.is_null() {
...
extern "C" fn xmlRegNewState(
    ...
    ret = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlRegState>() as u64)
    }) as xmlRegStatePtr;
    if ret.is_null() {
...
extern "C" fn xmlRegFreeState(mut state: *mut crate::src::xmlregexp::_xmlAutomataState) {
    ...
    if !(unsafe { (*state).transTo }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*state).transTo as *mut libc::c_void)
        });
    }
...
extern "C" fn xmlRegFreeParserCtxt(mut ctxt: *mut crate::src::xmlregexp::_xmlAutomata) {
    ...
    if !(unsafe { (*ctxt).counters }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*ctxt).counters as *mut libc::c_void)
        });
    }
...
extern "C" fn xmlRegPrintTrans<'a1>(
    ...
    if (unsafe { (*(*(borrow(&trans)).unwrap()).atom).type_0 }) as u32
...
extern "C" fn xmlRegPrintState(
    ...
    while i < (unsafe { (*state).nbTrans }) {
        xmlRegPrintTrans(
            output,
            Some(unsafe { &mut *((*state).trans).offset(i as isize) }),
        );
        i += 1;
    }
}
        ...
        match (unsafe { (*atom).quant }) as u32 {
            ...
            8 => {
                ...
                if (unsafe { (*atom).min }) == 0 as i32 && (unsafe { (*atom).start0 }).is_null() {
                    ...
                    (unsafe {
                        (*((*ctxt).counters).offset(counter as isize)).min = (*atom).min - 1 as i32
                    });
                    (unsafe {
                        (*((*ctxt).counters).offset(counter as isize)).max = (*atom).max - 1 as i32
                    });
        ...
        if !((unsafe { (*((*to).trans).offset(transnr as isize)).to }) < 0 as i32) {
            if (unsafe { (*((*to).trans).offset(transnr as isize)).atom }).is_null() {
                if (unsafe { (*((*to).trans).offset(transnr as isize)).to }) != fromnr {
                    ...
                    } else if (unsafe { (*((*to).trans).offset(transnr as isize)).counter })
                        >= 0 as i32
                    {
            ...
            if !((unsafe { (*state).nbTrans }) != 1 as i32) {
                    ...
                    || (unsafe { (*state).type_0 }) as u32 == XML_REGEXP_FINAL_STATE as i32 as u32)
                {
                    if (unsafe { (*((*state).trans).offset(0 as i32 as isize)).atom }).is_null()
                        ...
                        && (unsafe { (*((*state).trans).offset(0 as i32 as isize)).counter })
                            < 0 as i32
                        && (unsafe { (*((*state).trans).offset(0 as i32 as isize)).count })
                            < 0 as i32
                    {
                        newto = unsafe { (*((*state).trans).offset(0 as i32 as isize)).to };
                        if !((unsafe { (*state).type_0 }) as u32
                            == XML_REGEXP_START_STATE as i32 as u32)
                        {
                            ...
                            while i < (unsafe { (*state).nbTransTo }) {
                                ...
                                while j < (unsafe { (*tmp).nbTrans }) {
                                    if (unsafe { (*((*tmp).trans).offset(j as isize)).to })
                                        == statenr
                                    {
                                        (unsafe {
                                            (*((*tmp).trans).offset(j as isize)).to = -(1 as i32)
                                        });
                                        xmlRegStateAddTrans(
                                            ctxt,
                                            tmp,
...
extern "C" fn xmlFAEliminateEpsilonTransitions(mut ctxt: *mut crate::src::xmlregexp::_xmlAutomata) {
    ...
    while statenr < (unsafe { (*ctxt).nbStates }) {
        state = unsafe { *((*ctxt).states).offset(statenr as isize) };
        if !state.is_null()
            && (unsafe { (*state).type_0 }) as u32 == XML_REGEXP_UNREACH_STATE as i32 as u32
        {
        ...
        if !state.is_null() {
            ...
            while transnr < (unsafe { (*state).nbTrans }) {
                    ...
                    && (unsafe { (*((*state).trans).offset(transnr as isize)).to }) >= 0 as i32
                {
                    ...
                    } else if (unsafe { (*((*state).trans).offset(transnr as isize)).count })
                        < 0 as i32
                    {
                        let mut newto: i32 =
                            unsafe { (*((*state).trans).offset(transnr as isize)).to };
    ...
    while statenr < (unsafe { (*ctxt).nbStates }) {
        state = unsafe { *((*ctxt).states).offset(statenr as isize) };
        if !state.is_null()
            && (unsafe { (*state).reached }) as u32 == XML_REGEXP_MARK_NORMAL as i32 as u32
        {
            xmlRegFreeState(state);
        ...
        || (unsafe { (*range2).type_0 }) as u32 == XML_REGEXP_EPSILON as i32 as u32
    {
        ...
        if (unsafe { (*range1).type_0 }) as u32 == (unsafe { (*range2).type_0 }) as u32 {
            if (unsafe { (*range1).type_0 }) as u32 != XML_REGEXP_CHARVAL as i32 as u32 {
                ret = 1 as i32;
            } else if (unsafe { (*range1).end }) < (unsafe { (*range2).start })
                || (unsafe { (*range2).end }) < (unsafe { (*range1).start })
            {
                ret = 0 as i32;
            } else {
                ret = 1 as i32;
                ...
                || ((unsafe { (*range2).type_0 }) as u32) < XML_REGEXP_LETTER as i32 as u32
            {
                ...
                match (unsafe { (*range1).type_0 }) as u32 {
                    100 => {
                        if (unsafe { (*range2).type_0 }) as u32
                            == XML_REGEXP_LETTER_UPPERCASE as i32 as u32
                            || (unsafe { (*range2).type_0 }) as u32
                                == XML_REGEXP_LETTER_LOWERCASE as i32 as u32
                            || (unsafe { (*range2).type_0 }) as u32
                                == XML_REGEXP_LETTER_TITLECASE as i32 as u32
                            || (unsafe { (*range2).type_0 }) as u32
    ...
    while statenr < (unsafe { (*ctxt).nbStates }) {
        ...
        if !state.is_null() {
            if !((unsafe { (*state).nbTrans }) < 2 as i32) {
                transnr = 0 as i32;
                while transnr < (unsafe { (*state).nbTrans }) {
                    t1 = (unsafe { &mut *((*state).trans).offset(transnr as isize) })
                        as *mut xmlRegTrans;
...
extern "C" fn xmlFARegExecSave(mut exec: *mut crate::src::xmlregexp::_xmlRegExecCtxt) {
    ...
    *fresh56 += 1;
    if (unsafe { (*exec).maxRollbacks }) == 0 as i32 {
        ...
        *fresh58 = tmp;
        tmp =
            (unsafe { &mut *((*exec).rollbacks).offset(len as isize) }) as *mut xmlRegExecRollback;
        (unsafe {
            memset(
    ...
    let fresh59 =
        unsafe { &mut ((*((*exec).rollbacks).offset((*exec).nbRollbacks as isize)).state) };
    *fresh59 = unsafe { (*exec).state };
    ...
    if (unsafe { (*(*exec).comp).nbCounters }) > 0 as i32 {
        if (unsafe { (*((*exec).rollbacks).offset((*exec).nbRollbacks as isize)).counts })
            .is_null()
        {
            let fresh60 = unsafe {
                &mut ((*((*exec).rollbacks).offset((*exec).nbRollbacks as isize)).counts)
            };
            *fresh60 = (unsafe {
                xmlMalloc.expect("non-null function pointer")(
    ...
    *fresh68 = 0 as xmlRegInputTokenPtr;
    ...
    's_105: while (unsafe { (*exec).status }) == 0 as i32
        ...
        && ((unsafe { *((*exec).inputString).offset((*exec).index as isize) }) as i32 != 0 as i32
            || !(unsafe { (*exec).state }).is_null()
                && (unsafe { (*(*exec).state).type_0 }) as u32
                    != XML_REGEXP_FINAL_STATE as i32 as u32)
    {
        let mut trans: Option<&'_ mut crate::src::xmlregexp::_xmlRegTrans> =
            Option::<&'_ mut crate::src::xmlregexp::_xmlRegTrans>::None;
            ...
            && (unsafe { (*exec).counts }).is_null()
        {
            if (unsafe { (*exec).transno }) < (unsafe { (*(*exec).state).nbTrans }) {
                trans = Some(
                    unsafe { &mut *((*(*exec).state).trans).offset((*exec).transno as isize) },
                );
                if (*(borrow(&trans)).unwrap()).to >= 0 as i32 {
        ...
        match current_block {
            17281240262373992796 => {
                ...
                loop {
                    ...
                    trans = Some(
                        unsafe {
                            &mut *((*(*exec).state).trans).offset((*exec).transno as isize)
                        },
                    );
                    if !((*(borrow(&trans)).unwrap()).to < 0 as i32) {
                        atom = (*(borrow_mut(&mut trans)).unwrap()).atom;
                        ...
                        if (*(borrow(&trans)).unwrap()).count >= 0 as i32 {
                            ...
                            current_block = 2872334340672008580;
                            break;
                        } else if (unsafe { *((*exec).inputString).offset((*exec).index as isize) })
                            as i32
                            != 0 as i32
                        {
                            codepoint = unsafe {
                            ...
                            if ret == 1 as i32
                                ...
                                && (unsafe { (*atom).max }) > 0 as i32
                            {
                                let mut to: *mut crate::src::xmlregexp::_xmlAutomataState = unsafe {
                                    *((*comp).states)
                                ...
                                if ret == 1 as i32 {
                                    ...
                                    match current_block {
                                        ...
                                        _ => {
                                            ...
                                            *fresh77 = unsafe {
                                                *((*comp).states).offset(
                                                    (*(borrow(&trans)).unwrap()).to as isize,
                                                )
                                            };
                                            (unsafe { (*exec).transno = 0 as i32 });
                ...
                match current_block {
                    ...
                    _ => {
                        if !((unsafe { (*exec).transno }) != 0 as i32
                            || (unsafe { (*(*exec).state).nbTrans }) == 0 as i32)
                        {
                            continue;
                        }
                    }
    ...
    if !(unsafe { (*exec).rollbacks }).is_null() {
        ...
        (unsafe {
            xmlFree.expect("non-null function pointer")((*exec).rollbacks as *mut libc::c_void)
        });
    }
...
pub extern "C" fn xmlRegNewExecCtxt(
    ...
    exec = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlRegExecCtxt>() as u64)
    ...
    *fresh84 = data;
    if (unsafe { (*comp).nbCounters }) > 0 as i32 {
        ...
        *fresh86 =
            (unsafe { &mut *((*exec).counts).offset((*comp).nbCounters as isize) }) as *mut i32;
    } else {
...
pub extern "C" fn xmlRegFreeExecCtxt(mut exec: *mut crate::src::xmlregexp::_xmlRegExecCtxt) {
    ...
    if !(unsafe { (*exec).inputStack }).is_null() {
        ...
        (unsafe {
            xmlFree.expect("non-null function pointer")((*exec).inputStack as *mut libc::c_void)
        });
    }
    if !(unsafe { (*exec).errString }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*exec).errString as *mut libc::c_void)
        });
    }
    (unsafe { xmlFree.expect("non-null function pointer")(exec as *mut libc::c_void) });
}
extern "C" fn xmlFARegExecSaveInputString(
    ...
    let fresh93 =
        unsafe { &mut ((*((*exec).inputStack).offset((*exec).inputStackNr as isize)).value) };
    *fresh93 = xmlStrdup(value);
    let fresh94 =
        unsafe { &mut ((*((*exec).inputStack).offset((*exec).inputStackNr as isize)).data) };
    *fresh94 = data;
...
extern "C" fn xmlRegCompactPushString(
    ...
    if comp.is_null()
        || (unsafe { (*comp).compact }).is_null()
        || (unsafe { (*comp).stringMap }).is_null()
    {
        return -(1 as i32);
    }
    if value.is_null() {
    ...
    while i < (unsafe { (*comp).nbstrings }) {
        ...
        if target > 0 as i32 && target <= (unsafe { (*comp).nbstates }) {
            target -= 1;
            if xmlRegStrEqualWildcard(unsafe { *((*comp).stringMap).offset(i as isize) }, value)
                != 0
            {
                (unsafe { (*exec).index = target });
                if (unsafe { ((*exec).callback).is_some() })
                    && !(unsafe { (*comp).transdata }).is_null()
                {
                    (unsafe {
                ...
                if (unsafe {
                    *((*comp).compact).offset((target * ((*comp).nbstrings + 1 as i32)) as isize)
...
extern "C" fn xmlRegExecPushStringInternal(
                ...
                if !((unsafe { (*trans).to }) < 0 as i32) {
                    ...
                    if (unsafe { (*trans).count }) == 0x123457 as i32 {
                        ...
                        if value.is_null() && final_0 != 0 {
                            ...
                            while i < (unsafe { (*(*exec).state).nbTrans }) {
                                ...
                                if !((unsafe { (*t).counter }) < 0 as i32 || t == trans) {
                                    ...
                                    count = unsafe {
                                        *((*exec).counts).offset((*t).counter as isize)
                                    };
                                    if count < (*(borrow(&counter)).unwrap()).max
                                        && !(unsafe { (*t).atom }).is_null()
                                        && xmlStrEqual(
                                            value,
                                            (unsafe { (*(*t).atom).valuep }) as *const xmlChar,
                                        ) != 0
                                    {
                                        ret = 0 as i32;
                                        break;
                        ...
                        while i_0 < (unsafe { (*(*exec).state).nbTrans }) {
                            ...
                            if !((unsafe { (*t_0).counter }) < 0 as i32 || t_0 == trans) {
                                ...
                                count_0 =
                                    unsafe { *((*exec).counts).offset((*t_0).counter as isize) };
                        ...
                        counter_1 = Some(
                            unsafe {
                                &mut *((*(*exec).comp).counters).offset((*trans).count as isize)
                            },
                        );
                        ...
                        if ret == 1 as i32 && (unsafe { (*trans).counter }) >= 0 as i32 {
                            ...
                            count_2 =
                                unsafe { *((*exec).counts).offset((*trans).counter as isize) };
                            counter_2 = Some(
                                unsafe {
                                    &mut *((*(*exec).comp).counters)
                    ...
                    if ret == 1 as i32 {
                        ...
                        if (unsafe { (*trans).counter }) >= 0 as i32 {
                            let fresh105 = unsafe {
                                &mut (*((*exec).counts).offset((*trans).counter as isize))
                            };
                            *fresh105 += 1;
                        }
                        ...
                        {
                            (unsafe {
                                *((*exec).counts).offset((*trans).count as isize) = 0 as i32
                            });
                        }
    ...
    if (unsafe { (*exec).status }) == 0 as i32 {
        return ((unsafe { (*(*exec).state).type_0 }) as u32 == XML_REGEXP_FINAL_STATE as i32 as u32)
            as i32;
    }
    return unsafe { (*exec).status };
}
...
pub extern "C" fn xmlRegExecPushString2(
    ...
    if (150 as i32) < lenn + lenp + 2 as i32 {
        str = (unsafe {
            xmlMallocAtomic.expect("non-null function pointer")((lenn + lenp + 2 as i32) as size_t)
        }) as *mut xmlChar;
        if str.is_null() {
            (unsafe { (*exec).status = -(1 as i32) });
            return -(1 as i32);
    ...
    *(borrow_mut(&mut nbneg)).unwrap() = 0 as i32;
    if !(unsafe { (*exec).comp }).is_null() && !(unsafe { (*(*exec).comp).compact }).is_null() {
        ...
        if !borrow(&terminal).is_none() {
            if (unsafe {
                *((*comp).compact).offset((state * ((*comp).nbstrings + 1 as i32)) as isize)
            }) == XML_REGEXP_FINAL_STATE as i32
            {
                    ...
                    if (*(borrow(&trans)).unwrap()).count == 0x123457 as i32 {
                        ...
                        if borrow(&counter).is_none() || count < (*(borrow(&counter)).unwrap()).max
                        {
                            if (unsafe { (*atom).neg }) != 0 {
                                ...
                                nb = nb + 1;
                                let fresh118 =
                                    unsafe { &mut (*values.offset(fresh117 as isize)) };
                                *fresh118 = (unsafe { (*atom).valuep2 }) as *mut xmlChar;
                            } else {
                                let mut fresh119 = nb;
                                nb = nb + 1;
                                let fresh120 =
                                    unsafe { &mut (*values.offset(fresh119 as isize)) };
                            ...
                            if !((*(borrow(&trans)).unwrap()).counter >= 0 as i32) {
                                        ...
                                        == XML_REGEXP_SINK_STATE as i32 as u32
                                {
                                    if (unsafe { (*atom).neg }) != 0 {
                                        ...
                                        nb = nb + 1;
                                        let fresh126 =
                                            unsafe { &mut (*values.offset(fresh125 as isize)) };
                                        *fresh126 = (unsafe { (*atom).valuep2 }) as *mut xmlChar;
                                    } else {
                                        let mut fresh127 = nb;
                                        nb = nb + 1;
                                        let fresh128 =
                                            unsafe { &mut (*values.offset(fresh127 as isize)) };
    ...
    if cur == 'L' as i32 {
        ...
        *fresh166 = unsafe { (*fresh166).offset(1) };
        ...
        blockName = xmlStrndup(
            start,
            (unsafe { ((*ctxt).cur).offset_from(start) }) as i64 as i32,
        );
    } else {
        (unsafe { (*ctxt).error = XML_REGEXP_COMPILE_ERROR as i32 });
...
pub extern "C" fn xmlRegFreeRegexp(mut regexp: *mut crate::src::xmlregexp::_xmlRegexp) {
    ...
    if !(unsafe { (*regexp).string }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*regexp).string as *mut libc::c_void)
        });
    }
    if !(unsafe { (*regexp).states }).is_null() {
        ...
        (unsafe {
            xmlFree.expect("non-null function pointer")((*regexp).states as *mut libc::c_void)
        });
    }
    if !(unsafe { (*regexp).atoms }).is_null() {
        ...
        (unsafe {
            xmlFree.expect("non-null function pointer")((*regexp).atoms as *mut libc::c_void)
        });
    }
    if !(unsafe { (*regexp).counters }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*regexp).counters as *mut libc::c_void)
        });
    }
    if !(unsafe { (*regexp).compact }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*regexp).compact as *mut libc::c_void)
        });
    }
    if !(unsafe { (*regexp).transdata }).is_null() {
        (unsafe {
            xmlFree.expect("non-null function pointer")((*regexp).transdata as *mut libc::c_void)
        });
    }
    if !(unsafe { (*regexp).stringMap }).is_null() {
        ...
        (unsafe {
            xmlFree.expect("non-null function pointer")((*regexp).stringMap as *mut libc::c_void)
        });
    }
