...
extern "C" fn xmlEscapeEntities(
    ...
    loop {
        ...
        if (unsafe { *in_0 }) as i32 == '<' as i32 {
            ...
            in_0 = unsafe { in_0.offset(1) };
        } else if (unsafe { *in_0 }) as i32 >= 0x20 as i32
            && ((unsafe { *in_0 }) as i32) < 0x80 as i32
            || (unsafe { *in_0 }) as i32 == '\n' as i32
...
extern "C" fn xmlSaveCtxtInit<'a1>(mut ctxt: *mut crate::src::xmlsave::_xmlSaveCtxt<'a1>) {
    ...
    if (unsafe { *__xmlTreeIndentString() }).is_null() || len == 0 as i32 {
        ...
        (unsafe {
            (*ctxt).indent[((*ctxt).indent_nr * (*ctxt).indent_size) as usize] = 0 as i32 as i8
        });
    }
...
extern "C" fn xmlNewSaveCtxt<'a1>(
    ...
    ret = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlSaveCtxt>() as u64)
    }) as xmlSaveCtxtPtr;
    if ret.is_null() {
    ...
    xmlSaveCtxtInit(ret);
    if (unsafe { (*ret).options }) & XML_SAVE_NO_EMPTY as i32 != 0
...
extern "C" fn xmlDtdDumpOutput<'a1>(
    ...
    if !(unsafe { (*dtd).notations }).is_null()
        && ((unsafe { (*dtd).doc }).is_null() || (unsafe { (*(*dtd).doc).intSubset }) == dtd)
    {
        xmlBufDumpNotationTable(
            unsafe { (*buf).buffer },
            (unsafe { (*dtd).notations }) as xmlNotationTablePtr,
        );
    }
...
extern "C" fn xmlNodeDumpOutputInternal<'a1>(
    ...
    loop {
        match (unsafe { (*cur).type_0 }) as u32 {
            ...
            1 => {
                if cur != root
                    && (unsafe { (*ctxt).format }) == 1 as i32
                    && (unsafe { *__xmlIndentTreeOutput() }) != 0
                {
                    xmlOutputBufferWrite(
                        buf,
                        (unsafe { (*ctxt).indent_size })
                ...
                if (unsafe { (*cur).parent }) != parent && !(unsafe { (*cur).children }).is_null()
                {
                        ...
                        && !(unsafe { (*(*cur).ns).prefix }).is_null()
                    {
                        xmlOutputBufferWriteString(
                            buf,
                            (unsafe { (*(*cur).ns).prefix }) as *const i8,
                        );
                        xmlOutputBufferWrite(buf, 1 as i32, b":\0" as *const u8 as *const i8);
                    }
                    xmlOutputBufferWriteString(buf, (unsafe { (*cur).name }) as *const i8);
        ...
        || (unsafe { (*ctxt).options }) & XML_SAVE_AS_HTML as i32 != 0
    {
            ...
            || (unsafe { (*ctxt).options }) & XML_SAVE_XHTML as i32 != 0
        {
            ...
            if (unsafe { (*ctxt).options }) & XML_SAVE_NO_XHTML as i32 == 0 as i32 {
                ...
                if !dtd.is_null() {
                    is_xhtml =
                        xmlIsXHTML(unsafe { (*dtd).SystemID }, unsafe { (*dtd).ExternalID });
    ...
    while !cur.is_null() {
            ...
            ) != 0
        {
            ...
            && ((unsafe { (*cur).children }).is_null()
                ...
                || (unsafe { *((*(*cur).children).content).offset(0 as i32 as isize) }) as i32
                    == 0 as i32)
                        ...
                        && (xhtmlIsEmpty(cur) == 1 as i32 && addmeta == 0 as i32)
                    {
                        ...
                        if addmeta == 1 as i32 {
                            ...
                            if !(unsafe { (*ctxt).encoding }).is_null() {
                                xmlOutputBufferWriteString(
                                    buf,
                                    (unsafe { (*ctxt).encoding }) as *const i8,
                                );
                            } else {
                                xmlOutputBufferWrite(
                                    buf,
