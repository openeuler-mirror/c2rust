...
extern "C" fn libxml_domnode_binary_insertion_sort_start(
    ...
    while i < size {
        ...
        ) <= 0 as i32)
        {
            ...
            while j >= location {
                let fresh0 =
                    unsafe { &mut (*dst.offset(j.wrapping_add(1 as i32 as u64) as isize)) };
    ...
    if start >= size.wrapping_sub(2 as i32 as u64) {
        ...
        ) > 0 as i32
        {
            let mut __SORT_SWAP_t: xmlNodePtr =
                unsafe { *dst.offset(size.wrapping_sub(2 as i32 as u64) as isize) };
            let fresh4 =
                unsafe { &mut (*dst.offset(size.wrapping_sub(2 as i32 as u64) as isize)) };
            *fresh4 = unsafe { *dst.offset(size.wrapping_sub(1 as i32 as u64) as isize) };
            let fresh5 =
                unsafe { &mut (*dst.offset(size.wrapping_sub(1 as i32 as u64) as isize)) };
    ...
    if A < B {
        ...
        while k < curr.wrapping_add(A).wrapping_add(B) {
            if i < A && j < curr.wrapping_add(A).wrapping_add(B) {
                if wrap_cmp(
                    unsafe { *storage.offset(i as isize) },
                    unsafe { *dst.offset(j as isize) },
                ) <= 0 as i32
                {
                    let fresh7 = i;
                    i = i.wrapping_add(1);
                ...
                <= (unsafe { (*stack.offset(1 as i32 as isize)).length })
        {
            ...
            *fresh18 = (*fresh18 as u64)
            ...
            if BCD != 0 && CD == 0 {
                ...
                let fresh19 =
                    unsafe { &mut ((*stack.offset((stack_curr - 3 as i32) as isize)).length) };
                *fresh19 = (*fresh19 as u64).wrapping_add(
                ...
                libxml_domnode_tim_sort_merge(dst, stack, stack_curr, store);
                let fresh20 =
                    unsafe { &mut ((*stack.offset((stack_curr - 2 as i32) as isize)).length) };
                *fresh20 = (*fresh20 as u64).wrapping_add(
...
extern "C" fn PUSH_NEXT(
    ...
    if run > len {
        libxml_domnode_binary_insertion_sort_start(
            unsafe { &mut *dst.offset(*curr as isize) },
            len,
            run,
        );
        len = run;
    ...
    if (unsafe { *curr }) == size {
        while (unsafe { *stack_curr }) > 1 as i32 as u64 {
            ...
            let fresh21 = unsafe {
                &mut ((*run_stack.offset((*stack_curr).wrapping_sub(2 as i32 as u64) as isize))
                    .length)
            };
            *fresh21 = (*fresh21 as u64).wrapping_add(
                unsafe {
                    (*run_stack.offset((*stack_curr).wrapping_sub(1 as i32 as u64) as isize)).length
                },
            ) as size_t as size_t;
...
pub extern "C" fn xmlXPathErr(mut ctxt: xmlXPathParserContextPtr, mut error: i32) {
    ...
    *fresh25 = (unsafe { xmlStrdup((*ctxt).base) }) as *mut i8;
    (unsafe {
        (*(*ctxt).context).lastError.int1 = ((*ctxt).cur).offset_from((*ctxt).base) as i64 as i32
    });
...
extern "C" fn xmlXPathCheckOpLimit(mut ctxt: xmlXPathParserContextPtr, mut opCount: u64) -> i32 {
    ...
    if opCount > (unsafe { (*xpctxt).opLimit })
        || (unsafe { (*xpctxt).opCount }) > (unsafe { (*xpctxt).opLimit }).wrapping_sub(opCount)
    {
...
extern "C" fn xmlXPathCompExprAdd(
            ...
            || op as u32 == XPATH_OP_COLLECT as i32 as u32)
    {
        if !value4.is_null() {
            let fresh35 =
                unsafe { &mut ((*((*comp).steps).offset((*comp).nbStep as isize)).value4) };
            *fresh35 =
                (unsafe { xmlDictLookup((*comp).dict, value4 as *const xmlChar, -(1 as i32)) })
        ...
        if !value5.is_null() {
            let fresh37 =
                unsafe { &mut ((*((*comp).steps).offset((*comp).nbStep as isize)).value5) };
            *fresh37 =
                (unsafe { xmlDictLookup((*comp).dict, value5 as *const xmlChar, -(1 as i32)) })
                    as *mut libc::c_void as *mut xmlChar as *mut libc::c_void;
    ...
    match (unsafe { (*op).op }) as u32 {
        ...
        10 => {
            ...
            match axis as u32 {
                ...
                2 => {
                    (unsafe {
                        fprintf(output, b" 'ancestors-or-self' \0" as *const u8 as *const i8)
                    });
                }
                3 => {
                ...
                12 => {
                    (unsafe {
                        fprintf(output, b" 'preceding-sibling' \0" as *const u8 as *const i8)
                    });
                }
                13 => {
...
extern "C" fn xmlXPathCacheWrapNodeSet(
    ...
    if !ctxt.is_null() && !(unsafe { (*ctxt).cache }).is_null() {
        let mut cache: xmlXPathContextCachePtr =
            (unsafe { (*ctxt).cache }) as xmlXPathContextCachePtr;
        if !(unsafe { (*cache).miscObjs }).is_null()
            && (unsafe { (*(*cache).miscObjs).number }) != 0 as i32
        {
            let mut ret: xmlXPathObjectPtr = 0 as *mut xmlXPathObject;
            let fresh46 = unsafe { &mut ((*(*cache).miscObjs).number) };
            *fresh46 -= 1;
            ret = (unsafe { *((*(*cache).miscObjs).items).offset(*fresh46 as isize) })
...
extern "C" fn xmlXPathCacheWrapString(
    ...
    if !ctxt.is_null() && !(unsafe { (*ctxt).cache }).is_null() {
        let mut cache: xmlXPathContextCachePtr =
            (unsafe { (*ctxt).cache }) as xmlXPathContextCachePtr;
        if !(unsafe { (*cache).stringObjs }).is_null()
            && (unsafe { (*(*cache).stringObjs).number }) != 0 as i32
        {
            let mut ret: xmlXPathObjectPtr = 0 as *mut xmlXPathObject;
            let fresh48 = unsafe { &mut ((*(*cache).stringObjs).number) };
            *fresh48 -= 1;
            ret = (unsafe { *((*(*cache).stringObjs).items).offset(*fresh48 as isize) })
                as xmlXPathObjectPtr;
            ...
            *fresh49 = val;
                ...
                && (unsafe { (*(*cache).miscObjs).number }) != 0 as i32
            {
                ...
                *fresh50 -= 1;
                ret_0 = (unsafe { *((*(*cache).miscObjs).items).offset(*fresh50 as isize) })
                    as xmlXPathObjectPtr;
...
extern "C" fn xmlXPathCacheNewNodeSet(
    ...
    if !ctxt.is_null() && !(unsafe { (*ctxt).cache }).is_null() {
        let mut cache: xmlXPathContextCachePtr =
            (unsafe { (*ctxt).cache }) as xmlXPathContextCachePtr;
        if !(unsafe { (*cache).nodesetObjs }).is_null()
            && (unsafe { (*(*cache).nodesetObjs).number }) != 0 as i32
        {
            let mut ret: xmlXPathObjectPtr = 0 as *mut xmlXPathObject;
            let fresh52 = unsafe { &mut ((*(*cache).nodesetObjs).number) };
            *fresh52 -= 1;
            ret = (unsafe { *((*(*cache).nodesetObjs).items).offset(*fresh52 as isize) })
                as xmlXPathObjectPtr;
            (unsafe { (*ret).type_0 = XPATH_NODESET });
            ...
            if !val.is_null() {
                    ...
                    || (unsafe { (*val).type_0 }) as u32 == XML_NAMESPACE_DECL as i32 as u32
                {
                    ...
                    let fresh53 = unsafe {
                        &mut (*((*(*ret).nodesetval).nodeTab).offset(0 as i32 as isize))
                    };
                    *fresh53 = val;
                ...
                && (unsafe { (*(*cache).miscObjs).number }) != 0 as i32
            {
                ...
                *fresh54 -= 1;
                ret_0 = (unsafe { *((*(*cache).miscObjs).items).offset(*fresh54 as isize) })
                    as xmlXPathObjectPtr;
...
extern "C" fn xmlXPathCacheNewCString(
    ...
    if !ctxt.is_null() && !(unsafe { (*ctxt).cache }).is_null() {
            ...
            && (unsafe { (*(*cache).stringObjs).number }) != 0 as i32
        {
            ...
            *fresh56 -= 1;
            ret = (unsafe { *((*(*cache).stringObjs).items).offset(*fresh56 as isize) })
                as xmlXPathObjectPtr;
            (unsafe { (*ret).type_0 = XPATH_STRING });
            ...
            *fresh57 = unsafe { xmlStrdup(val as *mut xmlChar) };
                ...
                && (unsafe { (*(*cache).miscObjs).number }) != 0 as i32
            {
                ...
                *fresh58 -= 1;
                ret_0 = (unsafe { *((*(*cache).miscObjs).items).offset(*fresh58 as isize) })
                    as xmlXPathObjectPtr;
                (unsafe { (*ret_0).type_0 = XPATH_STRING });
...
extern "C" fn xmlXPathCacheNewString(
    ...
    if !ctxt.is_null() && !(unsafe { (*ctxt).cache }).is_null() {
            ...
            && (unsafe { (*(*cache).stringObjs).number }) != 0 as i32
        {
            ...
            *fresh60 -= 1;
            ret = (unsafe { *((*(*cache).stringObjs).items).offset(*fresh60 as isize) })
                as xmlXPathObjectPtr;
            (unsafe { (*ret).type_0 = XPATH_STRING });
            if !val.is_null() {
                ...
                *fresh62 =
                    unsafe { xmlStrdup(b"\0" as *const u8 as *const i8 as *const xmlChar) };
            }
            return ret;
        } else {
            if !(unsafe { (*cache).miscObjs }).is_null()
                && (unsafe { (*(*cache).miscObjs).number }) != 0 as i32
            {
                ...
                *fresh63 -= 1;
                ret_0 = (unsafe { *((*(*cache).miscObjs).items).offset(*fresh63 as isize) })
                    as xmlXPathObjectPtr;
                (unsafe { (*ret_0).type_0 = XPATH_STRING });
                if !val.is_null() {
...
extern "C" fn xmlXPathCacheNewBoolean(
    ...
    if !ctxt.is_null() && !(unsafe { (*ctxt).cache }).is_null() {
        let mut cache: xmlXPathContextCachePtr =
            (unsafe { (*ctxt).cache }) as xmlXPathContextCachePtr;
        if !(unsafe { (*cache).booleanObjs }).is_null()
            && (unsafe { (*(*cache).booleanObjs).number }) != 0 as i32
        {
            let mut ret: xmlXPathObjectPtr = 0 as *mut xmlXPathObject;
            let fresh66 = unsafe { &mut ((*(*cache).booleanObjs).number) };
            *fresh66 -= 1;
            ret = (unsafe { *((*(*cache).booleanObjs).items).offset(*fresh66 as isize) })
                as xmlXPathObjectPtr;
                ...
                && (unsafe { (*(*cache).miscObjs).number }) != 0 as i32
            {
                ...
                *fresh67 -= 1;
                ret_0 = (unsafe { *((*(*cache).miscObjs).items).offset(*fresh67 as isize) })
                    as xmlXPathObjectPtr;
...
extern "C" fn xmlXPathCacheNewFloat(
    ...
    if !ctxt.is_null() && !(unsafe { (*ctxt).cache }).is_null() {
        let mut cache: xmlXPathContextCachePtr =
            (unsafe { (*ctxt).cache }) as xmlXPathContextCachePtr;
        if !(unsafe { (*cache).numberObjs }).is_null()
            && (unsafe { (*(*cache).numberObjs).number }) != 0 as i32
        {
            let mut ret: xmlXPathObjectPtr = 0 as *mut xmlXPathObject;
            let fresh68 = unsafe { &mut ((*(*cache).numberObjs).number) };
            *fresh68 -= 1;
            ret = (unsafe { *((*(*cache).numberObjs).items).offset(*fresh68 as isize) })
                as xmlXPathObjectPtr;
                ...
                && (unsafe { (*(*cache).miscObjs).number }) != 0 as i32
            {
                ...
                *fresh69 -= 1;
                ret_0 = (unsafe { *((*(*cache).miscObjs).items).offset(*fresh69 as isize) })
                    as xmlXPathObjectPtr;
                ...
                && number == number as i32 as f64
            {
                ...
                if value == 0 as i32 {
                    ...
                    while (unsafe { *cur }) as i32 != 0
                        && ((unsafe { ptr.offset_from(buffer) }) as i64) < buffersize as i64
                    {
                        let fresh83 = cur;
                ...
                while work_0[0 as i32 as usize] as i32 == ' ' as i32 {
                    ptr_0 =
                        (unsafe { &mut *work_0.as_mut_ptr().offset(0 as i32 as isize) }) as *mut i8;
                    loop {
                ...
                size =
                    (unsafe { strlen(work_0.as_mut_ptr()) }).wrapping_add(1 as i32 as u64) as i32;
                if size > buffersize {
    ...
    while i < (unsafe { (*cur).nodeNr }) {
        if !(unsafe { *((*cur).nodeTab).offset(i as isize) }).is_null()
            && (unsafe { (**((*cur).nodeTab).offset(i as isize)).type_0 }) as u32
                == XML_NAMESPACE_DECL as i32 as u32
            && (unsafe { (*(*((*cur).nodeTab).offset(i as isize) as xmlNsPtr)).next })
                == node as xmlNsPtr
            && (unsafe {
                xmlStrEqual(
                    (*ns).prefix,
    ...
    while i < (unsafe { (*val2).nodeNr }) {
        ...
        while j < initNr {
            ...
            if n1 == n2 {
                ...
                {
                    if (unsafe { (*(n1 as xmlNsPtr)).next })
                        == (unsafe { (*(n2 as xmlNsPtr)).next })
                        && (unsafe {
        ...
        while i < (unsafe { (*obj).nodeNr }) {
                    ...
                    == XML_NAMESPACE_DECL as i32 as u32
            {
                xmlXPathNodeSetFreeNs(
                    (unsafe { *((*obj).nodeTab).offset(i as isize) }) as xmlNsPtr,
                );
            }
            i += 1;
        }
        ...
        if !ret.is_null() {
            ...
            while i < (unsafe { (*val).nodeNr }) {
                if xmlXPathNodeSetAddUnique(
                    unsafe { (*ret).nodesetval },
                    unsafe { *((*val).nodeTab).offset(i as isize) },
                ) < 0 as i32
                {
                    break;
                }
...
pub extern "C" fn xmlXPathDifference(
    ...
    let mut cur: xmlNodePtr = 0 as *mut xmlNode;
    if nodes2.is_null()
        || (unsafe { (*nodes2).nodeNr }) == 0 as i32
        || (unsafe { (*nodes2).nodeTab }).is_null()
    {
        return nodes1;
    }
...
pub extern "C" fn xmlXPathFunctionLookupNS(
    ...
    if unsafe { ((*ctxt).funcLookupFunc).is_some() } {
        ...
        f = unsafe { (*ctxt).funcLookupFunc };
        ret = unsafe {
            f.expect("non-null function pointer")((*ctxt).funcLookupData, name, ns_uri)
        };
        if ret.is_some() {
            return ret;
        }
    ...
    if !(unsafe { (*ctxt).namespaces }).is_null() {
        ...
        while i < (unsafe { (*ctxt).nsNr }) {
            if !(unsafe { *((*ctxt).namespaces).offset(i as isize) }).is_null()
                && (unsafe {
                    xmlStrEqual((**((*ctxt).namespaces).offset(i as isize)).prefix, prefix)
                }) != 0
            {
...
extern "C" fn xmlXPathReleaseObject(mut ctxt: xmlXPathContextPtr, mut obj: xmlXPathObjectPtr) {
    ...
    if ctxt.is_null() || (unsafe { (*ctxt).cache }).is_null() {
        ...
        match (unsafe { (*obj).type_0 }) as u32 {
                    ...
                    if (unsafe { (*obj).boolval }) != 0 {
                        ...
                        *fresh168 = 0 as xmlNodeSetPtr;
                        ...
                        && ((unsafe { (*cache).nodesetObjs }).is_null()
                            || (unsafe { (*(*cache).nodesetObjs).number })
                                < (unsafe { (*cache).maxNodeset }))
                    {
                        if (unsafe { (*cache).nodesetObjs }).is_null() {
            ...
            2 => {
                if (unsafe { (*cache).booleanObjs }).is_null()
                    || (unsafe { (*(*cache).booleanObjs).number })
                        < (unsafe { (*cache).maxBoolean })
                {
                    if (unsafe { (*cache).booleanObjs }).is_null() {
        ...
        match current_block {
            13678349939556791712 => {
                if (unsafe { (*cache).miscObjs }).is_null()
                    || (unsafe { (*(*cache).miscObjs).number }) < (unsafe { (*cache).maxMisc })
                {
                    if (unsafe { (*cache).miscObjs }).is_null() {
                ...
                if !(unsafe { (*obj).nodesetval }).is_null() {
                    ...
                    if (unsafe { (*tmpset).nodeNr }) > 1 as i32 {
                        ...
                        if !(unsafe { *((*tmpset).nodeTab).offset(0 as i32 as isize) }).is_null()
                            && (unsafe { (**((*tmpset).nodeTab).offset(0 as i32 as isize)).type_0 })
                                as u32
                                == XML_NAMESPACE_DECL as i32 as u32
                        {
                            xmlXPathNodeSetFreeNs(
                                (unsafe { *((*tmpset).nodeTab).offset(0 as i32 as isize) })
                                    as xmlNsPtr,
                            );
                        }
                    }
...
pub extern "C" fn xmlXPathCastNumberToString(mut val: f64) -> *mut xmlChar {
    ...
    match xmlXPathIsInf(val) {
        ...
        -1 => {
            ret =
                unsafe { xmlStrdup(b"-Infinity\0" as *const u8 as *const i8 as *const xmlChar) };
        }
        _ => {
...
pub extern "C" fn xmlXPathCastNodeSetToString(mut ns: xmlNodeSetPtr) -> *mut xmlChar {
    if ns.is_null()
        || (unsafe { (*ns).nodeNr }) == 0 as i32
        || (unsafe { (*ns).nodeTab }).is_null()
    {
        ...
        while i < (unsafe { (*ctxt).valueNr }) {
            if !(unsafe { (*ctxt).context }).is_null() {
                xmlXPathReleaseObject(
                    unsafe { (*ctxt).context },
                    unsafe { *((*ctxt).valueTab).offset(i as isize) },
                );
            } else {
        ...
        if !string.is_null() && (unsafe { *string.offset(0 as i32 as isize) }) as i32 != 0 as i32 {
            if len == 1 as i32 {
                return ret.wrapping_add(
                    ((unsafe { *string.offset(0 as i32 as isize) }) as u32) << 8 as i32,
                );
            }
            if (unsafe { *string.offset(1 as i32 as isize) }) as i32 == 0 as i32 {
                ...
                return ((unsafe { *string.offset(0 as i32 as isize) }) as u32).wrapping_add(
                    ((unsafe { *string.offset(1 as i32 as isize) }) as u32) << 8 as i32,
                );
            }
        }
        if !(unsafe { (*tmp).children }).is_null()
            && (unsafe { (*tmp).type_0 }) as u32 != XML_DTD_NODE as i32 as u32
        {
    ...
    if !ns.is_null() {
        ...
        while i < (unsafe { (*ns).nodeNr }) {
            ...
            if !str2.is_null() {
                ...
                valuePush(
                    ctxt,
                    xmlXPathCacheObjectCopy(unsafe { (*ctxt).context }, s),
                );
                ret = xmlXPathCompareValues(ctxt, inf, strict);
                if ret != 0 {
                    break;
...
extern "C" fn xmlXPathEqualNodeSetString(
    ...
    while i < (unsafe { (*ns).nodeNr }) {
        if xmlXPathNodeValHash(unsafe { *((*ns).nodeTab).offset(i as isize) }) == hash {
            str2 = unsafe {
                xmlNodeGetContent(*((*ns).nodeTab).offset(i as isize) as *const xmlNode)
            };
            if !str2.is_null() && (unsafe { xmlStrEqual(str, str2) }) != 0 {
                ...
                && (unsafe { xmlStrEqual(str, b"\0" as *const u8 as *const i8 as *mut xmlChar) })
                    != 0
            {
                if !(neq != 0) {
                    return 1 as i32;
...
extern "C" fn xmlXPathEqualNodeSets(
    ...
    if neq == 0 as i32 {
        ...
        while i < (unsafe { (*ns1).nodeNr }) {
            j = 0 as i32;
            while j < (unsafe { (*ns2).nodeNr }) {
                if (unsafe { *((*ns1).nodeTab).offset(i as isize) })
                    == (unsafe { *((*ns2).nodeTab).offset(j as isize) })
                {
                    return 1 as i32;
                }
                j += 1;
    ...
    while i < (unsafe { (*ns1).nodeNr }) {
        ...
        while j < (unsafe { (*ns2).nodeNr }) {
            ...
            if (unsafe { *hashs1.offset(i as isize) }) != (unsafe { *hashs2.offset(j as isize) }) {
                ...
                ret = (unsafe {
                    xmlStrEqual(*values1.offset(i as isize), *values2.offset(j as isize))
                }) ^ neq;
                if ret != 0 {
                    break;
                }
...
extern "C" fn xmlXPathEqualValuesCommon(
    ...
    match (unsafe { (*arg1).type_0 }) as u32 {
        2 => match (unsafe { (*arg2).type_0 }) as u32 {
            ...
            3 => {
                ret = ((unsafe { (*arg1).boolval })
                    == xmlXPathCastNumberToBoolean(unsafe { (*arg2).floatval }))
                    as i32;
            }
            4 => {
                if (unsafe { (*arg2).stringval }).is_null()
                    || (unsafe { *((*arg2).stringval).offset(0 as i32 as isize) }) as i32
                        == 0 as i32
                {
                    ret = 0 as i32;
                } else {
        ...
        3 => {
            ...
            match (unsafe { (*arg2).type_0 }) as u32 {
                2 => {
                    ret = ((unsafe { (*arg2).boolval })
                        == xmlXPathCastNumberToBoolean(unsafe { (*arg1).floatval }))
                        as i32;
            ...
            match current_block_37 {
                14983914821605129355 => {
                    if xmlXPathIsNaN(unsafe { (*arg1).floatval }) != 0
                        || xmlXPathIsNaN(unsafe { (*arg2).floatval }) != 0
                    {
                        ret = 0 as i32;
                        ...
                        ret =
                            ((unsafe { (*arg1).floatval }) == (unsafe { (*arg2).floatval })) as i32;
                    }
                }
                _ => {}
            }
        ...
        4 => match (unsafe { (*arg2).type_0 }) as u32 {
            2 => {
                if (unsafe { (*arg1).stringval }).is_null()
                    || (unsafe { *((*arg1).stringval).offset(0 as i32 as isize) }) as i32
                        == 0 as i32
                {
                    ret = 0 as i32;
                } else {
...
pub extern "C" fn xmlXPathEqualValues(mut ctxt: xmlXPathParserContextPtr) -> i32 {
        ...
        || (unsafe { (*arg1).type_0 }) as u32 == XPATH_XSLT_TREE as i32 as u32
    {
        ...
        match (unsafe { (*arg2).type_0 }) as u32 {
            ...
            3 => {
                ret =
                    xmlXPathEqualNodeSetFloat(ctxt, arg1, unsafe { (*arg2).floatval }, 0 as i32);
            }
            4 => {
...
pub extern "C" fn xmlXPathNotEqualValues(mut ctxt: xmlXPathParserContextPtr) -> i32 {
        ...
        || (unsafe { (*arg1).type_0 }) as u32 == XPATH_XSLT_TREE as i32 as u32
    {
        ...
        match (unsafe { (*arg2).type_0 }) as u32 {
            ...
            3 => {
                ret =
                    xmlXPathEqualNodeSetFloat(ctxt, arg1, unsafe { (*arg2).floatval }, 1 as i32);
            }
            4 => {
...
pub extern "C" fn xmlXPathNextDescendant(
    ...
    if cur.is_null() {
        ...
        if (unsafe { (*(*(*ctxt).context).node).type_0 }) as u32 == XML_ATTRIBUTE_NODE as i32 as u32
            || (unsafe { (*(*(*ctxt).context).node).type_0 }) as u32
                == XML_NAMESPACE_DECL as i32 as u32
        {
            return 0 as xmlNodePtr;
        }
        ...
        match (unsafe { (*(*(*ctxt).context).node).type_0 }) as u32 {
            1 | 3 | 4 | 5 | 6 | 7 | 8 | 12 | 14 | 15 | 16 | 19 | 20 | 17 => {
                ...
                if (unsafe { (*(*(*(*ctxt).context).node).parent).type_0 }) as u32
                    ...
                    && ((unsafe {
                        *((*(*(*(*ctxt).context).node).parent).name).offset(0 as i32 as isize)
                    }) as i32
                        == ' ' as i32
                        || (unsafe {
                            xmlStrEqual(
    ...
    match (unsafe { (*cur).type_0 }) as u32 {
        1 | 3 | 4 | 5 | 6 | 7 | 8 | 12 | 14 | 15 | 16 | 17 | 19 | 20 => {
            ...
            if (unsafe { (*(*cur).parent).type_0 }) as u32 == XML_ELEMENT_NODE as i32 as u32
                && ((unsafe { *((*(*cur).parent).name).offset(0 as i32 as isize) }) as i32
                    == ' ' as i32
                    || (unsafe {
                        xmlStrEqual(
                            (*(*cur).parent).name,
...
pub extern "C" fn xmlXPathNextNamespace(
    ...
    if cur.is_null() {
        ...
        *fresh198 = unsafe {
        ...
        if !(unsafe { (*(*ctxt).context).tmpNsList }).is_null() {
            while !(unsafe {
                *((*(*ctxt).context).tmpNsList).offset((*(*ctxt).context).tmpNsNr as isize)
            })
            .is_null()
            {
    ...
    if (unsafe { (*(*ctxt).context).tmpNsNr }) > 0 as i32 {
        ...
        *fresh200 -= 1;
        return (unsafe { *((*(*ctxt).context).tmpNsList).offset(*fresh200 as isize) })
            as xmlNodePtr;
    } else {
        if !(unsafe { (*(*ctxt).context).tmpNsList }).is_null() {
            (unsafe {
...
pub extern "C" fn xmlXPathRoot(mut ctxt: xmlXPathParserContextPtr) {
    ...
    valuePush(
        ...
        xmlXPathCacheNewNodeSet(
            unsafe { (*ctxt).context },
            (unsafe { (*(*ctxt).context).doc }) as xmlNodePtr,
        ),
    );
}
#[no_mangle]
    ...
    if (unsafe { (*(*ctxt).context).contextSize }) >= 0 as i32 {
        valuePush(
            ...
            xmlXPathCacheNewFloat(
                unsafe { (*ctxt).context },
                (unsafe { (*(*ctxt).context).contextSize }) as f64,
            ),
        );
    } else {
...
pub extern "C" fn xmlXPathPositionFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    if (unsafe { (*(*ctxt).context).proximityPosition }) >= 0 as i32 {
        valuePush(
            ...
            xmlXPathCacheNewFloat(
                unsafe { (*ctxt).context },
                (unsafe { (*(*ctxt).context).proximityPosition }) as f64,
            ),
        );
    } else {
...
pub extern "C" fn xmlXPathCountFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    if cur.is_null() || (unsafe { (*cur).nodesetval }).is_null() {
        ...
        valuePush(
            ...
            xmlXPathCacheNewFloat(
                unsafe { (*ctxt).context },
                (unsafe { (*(*cur).nodesetval).nodeNr }) as f64,
            ),
        );
    }
...
pub extern "C" fn xmlXPathIdFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
        ...
        || (unsafe { (*obj).type_0 }) as u32 == XPATH_XSLT_TREE as i32 as u32
    {
        ...
        if !(unsafe { (*obj).nodesetval }).is_null() {
            ...
            while i < (unsafe { (*(*obj).nodesetval).nodeNr }) {
                tokens = xmlXPathCastNodeToString(
                    unsafe { *((*(*obj).nodesetval).nodeTab).offset(i as isize) },
                );
        ...
        valuePush(
            ctxt,
            xmlXPathCacheWrapNodeSet(unsafe { (*ctxt).context }, ret),
        );
        return;
    }
    ...
    ret = xmlXPathGetElementsByIds(
        unsafe { (*(*ctxt).context).doc },
        unsafe { (*obj).stringval },
    );
    valuePush(
        ctxt,
        xmlXPathCacheWrapNodeSet(unsafe { (*ctxt).context }, ret),
        ...
        || (unsafe { (*(*cur).nodesetval).nodeNr }) == 0 as i32
    {
        ...
        match (unsafe { (**((*(*cur).nodesetval).nodeTab).offset(i as isize)).type_0 }) as u32 {
            1 | 2 => {
                ...
                } else if (unsafe { (**((*(*cur).nodesetval).nodeTab).offset(i as isize)).ns })
                    .is_null()
                    || (unsafe {
                    ...
                    .is_null()
                {
                    ...
                    valuePush(
                        ctxt,
                        xmlXPathCacheWrapString(unsafe { (*ctxt).context }, fullname),
                    );
                }
            }
            _ => {
...
pub extern "C" fn xmlXPathStringFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    valuePush(
        ctxt,
        xmlXPathCacheConvertString(unsafe { (*ctxt).context }, cur),
    );
}
#[no_mangle]
pub extern "C" fn xmlXPathStringLengthFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    if nargs == 0 as i32 {
        ...
        if (unsafe { (*(*ctxt).context).node }).is_null() {
            ...
            valuePush(
                ...
                xmlXPathCacheNewFloat(
                    unsafe { (*ctxt).context },
                    (unsafe { xmlUTF8Strlen(content) }) as f64,
                ),
            );
    ...
    valuePush(
        ...
        xmlXPathCacheNewFloat(
            unsafe { (*ctxt).context },
            (unsafe { xmlUTF8Strlen((*cur).stringval) }) as f64,
        ),
    );
    xmlXPathReleaseObject(unsafe { (*ctxt).context }, cur);
}
...
pub extern "C" fn xmlXPathContainsFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    if !(unsafe { xmlStrstr((*hay).stringval, (*needle).stringval) }).is_null() {
        valuePush(
            ctxt,
            xmlXPathCacheNewBoolean(unsafe { (*ctxt).context }, 1 as i32),
        );
    } else {
        valuePush(
            ctxt,
            xmlXPathCacheNewBoolean(unsafe { (*ctxt).context }, 0 as i32),
        );
    }
    xmlXPathReleaseObject(unsafe { (*ctxt).context }, hay);
    xmlXPathReleaseObject(unsafe { (*ctxt).context }, needle);
...
pub extern "C" fn xmlXPathSubstringFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    if i < j {
        ...
        valuePush(
            ctxt,
            xmlXPathCacheNewString(unsafe { (*ctxt).context }, ret),
        );
        (unsafe { xmlFree.expect("non-null function pointer")(ret as *mut libc::c_void) });
    } else {
        valuePush(
...
pub extern "C" fn xmlXPathSubstringBeforeFunction(
    ...
    if !target.is_null() {
        ...
        valuePush(
            ...
            xmlXPathCacheNewString(
                unsafe { (*ctxt).context },
                unsafe { xmlBufContent(target as *const xmlBuf) },
            ),
        );
...
pub extern "C" fn xmlXPathSubstringAfterFunction(
    ...
    if !target.is_null() {
        ...
        if !point.is_null() {
            offset = (unsafe { point.offset_from((*str).stringval) }) as i64 as i32
                + (unsafe { xmlStrlen((*find).stringval) });
            (unsafe {
                xmlBufAdd(
                    target,
...
pub extern "C" fn xmlXPathNormalizeFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    while (unsafe { *source }) != 0 {
        if (unsafe { *source }) as i32 == 0x20 as i32
            || 0x9 as i32 <= (unsafe { *source }) as i32
                && (unsafe { *source }) as i32 <= 0xa as i32
            || (unsafe { *source }) as i32 == 0xd as i32
        {
            blank = 1 as i32;
        } else {
...
pub extern "C" fn xmlXPathNotFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    if !(unsafe { (*ctxt).value }).is_null()
        && (unsafe { (*(*ctxt).value).type_0 }) as u32 != XPATH_BOOLEAN as i32 as u32
    {
        xmlXPathBooleanFunction(ctxt, 1 as i32);
    }
    if (unsafe { (*ctxt).value }).is_null()
...
pub extern "C" fn xmlXPathLangFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    valuePush(
        ctxt,
        xmlXPathCacheNewBoolean(unsafe { (*ctxt).context }, ret),
    );
}
#[no_mangle]
pub extern "C" fn xmlXPathNumberFunction(mut ctxt: xmlXPathParserContextPtr, mut nargs: i32) {
    ...
    if nargs == 0 as i32 {
        if (unsafe { (*(*ctxt).context).node }).is_null() {
            valuePush(
                ctxt,
                xmlXPathCacheNewFloat(unsafe { (*ctxt).context }, 0.0f64),
            );
        } else {
            ...
            valuePush(
                ctxt,
                xmlXPathCacheNewFloat(unsafe { (*ctxt).context }, res),
            );
            (unsafe { xmlFree.expect("non-null function pointer")(content as *mut libc::c_void) });
        }
        return;
    ...
    valuePush(
        ctxt,
        xmlXPathCacheConvertNumber(unsafe { (*ctxt).context }, cur),
    );
}
#[no_mangle]
            ...
            if c as i32 & 0xe0 as i32 == 0xe0 as i32 {
                if (unsafe { *cur.offset(2 as i32 as isize) }) as i32 & 0xc0 as i32 != 0x80 as i32 {
                    ...
                    val |= (((unsafe { *cur.offset(1 as i32 as isize) }) as i32 & 0x3f as i32)
                        << 6 as i32) as u32;
                    val |=
                        ((unsafe { *cur.offset(2 as i32 as isize) }) as i32 & 0x3f as i32) as u32;
                    current_block = 10043043949733653460;
                }
            } else {
                (unsafe { *len = 2 as i32 });
...
extern "C" fn xmlXPathCompNumber(mut ctxt: xmlXPathParserContextPtr) {
    ...
    if (unsafe { *(*ctxt).cur }) as i32 != '.' as i32
        && (((unsafe { *(*ctxt).cur }) as i32) < '0' as i32
            || (unsafe { *(*ctxt).cur }) as i32 > '9' as i32)
    {
    ...
    if (unsafe { *(*ctxt).cur }) as i32 == 'e' as i32
        || (unsafe { *(*ctxt).cur }) as i32 == 'E' as i32
    {
        if (unsafe { *(*ctxt).cur }) as i32 != 0 {
...
extern "C" fn xmlXPathParseLiteral(mut ctxt: xmlXPathParserContextPtr) -> *mut xmlChar {
    ...
    if (unsafe { *(*ctxt).cur }) as i32 == '"' as i32 {
        ...
        q = unsafe { (*ctxt).cur };
        while (0x9 as i32 <= (unsafe { *(*ctxt).cur }) as i32
        ...
        if !(0x9 as i32 <= (unsafe { *(*ctxt).cur }) as i32
            && (unsafe { *(*ctxt).cur }) as i32 <= 0xa as i32
            || (unsafe { *(*ctxt).cur }) as i32 == 0xd as i32
            || 0x20 as i32 <= (unsafe { *(*ctxt).cur }) as i32)
        {
...
extern "C" fn xmlXPathCompVariableReference(mut ctxt: xmlXPathParserContextPtr) {
    ...
    if !(unsafe { (*ctxt).context }).is_null()
        && (unsafe { (*(*ctxt).context).flags }) & (1 as i32) << 1 as i32 != 0
    {
...
extern "C" fn xmlXPathCompPrimaryExpr(mut ctxt: xmlXPathParserContextPtr) {
    ...
    } else if 0x30 as i32 <= (unsafe { *(*ctxt).cur }) as i32
        && (unsafe { *(*ctxt).cur }) as i32 <= 0x39 as i32
        || (unsafe { *(*ctxt).cur }) as i32 == '.' as i32
...
extern "C" fn xmlXPathCompPathExpr(mut ctxt: xmlXPathParserContextPtr) {
    ...
    if (unsafe { *(*ctxt).cur }) as i32 == '$' as i32
        || (unsafe { *(*ctxt).cur }) as i32 == '(' as i32
        || 0x30 as i32 <= (unsafe { *(*ctxt).cur }) as i32
            && (unsafe { *(*ctxt).cur }) as i32 <= 0x39 as i32
        || (unsafe { *(*ctxt).cur }) as i32 == '\'' as i32
        || (unsafe { *(*ctxt).cur }) as i32 == '"' as i32
        || (unsafe { *(*ctxt).cur }) as i32 == '.' as i32
                ...
                && (unsafe { *((*ctxt).cur).offset(1 as i32 as isize) }) as i32 <= 0x39 as i32)
    {
        ...
        if !name.is_null()
            && !(unsafe { xmlStrstr(name, b"::\0" as *const u8 as *const i8 as *mut xmlChar) })
                .is_null()
        {
            lc = 1 as i32;
            (unsafe { xmlFree.expect("non-null function pointer")(name as *mut libc::c_void) });
...
extern "C" fn xmlXPathCompNodeTest(
    ...
    if (unsafe { *(*ctxt).cur }) as i32 == '(' as i32 {
        ...
        if (unsafe { xmlStrEqual(name, b"comment\0" as *const u8 as *const i8 as *mut xmlChar) })
            != 0
        {
            (unsafe { *type_0 = NODE_TYPE_COMMENT });
        } else if (unsafe {
            xmlStrEqual(name, b"node\0" as *const u8 as *const i8 as *mut xmlChar)
        }) != 0
        {
            (unsafe { *type_0 = NODE_TYPE_NODE });
        } else if (unsafe {
        ...
        }) != 0
        {
            ...
            xmlStrEqual(name, b"text\0" as *const u8 as *const i8 as *mut xmlChar)
        }) != 0
        {
...
extern "C" fn xmlXPathIsAxisName(mut name: *const xmlChar) -> xmlXPathAxisVal {
    ...
    match (unsafe { *name.offset(0 as i32 as isize) }) as i32 {
        ...
        99 => {
            if (unsafe { xmlStrEqual(name, b"child\0" as *const u8 as *const i8 as *mut xmlChar) })
                != 0
            {
                ret = AXIS_CHILD;
            }
        ...
        112 => {
            if (unsafe { xmlStrEqual(name, b"parent\0" as *const u8 as *const i8 as *mut xmlChar) })
                != 0
            {
                ret = AXIS_PARENT;
            }
        ...
        115 => {
            if (unsafe { xmlStrEqual(name, b"self\0" as *const u8 as *const i8 as *mut xmlChar) })
                != 0
            {
                ret = AXIS_SELF;
            }
        ...
        if (unsafe { *(*ctxt).cur }) as i32 == '*' as i32 {
            ...
            if !name.is_null() {
                ...
                if axis as u32 != 0 as i32 as u32 {
                    ...
                    if (unsafe { *(*ctxt).cur }) as i32 == ':' as i32
                        && (unsafe { *((*ctxt).cur).offset(1 as i32 as isize) }) as i32
                            == ':' as i32
                    {
        ...
        while (unsafe { *(*ctxt).cur }) as i32 == '/' as i32 {
                ...
                && (unsafe { *((*ctxt).cur).offset(1 as i32 as isize) }) as i32 == '/' as i32
            {
                ...
                *fresh331 = unsafe { (*fresh331).offset(2 as i32 as isize) };
                ...
                if (unsafe { *(*ctxt).cur }) as i32 != 0 as i32
                    && (0x41 as i32 <= (unsafe { *(*ctxt).cur }) as i32
                        && (unsafe { *(*ctxt).cur }) as i32 <= 0x5a as i32
                        || 0x61 as i32 <= (unsafe { *(*ctxt).cur }) as i32
                            && (unsafe { *(*ctxt).cur }) as i32 <= 0x7a as i32
...
extern "C" fn xmlXPathNodeSetFilter(
    ...
    filterOp = (unsafe { &mut *((*(*ctxt).comp).steps).offset(filterOpIndex as isize) })
        as *mut xmlXPathStepOp;
    ...
    (unsafe { (*set).nodeNr = j });
    if (unsafe { (*set).nodeMax }) > 10 as i32
        && (unsafe { (*set).nodeNr }) < (unsafe { (*set).nodeMax }) / 2 as i32
    {
...
extern "C" fn xmlXPathIsPositionalPredicate(
    ...
    if (unsafe { (*op).ch2 }) != -(1 as i32) {
        exprOp = (unsafe { &mut *((*(*ctxt).comp).steps).offset((*op).ch2 as isize) })
            as *mut xmlXPathStepOp;
    } else {
        return 0 as i32;
    }
...
extern "C" fn xmlXPathNodeCollectAndTest(
    ...
    if (unsafe { (*op).ch2 }) != -(1 as i32) {
        predOp = (unsafe { &mut *((*(*ctxt).comp).steps).offset((*op).ch2 as isize) })
            as *mut xmlXPathStepOp;
        ...
        loop {
            ...
            if !first.is_null() && !(unsafe { *first }).is_null() {
                ...
                if total % 256 as i32 == 0 as i32
                    && xmlXPathCmpNodesExt(unsafe { *first }, cur) >= 0 as i32
                {
                    current_block = 16313108704606884446;
                    break;
                }
            ...
            if !last.is_null() && !(unsafe { *last }).is_null() {
                ...
                if total % 256 as i32 == 0 as i32
                    && xmlXPathCmpNodesExt(cur, unsafe { *last }) >= 0 as i32
                {
                    current_block = 16313108704606884446;
                    break;
                }
        ...
        match current_block {
            6058993197908835384 => {
                if outSeq.is_null() {
                    ...
                    outSeq =
                        unsafe { mergeAndClear.expect("non-null function pointer")(outSeq, seq) };
                }
                break;
            }
    ...
    match (unsafe { (*op).op }) as u32 {
        ...
        7 => {
                ...
                && (unsafe { (*(*(*ctxt).value).nodesetval).nodeNr }) >= 1 as i32
            {
                ...
                (unsafe {
                    *first = *((*(*(*ctxt).value).nodesetval).nodeTab).offset(0 as i32 as isize)
                });
            }
            ...
            if (unsafe { (*(*ctxt).context).opLimit }) != 0 as i32 as u64
                && (!(unsafe { (*arg1).nodesetval }).is_null()
                    && xmlXPathCheckOpLimit(ctxt, (unsafe { (*(*arg1).nodesetval).nodeNr }) as u64)
                        < 0 as i32
                    || !(unsafe { (*arg2).nodesetval }).is_null()
                        && xmlXPathCheckOpLimit(
                            ctxt,
                            (unsafe { (*(*arg2).nodesetval).nodeNr }) as u64,
                        ) < 0 as i32)
            {
                xmlXPathReleaseObject(unsafe { (*ctxt).context }, arg1);
                xmlXPathReleaseObject(unsafe { (*ctxt).context }, arg2);
                ...
                *fresh349 = xmlXPathNodeSetMerge(
                    unsafe { (*arg1).nodesetval },
                    unsafe { (*arg2).nodesetval },
                );
                valuePush(ctxt, arg1);
                xmlXPathReleaseObject(unsafe { (*ctxt).context }, arg2);
                if total > cur {
        ...
        11 => {
            valuePush(
                ...
                xmlXPathCacheObjectCopy(
                    unsafe { (*ctxt).context },
                    (unsafe { (*op).value4 }) as xmlXPathObjectPtr,
                ),
            );
        }
        17 => {
...
extern "C" fn xmlXPathCompOpEval(
    ...
    match (unsafe { (*op).op }) as u32 {
        ...
        1 => {
            ...
            xmlXPathBooleanFunction(ctxt, 1 as i32);
            if !((unsafe { (*ctxt).value }).is_null()
                || (unsafe { (*(*ctxt).value).boolval }) == 0 as i32)
            {
                arg2 = valuePop(ctxt);
                total += xmlXPathCompOpEval(
                    ctxt,
        ...
        3 => {
            ...
            valuePush(
                ctxt,
                xmlXPathCacheNewBoolean(unsafe { (*ctxt).context }, equal),
            );
        }
        4 => {
            total += xmlXPathCompOpEval(
        ...
        7 => {
                        ...
                        ) < 0 as i32)
            {
                ...
                if (unsafe { (*arg1).nodesetval }).is_null()
                    || !(unsafe { (*arg2).nodesetval }).is_null()
                        && (unsafe { (*(*arg2).nodesetval).nodeNr }) != 0 as i32
                {
        ...
        13 => {
            ...
            match current_block_226 {
                ...
                _ => {
                    if (unsafe { (*ctxt).valueNr })
                        < (unsafe { (*ctxt).valueFrame }) + (unsafe { (*op).value })
                    {
                        (unsafe {
                            (*__xmlGenericError()).expect("non-null function pointer")(
                                *__xmlGenericErrorContext(),
                        ...
                        match current_block_226 {
                            ...
                            _ => {
                                ...
                                *fresh357 = oldFuncURI;
                                if (unsafe { (*ctxt).error }) == XPATH_EXPRESSION_OK as i32
                                    && (unsafe { (*ctxt).valueNr })
                                        != (unsafe { (*ctxt).valueFrame }) + 1 as i32
                                {
                                    xmlXPathErr(ctxt, XPATH_STACK_ERROR as i32);
                                    return 0 as i32;
                                }
        ...
        15 | 16 => {
                    ...
                    == XPATH_OP_VALUE as i32 as u32
            {
                ...
                val_0 = (unsafe { (*((*comp).steps).offset((*op).ch2 as isize)).value4 })
                    as xmlXPathObjectPtr;
                if !val_0.is_null()
            ...
            match current_block_226 {
                ...
                _ => {
                    ...
                    {
                        let mut f: i32 =
                            unsafe { (*((*comp).steps).offset((*op).ch2 as isize)).ch1 };
                        if f != -(1 as i32)
                            && (unsafe { (*((*comp).steps).offset(f as isize)).op }) as u32
                                == XPATH_OP_FUNCTION as i32 as u32
                            && (unsafe { (*((*comp).steps).offset(f as isize)).value5 }).is_null()
                            && (unsafe { (*((*comp).steps).offset(f as isize)).value }) == 0 as i32
                            && !(unsafe { (*((*comp).steps).offset(f as isize)).value4 })
                                .is_null()
                            && (unsafe {
                                xmlStrEqual(
                                    (*((*comp).steps).offset(f as isize)).value4 as *const xmlChar,
    ...
    if min_depth == 0 as i32 {
        if from_root != 0 {
            ...
            xmlXPathNodeSetAddUnique(
                unsafe { (**resultSeq).nodesetval },
                (unsafe { (*ctxt).doc }) as xmlNodePtr,
            );
        } else {
            if toBool != 0 {
                return 1 as i32;
                ...
                's_315: loop {
                    ...
                    if !(ret < 0 as i32) {
                        if ret == 1 as i32 {
                            ...
                            if xmlXPathNodeSetAddUnique(unsafe { (**resultSeq).nodesetval }, cur)
                                < 0 as i32
                            {
                                (unsafe { (*ctxt).lastError.domain = XML_FROM_XPATH as i32 });
                                (unsafe { (*ctxt).lastError.code = XML_ERR_NO_MEMORY as i32 });
...
pub extern "C" fn xmlXPathEvalPredicate(
    ...
    match (unsafe { (*res).type_0 }) as u32 {
        ...
        3 => {
            return ((unsafe { (*res).floatval }) == (unsafe { (*ctxt).proximityPosition }) as f64)
                as i32;
        }
        1 | 9 => {
        ...
        4 => {
            return (!(unsafe { (*res).stringval }).is_null()
                && (unsafe { xmlStrlen((*res).stringval) }) != 0 as i32) as i32;
        }
        _ => {
            (unsafe {
                (*__xmlGenericError()).expect("non-null function pointer")(
...
extern "C" fn xmlXPathOptimizeExpression(
        ...
        && (unsafe { (*op).ch2 }) == -(1 as i32)
    {
        ...
        if (unsafe { (*prevop).op }) as u32 == XPATH_OP_COLLECT as i32 as u32
            && (unsafe { (*prevop).value }) as xmlXPathAxisVal as u32
                == AXIS_DESCENDANT_OR_SELF as i32 as u32
            && (unsafe { (*prevop).ch2 }) == -(1 as i32)
            && (unsafe { (*prevop).value2 }) as xmlXPathTestVal as u32
                == NODE_TEST_TYPE as i32 as u32
            && (unsafe { (*prevop).value3 }) as xmlXPathTypeVal as u32
...
pub extern "C" fn xmlXPathEvalExpr(mut ctxt: xmlXPathParserContextPtr) {
    ...
    if !comp.is_null() {
        ...
        *fresh373 = comp;
        ...
        if (unsafe { (*(*ctxt).comp).nbStep }) > 1 as i32
            && (unsafe { (*(*ctxt).comp).last }) >= 0 as i32
        {
            if !(unsafe { (*ctxt).context }).is_null() {
                oldDepth = unsafe { (*(*ctxt).context).depth };
            }
    ...
    if !target.is_null() {
        ...
        while (unsafe { *cptr }) != 0 {
            if (unsafe { *cptr }) as i32 >= 'A' as i32 && (unsafe { *cptr }) as i32 <= 'Z' as i32
                || (unsafe { *cptr }) as i32 >= 'a' as i32
                    && (unsafe { *cptr }) as i32 <= 'z' as i32
                || (unsafe { *cptr }) as i32 >= '0' as i32
                    && (unsafe { *cptr }) as i32 <= '9' as i32
