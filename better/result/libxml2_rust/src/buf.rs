...
pub extern "C" fn xmlBufCreate() -> *mut crate::src::buf::_xmlBuf {
    ...
    ret = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlBuf>() as u64)
    }) as xmlBufPtr;
    if ret.is_null() {
        xmlBufMemoryError(
            0 as xmlBufPtr,
...
pub extern "C" fn xmlBufResize(mut buf: *mut crate::src::buf::_xmlBuf, mut size: u64) -> i32 {
        ...
        && !(unsafe { (*buf).contentIO }).is_null()
    {
        ...
        if (unsafe { (*buf).content }).is_null() {
            rebuf = (unsafe { xmlMallocAtomic.expect("non-null function pointer")(newSize) })
                as *mut xmlChar;
            (unsafe { (*buf).use_0 = 0 as i32 as size_t });
...
pub extern "C" fn xmlBufMergeBuffer(
    ...
    if !buffer.is_null()
        && !(unsafe { (*buffer).content }).is_null()
        && (unsafe { (*buffer).use_0 }) > 0 as i32 as u32
    {
        ret = xmlBufAdd(
            buf,
            unsafe { (*buffer).content },
            (unsafe { (*buffer).use_0 }) as i32,
        );
    }
    (unsafe { xmlBufferFree(buffer) });
    return ret;
