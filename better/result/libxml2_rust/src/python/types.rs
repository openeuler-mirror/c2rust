    ...
    match (unsafe { (*obj).type_0 }) as u32 {
        9 => {
                ...
                || (unsafe { (*(*obj).nodesetval).nodeTab }).is_null()
            {
                ...
                node = unsafe {
                    (**((*(*obj).nodesetval).nodeTab).offset(0 as i32 as isize)).children
                };
                while !node.is_null() {
                    len += 1;
                ...
                || (unsafe { (*(*obj).nodesetval).nodeNr }) == 0 as i32
            {
                ...
                while i_0 < (unsafe { (*(*obj).nodesetval).nodeNr }) {
                    ...
                    if (unsafe { (*node_0).type_0 }) as u32 == XML_NAMESPACE_DECL as i32 as u32 {
                        let mut ns: *mut PyObject = unsafe {
                            PyCapsule_New(
                        ...
                        let fresh17 = unsafe {
                            &mut (*((*(*obj).nodesetval).nodeTab).offset(i_0 as isize))
                        };
                        *fresh17 = 0 as xmlNodePtr;
                    } else {
                        (unsafe {
                            PyList_SetItem(ret, i_0 as Py_ssize_t, libxml_xmlNodePtrWrap(node_0))
                        });
                    }
                    i_0 += 1;
                }
        ...
        while (i as i64) < (unsafe { PyList_Size(obj) }) {
            ...
            if !(node.is_null() || (unsafe { (*node).ob_type }).is_null()) {
                    ...
                    == (unsafe { &mut PyCapsule_Type }) as *mut PyTypeObject
                {
                    ...
                    PyObject_HasAttrString(node, b"_o\0" as *const u8 as *const i8 as *mut i8)
                }) != 0
                    && (unsafe {
                    ...
                    }) != 0
                {
                    ...
                    wrapper = unsafe {
                        PyObject_GetAttrString(node, b"_o\0" as *const u8 as *const i8 as *mut i8)
                    };
                    if !wrapper.is_null() {
