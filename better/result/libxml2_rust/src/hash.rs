...
pub extern "C" fn xmlHashCreate(mut size: i32) -> *mut crate::src::hash::_xmlHashTable {
    ...
    table = (unsafe {
        xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<xmlHashTable>() as u64)
    }) as xmlHashTablePtr;
    if !table.is_null() {
...
extern "C" fn stubHashScannerFull(
    ...
    (unsafe {
        ((*stubdata).hashscanner).expect("non-null function pointer")(
            payload,
            (*stubdata).data,
            name as *mut xmlChar,
        )
            ...
            if !((unsafe { (*((*table).table).offset(i as isize)).valid }) == 0 as i32) {
                ...
                while !iter.is_null() {
                    ...
                    if nb != (unsafe { (*table).nbElems }) {
                        if iter
                            == (unsafe { &mut *((*table).table).offset(i as isize) })
                                as *mut _xmlHashEntry
                        {
                            if (unsafe { (*((*table).table).offset(i as isize)).valid }) == 0 as i32
                            {
                            ...
                            if (unsafe { (*((*table).table).offset(i as isize)).next }) != next {
                                iter = (unsafe { &mut *((*table).table).offset(i as isize) })
                                    as *mut _xmlHashEntry;
                            }
                        } else {
                            iter = next;
...
pub extern "C" fn xmlHashScanFull3<'a1>(
    ...
    if !((*(borrow_mut(&mut table)).unwrap()).table).is_null() {
        ...
        while i < (*(borrow(&table)).unwrap()).size {
            if !((unsafe { (*((*(borrow(&table)).unwrap()).table).offset(i as isize)).valid })
                == 0 as i32)
            {
                ...
                *fresh43 = 0 as *mut libc::c_void;
                ...
                if !prev.is_null() {
                    ...
                    *fresh44 = unsafe { (*entry).next };
                    (unsafe {
                        xmlFree.expect("non-null function pointer")(entry as *mut libc::c_void)
                    });
