...
extern "C" fn asciiToUTF8(
    ...
    while in_0 < inend
        && ((unsafe { out.offset_from(outstart) }) as i64 + 5 as i32 as i64)
            < (unsafe { *outlen }) as i64
    {
        let mut fresh0 = in_0;
...
pub extern "C" fn isolat1ToUTF8(
    ...
    while in_0 < inend && out < (unsafe { outend.offset(-(1 as i32 as isize)) }) {
        ...
        if (unsafe { instop.offset_from(in_0) }) as i64
            > (unsafe { outend.offset_from(out) }) as i64
        {
...
pub extern "C" fn xmlCleanupEncodingAliases() {
    ...
    (unsafe {
        xmlFree.expect("non-null function pointer")(xmlCharEncodingAliases as *mut libc::c_void)
    });
    ...
    while i < 99 as i32 {
        upper[i as usize] = ({
            ...
            if ::std::mem::size_of::<i8>() as u64 > 1 as i32 as u64 {
                ...
                __res = unsafe {
                    *(*__ctype_toupper_loc()).offset(*alias.offset(i as isize) as i32 as isize)
                };
            }
            __res
        }) as i8;
...
pub extern "C" fn xmlAddEncodingAlias<'a1>(
    ...
    let fresh43 = unsafe {
        &mut ((*xmlCharEncodingAliases.offset(xmlCharEncodingAliasesNb as isize)).name)
    };
    *fresh43 = unsafe { xmlMemStrdup.expect("non-null function pointer")(name) };
    let fresh44 = unsafe {
        &mut ((*xmlCharEncodingAliases.offset(xmlCharEncodingAliasesNb as isize)).alias)
    };
    ...
    while i < 499 as i32 {
        upper[i as usize] = ({
            ...
            if ::std::mem::size_of::<i8>() as u64 > 1 as i32 as u64 {
                ...
                __res = unsafe {
                    *(*__ctype_toupper_loc()).offset(*name.offset(i as isize) as i32 as isize)
                };
            }
            __res
        }) as i8;
...
pub extern "C" fn xmlInitCharEncodingHandlers() {
    ...
    (unsafe {
        handlers = xmlMalloc.expect("non-null function pointer")(
            (50 as i32 as u64)
                .wrapping_mul(::std::mem::size_of::<xmlCharEncodingHandlerPtr>() as u64),
...
pub extern "C" fn xmlRegisterCharEncodingHandler(
    ...
    if !handler.is_null() {
        if !(unsafe { (*handler).name }).is_null() {
            (unsafe {
                xmlFree.expect("non-null function pointer")((*handler).name as *mut libc::c_void)
            });
        }
...
extern "C" fn xmlEncInputChunk(
    ...
    if unsafe { ((*handler).input).is_some() } {
        ret = unsafe {
            ((*handler).input).expect("non-null function pointer")(out, outlen, in_0, inlen)
        };
        if ret > 0 as i32 {
            ret = 0 as i32;
        }
...
extern "C" fn xmlEncOutputChunk(
    ...
    if unsafe { ((*handler).output).is_some() } {
        ret = unsafe {
            ((*handler).output).expect("non-null function pointer")(out, outlen, in_0, inlen)
        };
        if ret > 0 as i32 {
            ret = 0 as i32;
        }
    ...
    loop {
        ...
        match ret {
            ...
            -2 => {
                ...
                charrefLen = unsafe {
                    snprintf(
                        &mut *charref.as_mut_ptr().offset(0 as i32 as isize) as *mut xmlChar
                            as *mut i8,
        ...
        match ret {
            ...
            -2 => {
                ...
                if ret < 0 as i32 || toconv != charrefLen {
                    ...
                    {
                        (unsafe {
                            *((*in_0).content).offset(0 as i32 as isize) = ' ' as i32 as xmlChar
                        });
                    }
                    break;
                } else {
...
pub extern "C" fn xmlCharEncCloseFunc(
    ...
    if handler_in_list == 0 as i32
        && (!(unsafe { (*handler).iconv_out }).is_null()
            || !(unsafe { (*handler).iconv_in }).is_null())
    {
        tofree = 1 as i32;
...
pub extern "C" fn xmlByteConsumed<'a1>(
    ...
    return (unsafe { (*in_0).consumed })
        .wrapping_add((unsafe { ((*in_0).cur).offset_from((*in_0).base) }) as i64 as u64)
        as i64;
}
