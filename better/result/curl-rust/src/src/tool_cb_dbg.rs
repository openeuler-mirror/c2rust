...
pub struct URLGlob {
    pub pattern: Option<
        crate::__laertes_array::CustomSlice<
            'static,
            crate::src::src::tool_cb_dbg::URLPattern,
            [crate::src::src::tool_cb_dbg::URLPattern; 100],
        >,
    >,
    pub size: u64,
    pub urllen: u64,
...
pub extern "C" fn tool_debug_cb(
    ...
    let mut operation: *mut crate::src::src::tool_cb_dbg::OperationConfig =
        userdata as *mut OperationConfig;
    let mut config: *mut crate::src::src::tool_cb_dbg::GlobalConfig = unsafe { (*operation).global };
    let mut output: *mut crate::src::lib::http2::_IO_FILE = unsafe { (*config).errors };
    let mut text: *const i8 = 0 as *const i8;
    let mut tv: crate::src::lib::openldap::timeval = timeval {
        tv_sec: 0,
        tv_usec: 0,
    };
    ...
    if (unsafe { (*config).trace_stream }).is_null() {
        if (unsafe { strcmp(b"-\0" as *const u8 as *const i8, (*config).trace_dump) }) == 0 {
            let mut fresh0 = unsafe { &mut ((*config).trace_stream) };
            *fresh0 = unsafe { stdout };
        } else if (unsafe { strcmp(b"%\0" as *const u8 as *const i8, (*config).trace_dump) }) == 0 {
            let mut fresh1 = unsafe { &mut ((*config).trace_stream) };
            *fresh1 = unsafe { (*config).errors };
        } else {
            let mut fresh2 = unsafe { &mut ((*config).trace_stream) };
