...
pub extern "C" fn parseconfig(
    ...
    let mut operation: *mut crate::src::src::tool_cb_dbg::OperationConfig = unsafe { (*global).last };
    let mut pathalloc: *mut i8 = 0 as *mut i8;
    if filename.is_null() || (unsafe { *filename }) == 0 {
        let mut home: *mut i8 = homedir(b".curlrc\0" as *const u8 as *const i8);
        if !home.is_null() {
    ...
    if !file.is_null() {
        ...
        while my_get_line(file, &mut buf, Some(&mut fileerror)) {
            ...
            if line.is_null() {
                ...
                while (unsafe { *line }) as i32 != 0 && Curl_isspace((unsafe { *line }) as u8 as i32) != 0 {
                ...
                match (unsafe { *line }) as i32 {
                    ...
                    _ => {
                        ...
                        while (unsafe { *line }) as i32 != 0
                            && Curl_isspace((unsafe { *line }) as u8 as i32) == 0
                            && !(!dashed_option
                        ...
                        res = getparameter(option, param, Some(&mut usedarg), global, operation)
                            as i32;
                        ...
                        if res == PARAM_NEXT_OPERATION as i32 {
                            ...
                            {
                                let mut fresh1 = unsafe { &mut ((*operation).next) };
                                *fresh1 = (unsafe { malloc(::std::mem::size_of::<OperationConfig>() as u64) })
                                    as *mut OperationConfig;
                                if !(unsafe { (*operation).next }).is_null() {
                                    config_init(unsafe { (*operation).next });
                                    let mut fresh2 = unsafe { &mut ((*(*operation).next).global) };
                                    *fresh2 = global;
                                    let mut fresh3 = unsafe { &mut ((*global).last) };
                                    *fresh3 = unsafe { (*operation).next };
                                    let mut fresh4 = unsafe { &mut ((*(*operation).next).prev) };
                                    *fresh4 = operation;
