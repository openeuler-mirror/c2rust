...
extern "C" fn print_category(mut category: u32) {
    let mut i: u32 = 0;
    i = 0 as i32 as u32;
...
extern "C" fn get_category_content(mut category: *const i8) -> i32 {
    ...
    while !(unsafe { categories[i as usize].opt }).is_null() {
        if curl_strequal(unsafe { categories[i as usize].opt }, category) != 0 {
            (unsafe { curl_mprintf(
                b"%s: %s\n\0" as *const u8 as *const i8,
...
pub extern "C" fn tool_help(mut category: *mut i8) {
    ...
    } else if curl_strequal(category, b"category\0" as *const u8 as *const i8) != 0 {
        get_categories();
...
extern "C" fn featcomp(mut p1: *const core::ffi::c_void, mut p2: *const core::ffi::c_void) -> i32 {
...
pub extern "C" fn tool_version_info() {
    ...
    if (unsafe { (*curlinfo).features }) != 0 {
        let mut featp: Option<
            crate::__laertes_array::CustomSlice<'static, *mut i8, [*mut i8; 29]>,
        > = Some(crate::__laertes_array::CustomSlice::new([0 as *mut i8; 29]));
                ...
                .wrapping_div(::std::mem::size_of::<feat>() as u64)
        {
            if (unsafe { (*curlinfo).features }) & (unsafe { feats[i as usize].bitmask }) != 0 {
                ...
                (*crate::__laertes_array::GetMut::<&mut _>::get_add_mut(
                    (featp).as_mut().unwrap(),
                    fresh0 as usize,
                )) = (unsafe { feats[i as usize].name }) as *mut i8;
            }
            i = i.wrapping_add(1);
        }
        (unsafe { qsort(
