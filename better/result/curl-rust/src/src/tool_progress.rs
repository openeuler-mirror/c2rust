...
pub extern "C" fn progress_meter<'a1>(
    ...
    if final_0 as i32 != 0 || diff > 500 as i32 as i64 {
        ...
        let mut spent: i64 = tvdiff(now, *(borrow_mut(&mut start)).unwrap()) / 1000 as i32 as i64;
        let mut dlpercen: [i8; 4] =
            *(unsafe { core::intrinsics::transmute::<&'_ [u8; 4], &'_ mut [i8; 4]>(b"--\0\0") });
        let mut ulpercen: [i8; 4] =
            *(unsafe { core::intrinsics::transmute::<&'_ [u8; 4], &'_ mut [i8; 4]>(b"--\0\0") });
        ...
        if unsafe { indexwrapped } {
            deltams = tvdiff(now, unsafe { speedstore[speedindex as usize].stamp });
        ...
        if dlknown as i32 != 0 && speed != 0 {
            let mut est: i64 = (unsafe { all_dltotal }) / speed;
            let mut left: i64 = ((unsafe { all_dltotal }) - all_dlnow) / speed;
...
#[no_mangle]
pub extern "C" fn progress_finalize<'a1>(
