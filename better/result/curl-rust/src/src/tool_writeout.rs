...
pub struct writeoutvar<'a> {
    ...
    pub writefunc: Option<
        unsafe extern "C" fn(
            ...
            _: Option<&'a crate::src::src::tool_writeout::writeoutvar<'a>>,
            _: *mut crate::src::src::tool_cb_hdr::per_transfer<'a>,
            _: u32,
            _: bool,
        ) -> i32,
    >,
...
extern "C" fn writeTime<'a1, 'a2, 'a3>(
    ...
    if (*((wovar).clone()).unwrap()).ci as u64 != 0 {
        if (unsafe { curl_easy_getinfo(
            (*per).curl,
            (*(wovar).unwrap()).ci,
            &mut us as *mut curl_off_t,
        ) }) as u64
            == 0
        {
            valid = 1 as i32 != 0;
...
extern "C" fn writeString<'a1, 'a2, 'a3>(
    mut stream: *mut crate::src::lib::http2::_IO_FILE,
    mut wovar: Option<&'a1 crate::src::src::tool_writeout::writeoutvar<'a2>>,
    mut per: *mut crate::src::src::tool_cb_hdr::per_transfer<'a3>,
    mut per_result: u32,
    ...
    if (*((wovar).clone()).unwrap()).ci as u64 != 0 {
        if (*((wovar).clone()).unwrap()).ci as u32 == CURLINFO_HTTP_VERSION as i32 as u32 {
        ...
        } else if (unsafe { curl_easy_getinfo(
            (*per).curl,
            (*(wovar).unwrap()).ci,
...
extern "C" fn writeLong<'a1, 'a2, 'a3>(
    mut stream: *mut crate::src::lib::http2::_IO_FILE,
    mut wovar: Option<&'a1 crate::src::src::tool_writeout::writeoutvar<'a2>>,
    mut per: *mut crate::src::src::tool_cb_hdr::per_transfer<'a3>,
    mut per_result: u32,
    ...
    if valid {
        ...
        } else if (*((wovar).clone()).unwrap()).id as u32 == VAR_HTTP_CODE as i32 as u32
            || (*((wovar).clone()).unwrap()).id as u32 == VAR_HTTP_CODE_PROXY as i32 as u32
        {
...
extern "C" fn writeOffset<'a1, 'a2, 'a3>(
    mut stream: *mut crate::src::lib::http2::_IO_FILE,
    mut wovar: Option<&'a1 crate::src::src::tool_writeout::writeoutvar<'a2>>,
    mut per: *mut crate::src::src::tool_cb_hdr::per_transfer<'a3>,
    mut _per_result: u32,
...
#[no_mangle]
pub extern "C" fn ourWriteOut<'a1>(
    ...
    'a1: 'static,
{
    let mut stream: *mut crate::src::lib::http2::_IO_FILE = unsafe { stdout };
                ...
                if '{' as i32 == (unsafe { *ptr.offset(1 as i32 as isize) }) as i32 {
                    ...
                    if end.is_null() {
                        ...
                        while !(unsafe { variables[i as usize].name }).is_null() {
                            if curl_strequal(ptr, unsafe { variables[i as usize].name }) != 0 {
                                match_0 = 1 as i32 != 0;
