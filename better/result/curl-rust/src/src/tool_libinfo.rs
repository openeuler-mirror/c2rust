...
impl std::default::Default for proto_name_pattern {
    fn default() -> Self {
        proto_name_pattern::new()
    }
}
#[no_mangle]
pub static mut curlinfo: *mut crate::src::lib::version::curl_version_info_data<'static> =
    0 as *mut crate::src::lib::version::curl_version_info_data<'_>;
#[no_mangle]
pub static mut built_in_protos: i64 = 0 as i32 as i64;
#[no_mangle]
pub extern "C" fn get_libcurl_info() -> u32 {
    ...
    (unsafe { curlinfo = curl_version_info(CURLVERSION_TENTH) });
    if (unsafe { curlinfo }).is_null() {
        return CURLE_FAILED_INIT;
    }
    (unsafe { built_in_protos = 0 as i32 as i64 });
    if !(unsafe { (*curlinfo).protocols }).is_null() {
        proto = unsafe { (*curlinfo).protocols };
        while !(unsafe { *proto }).is_null() {
            let mut p: *const crate::src::src::tool_libinfo::proto_name_pattern =
                0 as *const crate::src::src::tool_libinfo::proto_name_pattern;
            p = unsafe { possibly_built_in.as_ptr() };
            while !(unsafe { (*p).proto_name }).is_null() {
                if curl_strequal(unsafe { *proto }, unsafe { (*p).proto_name }) != 0 {
                    (unsafe { built_in_protos |= (*p).proto_pattern });
                    break;
                } else {
                    p = unsafe { p.offset(1) };
                }
