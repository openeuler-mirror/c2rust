...
extern "C" {
    ...
    fn signal(
        __sig: i32,
        __handler: Option<unsafe extern "C" fn(_: i32) -> ()>,
    ...
    *fresh2 = *fresh1;
    if !(unsafe { (*config).first }).is_null() {
        ...
        if result as u64 == 0 {
            ...
            if result as u64 == 0 {
                config_init(unsafe { (*config).first });
                let mut fresh3 = unsafe { &mut ((*(*config).first).global) };
                *fresh3 = config;
            } else {
                (unsafe { errorf(
                    config,
...
extern "C" fn free_globalconfig<'a1>(
    mut config: Option<&'a1 mut crate::src::src::tool_cb_dbg::GlobalConfig>,
) {
    (unsafe { free((*(borrow_mut(&mut config)).unwrap()).trace_dump as *mut libc::c_void) });
    let mut fresh4 = &mut ((*(borrow_mut(&mut config)).unwrap()).trace_dump);
    *fresh4 = 0 as *mut i8;
    if (*(borrow(&config)).unwrap()).errors_fopened as i32 != 0
        && !((*(borrow_mut(&mut config)).unwrap()).errors).is_null()
    {
        (unsafe { fclose((*(borrow_mut(&mut config)).unwrap()).errors) });
    }
    let mut fresh5 = &mut ((*(borrow_mut(&mut config)).unwrap()).errors);
    *fresh5 = 0 as *mut FILE;
    if (*(borrow(&config)).unwrap()).trace_fopened as i32 != 0
        && !((*(borrow_mut(&mut config)).unwrap()).trace_stream).is_null()
    {
        (unsafe { fclose((*(borrow_mut(&mut config)).unwrap()).trace_stream) });
    }
    let mut fresh6 = &mut ((*(borrow_mut(&mut config)).unwrap()).trace_stream);
    *fresh6 = 0 as *mut FILE;
    (unsafe { free((*(borrow_mut(&mut config)).unwrap()).libcurl as *mut libc::c_void) });
    let mut fresh7 = &mut ((*(borrow_mut(&mut config)).unwrap()).libcurl);
    *fresh7 = 0 as *mut i8;
}
extern "C" fn main_free<'a1>(
    mut config: Option<&'a1 mut crate::src::src::tool_cb_dbg::GlobalConfig>,
) {
    curl_global_cleanup();
...
fn main_0(mut argc: i32, mut argv: *mut *mut i8) -> i32 {
    ...
    (unsafe { signal(
        13 as i32,
        core::intrinsics::transmute::<isize, Option<unsafe extern "C" fn(_: i32) -> ()>>(
