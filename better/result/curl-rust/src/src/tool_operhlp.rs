...
#[no_mangle]
pub extern "C" fn clean_getout(mut config: *mut crate::src::src::tool_cb_dbg::OperationConfig) {
    if !config.is_null() {
        let mut next: *mut crate::src::src::tool_cb_dbg::getout = 0 as *mut getout;
        let mut node: *mut crate::src::src::tool_cb_dbg::getout = unsafe { (*config).url_list };
        while !node.is_null() {
            next = unsafe { (*node).next };
            (unsafe { free((*node).url as *mut libc::c_void) });
            let mut fresh0 = unsafe { &mut ((*node).url) };
            *fresh0 = 0 as *mut i8;
            (unsafe { free((*node).outfile as *mut libc::c_void) });
            let mut fresh1 = unsafe { &mut ((*node).outfile) };
            *fresh1 = 0 as *mut i8;
            (unsafe { free((*node).infile as *mut libc::c_void) });
            let mut fresh2 = unsafe { &mut ((*node).infile) };
            *fresh2 = 0 as *mut i8;
            (unsafe { free(node as *mut libc::c_void) });
            node = 0 as *mut getout;
            node = next;
        }
        let mut fresh3 = unsafe { &mut ((*config).url_list) };
        *fresh3 = 0 as *mut getout;
    }
}
#[no_mangle]
pub extern "C" fn output_expected(mut url: *const i8, mut uploadfile: *const i8) -> bool {
    ...
    if curl_strnequal(
        ...
        url,
        unsafe { strlen(b"http://\0" as *const u8 as *const i8) },
    ) != 0
        || curl_strnequal(
            b"https://\0" as *const u8 as *const i8,
            url,
            unsafe { strlen(b"https://\0" as *const u8 as *const i8) },
        ) != 0
    {
        return 1 as i32 != 0;
    }
...
#[no_mangle]
pub extern "C" fn stdin_upload(mut uploadfile: *const i8) -> bool {
    return if (unsafe { strcmp(uploadfile, b"-\0" as *const u8 as *const i8) }) == 0
        || (unsafe { strcmp(uploadfile, b".\0" as *const u8 as *const i8) }) == 0
    {
        1 as i32
    } else {
        0 as i32
...
#[no_mangle]
pub extern "C" fn add_file_name_to_url(mut url: *mut i8, mut filename: *const i8) -> *mut i8 {
    let mut ptr: *mut i8 = unsafe { strstr(url, b"://\0" as *const u8 as *const i8) };
    ...
    ptr = unsafe { strrchr(ptr, '/' as i32) };
    if ptr.is_null() || {
        ptr = unsafe { ptr.offset(1) };
        (unsafe { *ptr }) == 0
    } {
        let mut filep: *const i8 = unsafe { strrchr(filename, '/' as i32) };
        let mut file2: *mut i8 =
            unsafe { strrchr(if !filep.is_null() { filep } else { filename }, '\\' as i32) };
        let mut encfile: *mut i8 = 0 as *mut i8;
        if !file2.is_null() {
            filep = unsafe { file2.offset(1 as i32 as isize) };
        } else if !filep.is_null() {
            filep = unsafe { filep.offset(1) };
        } else {
            filep = filename;
        }
        ...
        if !encfile.is_null() {
            ...
            if !ptr.is_null() {
                urlbuffer = unsafe { curl_maprintf(b"%s%s\0" as *const u8 as *const i8, url, encfile) };
            } else {
                urlbuffer = unsafe { curl_maprintf(b"%s/%s\0" as *const u8 as *const i8, url, encfile) };
            }
            ...
            if urlbuffer.is_null() {
                ...
                (unsafe { free(url as *mut libc::c_void) });
                url = 0 as *mut i8;
                url = urlbuffer;
            }
        }
...
#[no_mangle]
pub extern "C" fn get_url_file_name<'a1>(
    ...
    *(borrow_mut(&mut filename)).unwrap() = 0 as *mut i8;
    pc = unsafe { strstr(url, b"://\0" as *const u8 as *const i8) };
    if !pc.is_null() {
        pc = unsafe { pc.offset(3 as i32 as isize) };
    } else {
        pc = url;
    }
    pc2 = unsafe { strrchr(pc, '\\' as i32) };
    ...
    *(borrow_mut(&mut filename)).unwrap() = unsafe { strdup(pc) };
