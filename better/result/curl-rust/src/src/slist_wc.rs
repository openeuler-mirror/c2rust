...
#[no_mangle]
pub extern "C" fn slist_wc_append(
    mut list: *mut crate::src::src::slist_wc::slist_wc,
    mut data: *const i8,
) -> *mut crate::src::src::slist_wc::slist_wc {
    let mut new_item: *mut crate::src::lib::http2::curl_slist =
        curl_slist_append(0 as *mut curl_slist, data);
    if new_item.is_null() {
    ...
    if list.is_null() {
        ...
        let mut fresh0 = unsafe { &mut ((*list).first) };
        *fresh0 = new_item;
        let mut fresh1 = unsafe { &mut ((*list).last) };
        *fresh1 = new_item;
        return list;
    }
    let mut fresh2 = unsafe { &mut ((*(*list).last).next) };
    *fresh2 = new_item;
    let mut fresh3 = unsafe { &mut ((*list).last) };
    *fresh3 = unsafe { (*(*list).last).next };
    return list;
}
#[no_mangle]
pub extern "C" fn slist_wc_free_all(mut list: *mut crate::src::src::slist_wc::slist_wc) {
    if list.is_null() {
        return;
    }
    curl_slist_free_all(unsafe { (*list).first });
