...
extern "C" fn c_escape(mut str: *const i8, mut len: i64) -> *mut i8 {
...
pub extern "C" fn tool_setopt_enum(
    ...
    if !(unsafe { (*config).libcurl }).is_null() && !skip && ret as u64 == 0 {
        let mut nv: *const crate::src::src::tool_operate::NameValue =
            0 as *const crate::src::src::tool_operate::NameValue;
        nv = nvlist;
...
pub extern "C" fn tool_setopt_bitmask(
    ...
    if !(unsafe { (*config).libcurl }).is_null() && !skip && ret as u64 == 0 {
        ...
        let mut nv: *const crate::src::src::tool_operate::NameValueUnsigned =
            0 as *const crate::src::src::tool_operate::NameValueUnsigned;
        (unsafe { curl_msnprintf(
            preamble.as_mut_ptr(),
            ::std::mem::size_of::<[i8; 80]>() as u64,
            b"curl_easy_setopt(hnd, %s, \0" as *const u8 as *const i8,
...
extern "C" fn libcurl_generate_slist<'a1>(
    ...
    *(borrow_mut(&mut slistno)).unwrap() = unsafe { easysrc_slist_count };
    ret = unsafe { easysrc_addf(
                                                ...
                                                match current_block {
                                                    ...
                                                    _ => {
                                                            ...
                                                            && !(unsafe { (*part).headers }).is_null()
                                                        {
                                                            ...
                                                            if ret as u64 == 0 {
                                                                ret = unsafe { easysrc_addf (Some (& mut easysrc_code) , b"curl_mime_headers(part%d, slist%d, 1);\0" as * const u8 as * const i8 , mimeno , slistno ,) } ;
                                                                if !(ret as u64 != 0) {
                                                                    ret = unsafe { easysrc_addf(
                                                                        Some(&mut easysrc_code),
                                                                        b"slist%d = NULL;\0"
...
extern "C" fn libcurl_generate_mime<'a1>(
    mut curl: *mut crate::src::lib::http2::Curl_easy,
    mut config: *mut crate::src::src::tool_cb_dbg::GlobalConfig,
    ...
    *(borrow_mut(&mut mimeno)).unwrap() = unsafe { easysrc_mime_count };
    ret = unsafe { easysrc_addf(
        Some(&mut easysrc_decl),
