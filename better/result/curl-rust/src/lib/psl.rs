...
#[no_mangle]
pub extern "C" fn Curl_psl_destroy(mut pslcache: *mut crate::src::lib::http2::PslCache) {
    if !(unsafe { (*pslcache).psl }).is_null() {
        if unsafe { (*pslcache).dynamic } {
            (unsafe { psl_free((*pslcache).psl as *mut psl_ctx_t) });
        }
        let mut fresh0 = unsafe { &mut ((*pslcache).psl) };
        *fresh0 = 0 as *const psl_ctx_t;
        (unsafe { (*pslcache).dynamic = 0 as i32 != 0 });
    }
}
extern "C" fn now_seconds() -> i64 {
    let mut now: crate::src::lib::http2::curltime = Curl_now();
    return now.tv_sec;
}
#[no_mangle]
pub extern "C" fn Curl_psl_use(
    mut easy: *mut crate::src::lib::http2::Curl_easy,
) -> *const crate::src::lib::urlapi::psl_ctx_st {
    let mut pslcache: *mut crate::src::lib::http2::PslCache = unsafe { (*easy).psl };
    ...
    if (unsafe { (*pslcache).psl }).is_null() || (unsafe { (*pslcache).expires }) <= now {
        ...
        if (unsafe { (*pslcache).psl }).is_null() || (unsafe { (*pslcache).expires }) <= now {
            ...
            if !psl.is_null() {
                ...
                let mut fresh1 = unsafe { &mut ((*pslcache).psl) };
                *fresh1 = psl;
...
#[no_mangle]
pub extern "C" fn Curl_psl_release(mut easy: *mut crate::src::lib::http2::Curl_easy) {
    Curl_share_unlock(easy, CURL_LOCK_DATA_PSL);
}
