...
extern "C" fn convert_lineends(
    ...
    if (unsafe { ((*data).state).prev_block_had_trailing_cr() }) != 0 {
        if (unsafe { *startPtr }) as i32 == '\n' as i32 {
            ...
            let mut fresh0 = unsafe { &mut ((*data).state.crlf_conversions) };
            *fresh0 += 1;
        }
    ...
    if !inPtr.is_null() {
        while inPtr < (unsafe { startPtr.offset(size as isize).offset(-(1 as i32 as isize)) }) {
            ...
            ) }) == 0 as i32
            {
                ...
                (unsafe { *outPtr = *inPtr });
                let mut fresh2 = unsafe { &mut ((*data).state.crlf_conversions) };
                *fresh2 += 1;
...
#[no_mangle]
pub extern "C" fn Curl_write_plain<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut sockfd: i32,
    mut mem: *const core::ffi::c_void,
    mut len: u64,
...
extern "C" fn pausewrite(
    ...
    let mut s: Option<&'_ mut crate::src::lib::http2::UrlState> = Some(unsafe { &mut (*data).state });
    let mut i: u32 = 0;
    ...
    if newtype {
        Curl_dyn_init(
            unsafe { &mut (*((*(borrow_mut(&mut s)).unwrap()).tempwrite)
                .as_mut_ptr()
                .offset(i as isize))
            .b },
...
#[no_mangle]
pub extern "C" fn Curl_read_plain<'a1>(
    mut sockfd: i32,
    mut buf: *mut i8,
    mut bytesfromsocket: u64,
    mut n: Option<&'a1 mut i64>,
) -> u32 {
    let mut nread: i64 = unsafe { recv(sockfd, buf as *mut libc::c_void, bytesfromsocket, 0 as i32) };
    if -(1 as i32) as i64 == nread {
