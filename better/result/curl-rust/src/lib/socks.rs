...
extern "C" fn socksstate(mut data: *mut crate::src::lib::http2::Curl_easy, mut state: u32) {
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    let mut oldstate: u32 = unsafe { (*conn).cnnct.state };
...
#[no_mangle]
pub extern "C" fn Curl_SOCKS4<'a1>(
    ...
    let mut sx: Option<&'_ mut crate::src::lib::http2::connstate> = Some(unsafe { &mut (*conn).cnnct });
    ...
    match (*(borrow(&sx)).unwrap()).state as u32 {
        1 => {
            ...
            if !protocol4a {
                let mut rc: i32 =
                    Curl_resolv(data, hostname, remote_port, 0 as i32 != 0, Some(&mut dns));
        ...
        10 => {
            ...
            if !dns.is_null() {
                let mut fresh0 = unsafe { &mut ((*data).state.async_0.dns) };
                *fresh0 = dns;
                let mut fresh1 = unsafe { &mut ((*data).state.async_0) };
    ...
    match current_block_107 {
        1526476230797115868 => {
            ...
            if !dns.is_null() {
                ...
                if !hp.is_null() {
                    let mut saddr_in: *mut crate::src::lib::connect::sockaddr_in =
    ...
    match current_block_107 {
        10721739657148430059 => {
            ...
            if !proxy_user.is_null() {
                let mut plen: u64 = unsafe { strlen(proxy_user) };
...
#[no_mangle]
pub extern "C" fn Curl_SOCKS5<'a1>(
    mut proxy_user: *const i8,
    ...
    let hostname_len: u64 = unsafe { strlen(hostname) };
    ...
    match (*(borrow(&sx)).unwrap()).state as u32 {
        ...
        10 => {
            ...
            if !dns.is_null() {
                let mut fresh27 = unsafe { &mut ((*data).state.async_0.dns) };
                *fresh27 = dns;
                let mut fresh28 = unsafe { &mut ((*data).state.async_0) };
    ...
    match current_block_276 {
        ...
        6938158527927677584 => {
            ...
            if (unsafe { (*hp).ai_family }) == 2 as i32 {
                ...
                let mut i_0: i32 = 0;
                let mut saddr_in6: *mut crate::src::lib::connect::sockaddr_in6 =
