...
extern "C" {
    ...
    fn connect(__fd: i32, __addr: *const crate::src::lib::http2::sockaddr, __len: u32) -> i32;
    fn getpeername(
        __fd: i32,
        __addr: *mut crate::src::lib::http2::sockaddr,
        __len: *mut u32,
...
extern "C" fn tcpkeepalive(mut data: *mut crate::src::lib::http2::Curl_easy, mut sockfd: i32) {
    ...
    ) }) < 0 as i32
    {
        ...
        optval = curlx_sltosi(unsafe { (*data).set.tcp_keepidle });
        if (unsafe { setsockopt(
            sockfd,
            IPPROTO_TCP as i32,
            4 as i32,
        ...
        optval = curlx_sltosi(unsafe { (*data).set.tcp_keepintvl });
        if (unsafe { setsockopt(
            sockfd,
            IPPROTO_TCP as i32,
            5 as i32,
...
pub extern "C" fn Curl_timeleft(
    ...
    if timeout_set & 2 as i32 as u32 != 0 {
        maxtime_timeout_ms -= Curl_timediff(unsafe { *nowp }, unsafe { (*data).progress.t_startop });
        timeout_ms = maxtime_timeout_ms;
    }
    if timeout_set & 1 as i32 as u32 != 0 {
        connect_timeout_ms -= Curl_timediff(unsafe { *nowp }, unsafe { (*data).progress.t_startsingle });
...
extern "C" fn bindlocal(
    ...
    let mut sock: *mut crate::src::lib::http2::sockaddr =
        &mut sa as *mut Curl_sockaddr_storage as *mut sockaddr;
    let mut sizeof_sa: u32 = 0 as i32 as curl_socklen_t;
    let mut si4: *mut crate::src::lib::connect::sockaddr_in =
        &mut sa as *mut Curl_sockaddr_storage as *mut sockaddr_in;
    let mut si6: *mut crate::src::lib::connect::sockaddr_in6 =
        &mut sa as *mut Curl_sockaddr_storage as *mut sockaddr_in6;
    ...
    if !dev.is_null() && (unsafe { strlen(dev) }) < 255 as i32 as u64 {
        ...
        if !is_interface {
            ...
            rc = Curl_resolv(data, dev, 0 as i32, 0 as i32 != 0, Some(&mut h)) as i32;
        ...
        if done < 1 as i32 {
            let mut fresh1 = unsafe { &mut ((*data).state) };
...
extern "C" fn verifyconnect<'a1>(mut sockfd: i32, mut error: Option<&'a1 mut i32>) -> bool {
    let mut rc: bool = 1 as i32 != 0;
    let mut err: i32 = 0 as i32;
    let mut errSize: u32 = ::std::mem::size_of::<i32>() as u64 as curl_socklen_t;
    if 0 as i32
        != (unsafe { getsockopt(
            sockfd,
            1 as i32,
...
extern "C" fn ainext(
    ...
    let mut ai: *mut crate::src::lib::http2::Curl_addrinfo = unsafe { (*conn).tempaddr[tempindex as usize] };
    ...
    let mut fresh3 = unsafe { &mut ((*conn).tempaddr[tempindex as usize]) };
    *fresh3 = ai;
    return ai;
}
extern "C" fn trynextip(
    ...
    let mut fd_to_close: i32 = unsafe { (*conn).tempsock[tempindex as usize] };
    (unsafe { (*conn).tempsock[tempindex as usize] = -(1 as i32) });
    if sockindex == 0 as i32 {
        let mut ai: *mut crate::src::lib::http2::Curl_addrinfo =
            unsafe { (*conn).tempaddr[tempindex as usize] };
        while !ai.is_null() {
...
pub extern "C" fn Curl_persistconninfo(
    ...
    let mut fresh4 = unsafe { &mut ((*data).info.conn_scheme) };
...
#[no_mangle]
pub extern "C" fn Curl_addr2string<'a1>(
    mut sa: *mut crate::src::lib::http2::sockaddr,
    mut salen: u32,
    mut addr: *mut i8,
    mut port: Option<&'a1 mut i32>,
) -> bool {
    let mut si: *mut crate::src::lib::connect::sockaddr_in =
        0 as *mut crate::src::lib::connect::sockaddr_in;
    let mut si6: *mut crate::src::lib::connect::sockaddr_in6 =
        0 as *mut crate::src::lib::connect::sockaddr_in6;
    let mut su: *mut crate::src::lib::connect::sockaddr_un =
        0 as *mut crate::src::lib::connect::sockaddr_un;
...
#[no_mangle]
pub extern "C" fn Curl_conninfo_local<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut sockfd: i32,
    mut local_ip: *mut i8,
        ...
        match (unsafe { (*conn).socks_proxy.proxytype }) as u32 {
            5 | 7 => {
                pxresult = Curl_SOCKS5(
                    unsafe { (*conn).socks_proxy.user },
                    unsafe { (*conn).socks_proxy.passwd },
                    host,
                    port,
                    sockindex,
                    data,
...
extern "C" fn post_SOCKS<'a1>(
    ...
    *(borrow_mut(&mut connected)).unwrap() = 1 as i32 != 0;
    ...
    let mut fresh5 = unsafe { &mut ((*data).info.numconnects) };
    *fresh5 += 1;
}
#[no_mangle]
pub extern "C" fn Curl_is_connected<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut sockindex: i32,
    mut connected: Option<&'a1 mut bool>,
    ...
    while i < 2 as i32 as u32 {
        ...
        if !((unsafe { (*conn).tempsock[i as usize] }) == -(1 as i32)) {
            ...
            rc = Curl_socket_check(
                -(1 as i32),
                -(1 as i32),
                unsafe { (*conn).tempsock[i as usize] },
                0 as i32 as timediff_t,
            );
            if rc == 0 as i32 {
                if Curl_timediff(now, unsafe { (*conn).connecttime }) >= (unsafe { (*conn).timeoutms_per_addr[i as usize] })
                {
                    (unsafe { Curl_infof(
                        data,
                ...
                if i == 0 as i32 as u32
                    ...
                    && Curl_timediff(now, unsafe { (*conn).connecttime }) >= (unsafe { (*data).set.happy_eyeballs_timeout })
                {
                    let mut fresh6 = unsafe { &mut ((*conn).bits) };
                ...
                if verifyconnect(unsafe { (*conn).tempsock[i as usize] }, Some(&mut error)) {
                    (unsafe { (*conn).sock[sockindex as usize] = (*conn).tempsock[i as usize] });
                    let mut fresh7 = unsafe { &mut ((*conn).ip_addr) };
                ...
                verifyconnect(unsafe { (*conn).tempsock[i as usize] }, Some(&mut error));
            }
            if error != 0 {
                (unsafe { (*data).state.os_errno = error });
                ...
                if !(unsafe { (*conn).tempaddr[i as usize] }).is_null() {
                    ...
                    let mut buffer: [i8; 256] = [0; 256];
                    Curl_printable_address(
                        unsafe { (*conn).tempaddr[i as usize] },
...
extern "C" fn tcpnodelay(mut data: *mut crate::src::lib::http2::Curl_easy, mut sockfd: i32) {
...
extern "C" fn singleipconnect(
    ...
    let mut buffer: [i8; 256] = [0; 256];
    let mut sockp: Option<&'_ mut i32> =
        Some(unsafe { &mut *((*conn).tempsock).as_mut_ptr().offset(tempindex as isize) });
    *(borrow_mut(&mut sockp)).unwrap() = -(1 as i32);
    ...
    if addr.family == 2 as i32 || addr.family == 10 as i32 {
        result = bindlocal(
            ...
            sockfd,
            addr.family,
            Curl_ipv6_scope((unsafe { &mut addr._sa_ex_u.addr }) as *mut sockaddr),
        );
        if result as u64 != 0 {
            Curl_closesocket(data, conn, sockfd);
...
pub extern "C" fn Curl_connecthost(
    ...
    let mut fresh9 = unsafe { &mut ((*conn).tempaddr[1 as i32 as usize]) };
    *fresh9 = unsafe { (*remotehost).addr };
    let mut fresh10 = unsafe { &mut ((*conn).tempaddr[0 as i32 as usize]) };
    *fresh10 = *fresh9;
    let mut fresh11 = unsafe { &mut ((*conn).tempsock[1 as i32 as usize]) };
    *fresh11 = -(1 as i32);
    ...
    Curl_expire(
        data,
        unsafe { (*data).set.happy_eyeballs_timeout },
...
extern "C" fn conn_is_conn(
    ...
    if (unsafe { (*conn).connection_id }) == (unsafe { (*f).id_tofind }) {
        let mut fresh12 = unsafe { &mut ((*f).found) };
        *fresh12 = conn;
        return 1 as i32;
    }
    return 0 as i32;
...
#[no_mangle]
pub extern "C" fn Curl_getconnectinfo<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
        ...
        && (!(unsafe { (*data).multi_easy }).is_null() || !(unsafe { (*data).multi }).is_null())
    {
        ...
        Curl_conncache_foreach(
            ...
            if !(unsafe { (*data).multi_easy }).is_null() {
                unsafe { &mut (*(*data).multi_easy).conn_cache }
            } else {
                unsafe { &mut (*(*data).multi).conn_cache }
            },
...
#[no_mangle]
pub extern "C" fn Curl_connalive(mut conn: *mut crate::src::lib::http2::connectdata) -> bool {
...
pub extern "C" fn Curl_closesocket(
    ...
    if !conn.is_null() && (unsafe { ((*conn).fclosesocket).is_some() }) {
        if sock == (unsafe { (*conn).sock[1 as i32 as usize] }) && (unsafe { ((*conn).bits).sock_accepted() }) as i32 != 0 {
            let mut fresh13 = unsafe { &mut ((*conn).bits) };
...
#[no_mangle]
pub extern "C" fn Curl_socket<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut ai: *const crate::src::lib::http2::Curl_addrinfo,
    ...
    if unsafe { ((*data).set.fopensocket).is_some() } {
        Curl_set_in_callback(data, 1 as i32 != 0);
        *(borrow_mut(&mut sockfd)).unwrap() = unsafe { ((*data).set.fopensocket)
        ...
        *(borrow_mut(&mut sockfd)).unwrap() =
            unsafe { socket((*addr).family, (*addr).socktype, (*addr).protocol) };
    }
...
#[no_mangle]
pub extern "C" fn Curl_conncontrol(
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut ctrl: i32,
) {
    ...
    if !(ctrl == 2 as i32 && (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 9 as i32) as u32 != 0) {
        if closeit as bit != (unsafe { ((*conn).bits).close() }) {
            let mut fresh14 = unsafe { &mut ((*conn).bits) };
            (*fresh14).set_close(closeit as bit);
        }
    }
}
#[no_mangle]
pub extern "C" fn Curl_conn_data_pending(
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut sockindex: i32,
) -> bool {
