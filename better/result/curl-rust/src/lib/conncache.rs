...
extern "C" fn conn_llist_dtor(
    ...
    let mut fresh0 = unsafe { &mut ((*conn).bundle) };
    *fresh0 = 0 as *mut connectbundle;
}
extern "C" fn bundle_create<'a1>(
    mut bundlep: Option<&'a1 mut *mut crate::src::lib::http2::connectbundle>,
) -> u32 {
    *(borrow_mut(&mut bundlep)).unwrap() = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
        ::std::mem::size_of::<connectbundle>() as u64,
    ) }) as *mut connectbundle;
    if (*(borrow_mut(&mut bundlep)).unwrap()).is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (unsafe { (**(borrow_mut(&mut bundlep)).unwrap()).num_connections = 0 as i32 as size_t });
    (unsafe { (**(borrow_mut(&mut bundlep)).unwrap()).multiuse = 0 as i32 });
    Curl_llist_init(
        Some(unsafe { &mut (**(borrow_mut(&mut bundlep)).unwrap()).conn_list }),
        unsafe { core::intrinsics::transmute::<
            ...
            Option<
                unsafe extern "C" fn(_: *mut core::ffi::c_void, _: *mut core::ffi::c_void) -> (),
            >,
        >(Some(conn_llist_dtor)) },
    );
    return CURLE_OK;
}
extern "C" fn bundle_destroy(mut bundle: *mut crate::src::lib::http2::connectbundle) {
    if bundle.is_null() {
        return;
    }
    Curl_llist_destroy(unsafe { &mut (*bundle).conn_list }, 0 as *mut libc::c_void);
    (unsafe { Curl_cfree.expect("non-null function pointer")(bundle as *mut libc::c_void) });
}
extern "C" fn bundle_add_conn(
    mut bundle: *mut crate::src::lib::http2::connectbundle,
    mut conn: *mut crate::src::lib::http2::connectdata,
) {
    Curl_llist_insert_next(
        unsafe { &mut (*bundle).conn_list },
        unsafe { (*bundle).conn_list.tail },
        conn as *const libc::c_void,
        unsafe { &mut (*conn).bundle_node },
    );
    let mut fresh1 = unsafe { &mut ((*conn).bundle) };
    *fresh1 = bundle;
    let mut fresh2 = unsafe { &mut ((*bundle).num_connections) };
...
extern "C" fn bundle_remove_conn(
    ...
    while !curr.is_null() {
        if (unsafe { (*curr).ptr }) == conn as *mut libc::c_void {
            Curl_llist_remove(unsafe { &mut (*bundle).conn_list }, curr, 0 as *mut libc::c_void);
            let mut fresh3 = unsafe { &mut ((*bundle).num_connections) };
            *fresh3 = (*fresh3).wrapping_sub(1);
            let mut fresh4 = unsafe { &mut ((*conn).bundle) };
...
pub extern "C" fn Curl_conncache_init(
    ...
    *fresh5 = curl_easy_init();
    ...
    if rc != 0 {
        Curl_close(Some(unsafe { &mut (*connc).closure_handle }));
    } else {
        let mut fresh6 = unsafe { &mut ((*(*connc).closure_handle).state.conn_cache) };
        *fresh6 = connc;
    }
    return rc;
}
#[no_mangle]
pub extern "C" fn Curl_conncache_destroy<'a1>(
...
#[no_mangle]
pub extern "C" fn Curl_conncache_size(mut data: *mut crate::src::lib::http2::Curl_easy) -> u64 {
    let mut num: u64 = 0;
...
#[no_mangle]
pub extern "C" fn Curl_conncache_find_bundle<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut connc: *mut crate::src::lib::http2::conncache,
    mut hostp: Option<&'a1 mut *const i8>,
    ...
    if !connc.is_null() {
        ...
        bundle = Curl_hash_pick(
            unsafe { &mut (*connc).hash },
            key.as_mut_ptr() as *mut libc::c_void,
            unsafe { strlen(key.as_mut_ptr()) },
        ) as *mut connectbundle;
    }
    return bundle;
}
...
extern "C" fn conncache_remove_bundle(
    mut connc: *mut crate::src::lib::http2::conncache,
    mut bundle: *mut crate::src::lib::http2::connectbundle,
) {
    ...
    Curl_hash_start_iterate(Some(unsafe { &mut (*connc).hash }), Some(&mut iter));
    he = Curl_hash_next_element(&mut iter);
    while !he.is_null() {
        if (unsafe { (*he).ptr }) == bundle as *mut libc::c_void {
            Curl_hash_delete(
                unsafe { &mut (*connc).hash },
                (unsafe { ((*he).key).as_mut_ptr() }) as *mut libc::c_void,
                unsafe { (*he).key_len },
            );
            return;
        }
...
#[no_mangle]
pub extern "C" fn Curl_conncache_add_conn(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut current_block: u64;
    let mut result: u32 = CURLE_OK;
    let mut bundle: *mut crate::src::lib::http2::connectbundle = 0 as *mut connectbundle;
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    let mut connc: *mut crate::src::lib::http2::conncache = unsafe { (*data).state.conn_cache };
    ...
    match current_block {
        17407779659766490442 => {
            ...
            let mut fresh7 = unsafe { &mut ((*connc).next_connection_id) };
            let mut fresh8 = *fresh7;
            *fresh7 = *fresh7 + 1;
            (unsafe { (*conn).connection_id = fresh8 });
            let mut fresh9 = unsafe { &mut ((*connc).num_conn) };
...
pub extern "C" fn Curl_conncache_remove_conn(
    ...
    let mut bundle: *mut crate::src::lib::http2::connectbundle = unsafe { (*conn).bundle };
    let mut connc: *mut crate::src::lib::http2::conncache = unsafe { (*data).state.conn_cache };
    if !bundle.is_null() {
        if lock {
            if !(unsafe { (*data).share }).is_null() {
        ...
        let mut fresh10 = unsafe { &mut ((*conn).bundle) };
        *fresh10 = 0 as *mut connectbundle;
        if !connc.is_null() {
            let mut fresh11 = unsafe { &mut ((*connc).num_conn) };
...
pub extern "C" fn Curl_conncache_foreach(
    ...
    he = Curl_hash_next_element(&mut iter);
    while !he.is_null() {
        let mut bundle: *mut crate::src::lib::http2::connectbundle =
            0 as *mut crate::src::lib::http2::connectbundle;
        bundle = (unsafe { (*he).ptr }) as *mut connectbundle;
        he = Curl_hash_next_element(&mut iter);
        curr = unsafe { (*bundle).conn_list.head };
        while !curr.is_null() {
            let mut conn: *mut crate::src::lib::http2::connectdata =
                (unsafe { (*curr).ptr }) as *mut connectdata;
            curr = unsafe { (*curr).next };
...
extern "C" fn conncache_find_first_connection<'a1>(
    mut connc: Option<&'a1 mut crate::src::lib::http2::conncache>,
    ...
    Curl_hash_start_iterate(
        Some(&mut (*(borrow_mut(&mut connc)).unwrap()).hash),
        Some(&mut iter),
    );
    he = Curl_hash_next_element(&mut iter);
    while !he.is_null() {
...
pub extern "C" fn Curl_conncache_return_conn(
    ...
    let mut maxconnects: u64 = (if (unsafe { (*(*data).multi).maxconnects }) < 0 as i32 as i64 {
...
pub extern "C" fn Curl_conncache_extract_bundle(
    ...
    while !curr.is_null() {
        ...
        if (unsafe { (*conn).easyq.size }) == 0 {
            score = Curl_timediff(now, unsafe { (*conn).lastused });
            if score > highscore {
                highscore = score;
                conn_candidate = conn;
            }
    ...
    if !conn_candidate.is_null() {
        ...
        let mut fresh12 = unsafe { &mut ((*(*data).state.conn_cache).num_conn) };
        *fresh12 = (*fresh12).wrapping_sub(1);
    }
    return conn_candidate;
}
#[no_mangle]
pub extern "C" fn Curl_conncache_extract_oldest(
    mut data: *mut crate::src::lib::http2::Curl_easy,
) -> *mut crate::src::lib::http2::connectdata {
    let mut connc: *mut crate::src::lib::http2::conncache = unsafe { (*data).state.conn_cache };
    let mut iter: crate::src::lib::conncache::Curl_hash_iterator<'_> = Curl_hash_iterator {
    ...
    if !conn_candidate.is_null() {
        ...
        let mut fresh13 = unsafe { &mut ((*connc).num_conn) };
...
#[no_mangle]
pub extern "C" fn Curl_conncache_close_all_connections<'a1>(
    mut connc: Option<&'a1 mut crate::src::lib::http2::conncache>,
) {
    let mut conn: *mut crate::src::lib::http2::connectdata = 0 as *mut connectdata;
    ...
    let mut fresh14 = unsafe { &mut ((*(*(borrow_mut(&mut connc)).unwrap()).closure_handle)
        .state
        .buffer) };
    *fresh14 = buffer.as_mut_ptr();
    (unsafe { (*(*(borrow_mut(&mut connc)).unwrap()).closure_handle)
        .set
        .buffer_size = 1024 as i32 as i64 });
    ...
    let mut fresh15 = unsafe { &mut ((*(*(borrow_mut(&mut connc)).unwrap()).closure_handle)
        .state
        .buffer) };
    *fresh15 = 0 as *mut i8;
    if !((*(borrow_mut(&mut connc)).unwrap()).closure_handle).is_null() {
        ...
        Curl_close(Some(
            &mut (*(borrow_mut(&mut connc)).unwrap()).closure_handle,
        ));
