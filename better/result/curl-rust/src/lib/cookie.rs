...
pub type __compar_fn_t<'a1, 'a2> = Option<
    unsafe extern "C" fn(
        _: Option<&'a1 core::ffi::c_void>,
        _: Option<&'a2 core::ffi::c_void>,
    ) -> i32,
...
extern "C" fn freecookie(mut co: *mut crate::src::lib::http2::Cookie) {
...
extern "C" fn tailmatch(mut cooke_domain: *const i8, mut hostname: *const i8) -> bool {
    let mut cookie_domain_len: u64 = unsafe { strlen(cooke_domain) };
    let mut hostname_len: u64 = unsafe { strlen(hostname) };
...
extern "C" fn get_top_domain<'a1>(
    ...
    if !domain.is_null() {
        ...
        last = Curl_memrchr(domain as *const libc::c_void, '.' as i32, len) as *const i8;
        if !last.is_null() {
            first = Curl_memrchr(
                domain as *const libc::c_void,
                '.' as i32,
                (unsafe { last.offset_from(domain) }) as i64 as size_t,
            ) as *const i8;
            if !first.is_null() {
...
extern "C" fn cookie_hash_domain(mut domain: *const i8, len: u64) -> u64 {
    ...
    while domain < end {
        ...
        h ^= Curl_raw_toupper(unsafe { *fresh0 }) as u64;
    }
    return h.wrapping_rem(256 as i32 as u64);
}
extern "C" fn cookiehash(domain: *const i8) -> u64 {
...
#[no_mangle]
pub extern "C" fn Curl_cookie_loadfiles(mut data: *mut crate::src::lib::http2::Curl_easy) {
    let mut list: *mut crate::src::lib::http2::curl_slist = unsafe { (*data).state.cookielist };
    if !list.is_null() {
        ...
        while !list.is_null() {
            ...
            if newcookies.is_null() {
                ...
                let mut fresh1 = unsafe { &mut ((*data).cookies) };
                *fresh1 = newcookies;
            }
            list = unsafe { (*list).next };
        }
        curl_slist_free_all(unsafe { (*data).state.cookielist });
        let mut fresh2 = unsafe { &mut ((*data).state.cookielist) };
        *fresh2 = 0 as *mut curl_slist;
        Curl_share_unlock(data, CURL_LOCK_DATA_COOKIE);
    }
}
extern "C" fn strstore<'a1>(mut str: Option<&'a1 mut *mut i8>, mut newstr: *const i8) {
    (unsafe { Curl_cfree.expect("non-null function pointer")(
        *(borrow_mut(&mut str)).unwrap() as *mut libc::c_void
    ) });
    *(borrow_mut(&mut str)).unwrap() = unsafe { Curl_cstrdup.expect("non-null function pointer")(newstr) };
}
extern "C" fn remove_expired(mut cookies: *mut crate::src::lib::http2::CookieInfo) {
    let mut co: *mut crate::src::lib::http2::Cookie = 0 as *mut Cookie;
    let mut nx: *mut crate::src::lib::http2::Cookie = 0 as *mut Cookie;
    let mut now: i64 = unsafe { time(0 as *mut time_t) };
    let mut i: u32 = 0;
    ...
    while i < 256 as i32 as u32 {
        ...
        while !co.is_null() {
            ...
            if (unsafe { (*co).expires }) != 0 && (unsafe { (*co).expires }) < now {
                if pv.is_null() {
                    let mut fresh3 = unsafe { &mut ((*cookies).cookies[i as usize]) };
                    *fresh3 = unsafe { (*co).next };
                } else {
                    let mut fresh4 = unsafe { &mut ((*pv).next) };
                    *fresh4 = unsafe { (*co).next };
                }
                let mut fresh5 = unsafe { &mut ((*cookies).numcookies) };
                *fresh5 -= 1;
                freecookie(co);
            } else {
...
extern "C" fn bad_domain(mut domain: *const i8) -> bool {
    return (unsafe { strchr(domain, '.' as i32) }).is_null()
        && Curl_strcasecompare(domain, b"localhost\0" as *const u8 as *const i8) == 0;
}
#[no_mangle]
pub extern "C" fn Curl_cookie_add(
    ...
    if httpheader {
        ...
        let mut linelength: u64 = unsafe { strlen(lineptr) };
        ...
        loop {
                ...
                ) })
            {
                ...
                let mut len: u64 = unsafe { strlen(what.as_mut_ptr()) };
                let mut nlen: u64 = unsafe { strlen(name.as_mut_ptr()) };
                    ...
                    && name[1 as i32 as usize] as i32 == '_' as i32
                {
                    ...
                    {
                        let mut fresh6 = unsafe { &mut ((*co).prefix) };
                    ...
                    {
                        let mut fresh7 = unsafe { &mut ((*co).prefix) };
                ...
                if (unsafe { (*co).name }).is_null() {
                    ...
                    if Curl_strcasecompare(b"secure\0" as *const u8 as *const i8, name.as_mut_ptr())
                        != 0
                    {
                ...
                if !done {
                    if Curl_strcasecompare(b"path\0" as *const u8 as *const i8, name.as_mut_ptr())
                        != 0
                    {
                        strstore(Some(unsafe { &mut (*co).path }), whatptr);
                        if (unsafe { (*co).path }).is_null() {
                            ...
                            let mut fresh10 = unsafe { &mut ((*co).spath) };
                    ...
                    ) != 0
                    {
                        ...
                        is_ip =
                            Curl_host_is_ipnum(if !domain.is_null() { domain } else { whatptr });
                        if domain.is_null()
                            || is_ip as i32 != 0 && (unsafe { strcmp(whatptr, domain) }) == 0
                            || !is_ip && tailmatch(whatptr, domain) as i32 != 0
                        {
                            strstore(Some(unsafe { &mut (*co).domain }), whatptr);
                    ...
                    {
                        strstore(Some(unsafe { &mut (*co).version }), whatptr);
                    ...
                    {
                        strstore(Some(unsafe { &mut (*co).maxage }), whatptr);
                    ...
                    {
                        strstore(Some(unsafe { &mut (*co).expirestr }), whatptr);
        ...
        if !(unsafe { (*co).maxage }).is_null() {
            ...
            offt = curlx_strtoofft(
                if (unsafe { *(*co).maxage }) as i32 == '"' as i32 {
                    unsafe { &mut *((*co).maxage).offset(1 as i32 as isize) }
                } else {
                    unsafe { &mut *((*co).maxage).offset(0 as i32 as isize) }
                },
                Option::<&'_ mut *mut i8>::None,
                10 as i32,
                Some(unsafe { &mut (*co).expires }),
            );
            if offt as u32 == CURL_OFFT_FLOW as i32 as u32 {
                ...
                if (unsafe { (*co).expires }) == 0 {
                    ...
                    let mut fresh11 = unsafe { &mut ((*co).expires) };
                    *fresh11 += now;
                }
            }
        ...
        if !badcookie && (unsafe { (*co).domain }).is_null() {
            if !domain.is_null() {
                let mut fresh12 = unsafe { &mut ((*co).domain) };
        ...
        if !badcookie && (unsafe { (*co).path }).is_null() && !path.is_null() {
            ...
            if queryp.is_null() {
                ...
                endslash = Curl_memrchr(
                    ...
                    '/' as i32,
                    (unsafe { queryp.offset_from(path) }) as i64 as size_t,
                ) as *mut i8;
            }
            if !endslash.is_null() {
                let mut pathlen: u64 =
                    ((unsafe { endslash.offset_from(path) }) as i64 + 1 as i32 as i64) as size_t;
                let mut fresh13 = unsafe { &mut ((*co).path) };
                *fresh13 = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
                ...
                if !(unsafe { (*co).path }).is_null() {
                    ...
                    let mut fresh14 = unsafe { &mut ((*co).spath) };
        ...
        while !ptr_0.is_null() && !badcookie {
            ...
            match fields {
                0 => {
                    ...
                    let mut fresh15 = unsafe { &mut ((*co).domain) };
                ...
                2 => {
                    ...
                    {
                        let mut fresh16 = unsafe { &mut ((*co).path) };
                        *fresh16 = unsafe { Curl_cstrdup.expect("non-null function pointer")(ptr_0) };
                        if (unsafe { (*co).path }).is_null() {
                            badcookie = 1 as i32 != 0;
                        } else {
                            let mut fresh17 = unsafe { &mut ((*co).spath) };
                        ...
                        let mut fresh18 = unsafe { &mut ((*co).path) };
                        *fresh18 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                        ...
                        let mut fresh19 = unsafe { &mut ((*co).spath) };
                ...
                5 => {
                    let mut fresh20 = unsafe { &mut ((*co).name) };
                    *fresh20 = unsafe { Curl_cstrdup.expect("non-null function pointer")(ptr_0) };
                    ...
                    {
                        let mut fresh21 = unsafe { &mut ((*co).prefix) };
                    ...
                    {
                        let mut fresh22 = unsafe { &mut ((*co).prefix) };
                        *fresh22 = (*fresh22 as i32 | (1 as i32) << 1 as i32) as u8;
                    }
                    current_block_187 = 4402115142504265260;
                }
                6 => {
                    let mut fresh23 = unsafe { &mut ((*co).value) };
        ...
        if 6 as i32 == fields {
            let mut fresh24 = unsafe { &mut ((*co).value) };
            *fresh24 =
    ...
    let mut fresh25 = unsafe { &mut ((*c).lastct) };
    *fresh25 += 1;
    ...
    if !data.is_null()
        && (!domain.is_null() && !(unsafe { (*co).domain }).is_null() && !Curl_host_is_ipnum(unsafe { (*co).domain }))
    {
    ...
    while !clist.is_null() {
        if Curl_strcasecompare(unsafe { (*clist).name }, unsafe { (*co).name }) != 0 {
            if !(unsafe { (*clist).domain }).is_null() && !(unsafe { (*co).domain }).is_null() {
                if Curl_strcasecompare(unsafe { (*clist).domain }, unsafe { (*co).domain }) != 0
            ...
            if replace_old {
                if !(unsafe { (*clist).spath }).is_null() && !(unsafe { (*co).spath }).is_null() {
                    if (unsafe { (*clist).secure }) as i32 != 0 && !(unsafe { (*co).secure }) && !secure {
                        ...
                        if Curl_strncasecompare(unsafe { (*clist).spath }, unsafe { (*co).spath }, cllen) != 0 {
                            freecookie(co);
                            return 0 as *mut Cookie;
                        }
                    } else if Curl_strcasecompare(unsafe { (*clist).spath }, unsafe { (*co).spath }) != 0 {
                        replace_old = 1 as i32 != 0;
                    } else {
                        replace_old = 0 as i32 != 0;
                    }
            ...
            if replace_old {
                let mut fresh26 = unsafe { &mut ((*co).next) };
    ...
    if !replace_old {
        if !lastc.is_null() {
            let mut fresh27 = unsafe { &mut ((*lastc).next) };
            *fresh27 = co;
        } else {
            let mut fresh28 = unsafe { &mut ((*c).cookies[myhash as usize]) };
            *fresh28 = co;
        }
        let mut fresh29 = unsafe { &mut ((*c).numcookies) };
        *fresh29 += 1;
    }
...
pub extern "C" fn Curl_cookie_init(
    ...
    if inc.is_null() {
        ...
        let mut fresh30 = unsafe { &mut ((*c).filename) };
        ...
        11650488183268122163 => {
                ...
                if line.is_null() {
                    ...
                    while !(Curl_get_line(line, 5000 as i32, fp)).is_null() {
                        if curl_strnequal(
                            ...
                            line,
                            unsafe { strlen(b"Set-Cookie:\0" as *const u8 as *const i8) },
                        ) != 0
                        {
            ...
            match current_block {
                ...
                _ => {
                    ...
                    if !data.is_null() {
                        let mut fresh31 = unsafe { &mut ((*data).state) };
...
extern "C" fn cookie_sort(
    mut p1: *const core::ffi::c_void,
    mut p2: *const core::ffi::c_void,
) -> i32 {
    let mut c1: *mut crate::src::lib::http2::Cookie = unsafe { *(p1 as *mut *mut Cookie) };
    let mut c2: *mut crate::src::lib::http2::Cookie = unsafe { *(p2 as *mut *mut Cookie) };
    let mut l1: u64 = 0;
    let mut l2: u64 = 0;
...
extern "C" fn dup_cookie(
    mut src: *mut crate::src::lib::http2::Cookie,
) -> *mut crate::src::lib::http2::Cookie {
    let mut current_block: u64;
    let mut d: *mut crate::src::lib::http2::Cookie = (unsafe { Curl_ccalloc
    ...
    if !d.is_null() {
        if !(unsafe { (*src).expirestr }).is_null() {
            let mut fresh32 = unsafe { &mut ((*d).expirestr) };
        ...
        match current_block {
            7815301370352969686 => {
                if !(unsafe { (*src).domain }).is_null() {
                    let mut fresh33 = unsafe { &mut ((*d).domain) };
                ...
                match current_block {
                    ...
                    _ => {
                        if !(unsafe { (*src).path }).is_null() {
                            let mut fresh34 = unsafe { &mut ((*d).path) };
                            *fresh34 =
                        ...
                        match current_block {
                            ...
                            _ => {
                                if !(unsafe { (*src).spath }).is_null() {
                                    let mut fresh35 = unsafe { &mut ((*d).spath) };
                                ...
                                match current_block {
                                    ...
                                    _ => {
                                        if !(unsafe { (*src).name }).is_null() {
                                            let mut fresh36 = unsafe { &mut ((*d).name) };
                                        ...
                                        match current_block {
                                            ...
                                            _ => {
                                                if !(unsafe { (*src).value }).is_null() {
                                                    let mut fresh37 = unsafe { &mut ((*d).value) };
                                                ...
                                                match current_block {
                                                    ...
                                                    _ => {
                                                        if !(unsafe { (*src).maxage }).is_null() {
                                                            let mut fresh38 = unsafe { &mut ((*d).maxage) };
        ...
        } != 0
        {
            if (unsafe { (*co).domain }).is_null()
                ...
                || (!(unsafe { (*co).tailmatch }) || is_ip as i32 != 0)
                    && Curl_strcasecompare(host, unsafe { (*co).domain }) != 0
            {
                if (unsafe { (*co).spath }).is_null() || pathmatch(unsafe { (*co).spath }, path) as i32 != 0 {
                    ...
                    let mut fresh40 = unsafe { &mut ((*newco).next) };
                    *fresh40 = mainco;
                    mainco = newco;
            ...
            if matches != 0 {
                ...
                if array.is_null() {
                    ...
                    while !co.is_null() {
                        let mut fresh41 = i;
                        i = i.wrapping_add(1);
                        let mut fresh42 = unsafe { &mut (*array.offset(fresh41 as isize)) };
                        *fresh42 = co;
                        co = unsafe { (*co).next };
                    }
                    (unsafe { qsort(
                    ...
                    while i < matches.wrapping_sub(1 as i32 as u64) {
                        let mut fresh43 = unsafe { &mut ((**array.offset(i as isize)).next) };
                        *fresh43 = unsafe { *array.offset(i.wrapping_add(1 as i32 as u64) as isize) };
                        i = i.wrapping_add(1);
                    }
                    let mut fresh44 = unsafe { &mut ((**array
...
#[no_mangle]
pub extern "C" fn Curl_cookie_clearall(mut cookies: *mut crate::src::lib::http2::CookieInfo) {
    if !cookies.is_null() {
        let mut i: u32 = 0;
        i = 0 as i32 as u32;
        while i < 256 as i32 as u32 {
            Curl_cookie_freelist(unsafe { (*cookies).cookies[i as usize] });
            let mut fresh45 = unsafe { &mut ((*cookies).cookies[i as usize]) };
...
#[no_mangle]
pub extern "C" fn Curl_cookie_freelist(mut co: *mut crate::src::lib::http2::Cookie) {
...
#[no_mangle]
pub extern "C" fn Curl_cookie_clearsess(mut cookies: *mut crate::src::lib::http2::CookieInfo) {
    ...
    while i < 256 as i32 as u32 {
        if !(unsafe { (*cookies).cookies[i as usize] }).is_null() {
            ...
            while !curr.is_null() {
                ...
                if (unsafe { (*curr).expires }) == 0 {
                    ...
                    if prev == curr {
                        ...
                        let mut fresh46 = unsafe { &mut ((*prev).next) };
                        *fresh46 = next;
                    }
                    freecookie(curr);
                    let mut fresh47 = unsafe { &mut ((*cookies).numcookies) };
                    *fresh47 -= 1;
                } else {
                    prev = curr;
                }
            ...
            let mut fresh48 = unsafe { &mut ((*cookies).cookies[i as usize]) };
            *fresh48 = first;
        }
        i = i.wrapping_add(1);
    }
...
#[no_mangle]
pub extern "C" fn Curl_cookie_cleanup(mut c: *mut crate::src::lib::http2::CookieInfo) {
    if !c.is_null() {
        let mut i: u32 = 0;
...
extern "C" fn get_netscape_format(mut co: *const crate::src::lib::http2::Cookie) -> *mut i8 {
                    ...
                    while i < 256 as i32 as u32 {
                        ...
                        while !co.is_null() {
                            if !(unsafe { (*co).domain }).is_null() {
                                ...
                                let mut fresh50 = unsafe { &mut (*array.offset(fresh49 as isize)) };
                                *fresh50 = co;
                            }
                            co = unsafe { (*co).next };
                        }
...
#[no_mangle]
pub extern "C" fn Curl_cookie_list(
    mut data: *mut crate::src::lib::http2::Curl_easy,
) -> *mut crate::src::lib::http2::curl_slist {
    let mut list: *mut crate::src::lib::http2::curl_slist = 0 as *mut curl_slist;
    Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE);
...
#[no_mangle]
pub extern "C" fn Curl_flush_cookies(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut cleanup: bool,
) {
    ...
    if !(unsafe { (*data).set.str_0[STRING_COOKIEJAR as i32 as usize] }).is_null() {
        ...
        if cleanup as i32 != 0 && !(unsafe { (*data).state.cookielist }).is_null() {
            curl_slist_free_all(unsafe { (*data).state.cookielist });
            let mut fresh51 = unsafe { &mut ((*data).state.cookielist) };
        ...
        && ((unsafe { (*data).share }).is_null() || (unsafe { (*data).cookies }) != (unsafe { (*(*data).share).cookies }))
    {
        ...
        let mut fresh52 = unsafe { &mut ((*data).cookies) };
