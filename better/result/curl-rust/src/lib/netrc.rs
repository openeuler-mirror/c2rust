...
extern "C" fn parsenetrc<'a1, 'a2, 'a3, 'a4>(
    mut host: *const i8,
        ...
        's_69: while !done && !(unsafe { fgets(netrcbuffer.as_mut_ptr(), netrcbuffsize, file) }).is_null() {
            ...
            while !tok.is_null() {
                    ...
                    && (!password.is_null() && (unsafe { *password }) as i32 != 0)
                {
                    ...
                    match state as u32 {
                        0 => {
                            if Curl_strcasecompare(b"macdef\0" as *const u8 as *const i8, tok) != 0
                            {
                                state = MACDEF;
                        ...
                        2 => {
                            ...
                            } else if Curl_strcasecompare(b"login\0" as *const u8 as *const i8, tok)
                                != 0
                            {
...
#[no_mangle]
pub extern "C" fn Curl_parsenetrc<'a1, 'a2, 'a3, 'a4>(
    mut host: *const i8,
    mut loginp: Option<&'a1 mut *mut i8>,
    mut passwordp: Option<&'a2 mut *mut i8>,
    mut login_changed: Option<&'a3 mut bool>,
    ...
    if netrcfile.is_null() {
        ...
        let mut homea: *mut i8 = curl_getenv(b"HOME\0" as *const u8 as *const i8);
        if !homea.is_null() {
            home = homea;
        } else {
