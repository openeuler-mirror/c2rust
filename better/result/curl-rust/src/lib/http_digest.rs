...
pub extern "C" fn Curl_input_digest(
    ...
    let mut digest: Option<&'_ mut crate::src::lib::http2::digestdata> =
        Option::<&'_ mut crate::src::lib::http2::digestdata>::None;
    if proxy {
        digest = Some(unsafe { &mut (*data).state.proxydigest });
    } else {
        digest = Some(unsafe { &mut (*data).state.digest });
    }
    if curl_strnequal(
        ...
        || Curl_isspace((unsafe { *header.offset(6 as i32 as isize) }) as u8 as i32) == 0
    {
        return CURLE_BAD_CONTENT_ENCODING;
    }
    header = unsafe { header.offset(strlen(b"Digest\0" as *const u8 as *const i8) as isize) };
...
pub extern "C" fn Curl_output_digest(
    ...
    *(borrow_mut(&mut allocuserpwd)).unwrap() = 0 as *mut i8;
    ...
    if (*(borrow(&authp)).unwrap()).iestyle() != 0 {
        tmp = unsafe { strchr(uripath as *mut i8, '?' as i32) };
        if !tmp.is_null() {
            let mut urilen: u64 = (unsafe { tmp.offset_from(uripath as *mut i8) }) as i64 as size_t;
            path = (unsafe { curl_maprintf(b"%.*s\0" as *const u8 as *const i8, urilen as i32, uripath) })
                as *mut u8;
        }
    }
    if tmp.is_null() {
        path = (unsafe { Curl_cstrdup.expect("non-null function pointer")(uripath as *mut i8) }) as *mut u8;
    }
    if path.is_null() {
...
#[no_mangle]
pub extern "C" fn Curl_http_auth_cleanup_digest(mut data: *mut crate::src::lib::http2::Curl_easy) {
    Curl_auth_digest_cleanup(Some(unsafe { &mut (*data).state.digest }));
    Curl_auth_digest_cleanup(Some(unsafe { &mut (*data).state.proxydigest }));
}
