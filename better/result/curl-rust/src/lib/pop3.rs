...
pub type Curl_MD5_final_func<'a1, 'a2> = Option<
    unsafe extern "C" fn(_: Option<&'a1 mut u8>, _: Option<&'a2 mut core::ffi::c_void>) -> (),
>;
pub type Curl_MD5_update_func<'a1, 'a2> = Option<
    unsafe extern "C" fn(_: Option<&'a1 mut core::ffi::c_void>, _: Option<&'a2 u8>, _: u32) -> (),
>;
pub type Curl_MD5_init_func<'a1> =
    Option<unsafe extern "C" fn(_: Option<&'a1 mut core::ffi::c_void>) -> ()>;
...
pub static mut Curl_handler_pop3: crate::src::lib::http2::Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            setup_connection: Some(pop3_setup_connection),
            do_it: Some(pop3_do),
            done: Some(pop3_done),
            do_more: None,
            connect_it: Some(pop3_connect),
            connecting: Some(pop3_multi_statemach),
            doing: Some(pop3_doing),
            proto_getsock: Some(pop3_getsock),
            doing_getsock: Some(pop3_getsock),
            domore_getsock: None,
            perform_getsock: None,
            disconnect: Some(pop3_disconnect),
            readwrite: None,
...
static mut saslpop3: crate::src::lib::http2::SASLproto =  {
    {
        let mut init = SASLproto {
            ...
            sendauth: Some(pop3_perform_auth),
            sendcont: Some(pop3_continue_auth),
            getmessage: Some(pop3_get_message),
        };
        init
    }
};
extern "C" fn pop3_to_pop3s(mut conn: *mut crate::src::lib::http2::connectdata) {
...
extern "C" fn pop3_endofresp(
    ...
    let mut pop3c: Option<&'_ mut crate::src::lib::http2::pop3_conn> =
        Some(unsafe { &mut (*conn).proto.pop3c });
...
extern "C" fn state(mut data: *mut crate::src::lib::http2::Curl_easy, mut newstate: u32) {
    let mut pop3c: Option<&'_ mut crate::src::lib::http2::pop3_conn> =
        Some(unsafe { &mut (*(*data).conn).proto.pop3c });
    (*(borrow_mut(&mut pop3c)).unwrap()).state = newstate;
}
extern "C" fn pop3_perform_capa(
...
extern "C" fn pop3_perform_upgrade_tls(
    ...
    if result as u64 == 0 {
        if (*(borrow(&pop3c)).unwrap()).state as u32 != POP3_UPGRADETLS as i32 as u32 {
...
extern "C" fn pop3_perform_apop(
    ...
    ctxt = Curl_MD5_init(unsafe { Curl_DIGEST_MD5.as_ptr() });
    if ctxt.is_null() {
    ...
    Curl_MD5_update(
        ...
        curlx_uztoui(unsafe { strlen((*(borrow(&pop3c)).unwrap()).apoptimestamp) }),
    );
    Curl_MD5_update(
        ctxt,
        (unsafe { (*conn).passwd }) as *const u8,
        curlx_uztoui(unsafe { strlen((*conn).passwd) }),
    );
...
extern "C" fn pop3_perform_authentication(
    ...
    if (*(borrow(&pop3c)).unwrap()).authtypes
        & (*(borrow(&pop3c)).unwrap()).preftype
    ...
    if result as u64 == 0 && progress as u32 == SASL_IDLE as i32 as u32 {
        ...
        } else if (*(borrow(&pop3c)).unwrap()).authtypes
            & (*(borrow(&pop3c)).unwrap()).preftype
...
extern "C" fn pop3_perform_command(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    let mut pop3: *mut crate::src::lib::http2::POP3 = unsafe { (*data).req.p.pop3 };
            ...
            && (unsafe { *line.offset(len.wrapping_sub(2 as i32 as u64) as isize) }) as i32 == '>' as i32
        {
            ...
            while i < len.wrapping_sub(2 as i32 as u64) {
                if (unsafe { *line.offset(i as isize) }) as i32 == '<' as i32 {
                    let mut timestamplen: u64 = len.wrapping_sub(1 as i32 as u64).wrapping_sub(i);
                    ...
                    *fresh3 = (unsafe { Curl_ccalloc.expect("non-null function pointer")(
                    ...
                    (unsafe { *((*(borrow(&pop3c)).unwrap()).apoptimestamp).offset(timestamplen as isize) =
                        '\u{0}' as i32 as i8 });
                    at = unsafe { strchr((*(borrow(&pop3c)).unwrap()).apoptimestamp, '@' as i32) };
                    if at.is_null() {
...
extern "C" fn pop3_state_capa_resp(
    ...
    if pop3code == '*' as i32 {
        ...
        {
            (*(borrow_mut(&mut pop3c)).unwrap()).authtypes |= ((1 as i32) << 0 as i32) as u32;
        ...
        {
            (*(borrow_mut(&mut pop3c)).unwrap()).authtypes |= ((1 as i32) << 2 as i32) as u32;
...
extern "C" fn pop3_state_command_resp(
    ...
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    let mut pop3: *mut crate::src::lib::http2::POP3 = unsafe { (*data).req.p.pop3 };
    let mut pop3c: *mut crate::src::lib::http2::pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut pp: Option<&'_ mut crate::src::lib::http2::pingpong> = Some(unsafe { &mut (*pop3c).pp });
    if pop3code != '+' as i32 {
        state(data, POP3_STOP);
        ...
        if !((*(borrow(&pp)).unwrap()).cache).is_null() {
            if (unsafe { ((*data).set).opt_no_body() }) == 0 {
                result = Curl_pop3_write(
                    ...
                    (*(borrow_mut(&mut pp)).unwrap()).cache_size,
                );
                if result as u64 != 0 {
                    return result;
                }
...
extern "C" fn pop3_statemachine(
    ...
    let mut pop3c: *mut crate::src::lib::http2::pop3_conn = unsafe { &mut (*conn).proto.pop3c };
    let mut pp: *mut crate::src::lib::http2::pingpong = unsafe { &mut (*pop3c).pp };
...
extern "C" fn pop3_multi_statemach(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut done: *mut bool,
) -> u32 {
    let mut result: u32 = CURLE_OK;
    ...
    (unsafe { *done = if (*(borrow(&pop3c)).unwrap()).state as u32 == POP3_STOP as i32 as u32 {
        1 as i32
    } else {
        0 as i32
    } != 0 });
...
extern "C" fn pop3_block_statemach(
    ...
    let mut pop3c: Option<&'_ mut crate::src::lib::http2::pop3_conn> =
        Some(unsafe { &mut (*conn).proto.pop3c });
    while (*(borrow(&pop3c)).unwrap()).state as u32 != POP3_STOP as i32 as u32 && result as u64 == 0
    {
        result = Curl_pp_statemach(
            data,
            &mut (*(borrow_mut(&mut pop3c)).unwrap()).pp,
...
extern "C" fn pop3_init(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut pop3: *mut crate::src::lib::http2::POP3 = 0 as *mut crate::src::lib::http2::POP3;
    let mut fresh7 = unsafe { &mut ((*data).req.p.pop3) };
...
extern "C" fn pop3_getsock(
    ...
    return Curl_pp_getsock(data, Some(unsafe { &mut (*conn).proto.pop3c.pp }), socks);
}
extern "C" fn pop3_connect(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut done: *mut bool,
) -> u32 {
    let mut result: u32 = CURLE_OK;
    ...
    *fresh8 = Some(pop3_statemachine);
    let mut fresh9 = &mut ((*(borrow_mut(&mut pp)).unwrap()).endofresp);
    *fresh9 = Some(pop3_endofresp);
    (unsafe { (*pop3c).preftype = !(0 as u32) });
    Curl_sasl_init(Some(unsafe { &mut (*pop3c).sasl }), unsafe { &saslpop3 });
...
extern "C" fn pop3_disconnect(
    ...
    Curl_sasl_cleanup(
        conn,
        (*(borrow_mut(&mut pop3c)).unwrap()).sasl.authused as u32,
    );
    (unsafe { Curl_cfree.expect("non-null function pointer")(
        (*(borrow_mut(&mut pop3c)).unwrap()).apoptimestamp as *mut libc::c_void,
    ) });
...
extern "C" fn pop3_parse_url_options(mut conn: *mut crate::src::lib::http2::connectdata) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut pop3c: Option<&'_ mut crate::src::lib::http2::pop3_conn> =
        Some(unsafe { &mut (*conn).proto.pop3c });
    ...
    while result as u64 == 0 && !ptr.is_null() && (unsafe { *ptr }) as i32 != 0 {
        ...
        ) != 0
        {
            ...
            {
                (*(borrow_mut(&mut pop3c)).unwrap()).preftype = ((1 as i32) << 1 as i32) as u32;
    ...
    if (*(borrow(&pop3c)).unwrap()).preftype != ((1 as i32) << 1 as i32) as u32 {
        match (*(borrow(&pop3c)).unwrap()).sasl.prefmech as i32 {
            ...
            _ => {
                (*(borrow_mut(&mut pop3c)).unwrap()).preftype = ((1 as i32) << 2 as i32) as u32;
            }
        }
    }
    return result;
}
extern "C" fn pop3_parse_url_path(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut pop3: *mut crate::src::lib::http2::POP3 = unsafe { (*data).req.p.pop3 };
    ...
    return Curl_urldecode(
        ...
        path,
        0 as i32 as size_t,
        Some(unsafe { &mut (*pop3).id }),
        Option::<&'_ mut u64>::None,
        REJECT_CTRL,
    );
}
extern "C" fn pop3_parse_custom_request(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut pop3: *mut crate::src::lib::http2::POP3 = unsafe { (*data).req.p.pop3 };
                    ...
                    *fresh14 = (*fresh14).wrapping_add(1);
                    if i != 0 {
                        result = Curl_client_write(
                            data,
                            (1 as i32) << 0 as i32,
                            unsafe { &mut *str.offset(last as isize) },
                            i.wrapping_sub(last),
                        );
                        if result as u64 != 0 {
                            return result;
