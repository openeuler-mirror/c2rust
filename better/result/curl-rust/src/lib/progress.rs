...
extern "C" fn time2str(mut r: *mut i8, mut seconds: i64) {
    let mut h: i64 = 0;
    ...
    if h <= 99 as i64 {
        let mut m: i64 = (seconds - h * 3600 as i64) / 60 as i64;
        let mut s: i64 = seconds - h * 3600 as i64 - m * 60 as i64;
...
extern "C" fn max5data(mut bytes: i64, mut max5: *mut i8) -> *mut i8 {
...
#[no_mangle]
pub extern "C" fn Curl_pgrsDone(mut data: *mut crate::src::lib::http2::Curl_easy) -> i32 {
    let mut rc: i32 = 0;
    (unsafe { (*data).progress.lastshow = 0 as i32 as time_t });
    rc = Curl_pgrsUpdate(data);
    if rc != 0 {
        return rc;
    }
    if (unsafe { (*data).progress.flags }) & (1 as i32) << 4 as i32 == 0 && (unsafe { ((*data).progress).callback() }) == 0 {
        (unsafe { curl_mfprintf((*data).set.err, b"\n\0" as *const u8 as *const i8) });
    }
    (unsafe { (*data).progress.speeder_c = 0 as i32 });
    return 0 as i32;
}
#[no_mangle]
pub extern "C" fn Curl_pgrsResetTransferSizes(mut data: *mut crate::src::lib::http2::Curl_easy) {
    Curl_pgrsSetDownloadSize(data, -(1 as i32) as curl_off_t);
    Curl_pgrsSetUploadSize(data, -(1 as i32) as curl_off_t);
}
#[no_mangle]
pub extern "C" fn Curl_pgrsTime(
    ...
    match timer as u32 {
        1 => {
            (unsafe { (*data).progress.t_startop = now });
        }
        2 => {
            (unsafe { (*data).progress.t_startsingle = now });
            let mut fresh0 = unsafe { &mut ((*data).progress) };
            (*fresh0).set_is_t_startransfer_set(0 as i32 as bit);
        }
        9 => {
            (unsafe { (*data).progress.t_acceptdata = now });
        }
        3 => {
            delta = Some(unsafe { &mut (*data).progress.t_nslookup });
        }
        4 => {
            delta = Some(unsafe { &mut (*data).progress.t_connect });
        }
        5 => {
            delta = Some(unsafe { &mut (*data).progress.t_appconnect });
        }
        6 => {
            delta = Some(unsafe { &mut (*data).progress.t_pretransfer });
        }
        7 => {
            delta = Some(unsafe { &mut (*data).progress.t_starttransfer });
            if (unsafe { ((*data).progress).is_t_startransfer_set() }) != 0 {
                return now;
            } else {
                let mut fresh1 = unsafe { &mut ((*data).progress) };
                (*fresh1).set_is_t_startransfer_set(1 as i32 as bit);
            }
        }
        8 => {}
        10 => {
            (unsafe { (*data).progress.t_redirect = Curl_timediff_us(now, (*data).progress.start) });
        }
        0 | _ => {}
    }
    if !borrow(&delta).is_none() {
        let mut us: i64 = Curl_timediff_us(now, unsafe { (*data).progress.t_startsingle });
...
#[no_mangle]
pub extern "C" fn Curl_pgrsStartNow(mut data: *mut crate::src::lib::http2::Curl_easy) {
    (unsafe { (*data).progress.speeder_c = 0 as i32 });
    (unsafe { (*data).progress.start = Curl_now() });
    let mut fresh2 = unsafe { &mut ((*data).progress) };
    (*fresh2).set_is_t_startransfer_set(0 as i32 as bit);
    (unsafe { (*data).progress.ul_limit_start = (*data).progress.start });
    (unsafe { (*data).progress.dl_limit_start = (*data).progress.start });
    (unsafe { (*data).progress.ul_limit_size = 0 as i32 as curl_off_t });
    (unsafe { (*data).progress.dl_limit_size = 0 as i32 as curl_off_t });
    (unsafe { (*data).progress.downloaded = 0 as i32 as curl_off_t });
    (unsafe { (*data).progress.uploaded = 0 as i32 as curl_off_t });
    (unsafe { (*data).progress.flags &= (1 as i32) << 4 as i32 | (1 as i32) << 7 as i32 });
    Curl_ratelimit(data, unsafe { (*data).progress.start });
}
#[no_mangle]
pub extern "C" fn Curl_pgrsLimitWaitTime(
    ...
    if size < 0x7fffffffffffffff as i64 / 1000 as i32 as i64 {
        ...
        minimum = size / limit;
        if minimum < 0x7fffffffffffffff as i64 / 1000 as i32 as i64 {
...
#[no_mangle]
pub extern "C" fn Curl_pgrsSetDownloadCounter(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut size: i64,
) {
    (unsafe { (*data).progress.downloaded = size });
}
#[no_mangle]
pub extern "C" fn Curl_ratelimit(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut now: crate::src::lib::http2::curltime,
) {
    if (unsafe { (*data).set.max_recv_speed }) != 0 {
        if Curl_timediff(now, unsafe { (*data).progress.dl_limit_start }) >= 3000 as i32 as i64 {
            (unsafe { (*data).progress.dl_limit_start = now });
            (unsafe { (*data).progress.dl_limit_size = (*data).progress.downloaded });
        }
    }
    if (unsafe { (*data).set.max_send_speed }) != 0 {
        if Curl_timediff(now, unsafe { (*data).progress.ul_limit_start }) >= 3000 as i32 as i64 {
            (unsafe { (*data).progress.ul_limit_start = now });
            (unsafe { (*data).progress.ul_limit_size = (*data).progress.uploaded });
        }
    }
}
#[no_mangle]
pub extern "C" fn Curl_pgrsSetUploadCounter(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut size: i64,
) {
    (unsafe { (*data).progress.uploaded = size });
}
#[no_mangle]
pub extern "C" fn Curl_pgrsSetDownloadSize(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut size: i64,
) {
    if size >= 0 as i32 as i64 {
        (unsafe { (*data).progress.size_dl = size });
        (unsafe { (*data).progress.flags |= (1 as i32) << 6 as i32 });
    } else {
        (unsafe { (*data).progress.size_dl = 0 as i32 as curl_off_t });
        (unsafe { (*data).progress.flags &= !((1 as i32) << 6 as i32) });
    };
}
#[no_mangle]
pub extern "C" fn Curl_pgrsSetUploadSize(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut size: i64,
) {
    if size >= 0 as i32 as i64 {
        (unsafe { (*data).progress.size_ul = size });
        (unsafe { (*data).progress.flags |= (1 as i32) << 5 as i32 });
    } else {
        (unsafe { (*data).progress.size_ul = 0 as i32 as curl_off_t });
        (unsafe { (*data).progress.flags &= !((1 as i32) << 5 as i32) });
    };
}
extern "C" fn trspeed(mut size: i64, mut us: i64) -> i64 {
    ...
    } else if size < 0x7fffffffffffffff as i64 / 1000000 as i32 as i64 {
...
extern "C" fn progress_calc(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut now: crate::src::lib::http2::curltime,
) -> bool {
    let mut timetoshow: bool = 0 as i32 != 0;
    let p: *mut crate::src::lib::http2::Progress = unsafe { &mut (*data).progress };
    (unsafe { (*p).timespent = Curl_timediff_us(now, (*p).start) });
    (unsafe { (*p).dlspeed = trspeed((*p).downloaded, (*p).timespent) });
    (unsafe { (*p).ulspeed = trspeed((*p).uploaded, (*p).timespent) });
    if (unsafe { (*p).lastshow }) != now.tv_sec {
        let mut countindex: i32 = 0;
        let mut nowindex: i32 = (unsafe { (*p).speeder_c }) % (5 as i32 + 1 as i32);
        (unsafe { (*p).lastshow = now.tv_sec });
        timetoshow = 1 as i32 != 0;
        (unsafe { (*p).speeder[nowindex as usize] = (*p).downloaded + (*p).uploaded });
        (unsafe { (*p).speeder_time[nowindex as usize] = now });
        let mut fresh3 = unsafe { &mut ((*p).speeder_c) };
        *fresh3 += 1;
        countindex = (if (unsafe { (*p).speeder_c }) >= 5 as i32 + 1 as i32 {
            5 as i32 + 1 as i32
        } else {
            unsafe { (*p).speeder_c }
        }) - 1 as i32;
        if countindex != 0 {
            ...
            checkindex = if (unsafe { (*p).speeder_c }) >= 5 as i32 + 1 as i32 {
                (unsafe { (*p).speeder_c }) % (5 as i32 + 1 as i32)
            } else {
                0 as i32
            };
            span_ms = Curl_timediff(now, unsafe { (*p).speeder_time[checkindex as usize] });
            if 0 as i32 as i64 == span_ms {
                span_ms = 1 as i32 as timediff_t;
            }
            amount = (unsafe { (*p).speeder[nowindex as usize] }) - (unsafe { (*p).speeder[checkindex as usize] });
            if amount > 4294967 as i64 {
                (unsafe { (*p).current_speed = (amount as f64 / (span_ms as f64 / 1000.0f64)) as curl_off_t });
            } else {
                (unsafe { (*p).current_speed = amount * 1000 as i64 / span_ms });
            }
        } else {
            (unsafe { (*p).current_speed = (*p).ulspeed + (*p).dlspeed });
        }
    }
    return timetoshow;
}
extern "C" fn progress_meter(mut data: *mut crate::src::lib::http2::Curl_easy) {
    ...
    let mut timespent: i64 = (unsafe { (*data).progress.timespent }) / 1000000 as i32 as i64;
    if (unsafe { (*data).progress.flags }) & (1 as i32) << 7 as i32 == 0 {
        ...
        (unsafe { curl_mfprintf ((* data) . set . err , b"  %% Total    %% Received %% Xferd  Average Speed   Time    Time     Time  Current\n                                 Dload  Upload   Total   Spent    Left  Speed\n\0" as * const u8 as * const i8 ,) }) ;
        (unsafe { (*data).progress.flags |= (1 as i32) << 7 as i32 });
    }
    if (unsafe { (*data).progress.flags }) & (1 as i32) << 5 as i32 != 0 && (unsafe { (*data).progress.ulspeed }) > 0 as i64 {
        ulestimate = (unsafe { (*data).progress.size_ul }) / (unsafe { (*data).progress.ulspeed });
        if (unsafe { (*data).progress.size_ul }) > 10000 as i64 {
            ulpercen = (unsafe { (*data).progress.uploaded }) / ((unsafe { (*data).progress.size_ul }) / 100 as i64);
        } else if (unsafe { (*data).progress.size_ul }) > 0 as i64 {
            ulpercen = (unsafe { (*data).progress.uploaded }) * 100 as i32 as i64 / (unsafe { (*data).progress.size_ul });
        }
    }
    if (unsafe { (*data).progress.flags }) & (1 as i32) << 6 as i32 != 0 && (unsafe { (*data).progress.dlspeed }) > 0 as i64 {
        dlestimate = (unsafe { (*data).progress.size_dl }) / (unsafe { (*data).progress.dlspeed });
        if (unsafe { (*data).progress.size_dl }) > 10000 as i64 {
            dlpercen = (unsafe { (*data).progress.downloaded }) / ((unsafe { (*data).progress.size_dl }) / 100 as i64);
        } else if (unsafe { (*data).progress.size_dl }) > 0 as i64 {
            dlpercen = (unsafe { (*data).progress.downloaded }) * 100 as i32 as i64 / (unsafe { (*data).progress.size_dl });
        }
    }
    total_estimate = if ulestimate > dlestimate {
        ulestimate
    } else {
        dlestimate
    };
    ...
    total_expected_transfer = (if (unsafe { (*data).progress.flags }) & (1 as i32) << 5 as i32 != 0 {
        unsafe { (*data).progress.size_ul }
    } else {
        unsafe { (*data).progress.uploaded }
    }) + (if (unsafe { (*data).progress.flags }) & (1 as i32) << 6 as i32 != 0 {
        unsafe { (*data).progress.size_dl }
    } else {
        unsafe { (*data).progress.downloaded }
    });
    total_transfer = (unsafe { (*data).progress.downloaded }) + (unsafe { (*data).progress.uploaded });
    if total_expected_transfer > 10000 as i64 {
        ...
        total_percen = total_transfer * 100 as i32 as i64 / total_expected_transfer;
    }
    (unsafe { curl_mfprintf(
        (*data).set.err,
        b"\r%3ld %s  %3ld %s  %3ld %s  %s  %s %s %s %s %s\0" as *const u8 as *const i8,
        total_percen,
        max5data(
    ...
    (unsafe { fflush((*data).set.err) });
}
#[no_mangle]
pub extern "C" fn Curl_pgrsUpdate(mut data: *mut crate::src::lib::http2::Curl_easy) -> i32 {
    let mut now: crate::src::lib::http2::curltime = Curl_now();
    let mut showprogress: bool = progress_calc(data, now);
    if (unsafe { (*data).progress.flags }) & (1 as i32) << 4 as i32 == 0 {
        if unsafe { ((*data).set.fxferinfo).is_some() } {
            let mut result: i32 = 0;
            Curl_set_in_callback(data, 1 as i32 != 0);
            result = unsafe { ((*data).set.fxferinfo).expect("non-null function pointer")(
            ...
            if result != 0x10000001 as i32 {
                if result != 0 {
                    (unsafe { Curl_failf(data, b"Callback aborted\0" as *const u8 as *const i8) });
                }
                return result;
            }
        } else if unsafe { ((*data).set.fprogress).is_some() } {
            let mut result_0: i32 = 0;
            Curl_set_in_callback(data, 1 as i32 != 0);
            result_0 = unsafe { ((*data).set.fprogress).expect("non-null function pointer")(
                (*data).set.progress_client,
