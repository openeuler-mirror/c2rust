...
#[no_mangle]
pub extern "C" fn Curl_sasl_cleanup(
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut authused: u32,
) {
    if authused & ((1 as i32) << 9 as i32 | (1 as i32) << 10 as i32) as u32 != 0 {
        Curl_auth_gsasl_cleanup(Some(unsafe { &mut (*conn).gsasl }));
    }
    if authused == ((1 as i32) << 6 as i32) as u32 {
        Curl_auth_cleanup_ntlm(Some(unsafe { &mut (*conn).ntlm }));
    }
}
#[no_mangle]
pub extern "C" fn Curl_sasl_decode_mech<'a1>(
    mut ptr: *const i8,
    mut maxlen: u64,
    ...
    while !(unsafe { mechtable[i as usize].name }).is_null() {
            ...
            ) }) == 0
        {
            if !borrow(&len).is_none() {
                *(borrow_mut(&mut len)).unwrap() = unsafe { mechtable[i as usize].len };
            }
            if maxlen == (unsafe { mechtable[i as usize].len }) {
...
#[no_mangle]
pub extern "C" fn Curl_sasl_parse_url_auth_option<'a1>(
    ...
    if (unsafe { strncmp(value, b"*\0" as *const u8 as *const i8, len) }) == 0 {
        (*(borrow_mut(&mut sasl)).unwrap()).prefmech =
            (0xffff as i32 & !((1 as i32) << 5 as i32)) as u16;
    } else {
        let mut mechbit: u16 = Curl_sasl_decode_mech(value, len, Some(&mut mechlen));
...
#[no_mangle]
pub extern "C" fn Curl_sasl_init<'a1>(
    mut sasl: Option<&'a1 mut crate::src::lib::http2::SASL>,
...
extern "C" fn get_server_message<'a1>(
    mut sasl: *mut crate::src::lib::http2::SASL,
...
#[no_mangle]
pub extern "C" fn Curl_sasl_can_authenticate<'a1>(
    mut sasl: Option<&'a1 mut crate::src::lib::http2::SASL>,
    mut conn: *mut crate::src::lib::http2::connectdata,
) -> bool {
    ...
    if (*(borrow(&sasl)).unwrap()).authmechs as i32
        & (*(borrow(&sasl)).unwrap()).prefmech as i32
        & (1 as i32) << 5 as i32
        != 0
    {
        return 1 as i32 != 0;
...
#[no_mangle]
pub extern "C" fn Curl_sasl_start<'a1>(
    mut sasl: *mut crate::src::lib::http2::SASL,
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut force_ir: bool,
    ...
    *(borrow_mut(&mut progress)).unwrap() = SASL_IDLE;
        ...
        && (unsafe { *((*conn).passwd).offset(0 as i32 as isize) }) == 0
    {
        ...
        if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
            result = Curl_auth_create_external_message(unsafe { (*conn).user }, Some(&mut resp));
        }
                ...
                != 0
        {
            ...
            state2 = SASL_GSASL;
            result =
                Curl_auth_gsasl_start(data, unsafe { (*conn).user }, unsafe { (*conn).passwd }, Some(unsafe { &mut (*conn).gsasl }));
                ...
                != 0
        {
            ...
            && Curl_auth_is_digest_supported() as i32 != 0
        {
            ...
            && Curl_auth_is_ntlm_supported() as i32 != 0
        {
            ...
            if force_ir as i32 != 0 || (unsafe { ((*data).set).sasl_ir() }) as i32 != 0 {
                result = Curl_auth_create_login_message(unsafe { (*conn).user }, Some(&mut resp));
            }
        }
    }
    if result as u64 == 0 && !mech.is_null() {
        ...
        if (unsafe { (*(*sasl).params).maxirlen }) != 0
            && (unsafe { strlen(mech) }).wrapping_add(Curl_bufref_len(Some(&mut resp)))
                > (unsafe { (*(*sasl).params).maxirlen })
        {
        ...
        if result as u64 == 0 {
            result = unsafe { ((*(*sasl).params).sendauth).expect("non-null function pointer")(
                ...
                conn,
                mech,
                Curl_bufref_ptr(Some(&mut resp)) as *const i8,
            ) };
        }
        if result as u64 == 0 {
...
#[no_mangle]
pub extern "C" fn Curl_sasl_continue<'a1>(
    mut sasl: *mut crate::src::lib::http2::SASL,
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut code: i32,
    ...
    *(borrow_mut(&mut progress)).unwrap() = SASL_INPROGRESS;
    ...
    match (unsafe { (*sasl).state }) as u32 {
        ...
        3 => {
            result = Curl_auth_create_login_message(unsafe { (*conn).passwd }, Some(&mut resp));
        }
        4 => {
            result = Curl_auth_create_external_message(unsafe { (*conn).user }, Some(&mut resp));
        }
        15 => {
            ...
            if result as u64 == 0 && Curl_bufref_len(Some(&mut resp)) > 0 as i32 as u64 {
                newstate = SASL_GSASL;
            }
        }
        5 => {
        ...
        16 => {
            let mut fresh2 = unsafe { &mut ((*sasl).authmechs) };
            *fresh2 = (*fresh2 as i32 ^ (unsafe { (*sasl).authused }) as i32) as u16;
            return Curl_sasl_start(
                sasl,
                data,
                conn,
                unsafe { (*sasl).force_ir },
                borrow_mut(&mut progress),
            );
        }
        _ => {
            (unsafe { Curl_failf(
