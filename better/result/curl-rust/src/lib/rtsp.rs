...
pub static mut Curl_handler_rtsp: crate::src::lib::http2::Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            setup_connection: Some(rtsp_setup_connection),
            do_it: Some(rtsp_do),
            done: Some(rtsp_done),
            do_more: None,
            connect_it: Some(rtsp_connect),
            connecting: None,
            doing: None,
            proto_getsock: None,
            doing_getsock: Some(rtsp_getsock_do),
            domore_getsock: None,
            perform_getsock: None,
            disconnect: Some(rtsp_disconnect),
            readwrite: Some(rtsp_rtp_readwrite),
            connection_check: Some(rtsp_conncheck),
            attach: None,
            defport: 554 as i32,
...
extern "C" fn rtsp_setup_connection(
    ...
    let mut fresh0 = unsafe { &mut ((*data).req.p.rtsp) };
    *fresh0 = rtsp;
    if rtsp.is_null() {
...
extern "C" fn rtsp_connisdead(mut check: *mut crate::src::lib::http2::connectdata) -> bool {
    let mut sval: i32 = 0;
    let mut ret_val: bool = 1 as i32 != 0;
    sval = Curl_socket_check(
        unsafe { (*check).sock[0 as i32 as usize] },
        -(1 as i32),
        -(1 as i32),
...
extern "C" fn rtsp_disconnect(
    ...
    let mut fresh1 = unsafe { &mut ((*conn).proto.rtspc.rtp_buf) };
    *fresh1 = 0 as *mut i8;
    return CURLE_OK;
}
extern "C" fn rtsp_done(
    ...
    let mut rtsp: *mut crate::src::lib::http2::RTSP = unsafe { (*data).req.p.rtsp };
    let mut httpStatus: u32 = CURLE_OK;
...
extern "C" fn rtsp_do(
    ...
    let mut rtspreq: u32 = unsafe { (*data).set.rtspreq };
    let mut rtsp: *mut crate::src::lib::http2::RTSP = unsafe { (*data).req.p.rtsp };
    ...
    p_transport = Curl_checkheaders(data, b"Transport\0" as *const u8 as *const i8);
    if rtspreq as u32 == RTSPREQ_SETUP as i32 as u32 && p_transport.is_null() {
        if !(unsafe { (*data).set.str_0[STRING_RTSP_TRANSPORT as i32 as usize] }).is_null() {
            ...
            let mut fresh6 = unsafe { &mut ((*data).state.aptr.rtsp_transport) };
            *fresh6 = 0 as *mut i8;
            let mut fresh7 = unsafe { &mut ((*data).state.aptr.rtsp_transport) };
    ...
    if rtspreq as u32 == RTSPREQ_DESCRIBE as i32 as u32 {
            ...
            && !(unsafe { (*data).set.str_0[STRING_ENCODING as i32 as usize] }).is_null()
        {
            ...
            let mut fresh8 = unsafe { &mut ((*data).state.aptr.accept_encoding) };
            *fresh8 = 0 as *mut i8;
            let mut fresh9 = unsafe { &mut ((*data).state.aptr.accept_encoding) };
        ...
        && !(unsafe { (*data).state.aptr.uagent }).is_null()
    {
        ...
        let mut fresh10 = unsafe { &mut ((*data).state.aptr.uagent) };
        *fresh10 = 0 as *mut i8;
        let mut fresh11 = unsafe { &mut ((*data).state.aptr.uagent) };
        *fresh11 = 0 as *mut i8;
    } else if (Curl_checkheaders(data, b"User-Agent\0" as *const u8 as *const i8)).is_null()
    ...
    let mut fresh12 = unsafe { &mut ((*data).state.aptr.ref_0) };
    *fresh12 = 0 as *mut i8;
    if !(unsafe { (*data).state.referer }).is_null()
        && (Curl_checkheaders(data, b"Referer\0" as *const u8 as *const i8)).is_null()
    {
        let mut fresh13 = unsafe { &mut ((*data).state.aptr.ref_0) };
        *fresh13 = unsafe { curl_maprintf(
            b"Referer: %s\r\n\0" as *const u8 as *const i8,
            (*data).state.referer,
        ) };
    } else {
        let mut fresh14 = unsafe { &mut ((*data).state.aptr.ref_0) };
        *fresh14 = 0 as *mut i8;
    }
    ...
    if (unsafe { ((*data).state).use_range() }) as i32 != 0
        && rtspreq as u32
            ...
            != 0
    {
        if (Curl_checkheaders(data, b"Range\0" as *const u8 as *const i8)).is_null()
            && !(unsafe { (*data).state.range }).is_null()
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")(
                (*data).state.aptr.rangeline as *mut libc::c_void,
            ) });
            let mut fresh15 = unsafe { &mut ((*data).state.aptr.rangeline) };
            *fresh15 = 0 as *mut i8;
            let mut fresh16 = unsafe { &mut ((*data).state.aptr.rangeline) };
    ...
    if !(Curl_checkheaders(data, b"CSeq\0" as *const u8 as *const i8)).is_null() {
        (unsafe { Curl_failf(
            data,
    ...
    if !(Curl_checkheaders(data, b"Session\0" as *const u8 as *const i8)).is_null() {
        (unsafe { Curl_failf(
            data,
    ...
    let mut fresh17 = unsafe { &mut ((*data).state.aptr.userpwd) };
    *fresh17 = 0 as *mut i8;
    let mut fresh18 = unsafe { &mut ((*data).state.aptr.userpwd) };
    *fresh18 = 0 as *mut i8;
    if result as u64 != 0 {
        return result;
    }
        ...
        || rtspreq as u32 == RTSPREQ_GET_PARAMETER as i32 as u32
    {
        ...
        if putsize > 0 as i32 as i64 || postsize > 0 as i32 as i64 {
            if (Curl_checkheaders(data, b"Content-Length\0" as *const u8 as *const i8)).is_null() {
                result = unsafe { Curl_dyn_addf(
                    &mut req_buffer as *mut dynbuf,
                    b"Content-Length: %ld\r\n\0" as *const u8 as *const i8,
            ...
            let mut fresh19 = unsafe { &mut ((*data).state) };
    ...
    let mut fresh21 = unsafe { &mut ((*data).req) };
    (*fresh21).set_forbidchunk(1 as i32 as bit);
    result = Curl_dyn_add(&mut req_buffer, b"\r\n\0" as *const u8 as *const i8);
    if result as u64 != 0 {
        return result;
    }
    if postsize > 0 as i32 as i64 {
        result = Curl_dyn_addn(&mut req_buffer, unsafe { (*data).set.postfields }, postsize as size_t);
        if result as u64 != 0 {
            return result;
        }
    }
    ...
    let mut fresh22 = unsafe { &mut ((*data).state.rtsp_next_client_CSeq) };
    *fresh22 += 1;
...
extern "C" fn rtsp_rtp_readwrite(
    ...
    let mut rtspc: Option<&'_ mut crate::src::lib::http2::rtsp_conn> =
        Some(unsafe { &mut (*conn).proto.rtspc });
    ...
    if !((*(borrow_mut(&mut rtspc)).unwrap()).rtp_buf).is_null() {
        let mut newptr: *mut i8 = Curl_saferealloc(
            ...
            ((*(borrow(&rtspc)).unwrap()).rtp_bufsize + (unsafe { *nread })) as size_t,
        ) as *mut i8;
        if newptr.is_null() {
        ...
        *fresh24 = newptr;
        (unsafe { memcpy(
            ((*(borrow(&rtspc)).unwrap()).rtp_buf)
                .offset((*(borrow(&rtspc)).unwrap()).rtp_bufsize as isize)
                as *mut libc::c_void,
    ...
    while rtp_dataleft > 0 as i32 as i64 && (unsafe { *rtp.offset(0 as i32 as isize) }) as i32 == '$' as i32 {
        if rtp_dataleft > 4 as i32 as i64 {
            ...
            (*(borrow_mut(&mut rtspc)).unwrap()).rtp_channel =
                (unsafe { *rtp.offset(1 as i32 as isize) }) as u8 as i32;
...
#[no_mangle]
pub extern "C" fn Curl_rtsp_parseheader(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut header: *mut i8,
) -> u32 {
    ...
    if curl_strnequal(
        ...
        header,
        unsafe { strlen(b"CSeq:\0" as *const u8 as *const i8) },
    ) != 0
    {
        ...
        header,
        unsafe { strlen(b"Session:\0" as *const u8 as *const i8) },
    ) != 0
    {
        ...
        while (unsafe { *end }) as i32 != 0 && (unsafe { *end }) as i32 != ';' as i32 && Curl_isspace((unsafe { *end }) as u8 as i32) == 0
        {
        ...
        if !(unsafe { (*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize] }).is_null() {
            ...
            let mut fresh35 = unsafe { &mut ((*data).set.str_0[STRING_RTSP_SESSION_ID as i32 as usize]) };
