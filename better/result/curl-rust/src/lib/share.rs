...
#[no_mangle]
pub extern "C" fn curl_share_init() -> *mut crate::src::lib::asyn_thread::Curl_share {
    let mut share: *mut crate::src::lib::asyn_thread::Curl_share = (unsafe { Curl_ccalloc
        .expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<Curl_share>() as u64,
    ) }) as *mut Curl_share;
    if !share.is_null() {
        (unsafe { (*share).magic = 0x7e117a1e as i32 as u32 });
        (unsafe { (*share).specifier |= ((1 as i32) << CURL_LOCK_DATA_SHARE as i32) as u32 });
        if Curl_mk_dnscache(Some(unsafe { &mut (*share).hostcache })) != 0 {
            (unsafe { Curl_cfree.expect("non-null function pointer")(share as *mut libc::c_void) });
...
pub unsafe extern "C" fn curl_share_setopt(
    ...
    match option as u32 {
            ...
            match type_0 {
                ...
                4 => {
                    if ((*share).sslsession).is_null() {
                        ...
                        *fresh1 = Curl_ccalloc.expect("non-null function pointer")(
                ...
                5 => {
                    if Curl_conncache_init(&mut (*share).conn_cache, 103 as i32) != 0 {
                        res = CURLSHE_NOMEM;
                    }
                }
                3 | 6 => {}
        ...
        3 => {
            lockfunc = core::intrinsics::transmute::<
        ...
        4 => {
            unlockfunc = core::intrinsics::transmute::<
                *mut unsafe extern "C" fn(
                    _: *mut crate::src::lib::http2::Curl_easy,
                    _: u32,
                    _: *mut core::ffi::c_void,
...
#[no_mangle]
pub extern "C" fn curl_share_cleanup(
    mut share: *mut crate::src::lib::asyn_thread::Curl_share,
) -> u32 {
    if !(!share.is_null() && (unsafe { (*share).magic }) == 0x7e117a1e as i32 as u32) {
        return CURLSHE_INVALID;
    }
    if unsafe { ((*share).lockfunc).is_some() } {
        (unsafe { ((*share).lockfunc).expect("non-null function pointer")(
            0 as *mut CURL,
    ...
    if (unsafe { (*share).dirty }) != 0 {
        if unsafe { ((*share).unlockfunc).is_some() } {
            (unsafe { ((*share).unlockfunc).expect("non-null function pointer")(
                0 as *mut CURL,
                CURL_LOCK_DATA_SHARE,
                (*share).clientdata,
            ) });
    ...
    Curl_conncache_close_all_connections(Some(unsafe { &mut (*share).conn_cache }));
    Curl_conncache_destroy(Some(unsafe { &mut (*share).conn_cache }));
    Curl_hash_destroy(unsafe { &mut (*share).hostcache });
    Curl_cookie_cleanup(unsafe { (*share).cookies });
    if !(unsafe { (*share).sslsession }).is_null() {
        let mut i: u64 = 0;
        i = 0 as i32 as size_t;
        while i < (unsafe { (*share).max_ssl_sessions }) {
            Curl_ssl_kill_session(Some(unsafe { &mut *((*share).sslsession).offset(i as isize) }));
            i = i.wrapping_add(1);
        }
        (unsafe { Curl_cfree.expect("non-null function pointer")((*share).sslsession as *mut libc::c_void) });
    }
    Curl_psl_destroy(unsafe { &mut (*share).psl });
    if unsafe { ((*share).unlockfunc).is_some() } {
        (unsafe { ((*share).unlockfunc).expect("non-null function pointer")(
            0 as *mut CURL,
            CURL_LOCK_DATA_SHARE,
    ...
    (unsafe { (*share).magic = 0 as i32 as u32 });
    (unsafe { Curl_cfree.expect("non-null function pointer")(share as *mut libc::c_void) });
    return CURLSHE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_share_lock(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut type_0: u32,
    mut accesstype: u32,
) -> u32 {
    let mut share: *mut crate::src::lib::asyn_thread::Curl_share = unsafe { (*data).share };
    if share.is_null() {
        return CURLSHE_INVALID;
    }
    if (unsafe { (*share).specifier }) & ((1 as i32) << type_0 as u32) as u32 != 0 {
        if unsafe { ((*share).lockfunc).is_some() } {
            (unsafe { ((*share).lockfunc).expect("non-null function pointer")(
                data,
                type_0,
...
#[no_mangle]
pub extern "C" fn Curl_share_unlock(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut type_0: u32,
) -> u32 {
    let mut share: *mut crate::src::lib::asyn_thread::Curl_share = unsafe { (*data).share };
