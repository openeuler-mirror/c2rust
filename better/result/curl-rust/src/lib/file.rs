...
pub static mut Curl_handler_file: crate::src::lib::http2::Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            setup_connection: Some(file_setup_connection),
            do_it: Some(file_do),
            done: Some(file_done),
            do_more: None,
            connect_it: Some(file_connect),
            connecting: None,
            doing: None,
            proto_getsock: None,
            doing_getsock: None,
            ...
            disconnect: Some(file_disconnect),
            readwrite: None,
...
extern "C" fn file_setup_connection(
    ...
    let mut fresh0 = unsafe { &mut ((*data).req.p.file) };
...
extern "C" fn file_connect(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut done: *mut bool,
) -> u32 {
    let mut real_path: *mut i8 = 0 as *mut i8;
    let mut file: *mut crate::src::lib::http2::FILEPROTO = unsafe { (*data).req.p.file };
    let mut fd: i32 = 0;
    ...
    let mut fresh1 = unsafe { &mut ((*file).path) };
    *fresh1 = real_path;
    let mut fresh2 = unsafe { &mut ((*file).freepath) };
    *fresh2 = real_path;
    (unsafe { (*file).fd = fd });
...
extern "C" fn file_done(
    ...
    if !file.is_null() {
        ...
        let mut fresh3 = unsafe { &mut ((*file).freepath) };
        *fresh3 = 0 as *mut i8;
        let mut fresh4 = unsafe { &mut ((*file).path) };
        *fresh4 = 0 as *mut i8;
...
extern "C" fn file_upload(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut file: *mut crate::src::lib::http2::FILEPROTO = unsafe { (*data).req.p.file };
    ...
    let mut fresh5 = unsafe { &mut ((*data).req.upload_fromhere) };
    *fresh5 = buf;
    if dir.is_null() {
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    ...
    while result as u64 == 0 {
        ...
        if (unsafe { (*data).state.resume_from }) != 0 {
            if nread as curl_off_t <= (unsafe { (*data).state.resume_from }) {
                let mut fresh6 = unsafe { &mut ((*data).state.resume_from) };
...
extern "C" fn file_do(
    ...
    let mut bytecount: i64 = 0 as i32 as curl_off_t;
    let mut fd: i32 = 0;
    let mut file: *mut crate::src::lib::http2::FILEPROTO =
        0 as *mut crate::src::lib::http2::FILEPROTO;
    (unsafe { *done = 1 as i32 != 0 });
    ...
    {
        if !Curl_meets_timecondition(data, unsafe { (*data).info.filetime }) {
            (unsafe { *done = 1 as i32 != 0 });
            return CURLE_OK;
        }
    }
    if fstated {
        ...
        let mut accept_ranges: [i8; 24] = *(unsafe { core::intrinsics::transmute::<
            &'_ [u8; 24],
            &'_ mut [i8; 24],
        ...
        if expected_size >= 0 as i32 as i64 {
            ...
            result = Curl_client_write(
                ...
                unsafe { strlen(accept_ranges.as_mut_ptr()) },
            );
    ...
    if (unsafe { (*data).state.resume_from }) < 0 as i32 as i64 {
        ...
        let mut fresh7 = unsafe { &mut ((*data).state.resume_from) };
    ...
    while result as u64 == 0 {
        ...
        result = Curl_client_write(data, (1 as i32) << 0 as i32, buf, nread as size_t);
        if result as u64 != 0 {
            return result;
        }
