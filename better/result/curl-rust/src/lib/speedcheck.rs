...
#[no_mangle]
pub extern "C" fn Curl_speedinit(mut data: *mut crate::src::lib::http2::Curl_easy) {
    (unsafe { memset(
...
#[no_mangle]
pub extern "C" fn Curl_speedcheck(
    ...
    if (unsafe { (*data).progress.current_speed }) >= 0 as i32 as i64 && (unsafe { (*data).set.low_speed_time }) != 0 {
        if (unsafe { (*data).progress.current_speed }) < (unsafe { (*data).set.low_speed_limit }) {
            if (unsafe { (*data).state.keeps_speed.tv_sec }) == 0 {
                ...
                let mut howlong: i64 = Curl_timediff(now, unsafe { (*data).state.keeps_speed });
