...
extern "C" fn fileinfo_dtor(mut _user: *mut core::ffi::c_void, mut element: *mut core::ffi::c_void) {
    Curl_fileinfo_cleanup(element as *mut fileinfo);
}
#[no_mangle]
pub extern "C" fn Curl_wildcard_init<'a1>(
    mut wc: Option<&'a1 mut crate::src::lib::http2::WildcardData>,
) -> u32 {
    Curl_llist_init(
...
#[no_mangle]
pub extern "C" fn Curl_wildcard_dtor<'a1>(
    mut wc: Option<&'a1 mut crate::src::lib::http2::WildcardData>,
) {
    ...
    if ((*(borrow(&wc)).unwrap()).dtor).is_some() {
        (unsafe { ((*(borrow(&wc)).unwrap()).dtor).expect("non-null function pointer")(
            (*(borrow_mut(&mut wc)).unwrap()).protdata,
        ) });
    ...
    Curl_llist_destroy(
        &mut (*(borrow_mut(&mut wc)).unwrap()).filelist,
        0 as *mut libc::c_void,
    );
    (unsafe { Curl_cfree.expect("non-null function pointer")(
        (*(borrow_mut(&mut wc)).unwrap()).path as *mut libc::c_void,
    ) });
    let mut fresh2 = &mut ((*(borrow_mut(&mut wc)).unwrap()).path);
    *fresh2 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")(
        (*(borrow_mut(&mut wc)).unwrap()).pattern as *mut libc::c_void,
    ) });
