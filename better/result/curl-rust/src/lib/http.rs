...
pub static mut Curl_handler_http: crate::src::lib::http2::Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            setup_connection: Some(http_setup_conn),
            do_it: Some(Curl_http),
            done: Some(Curl_http_done),
            do_more: None,
            connect_it: Some(Curl_http_connect),
            connecting: None,
            doing: None,
            proto_getsock: None,
            doing_getsock: Some(http_getsock_do),
            domore_getsock: None,
...
pub static mut Curl_handler_https: crate::src::lib::http2::Curl_handler =  {
    {
        let mut init = Curl_handler {
            ...
            do_more: None,
            connect_it: Some(Curl_http_connect),
            connecting: Some(https_connecting),
            doing: None,
            proto_getsock: Some(https_getsock),
            doing_getsock: Some(http_getsock_do),
            domore_getsock: None,
            perform_getsock: None,
            disconnect: None,
...
extern "C" fn http_setup_conn(
    ...
    Curl_mime_initpart(unsafe { &mut (*http).form }, data);
    let mut fresh0 = unsafe { &mut ((*data).req.p.http) };
    *fresh0 = http;
...
pub extern "C" fn Curl_copy_header_value(mut header: *const i8) -> *mut i8 {
    ...
    start = header;
    while (unsafe { *start }) as i32 != 0 && Curl_isspace((unsafe { *start }) as u8 as i32) != 0 {
    ...
    while end > start && Curl_isspace((unsafe { *end }) as u8 as i32) != 0 {
...
extern "C" fn http_output_basic(
    ...
    if proxy {
        userp = Some(unsafe { &mut (*data).state.aptr.proxyuserpwd });
        user = unsafe { (*data).state.aptr.proxyuser };
        pwd = unsafe { (*data).state.aptr.proxypasswd };
    } else {
        userp = Some(unsafe { &mut (*data).state.aptr.userpwd });
    ...
    if !(result as u64 != 0) {
        if authorization.is_null() {
            ...
            *(borrow_mut(&mut userp)).unwrap() = unsafe { curl_maprintf(
...
extern "C" fn http_output_bearer(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut userp: Option<&'_ mut *mut i8> = Option::<&'_ mut *mut i8>::None;
    let mut result: u32 = CURLE_OK;
    userp = Some(unsafe { &mut (*data).state.aptr.userpwd });
    (unsafe { Curl_cfree.expect("non-null function pointer")(
...
extern "C" fn pickoneauth<'a1>(
    mut pick: Option<&'a1 mut crate::src::lib::http2::auth>,
    mut mask: u64,
) -> bool {
    let mut picked: bool = false;
    let mut avail: u64 =
        (*(borrow(&pick)).unwrap()).avail & (*(borrow(&pick)).unwrap()).want & mask;
    picked = 1 as i32 != 0;
            ...
            || (unsafe { (*data).state.authhost.picked }) == (1 as i32 as u64) << 5 as i32
        {
                ...
                || (unsafe { (*conn).proxy_ntlm_state }) as u32 != NTLMSTATE_NONE as i32 as u32
            {
                if (unsafe { ((*conn).bits).authneg() }) == 0 && (unsafe { (*conn).writesockfd }) != -(1 as i32) {
                    let mut fresh2 = unsafe { &mut ((*conn).bits) };
...
#[no_mangle]
pub extern "C" fn Curl_http_auth_act(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    ...
    {
        pickhost = pickoneauth(Some(unsafe { &mut (*data).state.authhost }), authmask);
        if !pickhost {
            ...
            || (unsafe { ((*conn).bits).authneg() }) as i32 != 0 && (unsafe { (*data).req.httpcode }) < 300 as i32)
    {
        pickproxy = pickoneauth(
            Some(unsafe { &mut (*data).state.authproxy }),
            authmask & !((1 as i32 as u64) << 6 as i32),
        );
        if !pickproxy {
            let mut fresh4 = unsafe { &mut ((*data).state) };
    ...
    if pickhost as i32 != 0 || pickproxy as i32 != 0 {
        ...
        let mut fresh5 = unsafe { &mut ((*data).req.newurl) };
        *fresh5 = 0 as *mut i8;
        let mut fresh6 = unsafe { &mut ((*data).req.newurl) };
        ...
        && (unsafe { ((*conn).bits).authneg() }) as i32 != 0
    {
        if (unsafe { (*data).state.httpreq }) as u32 != HTTPREQ_GET as i32 as u32
            && (unsafe { (*data).state.httpreq }) as u32 != HTTPREQ_HEAD as i32 as u32
        {
            let mut fresh7 = unsafe { &mut ((*data).req.newurl) };
            *fresh7 = unsafe { Curl_cstrdup.expect("non-null function pointer")((*data).state.url) };
            if (unsafe { (*data).req.newurl }).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
            let mut fresh8 = unsafe { &mut ((*data).state.authhost) };
    ...
    if (unsafe { (*authstatus).picked }) == (1 as i32 as u64) << 7 as i32 {
        ...
        if proxy as i32 != 0
            ...
            || !proxy
                ...
                && (Curl_checkheaders(data, b"Authorization\0" as *const u8 as *const i8)).is_null()
        {
...
pub extern "C" fn Curl_http_output_auth(
    ...
    if (unsafe { ((*data).state).this_is_a_follow() }) == 0
        ...
        || Curl_strcasecompare(unsafe { (*data).state.first_host }, unsafe { (*conn).host.name }) != 0
    {
        ...
        && httpreq as u32 != HTTPREQ_HEAD as i32 as u32
    {
        ...
        let mut fresh10 = unsafe { &mut ((*conn).bits) };
...
pub extern "C" fn Curl_http_input_auth(
    ...
    let mut authp: Option<&'_ mut crate::src::lib::http2::auth> =
        Option::<&'_ mut crate::src::lib::http2::auth>::None;
    if proxy {
        availp = Some(unsafe { &mut (*data).info.proxyauthavail });
        authp = Some(unsafe { &mut (*data).state.authproxy });
    } else {
        availp = Some(unsafe { &mut (*data).info.httpauthavail });
        authp = Some(unsafe { &mut (*data).state.authhost });
    }
    while (unsafe { *auth }) != 0 {
        if curl_strnequal(
            b"NTLM\0" as *const u8 as *const i8,
            auth,
            unsafe { strlen(b"NTLM\0" as *const u8 as *const i8) },
        ) != 0
            && is_valid_auth_separator(unsafe { *auth.offset(4 as i32 as isize) }) != 0
        {
            if (*(borrow(&authp)).unwrap()).avail & (1 as i32 as u64) << 3 as i32 != 0
                || (*(borrow(&authp)).unwrap()).avail & (1 as i32 as u64) << 5 as i32 != 0
                || Curl_auth_is_ntlm_supported() as i32 != 0
            {
                *(borrow_mut(&mut availp)).unwrap() |= (1 as i32 as u64) << 3 as i32;
                (*(borrow_mut(&mut authp)).unwrap()).avail |= (1 as i32 as u64) << 3 as i32;
                if (*(borrow(&authp)).unwrap()).picked == (1 as i32 as u64) << 3 as i32
                    || (*(borrow(&authp)).unwrap()).picked == (1 as i32 as u64) << 5 as i32
                {
                    let mut result: u32 = Curl_input_ntlm(data, proxy, auth);
                    if result as u64 == 0 {
                        let mut fresh11 = unsafe { &mut ((*data).state) };
                        (*fresh11).set_authproblem(0 as i32 as bit);
                        if (*(borrow(&authp)).unwrap()).picked == (1 as i32 as u64) << 5 as i32 {
                            *(borrow_mut(&mut availp)).unwrap() &= !((1 as i32 as u64) << 3 as i32);
                            (*(borrow_mut(&mut authp)).unwrap()).avail &=
                                !((1 as i32 as u64) << 3 as i32);
                            *(borrow_mut(&mut availp)).unwrap() |= (1 as i32 as u64) << 5 as i32;
                            (*(borrow_mut(&mut authp)).unwrap()).avail |=
                            ...
                            if result as u64 != 0 {
                                ...
                                let mut fresh12 = unsafe { &mut ((*data).state) };
            ...
            auth,
            unsafe { strlen(b"Digest\0" as *const u8 as *const i8) },
        ) != 0
            && is_valid_auth_separator(unsafe { *auth.offset(6 as i32 as isize) }) != 0
        {
            if (*(borrow(&authp)).unwrap()).avail & (1 as i32 as u64) << 1 as i32 != 0 as i32 as u64
            {
                (unsafe { Curl_infof(
                    data,
                ...
                *(borrow_mut(&mut availp)).unwrap() |= (1 as i32 as u64) << 1 as i32;
                (*(borrow_mut(&mut authp)).unwrap()).avail |= (1 as i32 as u64) << 1 as i32;
                ...
                if result_0 as u64 != 0 {
                    ...
                    let mut fresh14 = unsafe { &mut ((*data).state) };
            ...
            auth,
            unsafe { strlen(b"Basic\0" as *const u8 as *const i8) },
        ) != 0
            && is_valid_auth_separator(unsafe { *auth.offset(5 as i32 as isize) }) != 0
        {
            *(borrow_mut(&mut availp)).unwrap() |= (1 as i32 as u64) << 0 as i32;
            ...
            if (*(borrow(&authp)).unwrap()).picked == (1 as i32 as u64) << 0 as i32 {
                ...
                let mut fresh15 = unsafe { &mut ((*data).state) };
                (*fresh15).set_authproblem(1 as i32 as bit);
            }
        } else if curl_strnequal(
            b"Bearer\0" as *const u8 as *const i8,
            auth,
            unsafe { strlen(b"Bearer\0" as *const u8 as *const i8) },
        ) != 0
            && is_valid_auth_separator(unsafe { *auth.offset(6 as i32 as isize) }) != 0
        {
            *(borrow_mut(&mut availp)).unwrap() |= (1 as i32 as u64) << 6 as i32;
            ...
            if (*(borrow(&authp)).unwrap()).picked == (1 as i32 as u64) << 6 as i32 {
                ...
                let mut fresh16 = unsafe { &mut ((*data).state) };
        ...
        while (unsafe { *auth }) as i32 != 0 && Curl_isspace((unsafe { *auth }) as u8 as i32) != 0 {
            auth = unsafe { auth.offset(1) };
        }
    }
    return CURLE_OK;
}
extern "C" fn http_should_fail(mut data: *mut crate::src::lib::http2::Curl_easy) -> bool {
...
extern "C" fn readmoredata(
    ...
    let mut fresh17 = unsafe { &mut ((*data).req) };
        ...
        && (unsafe { (*data).set.max_send_speed }) < (unsafe { (*http).postsize })
    {
        ...
        if (unsafe { (*http).backup.postsize }) != 0 {
            let mut fresh18 = unsafe { &mut ((*http).postdata) };
            *fresh18 = unsafe { (*http).backup.postdata };
            (unsafe { (*http).postsize = (*http).backup.postsize });
            let mut fresh19 = unsafe { &mut ((*data).state.fread_func) };
            *fresh19 = unsafe { (*http).backup.fread_func };
            let mut fresh20 = unsafe { &mut ((*data).state.in_0) };
            *fresh20 = unsafe { (*http).backup.fread_in };
            let mut fresh21 = unsafe { &mut ((*http).sending) };
            *fresh21 += 1;
...
pub extern "C" fn Curl_buffer_send<'a1>(
        ...
        && (unsafe { (*conn).httpversion }) as i32 != 20 as i32
    {
        if (unsafe { (*data).set.max_send_speed }) != 0 && included_body_bytes > (unsafe { (*data).set.max_send_speed }) {
            let mut overflow: i64 = included_body_bytes - (unsafe { (*data).set.max_send_speed });
        ...
        let mut overflow_0: i64 = included_body_bytes - (unsafe { (*data).set.max_send_speed });
    ...
    if result as u64 == 0 {
        ...
        if bodylen != 0 {
            Curl_debug(
                data,
                CURLINFO_DATA_OUT,
                unsafe { ptr.offset(headlen as isize) },
                bodylen,
            );
        }
        *(borrow_mut(&mut bytes_written)).unwrap() += amount;
        if !http.is_null() {
            let mut fresh24 = unsafe { &mut ((*data).req.writebytecount) };
            *fresh24 = (*fresh24 as u64).wrapping_add(bodylen) as curl_off_t as curl_off_t;
            Curl_pgrsSetUploadCounter(data, unsafe { (*data).req.writebytecount });
            if amount as size_t != size {
                ...
                let mut fresh25 = unsafe { &mut ((*http).backup.fread_func) };
                *fresh25 = unsafe { (*data).state.fread_func };
                let mut fresh26 = unsafe { &mut ((*http).backup.fread_in) };
                *fresh26 = unsafe { (*data).state.in_0 };
                let mut fresh27 = unsafe { &mut ((*http).backup.postdata) };
                *fresh27 = unsafe { (*http).postdata };
                (unsafe { (*http).backup.postsize = (*http).postsize });
                let mut fresh28 = unsafe { &mut ((*data).state.fread_func) };
                *fresh28 = unsafe { core::intrinsics::transmute::<
                    Option<
                        unsafe extern "C" fn(
                            _: *mut i8,
                            _: u64,
                ...
                *fresh29 = data as *mut libc::c_void;
                let mut fresh30 = unsafe { &mut ((*http).postdata) };
                *fresh30 = ptr;
...
pub extern "C" fn Curl_compareheader(
    ...
    let mut hlen: u64 = unsafe { strlen(header) };
    let mut clen: u64 = 0;
    let mut len: u64 = 0;
    let mut start: *const i8 = 0 as *const i8;
    let mut end: *const i8 = 0 as *const i8;
...
extern "C" fn add_haproxy_protocol_header(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    ...
    if !(unsafe { (*(*data).conn).unix_domain_socket }).is_null() {
        result = Curl_dyn_add(&mut req, b"PROXY UNKNOWN\r\n\0" as *const u8 as *const i8);
    } else {
    ...
    if result as u64 == 0 {
        result = Curl_buffer_send(
            &mut req,
            data,
            Some(unsafe { &mut (*data).info.request_size }),
...
extern "C" fn https_connecting(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut done: *mut bool,
) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    result = Curl_ssl_connect_nonblocking(data, conn, 0 as i32 != 0, 0 as i32, done);
    if result as u64 != 0 {
        Curl_conncontrol(conn, 1 as i32);
    }
    return result;
...
pub extern "C" fn Curl_http_done(
    ...
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    let mut http: *mut crate::src::lib::http2::HTTP = unsafe { (*data).req.p.http };
    let mut fresh31 = unsafe { &mut ((*data).state.authhost) };
    (*fresh31).set_multipass(0 as i32 as bit);
    let mut fresh32 = unsafe { &mut ((*data).state.authproxy) };
    (*fresh32).set_multipass(0 as i32 as bit);
    Curl_unencode_cleanup(data);
    let mut fresh33 = unsafe { &mut ((*conn).seek_func) };
    *fresh33 = unsafe { (*data).set.seek_func };
    let mut fresh34 = unsafe { &mut ((*conn).seek_client) };
    *fresh34 = unsafe { (*data).set.seek_client };
    if http.is_null() {
        return CURLE_OK;
    }
    Curl_dyn_free(unsafe { &mut (*http).send_buffer });
    Curl_http2_done(data, premature);
    Curl_mime_cleanpart(unsafe { &mut (*http).form });
    Curl_dyn_reset(Some(unsafe { &mut (*data).state.headerb }));
    if status as u64 != 0 {
        return status;
    }
    if !premature
...
extern "C" fn expect100(
    ...
    if (unsafe { ((*data).state).disableexpect() }) == 0
        ...
        && ((unsafe { (*conn).httpversion }) as i32) < 20 as i32
    {
        let mut ptr: *const i8 = Curl_checkheaders(data, b"Expect\0" as *const u8 as *const i8);
        if !ptr.is_null() {
            ...
            result = Curl_dyn_add(req, b"Expect: 100-continue\r\n\0" as *const u8 as *const i8);
            if result as u64 == 0 {
...
pub extern "C" fn Curl_http_compile_trailers(
    ...
    while !trailers.is_null() {
        ...
        if !ptr.is_null() && (unsafe { *ptr.offset(1 as i32 as isize) }) as i32 == ' ' as i32 {
            result = Curl_dyn_add(b, unsafe { (*trailers).data });
            if result as u64 != 0 {
                return result;
            }
        ...
        while !headers.is_null() {
            ...
            if ptr.is_null() {
                ...
                if !ptr.is_null() {
                    ...
                    while (unsafe { *ptr }) as i32 != 0 && Curl_isspace((unsafe { *ptr }) as u8 as i32) != 0 {
                        ptr = unsafe { ptr.offset(1) };
                    }
                    if (unsafe { *ptr }) != 0 {
                        optr = 0 as *mut i8;
                ...
                if (unsafe { *ptr }) as i32 != 0 || !semicolonp.is_null() {
                    ...
                    if !(!(unsafe { (*data).state.aptr.host }).is_null()
                        && curl_strnequal(
                            ...
                            compare,
                            unsafe { strlen(b"Host:\0" as *const u8 as *const i8) },
                        ) != 0)
                    {
                        if !((unsafe { (*data).state.httpreq }) as u32 == HTTPREQ_POST_FORM as i32 as u32
                            && curl_strnequal(
                                ...
                                compare,
                                unsafe { strlen(b"Content-Type:\0" as *const u8 as *const i8) },
                            ) != 0)
                        {
                                ...
                                ) != 0)
                            {
                                if !((unsafe { ((*conn).bits).authneg() }) as i32 != 0
                                    && curl_strnequal(
                                        ...
                                        compare,
                                        unsafe { strlen(b"Content-Length:\0" as *const u8 as *const i8) },
                                    ) != 0)
                                {
                                    if !(!(unsafe { (*data).state.aptr.te }).is_null()
                                        && curl_strnequal(
                                            ...
                                            compare,
                                            unsafe { strlen(b"Connection:\0" as *const u8 as *const i8) },
                                        ) != 0)
                                    {
                                            ...
                                            ) != 0)
                                        {
                                            if !((curl_strnequal(
                                                ...
                                                || curl_strnequal(
                                                    ...
                                                    compare,
                                                    unsafe { strlen(b"Cookie:\0" as *const u8 as *const i8) },
                                                ) != 0)
...
pub extern "C" fn Curl_add_timecondition(
    ...
    let mut tm: Option<&'_ crate::src::lib::altsvc::tm> =
        Option::<&'_ crate::src::lib::altsvc::tm>::None;
    ...
    result = Curl_gmtime(unsafe { (*data).set.timevalue }, &mut keeptime);
    if result as u64 != 0 {
...
#[no_mangle]
pub extern "C" fn Curl_http_method<'a1, 'a2>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut method: Option<&'a1 mut *const i8>,
    mut reqp: Option<&'a2 mut u32>,
) {
    let mut httpreq: u32 = unsafe { (*data).state.httpreq };
...
#[no_mangle]
pub extern "C" fn Curl_http_useragent(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    if !(Curl_checkheaders(data, b"User-Agent\0" as *const u8 as *const i8)).is_null() {
        (unsafe { Curl_cfree.expect("non-null function pointer")(
            (*data).state.aptr.uagent as *mut libc::c_void,
        ) });
        let mut fresh38 = unsafe { &mut ((*data).state.aptr.uagent) };
        *fresh38 = 0 as *mut i8;
    }
    return CURLE_OK;
}
...
pub extern "C" fn Curl_http_host(
    ...
    if (unsafe { ((*data).state).this_is_a_follow() }) == 0 {
        ...
        let mut fresh39 = unsafe { &mut ((*data).state.first_host) };
    ...
    let mut fresh40 = unsafe { &mut ((*data).state.aptr.host) };
    *fresh40 = 0 as *mut i8;
            ...
            || Curl_strcasecompare(unsafe { (*data).state.first_host }, unsafe { (*conn).host.name }) != 0)
    {
        ...
        if (unsafe { *cookiehost }) == 0 {
            ...
            let mut fresh41 = unsafe { &mut ((*data).state.aptr.cookiehost) };
            *fresh41 = 0 as *mut i8;
            let mut fresh42 = unsafe { &mut ((*data).state.aptr.cookiehost) };
            *fresh42 = cookiehost;
        }
        if (unsafe { strcmp(b"Host:\0" as *const u8 as *const i8, ptr) }) != 0 {
            let mut fresh43 = unsafe { &mut ((*data).state.aptr.host) };
            *fresh43 = unsafe { curl_maprintf(
            ...
            let mut fresh44 = unsafe { &mut ((*data).state.aptr.host) };
            *fresh44 = 0 as *mut i8;
        }
    } else {
        ...
        {
            let mut fresh45 = unsafe { &mut ((*data).state.aptr.host) };
            *fresh45 = unsafe { curl_maprintf(
            ...
            let mut fresh46 = unsafe { &mut ((*data).state.aptr.host) };
...
pub extern "C" fn Curl_http_target(
    ...
    if (unsafe { ((*conn).bits).httpproxy() }) as i32 != 0 && (unsafe { ((*conn).bits).tunnel_proxy() }) == 0 {
        ...
        let mut h: *mut crate::src::lib::urlapi::Curl_URL = curl_url_dup(unsafe { (*data).state.uh });
        if h.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        if (unsafe { (*conn).host.dispname }) != (unsafe { (*conn).host.name }) as *const i8 {
            uc = curl_url_set(h, CURLUPART_HOST, unsafe { (*conn).host.name }, 0 as i32 as u32);
            if uc as u64 != 0 {
                curl_url_cleanup(h);
                return CURLE_OUT_OF_MEMORY;
            }
        }
        uc = curl_url_set(h, CURLUPART_FRAGMENT, 0 as *const i8, 0 as i32 as u32);
        if uc as u64 != 0 {
            curl_url_cleanup(h);
            return CURLE_OUT_OF_MEMORY;
        }
        if Curl_strcasecompare(b"http\0" as *const u8 as *const i8, unsafe { (*data).state.up.scheme }) != 0 {
            uc = curl_url_set(h, CURLUPART_USER, 0 as *const i8, 0 as i32 as u32);
            if uc as u64 != 0 {
                curl_url_cleanup(h);
                return CURLE_OUT_OF_MEMORY;
            }
            uc = curl_url_set(h, CURLUPART_PASSWORD, 0 as *const i8, 0 as i32 as u32);
            if uc as u64 != 0 {
                curl_url_cleanup(h);
                return CURLE_OUT_OF_MEMORY;
            }
        ...
        result = Curl_dyn_add(
            r,
            if !(unsafe { (*data).set.str_0[STRING_TARGET as i32 as usize] }).is_null() {
                unsafe { (*data).set.str_0[STRING_TARGET as i32 as usize] }
            } else {
                url
            },
        );
        ...
        if Curl_strcasecompare(b"ftp\0" as *const u8 as *const i8, unsafe { (*data).state.up.scheme }) != 0 {
            if (unsafe { ((*data).set).proxy_transfer_mode() }) != 0 {
                ...
                {
                    match Curl_raw_toupper(unsafe { *type_0.offset(6 as i32 as isize) }) as i32 {
                        65 | 68 | 73 => {}
                        _ => {
                            type_0 = 0 as *mut i8;
                        }
...
#[no_mangle]
pub extern "C" fn Curl_http_body<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    mut httpreq: u32,
    ...
    match httpreq as u32 {
        3 => {
            let mut fresh47 = unsafe { &mut ((*http).sendit) };
        ...
        2 => {
            ...
            result = Curl_getformdata(
                data,
                unsafe { &mut (*http).form },
                unsafe { (*data).set.httppost },
                unsafe { (*data).state.fread_func },
            );
            if result as u64 != 0 {
                return result;
            }
            let mut fresh48 = unsafe { &mut ((*http).sendit) };
            *fresh48 = unsafe { &mut (*http).form };
        }
        _ => {
            let mut fresh49 = unsafe { &mut ((*http).sendit) };
            *fresh49 = 0 as *mut curl_mimepart;
        }
    }
    if !(unsafe { (*http).sendit }).is_null() {
        let mut cthdr: *const i8 =
            Curl_checkheaders(data, b"Content-Type\0" as *const u8 as *const i8);
        ...
        curl_mime_headers(unsafe { (*http).sendit }, unsafe { (*data).set.headers }, 0 as i32);
        result =
            Curl_mime_prepare_headers(unsafe { (*http).sendit }, cthdr, 0 as *const i8, MIMESTRATEGY_FORM);
        curl_mime_headers(unsafe { (*http).sendit }, 0 as *mut curl_slist, 0 as i32);
        if result as u64 == 0 {
            result = Curl_mime_rewind(unsafe { (*http).sendit });
        }
        if result as u64 != 0 {
            return result;
        }
    ...
    ptr = Curl_checkheaders(data, b"Transfer-Encoding\0" as *const u8 as *const i8);
    if !ptr.is_null() {
        let mut fresh50 = unsafe { &mut ((*data).req) };
                    ...
                    && (unsafe { (*data).state.infilesize }) == -(1 as i32) as i64)
        {
            if !((unsafe { ((*conn).bits).authneg() }) != 0) {
                if Curl_use_http_1_1plus(data, conn) {
                    if ((unsafe { (*conn).httpversion }) as i32) < 20 as i32 {
                        let mut fresh51 = unsafe { &mut ((*data).req) };
    ...
    match httpreq as u32 {
        4 => {
            ...
            if (unsafe { (*http).postsize }) != -(1 as i32) as i64
                ...
                && ((unsafe { ((*conn).bits).authneg() }) as i32 != 0
                    || (Curl_checkheaders(data, b"Content-Length\0" as *const u8 as *const i8))
                        .is_null())
            {
                result = unsafe { Curl_dyn_addf(
                    r,
            ...
            Curl_pgrsSetUploadSize(data, unsafe { (*http).postsize });
            result = Curl_buffer_send(
                r,
                data,
                Some(unsafe { &mut (*data).info.request_size }),
        ...
        2 | 3 => {
            if (unsafe { ((*conn).bits).authneg() }) != 0 {
                result = Curl_dyn_add(r, b"Content-Length: 0\r\n\r\n\0" as *const u8 as *const i8);
                if result as u64 != 0 {
                    return result;
                }
                result = Curl_buffer_send(
                ...
                ptr = Curl_checkheaders(data, b"Expect\0" as *const u8 as *const i8);
                if !ptr.is_null() {
                    ...
                    || (unsafe { (*http).postsize }) < 0 as i32 as i64
                {
                    ...
                    let mut fresh54 = unsafe { &mut ((*data).state) };
                ...
                Curl_pgrsSetUploadSize(data, unsafe { (*http).postsize });
                let mut fresh55 = unsafe { &mut ((*data).state.fread_func) };
                *fresh55 = unsafe { core::intrinsics::transmute::<
                    Option<
                        unsafe extern "C" fn(
                            _: *mut i8,
                            _: u64,
                    ...
                    >,
                >(Some(Curl_mime_read)) };
                let mut fresh56 = unsafe { &mut ((*data).state.in_0) };
        ...
        1 => {
            ...
            if (Curl_checkheaders(data, b"Content-Type\0" as *const u8 as *const i8)).is_null() {
                result = Curl_dyn_add(
                    r,
            ...
            ptr = Curl_checkheaders(data, b"Expect\0" as *const u8 as *const i8);
            if !ptr.is_null() {
                let mut fresh57 = unsafe { &mut ((*data).state) };
                ...
                || (unsafe { (*http).postsize }) < 0 as i32 as i64
            {
                ...
                let mut fresh58 = unsafe { &mut ((*data).state) };
            ...
            if !(unsafe { (*data).set.postfields }).is_null() {
                    ...
                    && (unsafe { (*http).postsize }) < (64 as i32 * 1024 as i32) as i64
                {
                    ...
                    if (unsafe { ((*data).req).upload_chunky() }) == 0 {
                        result =
                            Curl_dyn_addn(r, unsafe { (*data).set.postfields }, (unsafe { (*http).postsize }) as size_t);
                        ...
                        if (unsafe { (*http).postsize }) != 0 {
                            ...
                            if result as u64 == 0 {
                                ...
                                result = Curl_dyn_addn(
                                    ...
                                    (unsafe { (*http).postsize }) as size_t,
                                );
                                if result as u64 == 0 {
                                    result = Curl_dyn_add(r, b"\r\n\0" as *const u8 as *const i8);
                                }
                    ...
                    let mut fresh59 = unsafe { &mut ((*http).postdata) };
                    *fresh59 = (unsafe { (*data).set.postfields }) as *const i8;
                    (unsafe { (*http).sending = HTTPSEND_BODY });
                    let mut fresh60 = unsafe { &mut ((*data).state.fread_func) };
                    *fresh60 = unsafe { core::intrinsics::transmute::<
                        Option<
                            unsafe extern "C" fn(
                                _: *mut i8,
                                _: u64,
                        ...
                        >,
                    >(Some(readmoredata)) };
                    let mut fresh61 = unsafe { &mut ((*data).state.in_0) };
                ...
                if (unsafe { ((*data).req).upload_chunky() }) as i32 != 0 && (unsafe { ((*conn).bits).authneg() }) as i32 != 0
                {
                    ...
                    Curl_pgrsSetUploadSize(
                        data,
                        if (unsafe { (*http).postsize }) != 0 {
                            unsafe { (*http).postsize }
                        } else {
                            -(1 as i32) as i64
                        },
                    );
                    if (unsafe { ((*conn).bits).authneg() }) == 0 {
                        let mut fresh62 = unsafe { &mut ((*http).postdata) };
            ...
            if result as u64 != 0 {
                ...
                Curl_setup_transfer(
                    ...
                    -(1 as i32) as curl_off_t,
                    1 as i32 != 0,
                    if !(unsafe { (*http).postdata }).is_null() {
                        0 as i32
                    } else {
                        -(1 as i32)
                    },
...
pub extern "C" fn Curl_http_cookies(
    ...
    if !(unsafe { (*data).set.str_0[STRING_COOKIE as i32 as usize] }).is_null()
        && (Curl_checkheaders(data, b"Cookie\0" as *const u8 as *const i8)).is_null()
    {
        ...
        if !(unsafe { (*data).cookies }).is_null() && (unsafe { ((*data).state).cookie_engine() }) as i32 != 0 {
            ...
            let secure_context: bool =
                if (unsafe { (*(*conn).handler).protocol }) & ((1 as i32) << 1 as i32) as u32 != 0
                    || Curl_strcasecompare(b"localhost\0" as *const u8 as *const i8, host) != 0
            ...
            co = Curl_cookie_getlist(unsafe { (*data).cookies }, host, unsafe { (*data).state.up.path }, secure_context);
        ...
        if !co.is_null() {
            ...
            while !co.is_null() {
                if !(unsafe { (*co).value }).is_null() {
                    if 0 as i32 == count {
                        result = Curl_dyn_add(r, b"Cookie: \0" as *const u8 as *const i8);
                        if result as u64 != 0 {
                            break;
                        }
                    }
...
pub extern "C" fn Curl_http_range(
    ...
    if (unsafe { ((*data).state).use_range() }) != 0 {
        if (httpreq as u32 == HTTPREQ_GET as i32 as u32
            || httpreq as u32 == HTTPREQ_HEAD as i32 as u32)
            && (Curl_checkheaders(data, b"Range\0" as *const u8 as *const i8)).is_null()
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")(
                (*data).state.aptr.rangeline as *mut libc::c_void,
            ) });
            let mut fresh63 = unsafe { &mut ((*data).state.aptr.rangeline) };
            *fresh63 = unsafe { curl_maprintf(
            ...
            && (Curl_checkheaders(data, b"Content-Range\0" as *const u8 as *const i8)).is_null()
        {
            (unsafe { Curl_cfree.expect("non-null function pointer")(
                (*data).state.aptr.rangeline as *mut libc::c_void,
            ) });
            if (unsafe { (*data).set.set_resume_from }) < 0 as i32 as i64 {
                let mut fresh64 = unsafe { &mut ((*data).state.aptr.rangeline) };
                *fresh64 = unsafe { curl_maprintf(
                ...
                let mut total_expected_size: i64 =
                    (unsafe { (*data).state.resume_from }) + (unsafe { (*data).state.infilesize });
                let mut fresh65 = unsafe { &mut ((*data).state.aptr.rangeline) };
                *fresh65 = unsafe { curl_maprintf(
                ...
                let mut fresh66 = unsafe { &mut ((*data).state.aptr.rangeline) };
        ...
        && (unsafe { (*data).state.resume_from }) != 0
    {
        ...
        if (unsafe { (*data).state.resume_from }) != 0 && (unsafe { ((*data).state).this_is_a_follow() }) == 0 {
            ...
            if (unsafe { (*data).state.infilesize }) > 0 as i32 as i64 {
                let mut fresh67 = unsafe { &mut ((*data).state.infilesize) };
...
#[no_mangle]
pub extern "C" fn Curl_http_firstwrite<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    ...
    if (unsafe { ((*data).req).ignore_cl() }) != 0 {
        ...
        *fresh68 = -(1 as i32) as curl_off_t;
        ...
        if (unsafe { (*data).set.max_filesize }) != 0 && (*(borrow(&k)).unwrap()).size > (unsafe { (*data).set.max_filesize })
        {
            (unsafe { Curl_failf(
                data,
    ...
    if (unsafe { (*data).state.resume_from }) != 0
        && (*(borrow(&k)).unwrap()).content_range() == 0
        && (unsafe { (*data).state.httpreq }) as u32 == HTTPREQ_GET as i32 as u32
        && (*(borrow(&k)).unwrap()).ignorebody() == 0
    {
        if (*(borrow(&k)).unwrap()).size == (unsafe { (*data).state.resume_from }) {
            (unsafe { Curl_infof(
                data,
...
#[no_mangle]
pub extern "C" fn Curl_transferencode(mut data: *mut crate::src::lib::http2::Curl_easy) -> u32 {
    if (Curl_checkheaders(data, b"TE\0" as *const u8 as *const i8)).is_null()
        && (unsafe { ((*data).set).http_transfer_encoding() }) as i32 != 0
    {
        let mut cptr: *mut i8 = Curl_checkheaders(data, b"Connection\0" as *const u8 as *const i8);
        (unsafe { Curl_cfree.expect("non-null function pointer")((*data).state.aptr.te as *mut libc::c_void) });
        let mut fresh69 = unsafe { &mut ((*data).state.aptr.te) };
        *fresh69 = 0 as *mut i8;
        if !cptr.is_null() {
        ...
        let mut fresh70 = unsafe { &mut ((*data).state.aptr.te) };
...
pub extern "C" fn Curl_http(
        ...
        if ((unsafe { (*conn).httpversion }) as i32) < 20 as i32 {
            match unsafe { (*conn).negnpn } {
                3 => {
                    ...
                    result = Curl_http2_switched(data, 0 as *const i8, 0 as i32 as size_t);
                    if result as u64 != 0 {
                        return result;
                    }
                }
    ...
    let mut fresh71 = unsafe { &mut ((*data).state.aptr.ref_0) };
    *fresh71 = 0 as *mut i8;
    if !(unsafe { (*data).state.referer }).is_null()
        && (Curl_checkheaders(data, b"Referer\0" as *const u8 as *const i8)).is_null()
    {
        let mut fresh72 = unsafe { &mut ((*data).state.aptr.ref_0) };
    ...
    if (Curl_checkheaders(data, b"Accept-Encoding\0" as *const u8 as *const i8)).is_null()
        && !(unsafe { (*data).set.str_0[STRING_ENCODING as i32 as usize] }).is_null()
    {
        (unsafe { Curl_cfree.expect("non-null function pointer")(
            (*data).state.aptr.accept_encoding as *mut libc::c_void,
        ) });
        let mut fresh73 = unsafe { &mut ((*data).state.aptr.accept_encoding) };
        *fresh73 = 0 as *mut i8;
        let mut fresh74 = unsafe { &mut ((*data).state.aptr.accept_encoding) };
        *fresh74 = unsafe { curl_maprintf(
        ...
        let mut fresh75 = unsafe { &mut ((*data).state.aptr.accept_encoding) };
        *fresh75 = 0 as *mut i8;
    }
    ...
    p_accept = if !(Curl_checkheaders(data, b"Accept\0" as *const u8 as *const i8)).is_null() {
        0 as *const i8
    } else {
        b"Accept: */*\r\n\0" as *const u8 as *const i8
    };
    ...
    if (unsafe { ((*conn).bits).altused() }) as i32 != 0
        && (Curl_checkheaders(data, b"Alt-Used\0" as *const u8 as *const i8)).is_null()
    {
    ...
    let mut fresh76 = unsafe { &mut ((*data).state.aptr.userpwd) };
    *fresh76 = 0 as *mut i8;
    (unsafe { Curl_cfree.expect("non-null function pointer")(
        (*data).state.aptr.proxyuserpwd as *mut libc::c_void,
    ) });
    let mut fresh77 = unsafe { &mut ((*data).state.aptr.proxyuserpwd) };
    *fresh77 = 0 as *mut i8;
    ...
    if result as u64 == 0 {
        let mut fresh78 = unsafe { &mut ((*http).postdata) };
    ...
    {
        let mut fresh79 = unsafe { &mut ((*data).req) };
    ...
    if (unsafe { (*data).req.writebytecount }) != 0 {
        ...
        if (unsafe { (*http).postsize }) == 0 {
            ...
            let mut fresh80 = unsafe { &mut ((*data).req) };
    ...
    if (unsafe { (*conn).httpversion }) as i32 == 20 as i32 && (unsafe { ((*data).req).upload_chunky() }) as i32 != 0 {
        let mut fresh81 = unsafe { &mut ((*data).req) };
        (*fresh81).set_upload_chunky(0 as i32 as bit);
    }
    return result;
}
extern "C" fn checkprefixmax(mut prefix: *const i8, mut buffer: *const i8, mut len: u64) -> bool {
    let mut ch: u64 = if (unsafe { strlen(prefix) }) < len {
...
extern "C" fn checkhttpprefix(
    ...
    let mut head: *mut crate::src::lib::http2::curl_slist = unsafe { (*data).set.http200aliases };
...
pub extern "C" fn Curl_http_header(
    ...
    let mut result: u32 = CURLE_OK;
    let mut k: Option<&'_ mut crate::src::lib::http2::SingleRequest> = Some(unsafe { &mut (*data).req });
    if (*(borrow(&k)).unwrap()).http_bodyless() == 0
        ...
        ) != 0
    {
        ...
        let mut offt: u32 = curlx_strtoofft(
            unsafe { headp.offset(strlen(b"Content-Length:\0" as *const u8 as *const i8) as isize) },
    ...
    ) != 0
    {
        ...
        if (unsafe { *contenttype }) == 0 {
            ...
            let mut fresh82 = unsafe { &mut ((*data).info.contenttype) };
            *fresh82 = 0 as *mut i8;
            let mut fresh83 = unsafe { &mut ((*data).info.contenttype) };
            *fresh83 = contenttype;
        }
            ...
            != 0
    {
        ...
        && curl_strnequal(
            b"Transfer-Encoding:\0" as *const u8 as *const i8,
            headp,
            unsafe { strlen(b"Transfer-Encoding:\0" as *const u8 as *const i8) },
        ) != 0
    {
        result = Curl_build_unencoding_stack(
            data,
            unsafe { headp.offset(strlen(b"Transfer-Encoding:\0" as *const u8 as *const i8) as isize) },
            1 as i32,
        );
        if result as u64 != 0 {
            return result;
        ...
        && curl_strnequal(
            ...
            headp,
            unsafe { strlen(b"Content-Encoding:\0" as *const u8 as *const i8) },
        ) != 0
        && !(unsafe { (*data).set.str_0[STRING_ENCODING as i32 as usize] }).is_null()
    {
        result = Curl_build_unencoding_stack(
            data,
            unsafe { headp.offset(strlen(b"Content-Encoding:\0" as *const u8 as *const i8) as isize) },
            0 as i32,
        );
        if result as u64 != 0 {
            return result;
        ...
        headp,
        unsafe { strlen(b"Retry-After:\0" as *const u8 as *const i8) },
    ) != 0
    {
        let mut retry_after: i64 = 0 as i32 as curl_off_t;
        let mut date: i64 = Curl_getdate_capped(
            unsafe { headp.offset(strlen(b"Retry-After:\0" as *const u8 as *const i8) as isize) },
        );
        ...
        && curl_strnequal(
            ...
            headp,
            unsafe { strlen(b"Content-Range:\0" as *const u8 as *const i8) },
        ) != 0
    {
        let mut ptr: *mut i8 =
            unsafe { headp.offset(strlen(b"Content-Range:\0" as *const u8 as *const i8) as isize) };
        while (unsafe { *ptr }) as i32 != 0 && Curl_isdigit((unsafe { *ptr }) as u8 as i32) == 0 && (unsafe { *ptr }) as i32 != '*' as i32
        {
            ptr = unsafe { ptr.offset(1) };
        }
        if Curl_isdigit((unsafe { *ptr }) as u8 as i32) != 0 {
            if curlx_strtoofft(
                ptr,
                Option::<&'_ mut *mut i8>::None,
                10 as i32,
                ...
                == 0
            {
                if (unsafe { (*data).state.resume_from }) == (*(borrow(&k)).unwrap()).offset {
        ...
        ) != 0
    {
        ...
        Curl_cookie_add(
            ...
            1 as i32 != 0,
            0 as i32 != 0,
            unsafe { headp.offset(strlen(b"Set-Cookie:\0" as *const u8 as *const i8) as isize) },
            host,
            unsafe { (*data).state.up.path },
            secure_context,
        );
        ...
        && curl_strnequal(
            ...
            headp,
            unsafe { strlen(b"Last-Modified:\0" as *const u8 as *const i8) },
        ) != 0
        && ((unsafe { (*data).set.timecondition }) as u32 != 0 || (unsafe { ((*data).set).get_filetime() }) as i32 != 0)
    {
        (*(borrow_mut(&mut k)).unwrap()).timeofdoc = Curl_getdate_capped(
            unsafe { headp.offset(strlen(b"Last-Modified:\0" as *const u8 as *const i8) as isize) },
        );
        if (unsafe { ((*data).set).get_filetime() }) != 0 {
            (unsafe { (*data).info.filetime = (*(borrow_mut(&mut k)).unwrap()).timeofdoc });
        }
    } else if curl_strnequal(
        b"WWW-Authenticate:\0" as *const u8 as *const i8,
        headp,
        unsafe { strlen(b"WWW-Authenticate:\0" as *const u8 as *const i8) },
    ) != 0
        && 401 as i32 == (*(borrow(&k)).unwrap()).httpcode
        || curl_strnequal(
            b"Proxy-authenticate:\0" as *const u8 as *const i8,
            headp,
            unsafe { strlen(b"Proxy-authenticate:\0" as *const u8 as *const i8) },
        ) != 0
            && 407 as i32 == (*(borrow(&k)).unwrap()).httpcode
    {
    ...
    } else if (*(borrow(&k)).unwrap()).httpcode >= 300 as i32
        && (*(borrow(&k)).unwrap()).httpcode < 400 as i32
        && curl_strnequal(
            b"Location:\0" as *const u8 as *const i8,
            headp,
            unsafe { strlen(b"Location:\0" as *const u8 as *const i8) },
        ) != 0
        && (unsafe { (*data).req.location }).is_null()
    {
        ...
        if (unsafe { *location }) == 0 {
            ...
            let mut fresh84 = unsafe { &mut ((*data).req.location) };
            *fresh84 = location;
            if (unsafe { ((*data).set).http_follow_location() }) != 0 {
                let mut fresh85 = unsafe { &mut ((*data).req.newurl) };
        ...
        && curl_strnequal(
            ...
            headp,
            unsafe { strlen(b"Strict-Transport-Security:\0" as *const u8 as *const i8) },
        ) != 0
        && (unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 0 as i32) as u32 != 0
    {
        let mut check: u32 = Curl_hsts_parse(
            unsafe { (*data).hsts },
            unsafe { (*data).state.up.hostname },
            unsafe { headp
        ...
        && curl_strnequal(
            ...
            headp,
            unsafe { strlen(b"Alt-Svc:\0" as *const u8 as *const i8) },
        ) != 0
        && ((unsafe { (*(*conn).handler).flags }) & ((1 as i32) << 0 as i32) as u32 != 0 || 0 as i32 != 0)
    {
        let mut id: u32 = (if (unsafe { (*conn).httpversion }) as i32 == 20 as i32 {
            ALPN_h2 as i32
        } else {
            ALPN_h1 as i32
        }) as alpnid;
        result = Curl_altsvc_parse(
            data,
            unsafe { (*data).asi },
            unsafe { headp.offset(strlen(b"Alt-Svc:\0" as *const u8 as *const i8) as isize) },
            id,
            unsafe { (*conn).host.name },
            curlx_uitous((unsafe { (*conn).remote_port }) as u32),
        );
        if result as u64 != 0 {
            return result;
        }
...
pub extern "C" fn Curl_http_statusline(
    ...
    let mut k: *mut crate::src::lib::http2::SingleRequest = unsafe { &mut (*data).req };
    ...
    match unsafe { (*k).httpcode } {
        304 => {
            if (unsafe { (*data).set.timecondition }) as u64 != 0 {
                let mut fresh86 = unsafe { &mut ((*data).info) };
...
#[no_mangle]
pub extern "C" fn Curl_http_readwrite_headers<'a1, 'a2>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut conn: *mut crate::src::lib::http2::connectdata,
    ...
    loop {
        ...
        if end_ptr.is_null() {
            ...
            rest_length = ((unsafe { end_ptr.offset_from((*(borrow(&k)).unwrap()).str_0) }) as i64
            ...
            *fresh87 = unsafe { end_ptr.offset(1 as i32 as isize) };
            full_length = (unsafe { ((*(borrow(&k)).unwrap()).str_0).offset_from(str_start) }) as i64 as size_t;
            result = Curl_dyn_addn(
                unsafe { &mut (*data).state.headerb },
                str_start as *const libc::c_void,
                full_length,
            ...
            headp = Curl_dyn_ptr(unsafe { &mut (*data).state.headerb });
            if 0xa as i32 == (unsafe { *headp }) as i32 || 0xd as i32 == (unsafe { *headp }) as i32 {
                            ...
                            && (unsafe { (*conn).proxy_ntlm_state }) as u32 == NTLMSTATE_TYPE2 as i32 as u32)
                {
                    ...
                    let mut fresh89 = unsafe { &mut ((*data).state) };
                ...
                headerlen = Curl_dyn_len(unsafe { &mut (*data).state.headerb });
                ...
                let mut fresh90 = unsafe { &mut ((*data).info.header_size) };
                *fresh90 += headerlen as i64;
                let mut fresh91 = unsafe { &mut ((*data).req.headerbytecount) };
                *fresh91 += headerlen as i64;
                ...
                (unsafe { (*data).req.deductheadercount = if 100 as i32 <= (*(borrow(&k)).unwrap()).httpcode
                    && 199 as i32 >= (*(borrow(&k)).unwrap()).httpcode
                {
                            ...
                            4 | 1 | 2 | 3 => {
                                ...
                                if (*(borrow(&k)).unwrap()).upload_done() == 0 {
                                        ...
                                        && (unsafe { ((*data).state).expect100header() }) as i32 != 0
                                    {
                                        ...
                                        let mut fresh93 = unsafe { &mut ((*data).req.newurl) };
                                        *fresh93 = unsafe { Curl_cstrdup.expect("non-null function pointer")(
                                        ...
                                        if (*(borrow(&k)).unwrap()).exp100 as u32
                                            > EXP100_SEND_DATA as i32 as u32
                                        {
                                            (*(borrow_mut(&mut k)).unwrap()).exp100 =
                                                EXP100_SEND_DATA;
                                            (*(borrow_mut(&mut k)).unwrap()).keepon |=
                                                (1 as i32) << 1 as i32;
                                        }
                                    } else {
                                        (unsafe { Curl_infof(
                                        ...
                                        (*(borrow_mut(&mut k)).unwrap())
                                            .set_upload_done(1 as i32 as bit);
                ...
                if (*(borrow(&k)).unwrap()).header() == 0 {
                    ...
                    if -(1 as i32) as i64 != (*(borrow(&k)).unwrap()).size {
                        Curl_pgrsSetDownloadSize(data, (*(borrow_mut(&mut k)).unwrap()).size);
                        (*(borrow_mut(&mut k)).unwrap()).maxdownload =
                            (*(borrow_mut(&mut k)).unwrap()).size;
                    }
                ...
                *fresh95 = *fresh95 + 1;
                ...
                let mut fresh97 = unsafe { &mut ((*data).info.header_size) };
                *fresh97 = (*fresh97 as u64).wrapping_add(Curl_dyn_len(unsafe { &mut (*data).state.headerb }))
                    as curl_off_t as curl_off_t;
                let mut fresh98 = unsafe { &mut ((*data).req.headerbytecount) };
                *fresh98 = (*fresh98 as u64).wrapping_add(Curl_dyn_len(unsafe { &mut (*data).state.headerb }))
                    as curl_off_t as curl_off_t;
                Curl_dyn_reset(Some(unsafe { &mut (*data).state.headerb }));
            }
            if !((unsafe { *(*(borrow(&k)).unwrap()).str_0 }) != 0) {
                break;
            }
        }
    }
