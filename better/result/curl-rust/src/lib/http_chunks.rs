...
#[no_mangle]
pub extern "C" fn Curl_httpchunk_init(mut data: *mut crate::src::lib::http2::Curl_easy) {
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    let mut chunk: Option<&'_ mut crate::src::lib::http2::Curl_chunker> = Some(unsafe { &mut (*conn).chunk });
    (*(borrow_mut(&mut chunk)).unwrap()).hexindex = 0 as i32 as u8;
    (*(borrow_mut(&mut chunk)).unwrap()).state = CHUNK_HEX;
    Curl_dyn_init(unsafe { &mut (*conn).trailer }, 4096 as i32 as size_t);
}
#[no_mangle]
pub extern "C" fn Curl_httpchunk_read<'a1>(
    ...
    let mut conn: *mut crate::src::lib::http2::connectdata = unsafe { (*data).conn };
    let mut ch: *mut crate::src::lib::http2::Curl_chunker = unsafe { &mut (*conn).chunk };
    ...
    (unsafe { *wrote = 0 as i32 as size_t });
    if (unsafe { ((*data).set).http_te_skip() }) as i32 != 0 && (*(borrow(&k)).unwrap()).ignorebody() == 0 {
    ...
    while length != 0 {
        ...
        match (unsafe { (*ch).state }) as u32 {
            0 => {
                if Curl_isxdigit((unsafe { *datap }) as i32) != 0 {
                    if ((unsafe { (*ch).hexindex }) as i32) < 8 as i32 * 2 as i32 {
                        (unsafe { (*ch).hexbuffer[(*ch).hexindex as usize] = *datap });
                        datap = unsafe { datap.offset(1) };
                        length -= 1;
                        let mut fresh0 = unsafe { &mut ((*ch).hexindex) };
                    ...
                    if 0 as i32 == (unsafe { (*ch).hexindex }) as i32 {
                        return CHUNKE_ILLEGAL_HEX;
                    }
                    (unsafe { (*ch).hexbuffer[(*ch).hexindex as usize] = 0 as i32 as i8 });
                    result = CURLE_OK as i32 as CURLcode;
                    if result as u64 != 0 {
                        return CHUNKE_ILLEGAL_HEX;
                    }
                    if curlx_strtoofft(
                        unsafe { ((*ch).hexbuffer).as_mut_ptr() },
                        Some(&mut endptr),
                        16 as i32,
                        Some(unsafe { &mut (*ch).datasize }),
                    ) as u64
                        != 0
                    {
                        return CHUNKE_ILLEGAL_HEX;
                    }
                    (unsafe { (*ch).state = CHUNK_LF });
                }
            }
            1 => {
                if (unsafe { *datap }) as i32 == 0xa as i32 {
                    if 0 as i32 as i64 == (unsafe { (*ch).datasize }) {
                        (unsafe { (*ch).state = CHUNK_TRAILER });
                    } else {
                        (unsafe { (*ch).state = CHUNK_DATA });
                    }
                }
                datap = unsafe { datap.offset(1) };
                length -= 1;
            ...
            2 => {
                piece = curlx_sotouz(if (unsafe { (*ch).datasize }) >= length {
                    length
                } else {
                    unsafe { (*ch).datasize }
                });
                if (unsafe { ((*data).set).http_te_skip() }) == 0 && (*(borrow(&k)).unwrap()).ignorebody() == 0 {
                    if (unsafe { ((*data).set).http_ce_skip() }) == 0
                        && !((*(borrow(&k)).unwrap()).writer_stack).is_null()
                    {
                        ...
                        result = Curl_client_write(data, (1 as i32) << 0 as i32, datap, piece);
                    }
                    if result as u64 != 0 {
                        *(borrow_mut(&mut extrap)).unwrap() = result;
                        return CHUNKE_PASSTHRU_ERROR;
                ...
                (unsafe { *wrote = (*wrote as u64).wrapping_add(piece) as size_t as size_t });
                let mut fresh1 = unsafe { &mut ((*ch).datasize) };
                *fresh1 = (*fresh1 as u64).wrapping_sub(piece) as curl_off_t as curl_off_t;
                datap = unsafe { datap.offset(piece as isize) };
                length = (length as u64).wrapping_sub(piece) as curl_off_t as curl_off_t;
                if 0 as i32 as i64 == (unsafe { (*ch).datasize }) {
                    (unsafe { (*ch).state = CHUNK_POSTLF });
                }
            }
            3 => {
                if (unsafe { *datap }) as i32 == 0xa as i32 {
                    Curl_httpchunk_init(data);
                } else if (unsafe { *datap }) as i32 != 0xd as i32 {
            ...
            5 => {
                if (unsafe { *datap }) as i32 == 0xd as i32 || (unsafe { *datap }) as i32 == 0xa as i32 {
                    let mut tr: *mut i8 = Curl_dyn_ptr(unsafe { &mut (*conn).trailer });
                    if !tr.is_null() {
                        let mut trlen: u64 = 0;
                        result = Curl_dyn_add(
                            unsafe { &mut (*conn).trailer },
                            b"\r\n\0" as *const u8 as *const i8 as *mut i8,
                        );
                        if result as u64 != 0 {
                            return CHUNKE_OUT_OF_MEMORY;
                        }
                        tr = Curl_dyn_ptr(unsafe { &mut (*conn).trailer });
                        trlen = Curl_dyn_len(unsafe { &mut (*conn).trailer });
                        result = CURLE_OK as i32 as CURLcode;
                        if result as u64 != 0 {
                            return CHUNKE_BAD_CHUNK;
                        }
                        if (unsafe { ((*data).set).http_te_skip() }) == 0 {
                            result = Curl_client_write(data, (1 as i32) << 1 as i32, tr, trlen);
                            if result as u64 != 0 {
                                *(borrow_mut(&mut extrap)).unwrap() = result;
                                return CHUNKE_PASSTHRU_ERROR;
                            }
                        }
                        Curl_dyn_reset(Some(unsafe { &mut (*conn).trailer }));
                        (unsafe { (*ch).state = CHUNK_TRAILER_CR });
                        if (unsafe { *datap }) as i32 == 0xa as i32 {
                        ...
                        (unsafe { (*ch).state = CHUNK_TRAILER_POSTCR });
                        current_block_101 = 15586796709793571329;
                    }
                } else {
            ...
            7 => {
                if (unsafe { *datap }) as i32 != 0xd as i32 && (unsafe { *datap }) as i32 != 0xa as i32 {
                    ...
                    if (unsafe { *datap }) as i32 == 0xd as i32 {
                        datap = unsafe { datap.offset(1) };
                        length -= 1;
                    }
                    (unsafe { (*ch).state = CHUNK_STOP });
                }
            }
            4 => {
                if (unsafe { *datap }) as i32 == 0xa as i32 {
                    length -= 1;
                    (unsafe { (*ch).datasize = curlx_sotouz(length) as curl_off_t });
                    return CHUNKE_STOP;
                } else {
                    return CHUNKE_BAD_CHUNK;
                }
...
#[no_mangle]
pub extern "C" fn Curl_chunked_strerror(mut code: i32) -> *const i8 {
    match code as i32 {
        1 => return b"Too long hexadecimal number\0" as *const u8 as *const i8,
        2 => {
            return b"Illegal or missing hexadecimal sequence\0" as *const u8 as *const i8;
        }
