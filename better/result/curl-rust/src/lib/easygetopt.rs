...
extern "C" fn lookup(
    ...
    if !name.is_null() || id as u32 != 0 {
        let mut o: *mut crate::src::lib::easygetopt::curl_easyoption =
            (unsafe { &mut *Curl_easyopts.as_mut_ptr().offset(0 as i32 as isize) }) as *mut curl_easyoption;
        loop {
            if !name.is_null() {
                if Curl_strcasecompare(unsafe { (*o).name }, name) != 0 {
                    return o;
                }
