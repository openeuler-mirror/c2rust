...
extern "C" {
    ...
    fn waitpid(__pid: i32, __stat_loc: *mut i32, __options: i32) -> i32;
    fn getpwuid_r(
        __uid: u32,
...
extern "C" fn ntlm_wb_cleanup<'a1>(mut ntlm: Option<&'a1 mut crate::src::lib::http2::ntlmdata>) {
    if (*(borrow(&ntlm)).unwrap()).ntlm_auth_hlpr_socket != -(1 as i32) {
        (unsafe { close((*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_socket) });
    ...
    (unsafe { Curl_cfree.expect("non-null function pointer")(
        (*(borrow_mut(&mut ntlm)).unwrap()).challenge as *mut libc::c_void,
    ) });
...
extern "C" fn ntlm_wb_response<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut ntlm: Option<&'a1 mut crate::src::lib::http2::ntlmdata>,
    mut input: *const i8,
    mut state: u32,
    ...
    let mut len_in: u64 = unsafe { strlen(input) };
    ...
    loop {
        ...
        let mut written: i64 = unsafe { send(
        ...
        11812396948646013369 => {
            loop {
                ...
                if size == -(1 as i32) as i64 {
                    ...
                    if Curl_dyn_addn(&mut b, buf as *const libc::c_void, size as size_t) as u64 != 0
                    {
                        current_block = 119169402052537339;
                        break;
                    }
                        ...
                        && (unsafe { *ptr.offset(1 as i32 as isize) }) as i32 == 'W' as i32)
                    {
                        if !(len_out < 4 as i32 as u64) {
                                    ...
                                    || (unsafe { *ptr.offset(2 as i32 as isize) }) as i32 != ' ' as i32))
                            {
                                ...
                                {
                                    let mut fresh2 =
                                        &mut ((*(borrow_mut(&mut ntlm)).unwrap()).response);
...
pub extern "C" fn Curl_input_ntlm_wb(
    ...
    while (unsafe { *header }) as i32 != 0 && Curl_isspace((unsafe { *header }) as u8 as i32) != 0 {
        header = unsafe { header.offset(1) };
    }
    if (unsafe { *header }) != 0 {
        let mut fresh3 = unsafe { &mut ((*ntlm).challenge) };
...
pub extern "C" fn Curl_output_ntlm_wb(
    ...
    let mut ntlm: Option<&'_ mut crate::src::lib::http2::ntlmdata> =
        Option::<&'_ mut crate::src::lib::http2::ntlmdata>::None;
    ...
    if proxy {
        allocuserpwd = Some(unsafe { &mut (*data).state.aptr.proxyuserpwd });
        userp = unsafe { (*conn).http_proxy.user };
        ntlm = Some(unsafe { &mut (*conn).proxyntlm });
        state = Some(unsafe { &mut (*conn).proxy_ntlm_state });
        authp = Some(unsafe { &mut (*data).state.authproxy });
    } else {
        allocuserpwd = Some(unsafe { &mut (*data).state.aptr.userpwd });
        userp = unsafe { (*conn).user };
        ntlm = Some(unsafe { &mut (*conn).ntlm });
        state = Some(unsafe { &mut (*conn).http_ntlm_state });
    ...
    match *(borrow(&state)).unwrap() as u32 {
        2 => {
            let mut input: *mut i8 = unsafe { curl_maprintf(
            ...
            *(borrow_mut(&mut allocuserpwd)).unwrap() = unsafe { curl_maprintf(
...
#[no_mangle]
pub extern "C" fn Curl_http_auth_cleanup_ntlm_wb(
    mut conn: *mut crate::src::lib::http2::connectdata,
) {
    ntlm_wb_cleanup(Some(unsafe { &mut (*conn).ntlm }));
    ntlm_wb_cleanup(Some(unsafe { &mut (*conn).proxyntlm }));
}
