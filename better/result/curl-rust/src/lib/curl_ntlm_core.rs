...
pub struct HMAC_params {
    ...
    pub hmac_hupdate:
        Option<unsafe extern "C" fn(_: *mut core::ffi::c_void, _: *const u8, _: u32) -> ()>,
...
pub extern "C" fn Curl_ntlm_core_mk_lm_hash(
    ...
    let mut len: u64 = if (unsafe { strlen(password) }) < 14 as i32 as u64 {
...
extern "C" fn ascii_to_unicode_le(mut dest: *mut u8, mut src: *const i8, mut srclen: u64) {
    let mut i: u64 = 0;
...
pub extern "C" fn Curl_ntlm_core_mk_nt_hash(
    ...
    let mut len: u64 = unsafe { strlen(password) };
...
extern "C" fn time2filetime<'a1>(
    mut ft: Option<&'a1 mut crate::src::lib::curl_ntlm_core::ms_filetime>,
    mut t: i64,
) {
    t = (t + 11644473600 as i64) * 10000000 as i32 as i64;
    (*(borrow_mut(&mut ft)).unwrap()).dwLowDateTime = (t & 0xffffffff as u32 as i64) as u32;
...
#[no_mangle]
pub extern "C" fn Curl_ntlm_core_mk_ntlmv2_resp<'a1, 'a2, 'a3>(
    mut ntlmv2hash: *mut u8,
    ...
    let mut result: u32 = CURLE_OK;
    time2filetime(Some(&mut tw), unsafe { time(0 as *mut time_t) });
    ...
    (unsafe { curl_msnprintf(
        ...
        (*core::intrinsics::transmute::<&'_ [u8; 5], &'_ [i8; 5]>(b"\x01\x01\0\0\0"))
    ...
    (unsafe { memcpy(
        ...
        &mut *((*(borrow_mut(&mut ntlm)).unwrap()).nonce)
            .as_mut_ptr()
    ...
    result = Curl_hmacit(
        ...
        ntlmv2hash,
        16 as i32 as size_t,
        unsafe { ptr.offset(8 as i32 as isize) },
        ((44 as i32 - 16 as i32) as u32)
            .wrapping_add((*(borrow(&ntlm)).unwrap()).target_info_len)
            .wrapping_add(4 as i32 as u32)
...
pub extern "C" fn Curl_ntlm_core_mk_lmv2_resp(
    ...
    result = Curl_hmacit(
        ...
        ntlmv2hash,
        16 as i32 as size_t,
        unsafe { &mut *data.as_mut_ptr().offset(0 as i32 as isize) },
        16 as i32 as size_t,
        hmac_output.as_mut_ptr(),
    );
    if result as u64 != 0 {
