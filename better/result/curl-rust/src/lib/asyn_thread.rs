...
#[no_mangle]
pub extern "C" fn Curl_resolver_init<'a1>(
    mut _easy: *mut crate::src::lib::http2::Curl_easy,
    mut resolver: Option<&'a1 mut *mut core::ffi::c_void>,
) -> u32 {
    *(borrow_mut(&mut resolver)).unwrap() = unsafe { Curl_ccalloc.expect("non-null function pointer")(
        1 as i32 as size_t,
...
#[no_mangle]
pub extern "C" fn Curl_resolver_cleanup(mut resolver: *mut core::ffi::c_void) {
    (unsafe { Curl_cfree.expect("non-null function pointer")(resolver) });
}
#[no_mangle]
pub extern "C" fn Curl_resolver_duphandle<'a1>(
...
#[no_mangle]
pub extern "C" fn Curl_resolver_cancel(mut data: *mut crate::src::lib::http2::Curl_easy) {
    destroy_async_data(Some(unsafe { &mut (*data).state.async_0 }));
}
extern "C" fn conn_thread_sync_data<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
) -> Option<&'a1 mut crate::src::lib::asyn_thread::thread_sync_data> {
    return Some(unsafe { &mut (*(*data).state.async_0.tdata).tsd });
}
extern "C" fn destroy_thread_sync_data(
    mut tsd: *mut crate::src::lib::asyn_thread::thread_sync_data,
) {
    ...
    if !(unsafe { (*tsd).res }).is_null() {
        Curl_freeaddrinfo(unsafe { (*tsd).res });
    }
...
extern "C" fn init_thread_sync_data<'a1>(
    ...
    let mut tsd: *mut crate::src::lib::asyn_thread::thread_sync_data = unsafe { &mut (*td).tsd };
    (unsafe { memset(
        tsd as *mut libc::c_void,
        0 as i32,
        ::std::mem::size_of::<thread_sync_data>() as u64,
    ) });
    let mut fresh0 = unsafe { &mut ((*tsd).td) };
    *fresh0 = td;
    (unsafe { (*tsd).port = port });
    (unsafe { (*tsd).done = 1 as i32 });
    (unsafe { (*tsd).hints = *(hints).unwrap() });
    let mut fresh1 = unsafe { &mut ((*tsd).mtx) };
    *fresh1 = (unsafe { Curl_cmalloc.expect("non-null function pointer")(
    ...
    if !(unsafe { (*tsd).mtx }).is_null() {
        ...
        ) }) < 0 as i32
        {
            ...
            let mut fresh2 = unsafe { &mut ((*tsd).hostname) };
...
extern "C" fn getaddrinfo_complete(mut data: *mut crate::src::lib::http2::Curl_easy) -> i32 {
    let mut tsd: Option<&'_ mut crate::src::lib::asyn_thread::thread_sync_data> =
        conn_thread_sync_data(data);
    let mut rc: i32 = 0;
    rc = Curl_addrinfo_callback(
        data,
        (*(borrow_mut(&mut tsd)).unwrap()).sock_error,
        (*(borrow_mut(&mut tsd)).unwrap()).res,
    ) as i32;
    let mut fresh3 = &mut ((*(borrow_mut(&mut tsd)).unwrap()).res);
    *fresh3 = 0 as *mut Curl_addrinfo;
    return rc;
}
extern "C" fn getaddrinfo_thread(mut arg: *mut core::ffi::c_void) -> u32 {
    let mut tsd: *mut crate::src::lib::asyn_thread::thread_sync_data = arg as *mut thread_sync_data;
    let mut td: *mut crate::src::lib::asyn_thread::thread_data = unsafe { (*tsd).td };
    ...
    rc = Curl_getaddrinfo_ex(
        unsafe { (*tsd).hostname },
        service.as_mut_ptr(),
        unsafe { &mut (*tsd).hints },
        Some(unsafe { &mut (*tsd).res }),
    );
    if rc != 0 {
...
extern "C" fn destroy_async_data<'a1>(
    mut async_0: Option<&'a1 mut crate::src::lib::http2::Curl_async>,
) {
    if !((*(borrow_mut(&mut async_0)).unwrap()).tdata).is_null() {
        let mut td: *mut crate::src::lib::asyn_thread::thread_data =
            (*(borrow_mut(&mut async_0)).unwrap()).tdata;
        let mut done: i32 = 0;
        let mut sock_rd: i32 = unsafe { (*td).tsd.sock_pair[0 as i32 as usize] };
        let mut data: *mut crate::src::lib::http2::Curl_easy = unsafe { (*td).tsd.data };
        ...
        if done == 0 {
            Curl_thread_destroy(unsafe { (*td).thread_hnd });
        } else {
            if !(unsafe { (*td).thread_hnd }).is_null() {
                Curl_thread_join(Some(unsafe { &mut (*td).thread_hnd }));
            }
    ...
    *fresh4 = 0 as *mut thread_data;
    (unsafe { Curl_cfree.expect("non-null function pointer")(
        (*(borrow_mut(&mut async_0)).unwrap()).hostname as *mut libc::c_void,
    ) });
    let mut fresh5 = &mut ((*(borrow_mut(&mut async_0)).unwrap()).hostname);
    *fresh5 = 0 as *mut i8;
}
extern "C" fn init_resolve_thread<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut hostname: *const i8,
    mut port: i32,
    mut hints: Option<&'a1 crate::src::lib::asyn_thread::addrinfo>,
) -> bool {
    let mut td: *mut crate::src::lib::asyn_thread::thread_data = (unsafe { Curl_ccalloc
        .expect("non-null function pointer")(
        1 as i32 as size_t,
        ::std::mem::size_of::<thread_data>() as u64,
    ) }) as *mut thread_data;
    let mut err: i32 = 12 as i32;
    let mut asp: Option<&'_ mut crate::src::lib::http2::Curl_async> =
        Some(unsafe { &mut (*data).state.async_0 });
    let mut fresh6 = unsafe { &mut ((*data).state.async_0.tdata) };
    *fresh6 = td;
    if !td.is_null() {
        ...
        *fresh7 = 0 as *mut Curl_dns_entry;
        let mut fresh8 = unsafe { &mut ((*td).thread_hnd) };
        ...
        if init_thread_sync_data(td, hostname, port, (hints).clone()) == 0 {
            ...
            *fresh10 = unsafe { Curl_cstrdup.expect("non-null function pointer")(hostname) };
            if !((*(borrow_mut(&mut asp)).unwrap()).hostname).is_null() {
                ...
                let mut fresh11 = unsafe { &mut ((*td).thread_hnd) };
                *fresh11 = Curl_thread_create(
                    Some(getaddrinfo_thread),
                    (unsafe { &mut (*td).tsd }) as *mut thread_sync_data as *mut libc::c_void,
                );
                if (unsafe { (*td).thread_hnd }).is_null() {
                    (unsafe { (*td).tsd.done = 1 as i32 });
...
extern "C" fn thread_wait_resolv<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    ...
    if Curl_thread_join(Some(unsafe { &mut (*td).thread_hnd })) != 0 {
        if !borrow(&entry).is_none() {
            result = getaddrinfo_complete(data) as CURLcode;
        }
    }
    let mut fresh12 = unsafe { &mut ((*data).state.async_0) };
    (*fresh12).set_done(1 as i32 as bit);
    if !borrow(&entry).is_none() {
        *(borrow_mut(&mut entry)).unwrap() = unsafe { (*data).state.async_0.dns };
    }
...
#[no_mangle]
pub extern "C" fn Curl_resolver_kill(mut data: *mut crate::src::lib::http2::Curl_easy) {
    let mut td: *mut crate::src::lib::asyn_thread::thread_data = unsafe { (*data).state.async_0.tdata };
    if !td.is_null() && !(unsafe { (*td).thread_hnd }).is_null() {
        thread_wait_resolv(
            data,
            Option::<&'_ mut *mut crate::src::lib::http2::Curl_dns_entry>::None,
            0 as i32 != 0,
        );
    } else {
...
#[no_mangle]
pub extern "C" fn Curl_resolver_wait_resolv<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut entry: Option<&'a1 mut *mut crate::src::lib::http2::Curl_dns_entry>,
) -> u32 {
...
#[no_mangle]
pub extern "C" fn Curl_resolver_is_resolved<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut entry: Option<&'a1 mut *mut crate::src::lib::http2::Curl_dns_entry>,
) -> u32 {
    let mut td: *mut crate::src::lib::asyn_thread::thread_data = unsafe { (*data).state.async_0.tdata };
    ...
    *(borrow_mut(&mut entry)).unwrap() = 0 as *mut Curl_dns_entry;
    ...
    if done != 0 {
        ...
        *(borrow_mut(&mut entry)).unwrap() = unsafe { (*data).state.async_0.dns };
        ...
        let mut elapsed: i64 = Curl_timediff(Curl_now(), unsafe { (*data).progress.t_startsingle });
        ...
        if (unsafe { (*td).poll_interval }) == 0 as i32 as u32 {
            ...
            let mut fresh13 = unsafe { &mut ((*td).poll_interval) };
        ...
        Curl_expire(data, (unsafe { (*td).poll_interval }) as timediff_t, EXPIRE_ASYNC_NAME);
    }
    return CURLE_OK;
}
#[no_mangle]
pub extern "C" fn Curl_resolver_getsock(
    ...
    let mut milli: i64 = 0;
    let mut ms: i64 = 0;
    let mut reslv: *mut crate::src::lib::asyn_thread::resdata =
        (unsafe { (*data).state.async_0.resolver }) as *mut resdata;
    let mut td: *mut crate::src::lib::asyn_thread::thread_data = unsafe { (*data).state.async_0.tdata };
    if !td.is_null() {
        (unsafe { *socks.offset(0 as i32 as isize) = (*td).tsd.sock_pair[0 as i32 as usize] });
        let mut fresh14 = unsafe { &mut ((*td).tsd.data) };
        *fresh14 = data;
        ret_val = (1 as i32) << 0 as i32;
    } else {
        ms = Curl_timediff(Curl_now(), unsafe { (*reslv).start });
...
#[no_mangle]
pub extern "C" fn Curl_resolver_getaddrinfo<'a1>(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut hostname: *const i8,
    mut port: i32,
