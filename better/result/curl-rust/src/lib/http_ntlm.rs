...
#[no_mangle]
pub extern "C" fn Curl_output_ntlm(
    mut data: *mut crate::src::lib::http2::Curl_easy,
    mut proxy: bool,
) -> u32 {
    ...
    if proxy {
        ...
        userp = unsafe { (*data).state.aptr.proxyuser };
        passwdp = unsafe { (*data).state.aptr.proxypasswd };
        service = if !(unsafe { (*data).set.str_0[STRING_PROXY_SERVICE_NAME as i32 as usize] }).is_null() {
            (unsafe { (*data).set.str_0[STRING_PROXY_SERVICE_NAME as i32 as usize] }) as *const i8
        } else {
        ...
        service = if !(unsafe { (*data).set.str_0[STRING_SERVICE_NAME as i32 as usize] }).is_null() {
...
#[no_mangle]
pub extern "C" fn Curl_http_auth_cleanup_ntlm(mut conn: *mut crate::src::lib::http2::connectdata) {
    Curl_auth_cleanup_ntlm(Some(unsafe { &mut (*conn).ntlm }));
    Curl_auth_cleanup_ntlm(Some(unsafe { &mut (*conn).proxyntlm }));
