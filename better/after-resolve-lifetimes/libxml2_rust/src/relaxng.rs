...
unsafe extern "C" fn xmlRelaxNGValidErrorPush<'a1>(
    ...
    if ((*ctxt).errTab).is_null() {
        ...
        *fresh57 = xmlMalloc
        ...
        let fresh58 = &mut (borrow_mut(&mut (*ctxt).err));
    ...
    if (*ctxt).errNr >= (*ctxt).errMax {
        ...
        *fresh59 = xmlRealloc
        ...
        let fresh60 = &mut (borrow_mut(&mut (*ctxt).err));
    ...
    let fresh69 = &mut (borrow_mut(&mut (*ctxt).err));
...
unsafe extern "C" fn xmlRelaxNGValidErrorPop<'a1>(mut ctxt: * mut crate::src::relaxng::_xmlRelaxNGValidCtxt<'a1>) {
    ...
    if (*ctxt).errNr <= 0 as i32 {
        let fresh72 = &mut (borrow_mut(&mut (*ctxt).err));
    ...
    *fresh73 -= 1;
    if (*ctxt).errNr > 0 as i32 {
        let fresh74 = &mut (borrow_mut(&mut (*ctxt).err));
        *fresh74 = Some(&mut *((*ctxt).errTab)
            .offset(((*ctxt).errNr - 1 as i32) as isize));
    } else {
        let fresh75 = &mut (borrow_mut(&mut (*ctxt).err));
...
unsafe extern "C" fn xmlRelaxNGPopErrors<'a1>(
    ...
    (*ctxt).errNr = level;
    if (*ctxt).errNr <= 0 as i32 {
        let fresh95 = &mut (borrow_mut(&mut (*ctxt).err));
...
pub unsafe extern "C" fn xmlRelaxNGNewValidCtxt<'a1, 'a2>(
    ...
    *fresh498 = *(__xmlGenericErrorContext()).unwrap();
    (*ret).errNr = 0 as i32;
    (*ret).errMax = 0 as i32;
    let fresh499 = &mut (borrow_mut(&mut (*ret).err));
