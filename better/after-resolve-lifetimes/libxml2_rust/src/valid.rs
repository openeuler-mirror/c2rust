    ...
    match (*ctree).type_0 as u32 {
        1 => {
            ...
            *(borrow_mut(&mut len)).unwrap() = *(borrow(& len)).unwrap() + 1;
            let fresh163 = &mut ((*crate::__laertes_array::GetMut::<&mut _>::get_offset_mut(names.as_mut().unwrap(), (fresh162 as isize))));
        ...
        2 => {
            ...
            *(borrow_mut(&mut len)).unwrap() = *(borrow(& len)).unwrap() + 1;
            let fresh165 = &mut ((*crate::__laertes_array::GetMut::<&mut _>::get_offset_mut(names.as_mut().unwrap(), (fresh164 as isize))));
            *fresh165 = (*ctree).name;
        }
        3 => {
    ...
    while i < nb_elements {
        ...
        *fresh173 = (*crate::__laertes_array::Get::<&_>::get_add((elements).as_ref().unwrap(), (i as usize)));
        if xmlValidateOneElement(&mut vctxt, (*parent).doc, parent) != 0 {
            ...
            let fresh175 = &mut ((*crate::__laertes_array::GetMut::<&mut _>::get_offset_mut(names.as_mut().unwrap(), (fresh174 as isize))));
