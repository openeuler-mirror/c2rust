...
pub unsafe extern "C" fn xmlSchematronNewParserCtxt<'a1>(
    ...
    *fresh27 = xmlDictLookup((*ret).dict, URL as *const xmlChar, -(1 as i32));
    let fresh28 = &mut (borrow_mut(&mut (*ret).includes));
...
unsafe extern "C" fn xmlSchematronGetNode<'a1, 'a2>(
    ...
    let fresh57 = &mut ((*(*(borrow_mut(&mut ctxt)).unwrap()).xctxt).doc);
    *fresh57 = (*cur).doc;
    let fresh58 = &mut ((*(*(borrow_mut(&mut ctxt)).unwrap()).xctxt).node);
    *fresh58 = cur;
...
pub unsafe extern "C" fn xmlSchematronNewValidCtxt<'a1, 'a2, 'a3>(
    ...
    (*ret).type_0 = 2 as i32;
    let fresh63 = &mut (borrow_mut(&mut (*ret).schema));
...
unsafe extern "C" fn xmlSchematronRunTest<'a1, 'a2, 'a3>(
    ...
    failed = 0 as i32;
    let fresh65 = &mut ((*(*(borrow_mut(&mut ctxt)).unwrap()).xctxt).doc);
    *fresh65 = instance;
    let fresh66 = &mut ((*(*(borrow_mut(&mut ctxt)).unwrap()).xctxt).node);
    *fresh66 = cur;
